...
def parse_args():
    ...
    ap.add_argument('--capacity-provider')
    args = ap.parse_args()
    return args
...
if args.capacity_provider:
    l.info("using capacity provider")
    runArguments["capacityProviderStrategy"] =  [{
        'capacityProvider': args.capacity_provider,
        'weight' : 100
    }]
else:
    runArguments["launchType"] = "EC2"

response = ecs.run_task(**runArguments)
