...
def prepare_task_definition(base_task_definition, target_task_family):
    ...
    # do not mount engine, uploaded, becase clean-base-for-staging worker does not have an access to efs through nat
    base_task_definition['volumes'] = list(filter(lambda volume: (volume['name'] not in ['engine', 'uploaded']),  base_task_definition['volumes']))
