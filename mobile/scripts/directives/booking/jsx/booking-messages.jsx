...
var BookingMessageInbox = React.createClass({
    ...
    render: function () {
        ...
        var classes = classNames({message: true, inbox: true, system: message.type != 'userText', 'new': message.readed == false});
        return (<div className={classes} ref={"booking_message_" + message.id}>
            <div className="message-top">
                ...
                <BookingMessageDate createDate={message['date']} updateDate={message['lastUpdate']} />
            </div>
            <div className="message-blk">
                <div className="message-text" dangerouslySetInnerHTML={{__html: message.body}}>
                </div>
            </div>
        </div>);
    }
...
var BookingMessageOutbox = React.createClass({
    ...
    handleClick: function (e) {
        var element = e.target, actionPopup = this.element.getElementsByClassName('popup-message');
        if (element.parentElement && element.parentElement.nodeName == 'A') {
            element = element.parentElement;
        }
        if (
            ...
            && element.href
            && (
                actionPopup.length == 0
                || !actionPopup[0].contains(element)
            )
            && !messagesProcessing
        ) {
            if (element.protocol && ['tel:', 'sms:', 'geo:', 'mailto:'].indexOf(element.protocol) > -1) {
    ...
    render: function () {
        ...
        return (<div className={classes} ref={"booking_message_" + message.id}>
            <div className={actionsClasses}>
                <ul>
                    { message.canEdit ? <li><a href onClick={this.editMessage}><i className="icon-edit"></i><span>{actions.edit}</span></a></li> : null }
                    { message.canDelete ? <li><a href onClick={this.deleteMessage}><i className="icon-delete"></i><span>{actions.delete}</span></a></li> : null }
                </ul>
            </div>
            <div className="message-top">
                <div className="row">
                    <BookingMessageDate createDate={message['date']} updateDate={message['lastUpdate']} />
                    <div className="author" onClick={this.openActions}>
                        <div>
                            {message.author}&nbsp;
                            {
                                this.isVisibleActions() ? <i className="icon-arrow-down"></i> : null
                            }
                        </div>
                    </div>
                </div>
            ...
            <div className="message-blk">
                <Spinner color="white" className={spinnerClasses}></Spinner>
                <div className={messageClasses} dangerouslySetInnerHTML={{__html: message.body}}></div>
            </div>
        </div>);
    }
});
var BookingMessageDate = React.createClass({
    ...
    render: function () {
        ...
        return (<div className="date">
            {
                messageUpdated ? <i className="icon-refresh-g"></i> : null
            }
            <span className="bold"><TimeAgo date={date.ts * 1000}></TimeAgo></span>
            {typeof date.fmt == 'string' ? "(" + date.fmt + ")" : "(" + $filter('date')($filter('fmt')(date.fmt), 'mediumDate') + ")"}
        </div>);
    }
});

var BookingMessages = React.createClass({
    ...
    pollScroll: function () {
        var scrollDomNode = this.scrollableDomEl, domNode = this.element, _this = this;
    ...
    deleteMessage: function (data) {
        this.props.onDeleteMessage(data.messageId).then(data.onSuccess, data.onError);
    },
    render: function () {
    ...
    renderMessage: function (message) {
        ...
        if (message.box == 'in') {
            return (<BookingMessageInbox message={message} key={messageId} ref={messageId}></BookingMessageInbox>)
        }
        return (<BookingMessageOutbox message={message} key={messageId} ref={messageId}></BookingMessageOutbox>)
    }
});
var BookingMessagesForm = React.createClass({
    ...
    render: function () {
        ...
        var Spinner = React.addons.Spinner;
        return (<form className="send-message" onSubmit={this.submit}>
            <button type="submit" disabled={this.state.disabled}>
                {this.state.loading ? <Spinner color="gray"></Spinner>
                    : translations.send}
            </button>
            {
                this.isEditMode() ? <a onClick={this.closeEditMode} className="reset"><i className="icon-clear-d"></i></a> : null
            }
            <div className="input">
                <textarea placeholder={translations.placeholder} ref="textarea" onChange={this.handleChange} value={this.state.message} disabled={this.state.loading}></textarea>
            </div>
        </form>);
    }
});