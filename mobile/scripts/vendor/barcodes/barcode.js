 ...
 * var settings = {
 ...
 Barcode(value, type, settings);
 */
var Barcode, BarcodeUtils;
(function () {
    ...
    BarcodeUtils = {
            ...
            compute: function (code, crc, type) {
                if (!crc) {
                    ...
                    for (var i = code.length - 1; i > -1; i--) {
                        v = BarcodeUtils.intval(code.charAt(i));
                        if (isNaN(v)) return ("");
                        sum += odd ? 3 * v : v;
                        odd = !odd;
                    }
        ...
        ean: {
            ...
            getDigit: function (code, type) {
                ...
                if (type == "ean8") {
                    ...
                    // process left part
                    for (var i = 0; i < 4; i++) {
                        result += this.encoding[BarcodeUtils.intval(code.charAt(i))][0];
                    }
                    ...
                    // process right part
                    for (var i = 4; i < 8; i++) {
                        result += this.encoding[BarcodeUtils.intval(code.charAt(i))][2];
                    }

                } else { // ean13
                    // extract first digit and get sequence
                    var seq = this.first[BarcodeUtils.intval(code.charAt(0))];

                    // process left part
                    for (var i = 1; i < 7; i++) {
                        result += this.encoding[BarcodeUtils.intval(code.charAt(i))][BarcodeUtils.intval(seq.charAt(i - 1))];
                    }
            ...
            compute: function (code, type) {
                ...
                for (i = code.length - 1; i > -1; i--) {
                    sum += (odd ? 3 : 1) * BarcodeUtils.intval(code.charAt(i));
                    odd = !odd;
                }
        ...
        upc: {
            getDigit: function (code) {
                ...
                return BarcodeUtils.ean.getDigit(code, 'ean13');
            },
            compute: function (code) {
                if (code.length < 12) {
                    code = '0' + code;
                }
                return BarcodeUtils.ean.compute(code, 'ean13').substr(1);
            }
        },
        msi: {
            ...
            computeMod10: function (code) {
                ...
                for (i = 0; i < code.length; i++) {
                    if (toPart1) {
                        n1 = 10 * n1 + BarcodeUtils.intval(code.charAt(i));
                    } else {
                        sum += BarcodeUtils.intval(code.charAt(i));
                    }
                    toPart1 = !toPart1;
                }
                var s1 = (2 * n1).toString();
                for (i = 0; i < s1.length; i++) {
                    sum += BarcodeUtils.intval(s1.charAt(i));
                }
                return (code + ((10 - sum % 10) % 10).toString());
            },
            computeMod11: function (code) {
                ...
                for (var i = code.length - 1; i >= 0; i--) {
                    sum += weight * BarcodeUtils.intval(code.charAt(i));
        ...
        code128: {
            ...
            getDigit: function (code) {
                ...
                while (i < code.length) {
                    ...
                    if (tableCActivated) {
                        value = BarcodeUtils.intval(code.charAt(i) + code.charAt(i + 1)); // Add two characters (numeric)
                        i += 2;
                    } else {
        ...
        digitToCanvas: function (settings, digit, hri) {
            return this.render(settings, this.bitStringTo2DArray(digit), hri);
        },
        digitToCanvas2D: function (settings, digit, hri) {
            return this.render(settings, digit, hri);
        },
        render: function (settings, digit, hri) {
            var canvas = document.createElement('canvas'),
                ctx = canvas.getContext('2d');

            var lines = digit.length,
                columns = digit[0].length,
                width = BarcodeUtils.intval(settings.barWidth),
                height = BarcodeUtils.intval(settings.barHeight),
                mw = Math.round(width / columns),
                mh = Math.round(height / lines),
                boxWidth = Math.round(mw * columns),
                boxHeight = Math.round(mh * lines),
            ...
            for (var y = 0; y < lines; y++) {
                ...
                if ((len > 0) && (current == '1')) {
                    ctx.fillRect((columns - len) * mw, y * mh, mw * len, mh);
                }
            }
            if (settings.showHRI) {
                var dim = ctx.measureText(hri);
                ctx.fillText(hri, Math.floor((columns * mw - dim.width) / 2), lines * mh + settings.fontSize + settings.marginHRI);
            }

            return canvas;
        },
        compute: function (type, code) {
            var crc, btype;
            if (['STD_25', 'INT_25', 'ITF', 'EAN_8', 'EAN_13'].indexOf(type) > -1) {
                if (type === 'ITF') {
                    type = 'INT_25';
                    crc = false;
                }
                btype = type.toLowerCase().replace(/[^a-z0-9]+/gi, "");
            }
            switch (type) {
                case "STD_25":
                case "INT_25":
                case "ITF":
                    code = BarcodeUtils.i25.compute(code, crc, btype);
                    break;
                case "EAN_8":
                case "EAN_13":
                    code = BarcodeUtils.ean.compute(code, btype);
                    break;
                case "UPC":
                case "UPC_A":
                case "UPC_E":
                    code = BarcodeUtils.upc.compute(code);
                    break;
                case "CODABAR":
                    code = code.replace(/[^0-9-$:/.+]/gi, '');
                    break;
                case "MSI":
                    code = BarcodeUtils.msi.compute(code, crc);
                    break;
            }
    ...
    Barcode = function (datas, type, settings) {
        ...
        if (typeof(settings) == "undefined")
            settings = {};
        for (var name in BarcodeUtils.settings) {
            if (BarcodeUtils.settings.hasOwnProperty(name) && settings[name] == undefined)
                settings[name] = BarcodeUtils.settings[name];
        }
        if (['STD_25', 'INT_25', 'ITF', 'EAN_8', 'EAN_13'].indexOf(type) > -1) {
            if (type === 'ITF') {
                type = 'INT_25';
        ...
        switch (type) {
            ...
            case "INT_25":
            case "ITF":
                digit = BarcodeUtils.i25.getDigit(code, crc, btype);
                hri = BarcodeUtils.i25.compute(code, crc, btype);
                break;
            case "EAN_8":
            case "EAN_13":
                digit = BarcodeUtils.ean.getDigit(code, btype);
                hri = BarcodeUtils.ean.compute(code, btype);
                break;
            case "UPC":
            case "UPC_A":
            case "UPC_E":
                digit = BarcodeUtils.upc.getDigit(code);
                hri = BarcodeUtils.upc.compute(code);
                break;
            case "CODE_11":
                digit = BarcodeUtils.code11.getDigit(code);
                hri = code;
                break;
            case "CODE_39":
                digit = BarcodeUtils.code39.getDigit(code);
                hri = code;
                break;
            case "CODE_93":
                digit = BarcodeUtils.code93.getDigit(code, crc);
                hri = code;
                break;
            case "CODE_128":
                digit = BarcodeUtils.code128.getDigit(code);
                hri = code;
                break;
            case "CODABAR":
                code = code.replace(/[^0-9-$:/.+]/gi, '');
                digit = BarcodeUtils.codabar.getDigit(code);
                hri = code;
                break;
            case "MSI":
                digit = BarcodeUtils.msi.getDigit(code, crc);
                hri = BarcodeUtils.msi.compute(code, crc);
                break;
            case "DATA_MATRIX":
                digit = BarcodeUtils.datamatrix.getDigit(code, rect);
                hri = code;
                b2d = true;
                break;
            case "PDF_417":
            case "PDF417":
                var pdf417 = new PDF417(code);
                digit = pdf417.bcode;
                hri = code;
                b2d = true;
                break;
            case "QR_CODE":
                var qr = new QRCode({text: code});
                digit = qr.getDigit();
                hri = code;
                b2d = true;
                break;
        ...
        return BarcodeUtils['digitToCanvas' + (b2d ? '2D' : '')](settings, digit, hri);
    }
