...
(function (window, angular, undefined) {
     ...
     *       functionality.
     */
    angular.module('jmdobry.angular-cache', ['ng', 'jmdobry.binary-heap', 'angular-fakestorage']);
    ...
    function $AngularCacheFactoryProvider() {
        ...
        this.setCacheDefaults = function (options) {
            ...
            try {
                ...
                localStorage.removeItem('angular-cache.test');
            } catch (e) {
         ...
         * @ignore
         */
        this.$get = ['$window', 'BinaryHeap', '$localStorage', function ($window, BinaryHeap, $localStorage) {
            var caches = {};
            ...
            function AngularCache(cacheId, options) {
                ...
                function _flushExpired() {
                    var now = new Date().getTime();
                    angular.forEach(expiresHeap.heap, function (item) {
                        if (config.deleteOnExpire !== 'none')
                ...
                function _saveItemToStorage(key) {
                    if (config.storageMode !== 'none' && storage && key in data) {
                        storage.removeItem(prefix + '.data.' + key); //Fix localStorage.setItem causes iPad QUOTA_EXCEEDED_ERR exception 22, issue #9895
                        storage.setItem(prefix + '.data.' + key, angular.toJson(data[key]));
                    }
                }
                ...
                this.put = function (key, value, options) {
                    ...
                    if (item.expires && (deleteOnExpire === 'aggressive' || deleteOnExpire === 'none')) {
