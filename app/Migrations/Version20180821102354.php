 ...
 */
final class Version20180821102354 extends AbstractMigration
{
    ...
    public const MIGRATE_UPDATES_SINCE = '2018-04-10';
    public const MIGRATION_DATE = '2018-11-16';
    ...
    public function up(Schema $schema): void
    {
        ...
        ");
        $this->addSql("UPDATE TripSegment JOIN Trip USING (TripID) SET TripSegment.MarketingAirlineConfirmationNumber = Trip.RecordLocator WHERE Trip.Category = 1 AND Trip.RecordLocator IS NOT NULL");
    }
    ...
    private function updateTable(string $table)
    {
        if ('TripSegment' === $table) {
            $this->connection->executeUpdate("UPDATE $table SET ExtPropertyMerged = FALSE WHERE ChangeDate >= " . self::MIGRATE_UPDATES_SINCE);
        } else {
            $this->connection->executeUpdate("UPDATE $table SET ExtPropertyMerged = FALSE WHERE UpdateDate >= " . self::MIGRATE_UPDATES_SINCE);
        }
        ...
        do {
            ...
            while ($nextRow = $statement->fetch(\PDO::FETCH_ASSOC)) {
                switch ($table) {
                    ...
                    case 'Reservation':
                        $insertedCounter += $this->reverseProcessResrevation($nextRow);
     ...
     * @throws \Doctrine\DBAL\DBALException
     */
    private function reverseProcessResrevation(array $reservation): int
    {
        $reservationDirectCopyMap = [
            'CancellationPolicy' => 'CancellationPolicy',
            'Fax' => 'Fax',
