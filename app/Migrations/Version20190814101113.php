...
class Version20190814101113 extends AbstractMigration implements ContainerAwareInterface
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $csv = 'X	ProgramTransferrable Currency	Type	MinimumTransfer	Receive	ValueMultiplier	NOTES																			
	Aeromexico (Club Premier)	Capital One	Airline	1000	750	0,75																				
	Air Canada Aeroplan (Altitude)	Capital One	Airline	1000	750	0,75																				
	...
	Jet Airways (JetPrivilege)	Citi	Airline	1000	1000	1,00	CURRENTLY UNAVAILABLE																			
	...
	Jet Airways (JetPrivilege)	Marriott	Airline	3000	500	0,17																				
	JetBlue Airways (trueBlue)	Marriott	Airline	3000	1000	0,33																				
	Korean Air (SkyPass)	Marriott	Airline	3000	1000	0,33																				
...
';
        ...
        $connection->executeQuery('DELETE FROM RewardsTransfer WHERE RewardsType = ' . RewardsTransfer::TYPE_MILEVALUE);

        foreach ($data as $col) {
            $multiplier = (float) str_replace(',', '.', $cols[5]);
            $ratio = round(1 / $multiplier);

            //$minimumTransferPoint = $cols[3];
            ...
            $connection->insert('RewardsTransfer', $row);
        }
    }
