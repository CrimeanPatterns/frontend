function makeSize(el, type) {
  switch (type) {
    ...
    case "5":
      el.style.background =
        "url('/images/levels/reached" + type + ".png') no-repeat";
      ...
      if (elName.offsetLeft < 10) {
        ...
        elName.style.top =
          el.offsetTop + el.offsetHeight / 2 - elName.offsetHeight / 2 + "px";
        elName.style.left =
          el.offsetLeft + el.offsetWidth - elName.offsetWidth - 5 + "px";
      }
  ...
  if (!tableEl.data("rendered")) {
    ...
    $("table." + tabID + "_elite_chart td." + tabID + "_eliteCell").each(
      function (e) {
        ...
        $(this).width(
          Math.floor(
            ($(this).data("initialLength") / levelsCount) * tableWidth,
          ) - gap,
        );
      },
    );
...
function showStatus(mode, errorCode, errorMessage) {
  ...
  if (errorMessage != "") s = s + ": " + errorMessage;
  alert(s);
}
...
function showMessagePopup(
  ...
  } else document.getElementById("messagePopup").style.width = "540px";
  if (style == "scroll") {
  ...
  if (noButtons) $("#messageButtons").css("display", "none");
  else $("#messageButtons").css("display", "block");
  if (okbuttonname) {
    $("#messageOKButton input").attr("value", okbuttonname.toString());
    $("button#messageOKButton").html(okbuttonname.toString());
  } else {
    $("#messageOKButton input").attr("value", "OK");
    $("button#messageOKButton").html("OK");
  }
  document.getElementById("messageOKButton").onclick = new Function(
    "cancelPopup()",
  );
  document.getElementById("messageCancelButton").style.display = "none";
  document.getElementById("messageCancelButton").onclick = new Function(
    "cancelPopup()",
  );
  $("#messageCancelButton input").css("width", "120px");
  $("button#messageCancelButton").css("width", "120px");
  $("#messageCancelButton input").attr("value", "Cancel");
  $("button#messageCancelButton").html("Cancel");
...
function showBecomeAFanPopup() {
  ...
  document.getElementById("messageText").innerHTML =
    '<a href="http://www.facebook.com/apps/application.php?id=75330755697" target="_blank" onclick="cancelPopup(); return true;"><img src="/images/becomeAFan.png" width="658" height="250" border="0"></a><br><br>Please go to Facebook, and click on the "Become a Fan" link'; /*checked by Alexi*/
  ...
  document.getElementById("messageOKButton").onclick = new Function(
    "window.open('http://www.facebook.com/apps/application.php?id=75330755697', 'facebook', ''); cancelPopup();",
  );
...
function showQuestionPopup(
  ...
  document.getElementById("messageHeader").innerHTML = header;
  document.getElementById("messageText").innerHTML = message;
  document.getElementById("messagePopup").style.height =
    $("#messagePopupBody").height() + 30 + "px";
...
function setHistoryRows(idx, cycle, accounts) {
  ...
  if (cycle) {
    ...
    if (rows > 5) {
      ...
      rows = 5;
    } else document.getElementById("checkHistory").style.overflow = "hidden";
  } else document.getElementById("checkHistory").style.overflow = "hidden";
  if (rows > 0)
    $("#checkHistory").animate(
      { height: 20 * rows },
  ...
  else document.getElementById("checkHistory").style.height = 20 * rows + "px";
  if (rows == 0) document.getElementById("checkHistory").style.display = "none";
  else document.getElementById("checkHistory").style.display = "";
  ...
  if (rows > 0) {
    ...
    var top = Math.round(
      getWindowHeight() / 2 - popupHeight / 2 + getScrollTop(),
    );
...
function checkAccountIdx(accounts, idx, forTrips) {
  ...
  if (idx < accounts.length) {
    ...
    if (accounts.length > 1)
      document.getElementById("checkPopupHeader").innerHTML =
        "Checking account " +
        (idx + 1) +
        " of " +
        accounts.length +
      ...
      document.getElementById("checkPopupHeader").innerHTML =
        'Checking <span style="font-weight: bold;">"' +
        account.displayName +
        '"</span> for ' +
        account.userName;
    ...
    document.getElementById("checkText").innerHTML =
      '<img src="/lib/images/progressCircle.gif"> Currently checking: <span style="font-weight: bold;">' +
      displayName +
      "</span>";
    document.getElementById("checkNote").innerHTML =
      "*All advertising links open in a new window, checking balances will not stop.";
    ...
    var onResponse = function (response) {
      if (accounts.length <= 1) {
        ...
        if (!forTrips)
          document.getElementById("checkStopButton").onclick = function () {
            stopChecking();
            reloadPage();
          };
        if (response.Detail != "") {
          showCheckDetails(response.Detail);
        } else checkSuccessfull(accounts, forTrips);
      } else {
    ...
    if (
      account.checkInBrowser == 1 ||
      (account.checkInBrowser == 2 &&
          ...
          function (params) {
            ...
            if (params.errorMessage) {
              ...
              response = {
                Status: "OK",
                Message:
                  icon + " " + account.displayName + " - Error occured.</span>",
                Detail: browserExt.formatAccountError(
                  params.errorMessage,
                  account.accountId,
                ),
              };
              ...
              if (typeof params.oldBalance != "undefined")
                if (params.oldBalance != params.properties.Balance) {
                  icon =
                    "<img src=/images/changed.gif title='Balance was changed within last 24 hours'>";
                  info =
                    "Successfully updated from <span style='font-weight: bold;'>" +
                    params.oldBalance +
                  ...
                  info =
                    "Balance remains the same: <span style='font-weight: bold;'>" +
                    params.properties.Balance +
                    "</span>";
              response = {
                Status: "OK",
      ...
      if (typeof activeTravelPlanId == "undefined" || activeTravelPlanId == 0)
        travelPlan = "";
      else travelPlan = "&travelPlan=" + activeTravelPlanId;
      checkRequest = $.ajax({
        url:
          "/account/ajaxCheck.php?ID=" +
          account.accountId +
          "&Cycle=" +
          (accounts.length > 1 ? "1" : "0") +
          "&CheckIts=" +
          (forTrips ? "1" : "0") +
          (idx == 0 ? "&ClearCache=1" : "") +
          travelPlan,
    ...
    checkRequestTimer = setTimeout(function () {
      checkTimedout = true;
      if (checkRequest) checkRequest.abort();
      checkTimedout = false;
      onResponse({
        Message:
          "<img src=/lib/images/error.gif> " +
...
function askAccountPassword(
  ...
  showMessagePopup(
    "",
    "Missing password for " + providerName,
    prompt +
      '<br><input class="inputTxt" id="accountPassword" type="password" style="width:454px;">',
  );
  setTimeout(
    "document.getElementById('accountPassword').focus(); setSavePasswordHandler();",
    500,
  );
  document.getElementById("messageCancelButton").style.display = "";
...
function showCheckDetails(details) {
  ...
  document.getElementById("checkAdMessage").innerHTML =
    '<div id=checkAdError style="text-align: left; padding: 5px; font-size: 16px;">' +
    details +
    "</div>";
}
...
function cycleAccountChecked(accounts, message, idx, forTrips) {
  ...
  if (idx == accounts.length - 1) {
    document.getElementById("checkText").innerHTML =
      '<span style="font-weight: bold;">All accounts checked</span>'; /*checked by Alexi*/
    $("#checkStopButton input").attr("value", "Close");
    document.getElementById("checkStopButton").onclick = function () {
      cancelPopup();
      reloadPage();
    ...
    setTimeout(function () {
      checkAccountIdx(accounts, idx + 1, forTrips);
    }, 10);
}
...
function stopChecking() {
  clearTimeout(checkRequestTimer);
  checkCancelled = true;
  if (browserExt.installed) browserExt.cancel();
  cancelPopup();
  document.getElementById("checkStopButton").onclick = stopChecking;
...
function getAccountInfoById(id) {
  ...
  if (link.length > 0) info = getAccountInfoFromLink(link);
...
function redirectAccount(
  ...
  if (autoLogin) {
    document.getElementById("checkPopupHeader").innerHTML =
      'Auto-login to <span style="font-weight: bold;">"' +
      programName +
      '"</span> website';
    document.getElementById("checkText").innerHTML =
      '<img src="/lib/images/progressCircle.gif"> We are attempting to automatically log you in';
  } else {
    document.getElementById("checkPopupHeader").innerHTML =
      'Redirecting to <span style="font-weight: bold;">"' +
      programName +
      '"</span> website';
    document.getElementById("checkText").innerHTML =
      '<img src="/lib/images/progressCircle.gif"> Unfortunately auto-login is not yet available for this program';
  }
  document.getElementById("checkNote").innerHTML =
    "*All advertising links open in a new window, auto-login will not stop.";
  if (document.getElementById("checkPopup").style.visibility != "visible")
    showPopupWindow(document.getElementById("checkPopup"), true);
...
function switchSection(id) {
  ...
  if (arrow.hasClass("closedArrow")) {
    ...
    var bodyInner = $(
      "#" + id + "_body > td > div.blockBody > div.blockBodyInner",
    );
...
function overRow(id) {
  var fader = document.getElementById("fader");
  if (fader.style.visibility == "visible") return false;
  if (showTimer > 0) clearTimeout(showTimer);
  ...
  if (popup && activePopup != id) {
    ...
    if (popup.innerHTML == "Loading..") {
      ...
      if (popup.getAttribute("accounts") > 1) {
        url =
          "/account/providerInfo.php?ID=" + popup.getAttribute("providerId");
        if (popup.getAttribute("checkInBrowser") != "0")
          params = browserExt.getBalancesOfProvider(
            popup.getAttribute("providerCode"),
          );
      } else
        url =
          "/account/moreInfo.php?TableName=" +
          popup.getAttribute("tableName") +
          "&ID=" +
          popup.getAttribute("rowId") +
          "&SubAccountID=" +
          popup.getAttribute("subAccountId") +
          "&Width=" +
          width;
      $.ajax({
        url: url,
    ...
    if (
      $(marker).offset().top > $(popup).offset().top + 10 &&
      $(marker).offset().top + $(marker).height() <
        $(popup).offset().top + $(popup).height() - 15
    )
...
function updateInfoWindow(popup, accountId, subAccountId) {
  ...
  } else expiration = $("tr#row" + accountId + " td.expiration a:first");
  if (expiration.length > 0) {
    template.find("td.value").html("").addClass("expiration");
    $("tr#row" + accountId + " td.expiration div.greenCorner")
      .clone()
      .appendTo(template.find("td.value"));
    ...
    if (row && row.getAttribute("checkinbrowser") > 0) template.remove();
  }
  table
    .children("tr:even")
    .filter(":has(td.name)")
...
function updatePopupSize(popup, top) {
  ...
  if (top < scrollOffset + 10) top = scrollOffset + 10;
...
function revealPassword(fieldId, link, accountId) {
  if (link.innerHTML == "Hide password") {
    var field = document.getElementById(
      document.getElementById("revealFieldId").value,
    );
    ...
    $(field2)
      .bind("focus", null, formControlFocused)
      .bind("blur", null, formControlBlurred);
    field2.focus();
  ...
  $("#revealPassword").keypress(function (event) {
    if (event.keyCode == "13") checkRevealPassword();
  });
...
function cancelReveal() {
  $("#revealPasswordBox").fadeOut(300, function () {
    document
      .getElementById(document.getElementById("revealFieldId").value)
      .focus();
  });
}
...
function checkRevealPassword() {
  ...
  $.ajax({
    ...
    data:
      "Password=" +
      encodeURIComponent(document.getElementById("revealPassword").value) +
      "&AccountID=" +
      encodeURIComponent(revealAccountId),
    error: function () {
      $("#revealPasswordProgress").hide();
...
function checkUserPasswordSuccess(response) {
  if (response.Error == "") {
    ...
    if (localPassword) field2.value = localPassword;
    else field2.value = response.Password;
    field.parentNode.replaceChild(field2, field);
    cancelReveal();
    $(field2)
      .bind("focus", null, formControlFocused)
    ...
    if ($("#revealPassword") != null) $("#revealPassword").focus().select();
  }
}
...
function adjustPopupSize(popup) {
  ...
  if (docHeight > maxHeight) maxHeight = docHeight;
...
function getScrollTop() {
  ...
  if (
    typeof parentScrollTop != "undefined" &&
    parentScrollTop > parentOffsetTop
  )
...
function getWindowHeight() {
  ...
  if (typeof parentHeight != "undefined") windowHeight = parentHeight;
  return windowHeight;
}
...
function cancelPopup() {
  if (activePopupWindow) {
    ...
    if (activePopupWindow.id == "framePopup")
      setTimeout(
        'document.getElementById("popupFrameContainer").innerHTML = ""',
        500,
      );
  }
...
function popupCancelled() {
  ...
  fader.style.visibility = "hidden";
  hidePopupWindow();
  if (reloadAfterClose == 1) reloadPage();
  activePopupWindow = null;
...
function showPolicy() {
  if (window.frameElement != null) parent.showPolicy();
  else {
    $.ajax({
...
function showFrame(src, width, title, onShow) {
  ...
  document.getElementById("popupFrameContainer").innerHTML =
    '<iframe name="popupFrame" id="popupFrame" src="' +
    src +
    '" width="100%" height="50" border="0" onload="popupFrameLoadComplete()" frameborder="0"></iframe>';
  frameTimer = null;
  frameLoaded = false;
...
function showLastReview(a, providerId) {
  ...
  if (reviewProviderId == providerId) return;
...
function expiredCouponsClick() {
  ...
  var url = document.location.href;
  url = url.replace(/#[^#]+/gi, "");
  url = url.replace(/[\&\?]ExpCoupons=\d+/gi, "");
  if (expiredCoupons) {
    if (url.indexOf("?") >= 0) url = url + "&";
    else url = url + "?";
...
function authorizeUser() {
  ...
  else location.href = "/?" + query_string();
}
...
function showLoginBox() {
  ...
  if (useHttps() && document.location.protocol.toLowerCase() != "https:") {
    ...
    if (backTo != "") url += "&BackTo=" + url_encode(backTo);
    document.location.href = url;
  } else {
    ...
    showFrame(url, 420, "Existing User", function () {
      var frame = document.getElementById("popupFrame");
      var frameDoc;
      if (frame.contentWindow) frameDoc = frame.contentWindow.document;
      else frameDoc = frame.contentDocument;
...
function useHttps() {
  ...
  return (
    host == "awardwallet.com" ||
    host == "iframe.awardwallet.com" ||
    host == "business.awardwallet.com"
  );
}
...
function showRegisterBox() {
  ...
  if (useHttps() && document.location.protocol.toLowerCase() != "https:") {
    ...
    if (acceptUserId != "") url += "&AcceptUserID=" + acceptUserId;
    document.location.href = url;
  } else {
    url = "/user/quickRegFrame.php?d=1"; // dummy
    if (acceptUserId != "") url += "&AcceptUserID=" + acceptUserId;
    var backTo = getParameterByName("BackTo");
    if (backTo != "") url += "&BackTo=" + encodeURIComponent(backTo);
    showFrame(
      url,
      815,
      "Quick Registration<br /><span class='alreadyAccount'>Already have an account? <a href='#' onclick='showLoginBox(); return false;'>Click to login</a> </span>",
      function () {
        var frame = document.getElementById("popupFrame");
        var frameDoc;
        if (frame.contentWindow) frameDoc = frame.contentWindow.document;
...
function alignAccountList() {
  $("div.redBar").each(function (index, bar) {
    ...
    if (tr.hasClass("afterName") || tr.hasClass("afterHead")) height -= 5;
    bar.height(height);
  });
...
function clearEmailFeild(feild) {
  if (feild.value == "Type your friend's email here...") feild.value = "";
}
...
function facebookShare(refCode) {
  ...
  if (browserExt.totals) browserMiles = browserExt.totals.all;
  $.ajax({
  ...
  FB.ui(
    {
      ...
      picture: info.Image,
      caption:
        "AwardWallet.com tracks frequent flyer miles and other loyalty program points for free.",
      description: " ",
...
function selectTab(tabsId, tab) {
  ...
  children.each(function (index, el) {
    ...
    if (el.id == activeId) className = "active";
...
function printCoupons(accountId, subAccountId) {
  var url =
    "/account/redirect?ID=" + accountId + "&Mode=download&deal=" + subAccountId;
...
function inviteFamilyMember(sender, userAgentId, email) {
  ...
  $("#email").keypress(function (event) {
    if (event.keyCode == Event.KEY_RETURN) sendFamilyInvite();
  });
  showPopupWindow($("#askEmailBox").get(0), true, function () {
...
function sendFamilyInvite() {
  $.ajax({
    url:
      "/invites/invite-member/" + document.getElementById("userAgentId").value,
    success: function (response) {
      if (response.success) {
        cancelInvite();
        reloadPage();
...
function sendEmailReminder(inviteCodeId, element, source) {
  $.ajax({
    url:
      "/agent/sendEmailReminder.php?InviteCodeID=" +
      inviteCodeId +
      "&Source=" +
      source,
    success: function (response) {
...
function changeProviderState(
  ...
  if (newState == "fixed" || newState == "added") {
    if (action == "openDialog") {
      ...
      if (checkCorrectInput(document.getElementById("userIDs").value, regExp)) {
        document.getElementById("justLink").innerHTML =
  ...
  } else if (newState == "broken") markAsEvent(newState, providerID, false);
}
...
function drawAddUserIDBox(
  ...
  showPopupWindow($("#askUserIDBox").get(0), true, function () {
    document.getElementById("userIDs").focus();
  });
}
...
function deleteAccountConfirmation(accountId) {
  if (window.confirm("Delete account?")) {
    if (browserExt.installed) browserExt.deleteAccount(accountId);
...
function deletePendingAccount(accountId, all) {
  var url;
  if (all) url = "/account/deletePending.php";
  else url = "/account/delete.php";
  showMessagePopup("info", "Deleting..", "Please wait..", true);
  $.ajax({
    url: url,
    type: "POST",
...
function skipPendingAccount(accountId, all) {
  ...
  if (all) setCookie("skipPendingAll", "1", exp);
  else setCookie("skipPending" + accountId, "1", exp);
  document.location.reload();
  return false;
}
...
function browserDetectNav(forExtension) {
  ...
  if (Edge) {
    browser[0] = Edge[0];
    browser[0] = browser[0].replace("Edg/", "Edge/");
  }
  //----- Opera ----
  else if (Maxthon) browser[0] = Maxthon[0];
  else if (!Opera == "" && !OperaV == "")
    browser[0] = OperaV[0].replace(/Version/, "Opera");
  else if (!Opera == "") browser[0] = Opera[0].replace(/OPR/, "Opera");
  //----- IE -----
  ...
  if (navigator.appName == "Netscape") {
    ...
    if (re.exec(ua) != null) {
      ...
      if (
        Math.round(outputData[1]) == 11 &&
        UA.match(/Windows NT 6\.3/i) &&
        ((!UA.match(/WOW64/i) && UA.match(/Win64/i)) || !isActiveX)
      ) {
  ...
  if (outputData != null) {
    ...
    if (forExtension && outputData[0] != "Edge") {
      if (
        outputData[0] == "Safari" &&
        Math.round(Math.round(outputData[1]) * 10 + Math.round(outputData[2])) <
          51
      )
...
function checkHtml5Form(form) {
  ...
  $(form)
    ...
    .each(function (i, elem) {
      ...
      if (
        typeof $(elem).attr("required") !== "undefined" &&
        $.trim($(elem).val()) == ""
      ) {
...
function reloadPage() {
  ...
  date.setTime(time);
  if (url.match(/r=/)) url = url.replace(/r=\d+/, "r=" + date.getTime());
  else {
  ...
  if (action) {
    if (action == "active")
      for (i = 0; i < allDatepickers.length; i++) {
        $("input[name=" + allDatepickers[i].name + "]").datepicker(
          allDatepickers[i].options,
        );
      }
...
function buttonAjax(obj, url, params, requestType, method, success, before) {
  ...
  this.addImage = function (obj, item) {
    ...
    var objPaddingLeft = parseFloat(
      $(obj).css("padding-left").replace(/px/, ""),
    );
    var objPaddingRight = parseFloat(
      $(obj).css("padding-right").replace(/px/, ""),
    );
    ...
    $(obj).append(
      '<img id="progress_' +
        id[item] +
        '" class="progressImage" src="/lib/images/progressCircle.gif" style="left:' +
        imageLeft +
        "px; top:" +
        imageTop +
        'px" />',
  ...
  $.ajax({
    ...
    success: function (json, textStatus) {
      ...
      if (obj) {
        if (obj[0] == "isArray") {
          for (i in obj) {
            if (i != 0) thisObj.removeImage(obj[i], i);
          }
        } else {
...
function strToValidDate(str, dateType) {
  if (dateType == undefined) dateType = "DATE_EU";
  if (dateType == "DATE_EU") {
...
function flashBackground(cell, color, flashCount, pause) {
  ...
  var switchLight = function () {
    if (!lightOn) cell.css("background-color", color);
    else cell.css("background-color", "");
    lightOn = !lightOn;
    flashes++;
    if (flashes < flashCount) setTimeout(switchLight, pause);
  };
...
function markDeal(dealID, status, action) {
  ...
  switch (action) {
    case "Apply":
      varMarkAsAction = varMarkAsApply;
      varMarkAsUnaction = varMarkAsUnapply;
      break;
    case "Follow":
      varMarkAsAction = varMarkAsFollow;
      varMarkAsUnaction = varMarkAsUnfollow;
      break;
  }
  buttonAjax(
    $("#mark" + action + "_" + dealID),
    "../../promos/mark" + action + ".json",
    { dealID: dealID, status: status },
    "json",
    "POST",
    function (data) {
      if (data.content != "OK") {
        ...
        if (status == 1) {
          $("#dealHead_" + dealID)
            .removeClass("listItem" + action)
            .removeClass("listItemDouble");
          $("#mark" + action + "_" + dealID)
            .removeClass("mark" + action + "Off")
            .addClass("mark" + action + "On");
          $("#mark" + action + "_" + dealID + " input").val(varMarkAsAction);
          markDealMsg[action][dealID] = 0;
        } else {
          $("#dealHead_" + dealID).addClass("listItem" + action);
          if (
            $("#dealHead_" + dealID).hasClass("listItemApply") &&
            $("#dealHead_" + dealID).hasClass("listItemFollow")
          )
            $("#dealHead_" + dealID).addClass("listItemDouble");
          $("#mark" + action + "_" + dealID)
            .addClass("mark" + action + "Off")
            .removeClass("mark" + action + "On");
...
function autoLoginAccountById(accountId) {
  ...
  showMessagePopup(
    "info",
    "Auto-login",
    "We are attempting to automatically log you in, please wait...",
    false,
    "Cancel",
  ); /*review*/
  onPopupCancelled = function () {
