...
var browserExt = browserExt;
if(!browserExt){
    browserExt = window.browserExt;
}
// End (1)
    ...
    /*update*/awardwallet = {
        ...
        onMessage: function (msg, callback) {
            ...
            if ((msg.command == 'setNextStep') && (awardwallet.state == 'check'))
                awardwallet.setNextStep(msg.params, callback);
            ...
            if (msg.command === 'keepTabOpen') {
                if (typeof(awardwallet.plugin) === 'object' && awardwallet.plugin !== null) {
        ...
        startCheck: function (sendPluginSource) {
            ...
            if (sendPluginSource) {
                ...
                if (awardwallet.saveScreenshot()) {
                    params.html2Cnv = awardwallet.html2Cnv;
                    params.saveScreenshot = true;
                }
            }
        ...
        loadHtml2Canvas: function() {
            if (awardwallet.plugin !=null && awardwallet.saveScreenshot() && awardwallet.html2Cnv==null) {
                browserAPI.log('loadHtml2Canvas');
                var d = new Date();
                var address = util.location.protocol + '//' + util.location.hostname;
                awardwallet.ajax(address + '/assets/common/vendors/html2canvas/index.js', function (content) {
                    browserAPI.log('loadHtml2Canvas loaded');
                    awardwallet.html2Cnv = content;
                    if (typeof(html2canvas) == 'undefined') {
                        browserAPI.log('setting html2canvas');
                        eval(content);
                    }
                    else
                        browserAPI.log('html2canvas already exists');
                });
            }
        },
        ...
        complete: function (params) {
            ...
            awardwallet.sendToPage('saveLog', {accountId: awardwallet.accountId, log: awardwallet.log});
            console.log('complete', awardwallet.accounts[awardwallet.accountId]);
...
//		browserAPI.log("sending to page: " + command + ', ' + document.getElementById('extParams').value);
            ...
            if (typeof(awardwallet.accounts[params.accountId].password) == 'undefined') {
                ...
                awardwallet.getPlugin(awardwallet.accounts[awardwallet.accountId].providerCode, function () {
                    ...
                    if (awardwallet.step == 'startFromCashback') {
                        ...
                        url = util.location.protocol + '//' + util.location.hostname + '/account/overview#' + url;
                    }
                    awardwallet.openTab(
                            url,
                            focusTab
        ...
        saveScreenshot: function () {
            return typeof(awardwallet.plugin) == 'object' && typeof(awardwallet.plugin.saveScreenshot) != 'undefined' && awardwallet.plugin.saveScreenshot;
        },
        ...
        getPlugin: function (providerCode, callback) {
            ...
            var url = util.location.protocol + '//' + util.location.hostname + '/engine/' + providerCode + '/extension.js?t=' + d.getTime() + '&v=' + encodeURIComponent(version);
            awardwallet.ajax(
                    url,
                    function (data) {
    ...
    var /*update*/browserAPI = {
        ...
        openTab: function (url, background, onSuccess, onError) {
            ...
            if (typeof(awardwallet.plugin.saveScreenshot) === 'undefined') {
                awardwallet.plugin.saveScreenshot = false;
            }
    ...
    var /*update*/provider = {
        ...
        init: function () {
            if (provider.readySent)
                return;
            provider.readySent = true;
        ...
        check: function (params) {
            ...
            if (typeof(params.saveScreenshot)!='undefined') {
                provider.saveScreenshot = params.saveScreenshot;
                if (params.saveScreenshot)
                    browserAPI.log('saving screenshot is on');
            }
            ...
            if (typeof(params.html2Cnv) == 'string') {
                browserAPI.log('setting html2canvas');
                eval(params.html2Cnv);
            }
            else
                browserAPI.log('html2Cnv not loaded');
        ...
        setNextStep: function (step, callback) {
            ...
            browserAPI.log('setNextStep: ' + step);
            browserAPI.send("awardwallet", "setNextStep", step, function() {
...
//		}
        ...
        showFader: function (text, onlyOnce) {
            ...
            message.innerHTML = "<tr><td><div style='background-image: url(" + util.location.protocol + "//awardwallet.com/lib/images/progressCircle.gif); " +
                    "border: none; width: 16px; height: 16px; float: left; margin-right: 10px;'></div></td> <td style='float: left;'>" +
                    text + "</td></tr>";
            $(message).show();
        ...
        logBody: function (step, takeScreenshot = false) {
            browserAPI.log('logging body');
            let content = document.documentElement.outerHTML;
            if (takeScreenshot && !provider.saveScreenshot){
                browserAPI.log('saving screenshot is off');
                takeScreenshot = false;
            }
            if (takeScreenshot && typeof(html2canvas)=='undefined'){
                browserAPI.log('can\'t save screenshot, html2canvas not loaded');
                takeScreenshot = false;
            }
            if (takeScreenshot) {
                html2canvas(document.body, {scale: 0.5})
                .then(function (canvas) {
                    if (typeof(document.documentElement) == 'object') {
                        browserAPI.log('save screenshot');
                    ...
                    browserAPI.log('problem with saving screenshot');
                    browserAPI.send('awardwallet', 'logEvent', {
                        type: 'file',
                        content: content,
                        step: step,
...
// init
    ...
    function awardwalletStart() {
        ...
        if (typeof(browserExt) === "object") {
            ...
            if (document.getElementById('extCommand') === null || document.getElementById('extCommand').value !== "content_scripts_ready")
                return;
...
// this is to prevent slow startup on sites which uses external scripts or images (topcashback)
    ...
    window.awardwallet = awardwallet;
// end init
