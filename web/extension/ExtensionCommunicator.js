...
// End (1)
    ...
    return function ExtensionCommunicator(onMessage) {
        ...
        this.sendToBg = function (command, params) {
            ...
            var safariExtensionId = 'com.awardwallet.macos.safari-extension (J3M2LK2HFC)';
            var allowedExtensionIds = [
                /* chrome prod */ 'lppkddfmnlpjbojooindbmcokchjgbib',
                /* chrome beta */ 'mnghodkdfjabhoeemdnappaaoablbnhj',
                /* edge prod */   'ohkankfogimcbcioalemeonlapdababm',
                /* new safari extension v2/v3 */ safariExtensionId
            ];
            ...
            if (browserExt.portCommSupported && allowedExtensionIds.includes(browserExt.extensionId)) {
                if (self.bgConnection === null) {
                    ...
                    var runtime;
                    if (browserExt.extensionId === safariExtensionId) {
                        runtime = browser.runtime;
                    } else {
                        runtime = chrome.runtime;
                    }
                    self.bgConnection = runtime.connect(browserExt.extensionId);
            ...
            return new Promise(function (resolve, reject) {
                if (tabScripts)
                    ...
                    $.when(
                        ...
                        // chrome.tabs.onUpdated firing when iframe URL change
                        // TODO: use the webNavigation API, which supports frame-level navigation events
                        var setWarning = 'console.warn("aw scripts injected twice, potential issue with navigation on provider site");'
                        var checkExecuteScript = 'var _attr = "scripts-injected"; if (document.body.hasAttribute(_attr)) { ' + setWarning + ' return; } document.body.setAttribute(_attr, String(true));';

                        tabScripts = "setTimeout(function(){ " + checkExecuteScript + " console.log('aw scripts start, owner tab id: " + code.tabId + ", my tab id: %target_tab_id%'); var myTabId = %target_tab_id%; var ownerTabId = " + code.tabId + ";\n\n" + code.jquery + "\n\n" + code.callbackManager + "\n\n" + code.forgeApiEmulator + "\n\n" + code.main + "\n\nconsole.log('aw scripts end'); }, 10);";
                        resolve(tabScripts);
                    });
                }
            });
                ...
                if (changeInfo.status === "complete") {
                    ...
                    completeTabTimers[tabId] = setTimeout(function(){
                        ...
                        self.loadTabScripts().then(function () {
                            ...
                            callback(tabId);
                            // we need to guarantee that scripts will be injected after callback was fired
        ...
        document.addEventListener(events.contentToPage, function (event) {
            ...
            if (detail.hasOwnProperty('callbackId')) {
                // skip event from v3 in compatibility mode
                return;
            }
