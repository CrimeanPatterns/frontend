...
$( document ).ready(function() {
    ...
    (function() {
        ...
        $('.plainlink').click(function() {
            var $this = $(this);
            var text = $this.text();
            if (text === 'Account Check Result' || text === 'Check Confirmation Result') {
                return;
            }

            function textToTabId(text) {
                var tabId = '';
                if (
                    text === 'Account Check Parameters'
                    || text === 'Load Login Form'
                    || text === 'Login'
                    || text === 'Parse'
                ) {
            ...
            var tabId = textToTabId(text);

            // active tab logic
            var activeTabText = $('ul.tab a.active').text();
            if (activeTabText === 'All') {
                return;
            } else if (text.match(/Parse Itinerary #/i)) {
                if (activeTabText === 'Parse Itineraries') {
                    return;
                }
                ...
                var $prev = findPreviousBlock($this);
                if ($prev && isBlock($prev.text()) && $prev.attr('href').endsWith('0')) {
                    var prevTabId = textToTabId($prev.text());
    ...
    function findPreviousBlock($curr) {
        while (!isBlock($curr.text())) {
            $curr = $curr.prev();
        }
        return $curr;
    }
    ...
    (function() {
        $('ul.tab a').click(function(e) {
            ...
            $('#CheckConfirmationResultTabId').show();

            if (hrefSelector === '#LoadLoginParseTabId') {
    ...
    function retriesPresent() {
        return $('.awlog-notice').text().match(/Checker signalized that retry is needed/i);
    }
...
function openLogs() {
    ...
    $('a.open-log-link').on('click', function(event) {
        ...
        $.ajax({
            ...
            success: function(data) {
                setTimeout(function() {
                    // prevent document.location.href leak
                    logWindow.document.write('<html lang="en"><body><iframe sandbox="allow-scripts" style="width: 100%; height: 100%;" srcdoc="' + quoteattr(data) + '"></iframe></body></html>');
                }, 1) // increase timeout to debug network on new tab
            }
        })
    });
}
