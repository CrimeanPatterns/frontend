define(['jquery-boot', 'lib/utils', 'jqueryui', 'translator-boot'], function ($, utils) {
    var Dialog = function (element) {
        var icon = element.dialog('option', 'type');
        if(icon)
            element.dialog('widget').find('.ui-dialog-title').prepend(icon);
        let cssClass = element.data('addclass');
        if (cssClass) {
            element.parent().addClass(cssClass);
        }
        let positionAt = element.data('position-at');
        if (positionAt) {
    ...
    Dialog.prototype = {
         ...
         *
         * @param {boolean} scrollable параметр, определяющий, может ли прокручиваться страница
         */
        open: function (scrollable = false) {
            this.element.dialog('open');
            if (this.getOption('modal') && !scrollable) {
