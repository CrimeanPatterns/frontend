...
define(['jquery-boot', 'lib/dialog', 'jqueryui', 'routing', 'translator-boot'], function ($, dialog) {
	...
	function calendarExport(){
		if (Calendars == null) {
			// todo fail!
			$.ajax({
				url: Routing.generate('aw_icalendar_ajax', {'action' : 'getPopupDataAcc', '_format': 'json', 'new': '1'}),
				type: "POST",
				data: {},
				dataType: 'json',
			...
			if (typeof(Calendars[-1]) !== "undefined") {
				...
				userDialog = dialog.fastCreate(
					Translator.trans('calendar.expire.auto.import'),
					calendarList,
					true,
					true,
					[
	...
	function showCalendarLink(agent, action) {
		...
		var manageLink = Routing.generate('aw_icalendar_ajax', {'action' : 'manageAcc', '_format': 'json', 'new': '1'});
		switch (action) {
			case "new":
				// todo fail!
				$.ajax(manageLink, {
	...
	function switchManage(remove, agent) {
		...
		if (remove) {
			var url = document.location.protocol + '//' + document.location.host + '/iCal/accExpire/' + Calendars[agent].Code;
