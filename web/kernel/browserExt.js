...
if (!showMessagePopup) {
  showMessagePopup = window.showMessagePopup;
}
var cancelPopup = cancelPopup;
if (!cancelPopup) {
  cancelPopup = window.cancelPopup;
}
// End (1)
var browserExt = {
  installed: false,
  ...
  onError: null,
  errorPrefix:
    "Something unexpected just happened while trying to register your account." +
  ...
  progressImage:
    "<img src='/lib/images/progressCircle.gif' " +
  ...
  sendCommand: function (command, params) {
    if (typeof params == "function") params = params.toString();

    if (browserExt.v2mode()) {
      // For page /account/list built via Webpack (1)
      if (!awardwallet) {
        window.awardwallet.processCommand(command, params);
        return;
      }
      // End (1)
  ...
  receiveCommand: function () {
    ...
    else browserExt.log("unknown command");
  },

  /* ---------------- commands from extension --------------- */

  complete: function (params) {
    if (browserExt.onComplete) browserExt.onComplete(params);
  },

  error: function (params) {
    if (browserExt.onError) browserExt.onError(params);
  },

  providerReady: function () {
    if (browserExt.onProviderReady) browserExt.onProviderReady();
    else browserExt.log("onProviderReady handler not set");
  },

  info: function (params) {
    if (browserExt.onInfo) browserExt.onInfo(params);
  },

  ready: function (params) {
    browserExt.log("browserExt ready, v" + params.version);
    if (browserExt.installed) return;
    ...
    if (browserExt.browser[0] == "Safari") {
      ...
      browserExt.log(
        "new ver: " + newVerInstalled + ", ready version: " + params.version,
      );
      ...
      if (
        ...
        browserExt.safariTimer = setTimeout(function () {
          browserExt.safariChecked = true;
          browserExt.ready(params);
        }, 1000);
  ...
  reportExtensionVersion: function () {
    ...
    date = d.getDate();
    if (
      browserExt.extensionVersion != reportedVersion ||
      date != reportedDate
    ) {
      $.ajax({
        url: Routing.generate("aw_extension_version_report", {
          v: browserExt.extensionVersion,
        }),
        method: "POST",
        success: function () {
  ...
  updateExtension: function (onComplete) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data = data.replace(/var \/\*update\*\//gim, "var update_");
        data =
          "function(){ awardwallet.update = " +
          JSON.stringify(data) +
          "; " +
          data +
          "updater = " +
        ...
        if (parseFloat(browserExt.extensionVersion) >= 1.2) {
  ...
  autoLoginAccountById: function (
    ...
    browserExt.updateExtension(function () {
      ...
      browserExt.xhr = $.ajax({
        ...
        success: function (data) {
          if (
            typeof data.requirePassword == "boolean" &&
            data.requirePassword &&
            typeof onRequirePassword != "undefined"
          ) {
    ...
    browserExt.updateExtension(function () {
      ...
      browserExt.onComplete = function (result) {
        ...
        $.ajax({
          ...
          data: result,
          success: function (data) {
            if (data.answer === "ok") onComplete(data.redirectUrl);
            else {
              if (version === "old") browserExt.onError(data.message);
              if (version === "new") onError(data.message);
            }
          },
          error: ajaxError,
        });
  ...
  accountsChanged: function (accounts) {
    ...
    for (var accountId in accounts) {
      ...
      if (
        row.length > 0 &&
        row.attr("data-checkinbrowser") == "1" &&
        row.find("a.multipleAccounts").length ==
          0 /* not business multiple accounts line */
      ) {
        ...
        if (typeof account.properties != "undefined") {
          if (typeof account.properties.Balance != "undefined") {
            ...
            if (goalTitle) {
              ...
              if (goal != null) {
                ...
                row
                  .find("td.balance .goal")
                  .attr(
                    "title",
                    "You goal " + goal + ", progress: " + progress + "%",
                  );
                row
                  .find("td.balance .goal .progress")
                  .css("width", progress + "%");
              }
            }
            //status
            if (account.properties.Status != null) {
              row
                .find("td.Status")
                .html(
                  "<div class='cont'>" + account.properties.Status + "</div>",
                );
              //alert(row.find('td.Status').attr('eliteLevels'));
              var eliteLevels = $.parseJSON(
                row.find("td.Status").attr("eliteLevels"),
              );
              var eliteLevelsCount = row
                .find("td.Status")
                .attr("eliteLevelsCount");
              var rank = 0;
              for (var key in eliteLevels) {
                var level = eliteLevels[key];
                if (
                  level.ValueText.toLowerCase() ==
                  account.properties.Status.toLowerCase()
                ) {
                  rank = level.Rank;
                  ...
                  if (trim(level.AllianceName) != "") {
                    ...
                    var bg =
                      "url(/images/alliances/" +
                      cell.attr("data-alliance") +
                      level.AllianceName.toLowerCase() +
                      ".png)";
              ...
              var elitizm =
                (eliteLevelsCount == 0
                  ? 0
                  : (rank / eliteLevelsCount).toFixed(2)) * 100;
              row
                .find("td.Status")
                .append(
                  "<div class='goal' title='Progress to the highest possible elite level on this program: " +
                    elitizm +
          ...
          if (typeof account.properties.ExpirationDate != "undefined") {
            ...
            if (date == null) date = account.properties.ExpirationDate;
            else date = formatDate(date, dateFormat);
  ...
  countTotal: function (balanceVal, kind, totals) {
    ...
    if (typeof totals[kind] == "undefined") totals[kind] = 0;
    ...
    if (browserExt.accounts != null)
      for (var accountId in browserExt.accounts) {
        ...
        if (
          ...
          typeof account.properties.Balance != "undefined"
        )
  ...
  setBusinessBalances: function (accounts, totals) {
    $("#tblAccounts a.checkAllLink").each(function (index, element) {
      ...
      for (var key in ids) {
        ...
        if (
          typeof accounts[accountId] != "undefined" &&
          typeof accounts[accountId].properties != "undefined" &&
      ...
      if (balanceSet) {
        ...
        browserExt.setRowBalance(
          formatNumberBy3(allBalance, ".", thousandsSeparator),
          row,
        );
  ...
  correctTotalBalance: function (totals) {
    ...
    for (var kind in totals) {
      if (kind != "all")
        browserExt.addTotal(
          row.find("tr.kind" + kind + " span.balance"),
          totals[kind],
        );
    }
  },
  ...
  fillPopupProperties: function (accountId) {
    if (!browserExt.installed) {
      //info in popup if extension is not installed
      $("#tabs_" + accountId + "_content").html(
        "In order to see detailed information on this program " +
          'you need to <a href="/extension/">install the AwardWallet browser extension</a>.',
      ); /*checked*/
      return;
    }
    if (!browserExt.accounts) {
      //info in popup if accounts array is null
      $("#tabs_" + accountId + "_content").html(
        "You do not have any information gathered for this program yet. " +
          "Please click the update button to retrieve the data.",
      ); /*checked*/
      return;
    }
    if (!browserExt.accounts[accountId]) {
      //info in popup if data of current accountId is null
      $("#tabs_" + accountId + "_content").html(
        "You do not have any information gathered for this program yet. " +
          "Please click the update button to retrieve the data.",
      ); /*checked*/
      ...
      /*==== main Properties ====*/
      var mainProps = $("#tabs_" + accountId + "_details").find(
        "table.mainProps",
      );
      ...
      else mainProps.find("td:contains(Balance:)").parent("tr").remove();
      //Expiration
      if (props.ExpirationDate != null && props.ExpirationDate != "")
        mainProps
          .find("td:contains(Expiration:) ~ td")
          .html(props.ExpirationDate);
      else mainProps.find("td:contains(Expiration:)").parent("tr").remove();
      //Status
      if (props.Status != null && props.Status != "")
        mainProps.find("td:contains(Status:) ~ td").html(props.Status);
      else mainProps.find("td:contains(Status:)").parent("tr").remove();
      //Account
      if (props.Number != null && props.Number != "")
        mainProps.find("td:contains(Account:) ~ td").html(props.Number);
      else mainProps.find("td:contains(Account:)").parent("tr").remove();

      /*==== extProps ====*/
      var extProps = $("#tabs_" + accountId + "_details").find(
        "table.extProps tbody tr",
      );
      for (i = 0; i < extProps.length; i++) {
        if ($(extProps[i]).attr("prop") != null) {
          var prop = $(extProps[i]).attr("prop");
          if (props[prop] != null && props[prop] != "")
            $("#tabs_" + accountId + "_details")
              .find("tr[prop=" + prop + "] td.value")
              .html(props[prop]);
          else
            $("#tabs_" + accountId + "_details")
              .find("tr[prop=" + prop + "]")
      ...
      if (data.errorMessage != null && data.errorMessage != "")
        $("#tabs_" + accountId + "_details")
          .find("div.accountErrorMessage")
          .find(".boxRed .center")
          .html(
            browserExt.formatAccountError(data.errorMessage, data.accountId),
          );
      else {
        $("#extRow" + accountId + " table tr td:eq(1) div.state")
          .removeClass("errorState")
          .addClass("successState")
          .attr("title", "Success");
        $("#tabs_" + accountId + "_details")
          .find("div.accountErrorMessage")
          .remove();
  ...
  available: function () {
    return (
      ...
      browserExt.validVersion(
        browserExt.extensionVersion,
        browserExt.requiredVersion(),
      )
    );
  },
  ...
  requiredVersion: function () {
    ...
    if (browserExt.browser == null) browserExt.supportedBrowser();
    if (browserExt.browser[0] == "MSIE") return "1.26";
    if (browserExt.browser[0] == "Edge") return "2.2";
    if (browserExt.browser[0] === "Firefox") {
      if (browserExt.browser[1] >= 57) return "2.0";
      else return "1.34";
    }
    if (browserExt.browser[0] == "Chrome") {
      if (browserExt.browser[1] >= 69) return "2.0";
      else return "1.36";
    }
    if (browserExt.browser[0] == "Safari") return "1.35";
  ...
  installExtension: function () {
    ...
    document.location.href =
      "/extension-install?BackTo=" + encodeURIComponent(document.location.href);
  },

  pushOnReady: function (handler) {
      console.log('pushOnReady');
    if (browserExt.available()) handler();
    else {
      if (browserExt.installed) browserExt.installExtension();
      else browserExt.onReady.push(handler);
    }
  },
  ...
  cancel: function () {
    if (browserExt.xhr && browserExt.xhr.readyState < 4) browserExt.xhr.abort();
  ...
  getBalancesOfProvider: function (providerCode) {
    if (browserExt.accounts == null) return {};
    var result = {};
    for (var accountId in browserExt.accounts) {
      var account = browserExt.accounts[accountId];
      if (
  ...
  autoLoginFallback: function (info, link) {
    if (link) return true;
      ...
      var url =
        "http://" + document.location.host + "/account/" + info.redirectUrl;
      browserExt.sendCommand(
        "execAwardWallet",
        ...
        if (response.partnerAccount.accountExist == 0) {
          ...
          if (extension) {
            ...
            browserExt.updateExtension(function () {
              ...
              showMessagePopup(
                ...
                browserExt.progressImage +
                  "Please wait while we are registering you for " +
                  response.partnerAccount.providerName +
              ...
              browserExt.autologin(
                ...
                function (error) {
                  ...
                  if (error != "code 1001") browserExt.onInfo = null;
          ...
          // registration via server
            ...
            $.ajax({
              ...
              data: {
                properties: properties,
                providerCode: response.partnerAccount.providerCode,
              },
              success: function (error) {
                // Registration failed
          ...
          showMessagePopup(
            ...
            "You already have an " +
              response.partnerAccount.providerName +
              " account added to your AwardWallet profile",
          );
      },
      error: ajaxError,
    });
  ...
  extensionV3supported: function(){
		var data = browserExt.browser;
		return (
            // Manifest V3 is supported generally in Chrome 88 or later
            (data[0] === 'Chrome' && data[1] >= 88)
		) || ['Safari', 'Firefox', 'Edge'].includes(data[0]);
	},

  autoRegMessage: function (response) {
    return (
      "We've signed you up for " +
      response.partnerAccount.providerName +
      " with the following credentials:<br/><br/>" +
      "Login: " +
      response.partnerAccount.login +
      "<br/>Password: " +
      response.partnerAccount.password +
  ...
  showRegisterProgress: function (response) {
    showMessagePopup(
      ...
      "Registration in progress",
      browserExt.progressImage +
        "Please wait, we are registering you on " +
        response.partnerAccount.providerName +
        ", this takes about 1 - 2 minutes",
      false,
      "Stop",
  ...
  getRegCompleteMessage: function (response) {
    return (
      "We've signed you up with the following credentials:<br/><br/>" +
      "Login: " +
      response.partnerAccount.login +
      "<br/>Password: " +
      response.partnerAccount.password +
      ...
      "<br/><br/>Please click the link in your email to confirm your registration and click next. " +
      "(please note that verification email could be in your spam folder)" +
      "<br/><br/>You are on your way to save " +
      response.partnerAccount.price +
      " on " +
      response.partnerAccount.nextAccount.providerName +
      " purchases."
    );
  },
  ...
  submitStat: function (code, success, error, accountId) {
    ...
    $.post("/extension/extensionStats.php", {
      providerCode: code,
      success: success,
      errorMessage: error,
      errorCode: errorCode,
  ...
  detectBrowser: function () {
    if (window.browserDetectNav) {
      browserExt.browser = window.browserDetectNav(true);
    } else {
  ...
  supportedBrowser: function () {
    var data = browserExt.browser;
    var result =
      (data[0] === "Edge" && data[1] >= 14 && data[2] >= 15063) ||
      (data[0] === "Edge" && data[1] >= 79) ||
      data[0] === "Safari" ||
      data[0] === "MSIE" ||
      data[0] === "Firefox" ||
      data[0] === "Chrome" ||
      (data[0] === "Opera" && typeof data[1] !== "undefined" && data[1] >= 15);
  ...
  showInfoBrowser: function () {
    var data = browserExt.browser;
    return data[0] === "Edge" && data[1] < 88 && data[2] < 15063;
  },
  ...
  saveLog: function (data) {
    $.ajax({
      ...
      data: JSON.stringify({ accountId: data.accountId, log: data.log }),
    });
  },
};
window.browserExt = browserExt;

browserExt.detectBrowser();
if (browserExt.browser[0] === "Edge" && browserExt.browser[1] < 79) {
  console.log("detected old edge browser, not chromium based");
  require(["extension-boot"], function () {});
} else {
  var getExtensionVersion = function () {
    var extParamsInput = document.getElementById("extParams");
    if (extParamsInput === null) {
      console.log("no extParams input");
      return;
    }
    var version_data = JSON.parse(extParamsInput.value);
    ...
    browserExt.portCommSupported =
      browserExt.browser[0] === "Chrome" ||
      (browserExt.browser[0] === "Edge" && browserExt.browser[1] >= 79);
    ...
    if (browserExt.browser[0] === "Safari") {
        ...
        if (typeof browserExt.extensionVersion === 'string') {
            try {
                extensionVersion = JSON.parse(browserExt.extensionVersion);
                if (typeof extensionVersion === 'object') {
                    extensionVersion = parseFloat(extensionVersion.version);
                } else {
                    extensionVersion = parseFloat(extensionVersion);
                }
            } catch (error) {
                extensionVersion = 0;
            }
        }
        if (extensionVersion >= 2.38) {
            browserExt.portCommSupported = true;
        }
    }
  ...
  var interval;
    ...
    if (browserExt.browser[0] === "Safari" && !browserExt.portCommSupported) {
        document.addEventListener("content_scripts_ready", function () {
            ...
            require(["extension-boot"], function () {
                if (version && version !== browserExt.extensionVersion) {
                    console.log(
                        "browserExt, detected old version: " +
                        version +
                        ", re-init new version: " +
                        browserExt.extensionVersion,
                    );
