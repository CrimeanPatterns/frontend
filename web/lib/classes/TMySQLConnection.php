...
//			$sSQL = preg_replace("/^select\s+/ims", "select /*! SQL_NO_CACHE */ ", $sSQL);
	...
	{
        $res = $this->OpenQuery("select COLUMN_NAME from information_schema.columns where table_schema=DATABASE() and table_name = '$sTable' and column_key='PRI'");
		$Result = $this->Fetch($res);
        // exclude multi-column primary keys, we do not support them
		$nextRow = $this->Fetch($res);
		$this->CloseQuery($res);
		if(is_array($Result) && !$nextRow)
