...
//			'file'      => '@'.$file,
    ...
    public function recognizeByRuCaptcha($key, $postDataExtended = array()) {
        ...
        if (isset($postdata['proxy']) && is_array($postdata['proxy'])) {
            // convert result of ProxyList::getCaptchaProxy to accepted format username:password@host:port
            $proxy = $postdata['proxy']['proxyAddress'] . ':' . $postdata['proxy']['proxyPort'];
            if (!empty($postdata['proxy']['proxyLogin'])) {
                $proxy = $postdata['proxy']['proxyLogin'] . ':' . $postdata['proxy']['proxyPassword'] . '@' . $proxy;
