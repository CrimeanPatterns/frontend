...
<?php
    ...
    public function Draw()
    {
        ...
        echo <<<'JS'
            <script>
                function autoSelect2MatcherList(term, text, opt) {
                ...
                const cookieSettings = $.cookie('AWManagerSettings');
                if (cookieSettings) {
                    const settings = JSON.parse(cookieSettings);
                    if (settings.searchableSelectBoxes) {
                        if (!(typeof $ === 'undefined' || typeof $.fn.select2 === 'undefined')) {
                            $(function () {
                                $('table#list-table select').map(function () {
                                    const $select = $(this);
                                    
                                    if ($select.find('option').length >= (settings.selectOptionsThreshold || 50)) {
                                        const $table = $select.closest('table'); // find the closest parent td
                                        
                                        if ($table.length === 0) {
                                            return;
                                        }
                                        
                                        const $td = $table.closest('td'); // find the closest parent td
                                        const width = $td.length ? ($td.width() - 15) : 'resolve'; // fallback if no td is found

                                        $select.select2({
                                            width: width, 
                                            dropdownAutoWidth: true,
                                            matcher: autoSelect2MatcherList
                                        });
                                    }
                                });
                            });
...
JS;
    ...
    public function DrawButtonsInternal()
    {
        ...
        if (!$this->Query->IsEmpty && $this->MultiEdit && !$this->ReadOnly) {
