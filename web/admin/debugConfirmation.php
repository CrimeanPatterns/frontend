...
function CheckForm() {
    ...
    $arFields = array_diff_key($objForm->GetFieldValues(), ['ProviderID' => true, 'ShowHeaders' => true]);
    ...
    try {
        ...
        $checker->logger->error($errorMessage);
        $checker->logger->error($e->getTraceAsString());
        foreach ($checker->itinerariesMaster->getItineraries() as $it) {
            $checker->itinerariesMaster->removeItinerary($it);
        }
    } finally {
    ...
    $providerEntity = getSymfonyContainer()->get('doctrine')->getRepository(\AwardWallet\MainBundle\Entity\Provider::class)->findOneBy(["code" => $providerCode]);
    ...
    $checker->logger->info('Check Confirmation Result', ['Header' => 2]);
	...
	echo '<strong>Log: </strong><a href="' . getSymfonyContainer()->get("router")->generate("aw_manager_loyalty_local_log", ["file" => $logName]) . '" target="_blank">'.$checker->http->LogDir.'/log.html</a> <small id="reltime"></small><br /><br />';
}
