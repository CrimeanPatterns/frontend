    ...
    try {
        ...
        if (!file_exists($sPath . "/../engine/" . $response->ProviderCode . "/functions.php")) {
            ...
            if ($server != LOCAL_SERVER_URL && getSymfonyContainer()->getParameter("selenium_consul_address") === null) {
                getSymfonyContainer()->get("aw.selenium_finder")->setServer("selenium-fixed.awardwallet.com");
                getSymfonyContainer()->get("aw.selenium_connector")->setNodeFinder(getSymfonyContainer()->get(\AwardWallet\WebdriverClient\NodeFinder::class));
            }
            ...
            try {
                for ($attempt = 1; $attempt <= $checkAttemptsCount && $attempt <= 7; $attempt++) {
                    try {
                        ...
                        $providerEntity = getSymfonyContainer()->get('doctrine')->getRepository(\AwardWallet\MainBundle\Entity\Provider::class)->findOneBy(["code" => $response->ProviderCode]);
                        ...
                        try {
                            ...
                            $checker->logger->error($e->getMessage(), ['HtmlEncode' => true]);
                        }
            ...
            $logName = FileName(basename($checker::archiveLogs($checker->http->LogDir, file_get_contents($checker->http->LogDir . "/log.html"))));
            ...
            echo '<strong>DisplayName: </strong>' . $response->DisplayName . '<br />
				  ...
				  <strong>Log: </strong><a href="' . getSymfonyContainer()->get("router")->generate("aw_manager_loyalty_local_log", ["file" => $logName]) . '" target="_blank">' . $checker->http->LogDir . '/log.html</a> <small id="reltime"></small><br /><br />
			';
...
function showAuth($endPoint, $server)
{
    ...
    $authUrl = $urlParts['scheme'] . '://' . $urlParts['host'] . '/api/oauth2/authorize.php?client_id=local&response_type=code&scope=debugProxy&state=' . strtolower($server) . '&redirect_uri=' . urlencode('http://awardwallet.docker/admin/save-debug-proxy-token');
