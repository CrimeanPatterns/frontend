...
$form = new TBaseForm([
    "ProviderCode" => [
        "Type" => "string",
        "Options" => ["" => ""] + SQLToArray("SELECT `Code` as ProviderID, `Code` FROM Provider WHERE CanCheckRewardAvailability <> 0 ORDER BY Code", "ProviderID", "Code"),
        "Required" => true,
    ],
    "ParseMode" => [
        "Type" => "string",
...
if ($form->IsPost && $form->Check()) {
    try {
        ...
        try {
            for ($attempt = 1; $attempt <= $checkAttemptsCount && $attempt <= $maxAttempts; $attempt++) {
                try {
                    ...
                    if (!empty($form->Fields["ReturnDate"]["Value"])) {
                        $returnDate = strtotime($form->Fields["ReturnDate"]["Value"], false);
                    } else {
                        $returnDate = null;
                    }
                    ...
                    //                if ($checker->http instanceof HttpBrowser)
                    //                    $checker->http->cleanup();
                    $checker->logger->info(var_export($checker->Answers ?? null, true), ['pre' => true]);
                }
            }
        } finally {
        ...
        $log = getSymfonyContainer()->get("router")->generate("aw_manager_loyalty_local_log", ["file" => $logName]);
