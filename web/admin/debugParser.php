...
if($form->IsPost && $form->Check()){
    try {
        $accountInfo = [
            ...
            "AccountID" => 0,
            "UserID" => $form->Fields["UserID"]["Value"],
        ...
        try {
            for ($attempt = 1; $attempt <= $checkAttemptsCount && $attempt <= $maxAttempts; $attempt++) {
                try {
                    ...
                    $providerEntity = getSymfonyContainer()->get('doctrine')->getRepository(\AwardWallet\MainBundle\Entity\Provider::class)->findOneBy(["code" => $form->Fields["ProviderCode"]["Value"]]);
