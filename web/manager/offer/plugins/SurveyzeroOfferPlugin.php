...
class SurveyzeroOfferPlugin extends OfferPlugin{
    public function searchUsers(){
        global $deltaId, $unitedId, $usairwaysId;
        $this->Log("Searching for Delta users with rank 0...");
        flush();
        set_time_limit(59);
        $u = 0;
        $users = getUsersByLevel($deltaId, 0);
        $exclude = array();
        ...
        foreach ($users as $user){
            if (!in_array($user, $exclude) && !in_array($user, $exclude2)){
                $q = new TQuery("select OfferUserID
                                 from OfferUser ou join Offer o on ou.OfferID = o.OfferID
                                 where UserID = $user AND Code = 'surveysplitters'");
                if ($q->EOF){
                    $params = array();
                    $params['URL'] = 'https://www.surveymonkey.com/s/Y9XCWGT?c='.$user;
