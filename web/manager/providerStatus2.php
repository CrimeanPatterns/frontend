...
while (!$q->EOF) {
    ...
    if ($loyaltyStat) {
        ...
        $q->Fields['ServerFailureRate'] = 100 - round($loyaltyStat->getSuccessCount() / $q->Fields['TotalCount'] * 100);
    }

    $q->Fields['ClientFailureRate'] = 100 - $q->Fields['SuccessRateBrowser'];
    $q->Fields['SortIndex'] = $q->Fields['Popularity'] * max($q->Fields['ClientFailureRate'], $q->Fields['ServerFailureRate']);
...
foreach ($rows as $row) {
    ...
    // --- Debug only ---
    $failureRate = sprintf('%d / %d', $row['ClientFailureRate'], $row['ServerFailureRate']);
    $sortIndex = number_format($row['SortIndex'], 0, '.', ' ');
...
<div class="dropdown-wrapper" data-provider-id="{$row['ProviderID']}">
    ...
    <ul class="dropdown-menu">
        ...
        <li class="divider"></li>
        <li><span>Failure rate: {$failureRate}</span></li>
        <li><span>Sort index: {$sortIndex}</span></li>
    </ul>
</div>
HTML;
