...
class TotalwineExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    ...
    public function isLoggedIn(Tab $tab): bool
    {
        ...
        $captcha = $tab->evaluate("//div[contains(@class, 'captcha-header')]", EvaluateOptions::new()->allowNull(true));

        if ($captcha) {
            $tab->showMessage(Message::MESSAGE_IDENTIFY_COMPUTER);
            $tab->evaluate('//input[@name="emailAddress"]', EvaluateOptions::new()->timeout(90));
        }
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        ...
        if (stripos($currentURL, "logout=true") !== false) {
            $tab->gotoUrl("https://www.totalwine.com/my-account");
        }
    ...
    public function parse(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
        ...
        $statement->addProperty("PointsToNextLevel", $tab->findText('//span[contains(@data-at, "loyalty-points-msg")]', FindTextOptions::new()->preg("/Collect ([\d\,\. ]+) point/ims")));
        // Status valid until
