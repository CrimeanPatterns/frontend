...
//        );
    ...
    public function Parse()
    {
        ...
        // Name
        $name = $this->waitForElement(WebDriverBy::xpath("//li[@class='loyallist-name'] | //h2[contains(text(), 'HI,')]/span"), 10);
        $this->saveResponse();
        $this->SetProperty('Name', beautifulName($name ? $name->getText() : $this->http->FindSingleNode("//li[@class='loyallist-name'] | //h2[contains(text(), 'HI,')]/span")));
        // Loyallist Number
        $this->SetProperty('LoyallistNumber', beautifulName($this->http->FindSingleNode("//p[@class='loyalty-number-value']")));

        // Available points
        $b = $this->waitForElement(WebDriverBy::xpath("//span[@class='lty-as-balance' and normalize-space(.) != '']"), 3);
        $this->saveResponse();

        if ($b) {
            $balance = $b->getText();
        } else {
            $balance = $this->http->FindSingleNode("//div[contains(@aria-valuetext, 'Value of loyalty tracker')]/@aria-valuetext", null, true, "/tracker (.+)/");
        }
        ...
        $this->SetProperty('PointsUntilNextRewardCard',
            $this->http->FindSingleNode('//div[contains(@class, "lty-next-award-text")]/following-sibling::div[contains(@class, "lty-reward-number")]')
            ?? $this->http->FindSingleNode('//div[contains(text(), "away from your next Reward")]', null, true, "/You're ([\.\,\d]+) points? away from your next Reward/ims")
        );
        // reward card balance
        $this->SetProperty('RewardCardBalance', $this->http->FindSingleNode('//div[contains(@class, "lty-reward-balance")]/following-sibling::div[contains(@class, "lty-reward-number")] | //h5[contains(text(), "REWARDS")]/following-sibling::div/div[last()]'));

        if ($pendingPoints = $this->waitForElement(WebDriverBy::xpath("//a[normalize-space(text()) = 'Pending points']"), 0)) {
            $pendingPoints->click();
            $this->waitForElement(WebDriverBy::xpath("//p[normalize-space(text()) = 'Total Pending Points']"), 3);
            $this->saveResponse();
        }

        // Pending Points
        // Base Points
        $this->SetProperty('BasePoints', $this->http->FindSingleNode('//div[p[contains(text(), "Base Points")]]/following-sibling::div/span | //div[p[contains(text(), "Base Points")]]/following-sibling::div/p'));
        // Bonus Points
        $this->SetProperty('BonusPoints', $this->http->FindSingleNode('//div[contains(text(), "Bonus Points")]/following-sibling::div/span | //div[p[contains(text(), "Bonus Points")]]/following-sibling::div/p'));
        // Power Points
        $this->SetProperty('PowerPoints', $this->http->FindSingleNode('//div[contains(text(), "Power Points")]/following-sibling::div/span | //div[p[contains(text(), "Power Points")]]/following-sibling::div/p'));
        // Total Pending Points
        $this->SetProperty('TotalPendingPoints', $this->http->FindSingleNode('//div[contains(text(), "Total Pending Points")]/following-sibling::div/span | //div[p[contains(text(), "Total Pending Points")]]/following-sibling::div/p'));
        ...
        $this->logger->debug("[Error]: {$this->http->currentUrl()}");
    }
