...
class ParkingConfirmation extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            ...
            'View Parking Pass' => ['View Parking Pass', 'View Reservations'],
        ],
    ];
    ...
    public function ParseParking(Email $email)
    {
        ...
        $conf = $this->http->FindSingleNode("//text()[starts-with(normalize-space(), 'Rental ID:')]", null, true, "/{$this->opt($this->t('Rental ID:'))}\s*(\d{5,})$/");

        if (empty($conf) && preg_match("/Your Parking Pass\s*[#]([A-Z\d]{5,})$/", $this->subject, $m)) {
            $conf = $m[1];
        }

        $p->general()
            ->confirmation($conf)
            ->status($this->http->FindSingleNode("//text()[{$this->starts($this->t('Reservation'))}]", null, true, "/{$this->opt($this->t('Reservation'))}\s+(\w+)/"));
