...
class TAccountCheckerSantander extends TAccountChecker
{
    ...
    public function InitBrowser()
    {
        ...
        $this->setProxyGoProxies(null, 'es'); // "Algo deu errado" workaround
    }
...
//        X-CCVisitId: -6c02aa33:16dc8d9382d:-30bc-129.80.155.72
    ...
    public function Login()
    {
        ...
        if ($message = $this->http->FindSingleNode("//span[@id = 'template-error' and normalize-space(.) != ''] | //div[@id = 'message-error-login' and not(contains(@class, 'util-hidden'))] | //span[contains(text(), 'Utilize um CPF ou CNPJ válido.') and not(contains(@class, 'util-hidden'))] | //p[contains(@class, 'modal-error-login-description')]")) {
            ...
            if (
                ...
                || $message == 'Confira os dados preenchidos ou recupere sua senha.'
            ) {
...
//            $selenium->disableImages();
            ...
            $loginBtn = $selenium->waitForElement(WebDriverBy::xpath("//a[@id = 'esfera-login-btn-occ'] | //span[@data-gtm-event-label = 'entre']"), 30);
            $this->savePageToLogs($selenium);

            if (!$loginBtn) {
                if ($this->http->FindSingleNode("//a[@id = 'esfera-login-btn-occ']/@id | //span[@data-gtm-event-label = 'entre']/@data-gtm-event-label")) {
                    $retry = true;
                }
            ...
            try {
                ...
                $selenium->driver->executeScript("document.querySelector('a#esfera-login-btn-occ, span[data-gtm-event-label = \"entre\"]').click();");
            }
//            $selenium->http->GetURL($url);
            ...
            if ($selenium->waitForElement(WebDriverBy::xpath("//a[@id = 'esfera-login-btn-occ'] | //span[@data-gtm-event-label = 'entre']"), 0)) {
                $this->savePageToLogs($selenium);
                $retry = true;
...
//            $pass->sendKeys($this->AccountFields['Pass']);
            ...
            $res = $selenium->waitFor(function () use ($selenium) {
                return
                    $selenium->waitForElement(WebDriverBy::xpath("//*[contains(text(), 'Olá,')] | //div[@id = 'message-error-login'] | //h2[contains(text(), 'Algo deu errado.')] | //div[@id = 'terms-and-conditions' and not(contains(@class, 'util-hidden'))] | //label[contains(text(), 'Declaro que li e aceito os')] | //div[@id='terms-and-conditions'] | //p[contains(@class, 'modal-error-login-description')]"), 0)
