...
class TAccountCheckerAmtrak extends TAccountChecker
{
    ...
    public function Parse($sessionStorage = null)
    {
        ...
        $this->setBalanceExpiration($sessionStorage);
    ...
    private function getHistoryData($sessionStorage = null)
    {
        $this->logger->notice(__METHOD__);
        $fromDate = (date('Y-m-d', strtotime('-2 years', strtotime('now'))));
        $toDate = (date('Y-m-d', strtotime('now')));
        $this->jsonHeaders['x-agr-session'] = $sessionStorage->global->uuid ?? null;
        $this->jsonHeaders['Referer'] = 'https://www.amtrak.com/guestrewards/account-overview/my-transactions.html';
        $this->jsonHeaders['Origin'] = null;
        $this->http->GetURL("https://www.amtrak.com/dotcom/member/transactions?agrNumber=$this->accountNumber&startDate={$fromDate}T21:00:00Z&endDate={$toDate}T20:59:59Z", $this->jsonHeaders);
    ...
    // refs #12609
    private function setBalanceExpiration($sessionStorage = null): void
    {
        ...
        if (!empty($sessionStorage->profile->transactionssummary)) {
            $this->sendNotification('check history // MI');
        }
        $this->history = $this->getHistoryData($sessionStorage);
