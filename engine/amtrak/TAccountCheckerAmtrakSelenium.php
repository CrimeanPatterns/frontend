...
//            $this->saveResponse();
    ...
    public function Login()
    {
        ...
        $this->waitForElement(WebDriverBy::xpath("
           ...
           | //div[contains(@id, 'password-error') and @style = 'display: block;']
           ...
           | //h1[contains(text(), 'Account Verification')]
        ...
        if ($message =
                $this->http->FindSingleNode("//div[contains(@class, 'error') and @aria-hidden=\"false\" and @style = 'display: block;']")
            ...
            if (
                $message == 'Something wrong with our System. Please try again'
                || $message == 'We are having trouble signing you in. Please try again later.'
            ) {
    ...
    public function parseQuestion()
    {
        ...
        $choice = $this->waitForElement(WebDriverBy::xpath("//*[@id='forgotpassword-simple--subheading']/*[self::div or self::h3][contains(text(),'Please select how you would like to receive your code')] | //h1[contains(text(), 'Account Verification')]"), 1);
        $delay = 0;

        if ($choice) {
            sleep(1);
            $this->saveResponse();
            $emailOption = $this->waitForElement(WebDriverBy::xpath('//label[@id="email_option"]'), 0);
            $continue = $this->waitForElement(WebDriverBy::xpath('//button[@id="continue"]'), 0);
...
//            $this->logger->debug(var_export($this->http->getDefaultHeaders(), true), ['pre' => true]);
    ...
    public function Parse()
    {
        ...
        $sessionStorage = $this->driver->executeScript("return sessionStorage.getItem('amtrak');");
        $sessionStorage = $this->http->JsonLog($sessionStorage);
        $amtrak->Parse($sessionStorage);
