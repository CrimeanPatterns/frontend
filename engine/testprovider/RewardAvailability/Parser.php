...
class Parser extends Success
{
    ...
    public function ParseCalendar(array $fields)
    {
        ...
        $this->logger->info(var_export($fields, true));
        if (in_array($fields['Currencies'][0], ['USD', 'AUD', 'RUB'])) {
            return $this->parseCalendarUSD($fields);
        }
        return ['fares' => []];
    }
    ...
    private function parseCalendarUSD(array $fields)
    {
        return [
            'fares' => [
                [
                    'date' => date('Y-m-d', $fields['DepDate'] - 60 * 60 * 24),
                    'redemptions' => ['miles' => 33000],
                    'payments'    => ['currency' => 'USD', 'taxes' => 3.3, 'fees' => null],
                    'cabin' => 'economy',
                    'brandedCabin' => 'Lowest possible',
                ],
                [
                    'date' => date('Y-m-d', $fields['DepDate']),
                    'redemptions' => ['miles' => 44000],
                    'payments'    => ['currency' => 'USD', 'taxes' => 4.4, 'fees' => null],
                    'cabin' => 'economy',
                    'brandedCabin' => 'Lowest possible',
                ],
                [
                    'date' => date('Y-m-d', $fields['DepDate'] + 60 * 60 * 24),
                    'redemptions' => ['miles' => 55000],
                    'payments'    => ['currency' => 'USD', 'taxes' => 5.5, 'fees' => null],
                    'cabin' => 'economy',
                    'brandedCabin' => 'Lowest possible',
                ],
            ]
