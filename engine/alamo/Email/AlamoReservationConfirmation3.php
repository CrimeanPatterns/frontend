...
class AlamoReservationConfirmation3 extends \TAccountChecker
{
    ...
    public static $dict = [
        'en' => [
            ...
            'Emerald Club #:'             => ['Emerald Club #:', 'Alamo Insiders', 'Emerald Club', 'Enterprise Plus'],
    ...
    private function parseEmail(Email $email): void
    {
        ...
        if (preg_match("/^({$this->opt($this->t('Your confirmation number is:'))})\s*([A-Z\d]{5,})$/", $confirmationText, $m)) {
            ...
            $confirmationTitle = $this->http->FindSingleNode("(//text()[{$this->eq($this->t('Your confirmation number is:'))}])[1]", null, true, "/^(.+?)(?:\s+is)?[\s:：]*$/u");
            $confirmation = $this->http->FindSingleNode("(//text()[{$this->eq($this->t('Your confirmation number is:'))}])[1]/following::text()[string-length(normalize-space())>1][1]", null, true, '/^[A-Z\d]{5,}$/');
        }
        ...
        if (preg_match("/{$this->opt($this->t('cancelledVariants'))}/iu", $r->getStatus())) {
            $r->setCancelled(true);
            $conf = $this->http->FindSingleNode("//text()[{$this->eq($this->t('The cancellation number is:'))}][1]/following::text()[string-length(normalize-space())>1][1]");
        ...
        $accounts = array_filter(preg_replace("/^\D+$/", '', $this->http->FindNodes("//text()[{$this->starts($this->t('Emerald Club #:'))}]/following::text()[string-length(normalize-space())>2][1]", null, false, '/^[A-Z\d]{7,}$/')));
