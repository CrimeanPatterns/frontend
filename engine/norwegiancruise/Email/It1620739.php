...
//            $this->year = $this->re("/{$this->opt($this->t('SAILING'))}\:\s*\d+\-\w+\-(\d{4})/u", $textPdf);
    ...
    {
        $taPos = strpos($text, $this->t('TRAVEL AGENT COPY'));
        $gPos = strpos($text, $this->t('GUEST COPY'));
        ...
        if (!empty($currency)) {
            ...
            if (preg_match("/{$this->opt($this->t('Guest Fare'))} *(-)? ?(\d[\d., ]*?)\s{2,}/", $header, $m)) {
                if (empty($m[1])) {
                    $c->price()
                        ->currency($currency)
                        ->cost(PriceHelper::parse(trim($m[2]), $currency));
                }
            } else {
                $c->price()
                    ->cost(null);
            ...
            if (preg_match("/{$this->opt($this->t('Guest Fare'))}.+\n([\s\S]+?)\n\s*{$this->opt($grossTotalName ?? $this->t('Gross Total'))} {2,}/", $header, $m)) {
                ...
                $commisionText = $this->re("/{$this->opt($this->t('Gross Total'))}(.+){$this->opt($this->t('Net Due'))}/su", $text);

                if (!empty($taPos)
                    && !empty($commisionText)
                    && preg_match("/^ *({$this->opt($this->t('Commission'))}) +(\d[\d,. ]+)\n/mu", $commisionText, $mat)) {
                    $c->price()->fee($mat[1], PriceHelper::parse(trim($mat[2]), $currency));
                }
