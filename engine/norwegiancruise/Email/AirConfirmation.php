...
{
    public $mailFiles = "norwegiancruise/it-261040374.eml, norwegiancruise/it-268917991.eml, norwegiancruise/it-473673768.eml, norwegiancruise/it-558127543.eml, norwegiancruise/it-848839918.eml, norwegiancruise/it-856313915.eml, norwegiancruise/it-906157294.eml";
    ...
    private function parseEmailPdf(Email $email, ?string $textPdf = null)
    {
        ...
        if (preg_match_all("/\n {0,5}{$this->opt($this->t('Conf #'))} {2,}Airline +.+\s*((?:\n+ {0,5}[A-Z\d]{5,7} {2,}.*|\n+ {12,}.*)+)\n\n/", $textPdf, $m)) {
            $m[1] = preg_replace("/\n+\s+Flight Schedule Change Notification/", "", $m[1]);
            $this->logger->debug(var_export($m[1], true));
        ...
        foreach ($segments as $sText) {
            ...
            //it-906157294.eml
            $depInfo = $s->getAirlineName() . $s->getFlightNumber() . $s->getDepDate();

            if (in_array($depInfo, $this->flightArray)) {
                $f->removeSegment($s);
            } else {
                $this->flightArray[] = $depInfo;
            }
        }
