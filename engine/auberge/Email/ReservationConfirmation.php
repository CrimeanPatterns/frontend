...
{
    public $mailFiles = "auberge/it-104282962.eml, auberge/it-141750203.eml, auberge/it-166651698.eml, auberge/it-198400618.eml, auberge/it-688394559.eml, auberge/it-85221835.eml, auberge/it-85460525.eml, auberge/it-86642184.eml, auberge/it-908144757.eml, auberge/it-91712077.eml";
    public $subjects = [
    ...
    public static $dictionary = [
        "en" => [
            'RESERVATION INFORMATION' => ['RESERVATION INFORMATION', 'Reservation Details', 'RESERVATION', 'Reservation'],
            ...
            'nameEnd'                 => [
                ...
                'to experience a new generation',
                ', Auberge Resorts Collection, for a',
                ', Auberge Resorts Collection. Please take a moment to review',
                ';',
                ', Auberge Resorts Collection',
                ', synonymous with warm', ],

            'Reservation #'             => ['Reservation #', 'Reservation Number', 'Confirmation Number', 'Confirmation Numb', 'Confirmation:', 'RESERVATION NO.'],
            ...
            'Nightly Rate'              => ['Nightly Rate', 'Average Nightly Rate', 'Average Daily Rate', 'NIGHTLY RATE', 'AVERAGE NIGHTLY RATE'],
            'Total'                     => ['Total', 'Room Total - Excludes Taxes & Fees', 'Total with Taxes', 'Reservation Total', 'Grand Total*', 'TOTAL'],
            'Accommodations'            => ['Accommodations', 'Accommodation'],
            'Auberge'                   => ['Auberge', '@aubergeresorts'],
            'Adults'                    => ['Adults', 'adult(s)', 'ADULTS'],
            'Guest Name'                => ['Guest Name', 'Reserved For:', 'GUEST NAME'],
    ...
    public function ParseHotel2(Email $email): void
    {
        ...
        $cancellationNumber = $this->http->FindSingleNode("//text()[normalize-space() = 'Cancellation Number']/following::text()[normalize-space()][1]", null, true, "/^(\d+)$/");
        ...
        if (!empty(trim($cancellation, ':')) && $cancellationNumber !== $cancellation) {
            $h->general()
        ...
        if (empty($inDate) && empty($inTime)) {
            if (!empty($startDate = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Nightly Rate'))}]/ancestor::td[1]/following::td[1]/descendant::text()[normalize-space()][1]", null, true, "/^(\d{1,2}\/\d{1,2}\/\d{1,2})\s/"))) {
        ...
        $roomType = str_replace([$this->t('Room Type'), ':'], "", $this->http->FindSingleNode("//text()[{$this->starts($this->t('Room Type'))} or {$this->starts($this->t('ROOM TYPE'))}]/following::text()[normalize-space()][1]/ancestor::td[1]", null, true, "/\:?\s*(.+)/"));
        ...
        if (empty($rate)) {
            $rate = implode('; ', $this->http->FindNodes("//text()[{$this->starts($this->t('Nightly Rate'))}]/following::*[normalize-space()][1]/descendant::text()[normalize-space()]"));
        }
        ...
        if (empty($h->getGuestCount())) {
            $guests = $this->http->FindSIngleNode("//text()[{$this->eq($this->t('Adults'))}]/following::text()[normalize-space()][1]", null, true, "/^(\d+)$/");
            ...
            $kids = $this->http->FindSIngleNode("//text()[{$this->eq($this->t('Children'))}]/following::text()[normalize-space()][1]", null, true, "/^(\d+)$/");
    ...
    private function detectDeadLine(Hotel $h, $cancellationText): void
    {
        ...
        || preg_match("#Non changeable Non refundable#i", $cancellationText, $m)
