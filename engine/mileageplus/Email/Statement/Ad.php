    ...
    protected function ParseEmail(Email $email)
    {
        ...
        if (empty($number)) {
            $number = $this->http->FindSingleNode("(//text()[contains(., 'XXXX') and starts-with(., 'MileagePlus')])[1]",
                null, true, "/MileagePlus\s*\#\s*X+(\d+)$/u");
        }

        if (empty($number)) {
            $number = $this->http->FindSingleNode("(//text()[starts-with(normalize-space(), 'MileagePlus')]/ancestor::td[1][contains(normalize-space(), 'XXXXX')])[1]",
                null, true, "/MileagePlus\s*\#\s*X+(\d+)$/u");
