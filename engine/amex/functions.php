...
//        }
    ...
    public function selenium($url = null)
    {
        ...
        try {
            ...
            if ($fingerprint !== null) {
                ...
                if ($this->AccountFields['Login'] == 'veresch80') {
                    $this->State['fingerprint'] = $selenium->seleniumOptions->fingerprint;
                    $this->State['setUserAgent'] = $fingerprint->getUseragent();
                }
            }
            ...
            $selenium->http->GetURL($url ?? "https://www.americanexpress.com/en-us/account/login?inav=iNavLnkLog");
            // login
            ...
            $rememberMe = $selenium->waitForElement(WebDriverBy::xpath('//label[@for="rememberMe"]'), 0);
            // Sign In
        ...
        if (
            ...
            if (
                ...
                foreach ($response->challengeQuestions as $challengeQuestion) {
                    ...
                    elseif (!in_array($deliveryMethod, ['EMAIL', 'SMS']) && $challengeQuestion->category == 'OTP_VOICE') {
                        $deliveryMethod = 'VOICE';
                        $device = $challengeQuestion->challengeOptions[0]->maskedValue;
                        $this->logger->notice("phone was found: {$device} OTP");
                        $question = str_replace("was sent to you.", "was sent to {$device}.", self::QUESTION_TSC);
                        $encryptedValue = $challengeQuestion->challengeOptions[0]->encryptedValue;
                    }
                }// foreach ($response->challengeQuestions as $challengeQuestion)
...
//                $this->sendNotification("Identification code was entered");
    ...
    public function ParseNorthAfrica()
    {
        ...
        if ($rewardURL = $this->http->FindSingleNode("(//a[contains(@id, 'wpf_action_ref_0onlsportletsAccountSummary')]/@href)[1]")) {
            ...
            $this->waitForElement(WebDriverBy::xpath("//a[@id = 'membership']"), 0)
            || !$this->http->FindNodes("//div[@class = 'olsr_cnt_db_wide2']")
        ) {
            $this->logger->notice("open balance page");
    ...
    public function ParseUSA()
    {
        ...
        */
        if (
            ...
            throw new CheckException("Sorry, we are unable to display this account right now. Please try again later.",
        ...
        $blueboxpublic = $this->http->getCookieByName('blueboxpublic');
        $axp_loyalty_summary =
...
//        $amexOffers = [];
        ...
        if ($countOfAccounts === 0) {
            ...
            if ($this->http->FindPreg("/errors\":\[\{\"error_code\":\"component_error\",\"message\":\"ErrorCode = IC_SYSTEM_SSI_ERROR Message = Failure during logon: Unable to connect to SSI : ssiprod.web.ipc.us.aexp.com\"/")) {
                throw new CheckException("Sorry, your rewards information is unavailable at this time. Please try again later.",
                    ACCOUNT_PROVIDER_ERROR);
            }
        }
        ...
        foreach ($accounts as $account) {
            ...
            // refs #24973
            $isSpecialCharacter = (strstr($account_token, "%") || strstr($account_token, '_')
                || strstr($account_token, '(') || strstr($account_token, ')')
                || strstr($account_token, '{') || strstr($account_token, '}')
                || strstr($account_token, '^') || strstr($account_token, "'")
                || strstr($account_token, '[') || strstr($account_token, ']')
                || strstr($account_token, ',') || strstr($account_token, "~"));
            $subAccount = $detectedCard = [
                'Code' => 'amex' . ($isSpecialCharacter ? md5($account_token) : $account_token) . $code,
            ...
            if ((isset($account['status']['card_status'][0]) && (in_array($account['status']['card_status'][0],
                        ['Cancelled', 'Canceled'])))
                || (isset($account['status']['account_status'][0]) && (in_array($account['status']['account_status'][0],
                        ['Cancelled', 'Canceled'])))) {
                // detected cards
            ...
            if ($account_token) {
                ...
                if (count($accounts) == 1) {
                    $this->http->GetURL("https://global.americanexpress.com/api/servicing/v1/arbitration",
                        array_merge(["account_token" => $account_token], $headers));
                    ...
                    if ($this->http->Response['code'] == 404) {
                        ...
                        $this->http->PostURL("https://functions.americanexpress.com/ReadArbitrationForAccounts.v1",
                            "{\"accountTokens\":[\"{$account_token}\"],\"productClass\":\"\"}", $postHeaders);
                    } else {
                        $this->http->GetURL("https://global.americanexpress.com/api/servicing/v2/layout?view=%2Fdashboard",
                            array_merge(["account_token" => $account_token], $headers));
                    }
                }// if (count($accounts) == 1)
...
//                $this->http->GetURL("https://global.americanexpress.com/api/servicing/v1/loyalty/details", array_merge(["account_tokens" => $account_token], $headers));
                $this->http->PostURL("https://functions.americanexpress.com/ReadLoyaltyAccounts.v1",
                    '{"accountTokens":["' . $account_token . '"]}', $headers);
                ...
                if ($this->http->Response['code'] == 401) {
                    ...
                    if (
                        ...
                        $this->http->GetURL("https://global.americanexpress.com/myca/reauth/intl/riskScoringController.do?request_type=authunreg_riskScoring&Details=true&Face={$this->lang}&destPage=https%3A%2F%2Fglobal.americanexpress.com%2Fdashboard",
                            ['Referer' => "https://global.americanexpress.com/dashboard"]);
                        $this->http->RetryCount = 0;
                        $this->http->GetURL("https://global.americanexpress.com/api/servicing/v1/loyalty/details",
                            array_merge(["account_tokens" => $account_token], $headers));
                        $this->http->RetryCount = 2;
                        $response = $this->http->JsonLog(null, 3, true);
                    }
                }// if ($this->http->Response['code'] == 401)
                ...
                if (isset($response[0])) {
                    ...
                    $account_number = ArrayVal($response[0], 'account_number',
                        ArrayVal($response[0], 'loyaltyAccountIdentifier', null));
                    ...
                    // new
                    $loyalty_linked_cards_list = ArrayVal($response[0], 'loyalty_linked_cards_list',
                        ArrayVal($response[0], 'relationships', []));
                    // deprecated
                    $linked_account = ArrayVal($response[0], 'linked_account', null);
                    $primary_account = ArrayVal($linked_account, 'primary_account', null);

                    // refs #20382, do not duplicate amex cards with MR balance for Australia
                    $balance_account_key = ArrayVal($response[0], 'account_key',
                        ArrayVal($response[0], 'accountToken', null));
                }
                    ...
                    if (isset($program_code, $account_number) && $program_code != 'cobrand' /*AccountID: 4501448 */) {
                        ...
                        switch ($program_code) {
                            case 'HLTN':
                                ...
                                $benefitSubAccounts = array_merge($benefitSubAccounts,
                                    $this->progressRewards($displayName, $code, $account_token));

                                // refs #20852
                                $originalCard['HistoryRows'] = $this->parseSubAccHistory($originalCard['Code'],
                                    array_merge(["account_token" => $account_token], $headers),
                                    $originalCard['Code'] . 'Analysis', $originalCard['DisplayName'], true);
                            ...
                            case 'CAL':// Membership Rewards® // AccountID: 8073548
                                $detectedCard['CardDescription'] = C_CARD_DESC_ACTIVE;
                                ...
                                if (!empty($loyalty_linked_cards_list)) {
                                    foreach ($loyalty_linked_cards_list as $loyalty_linked_card) {
                                        if (
                                            ...
                                            $detectedCard['CardDescription'] = sprintf(C_CARD_DESC_AMEX_LINKED,
                                                $subAccount['Number']);
                                            ...
                                            if (in_array($program_code, ['USMR', 'USPR', 'USCM'])) {
                                                ...
                                                $originalCard['HistoryRows'] = $this->parseSubAccHistory($historyCode,
                                                    array_merge(["account_token" => $account_token], $headers),
                                                    $originalCard['Code'], $originalCard['DisplayName']);
                                            }
                                            ...
                                            $this->logger->debug(var_export($loyalty_linked_card, true),
                                                ['pre' => true]);
...
//                                            break;// refs #24774
                                ...
                                if (isset($balance_account_key)) {
                                    ...
                                    if (
                                        ...
                                        && (
                                            ...
                                            || in_array($program_tier . '|' . $balance . "|" . $program_code,
                                                $collectedCards)
                                        )
                                        && (
                                            (
                                ...
                                if ($detectedCard['CardDescription'] == C_CARD_DESC_ACTIVE) {
                                    ...
                                    if (in_array($program_code, ['USMR', 'USPR', 'USCM', 'GB9'])) {
                                        ...
                                        $originalCard['HistoryRows'] = $this->parseSubAccHistory($subAccount['Code'],
                                            array_merge(["account_token" => $account_token], $headers),
                                            $originalCard['Code'], $originalCard['DisplayName']);
                                    } else {
                                        // refs #20852
                                        $subAccount['HistoryRows'] = $this->parseSubAccHistory($originalCard['Code'],
                                            array_merge(["account_token" => $account_token], $headers),
                                            $originalCard['Code'], $originalCard['DisplayName']);
                                    }
                                    ...
                                    if (
                                        ...
                                        && in_array($program_code, ['USMR', 'USPR', 'GB9', 'AUB08', 'CAL'])
                                        && (
...
//                                            || ($this->AccountFields['Login'] == 'surfman49' && $subAccount['Code'] == 'amexCZG19YSNJ3CXKYX1M19849285')
                                        ...
                                        if (in_array($program_code, ['USMR', 'USPR', 'USCM'])) {
                                            ...
                                            $this->http->RetryCount = 0;
                                            $this->http->PostURL("https://functions.americanexpress.com/ReadLoyaltyTransactionSummaries.v1",
                                                json_encode($data));
                                            $this->http->RetryCount = 2;
                                    ...
                                    if (in_array($program_code, [
                                            ...
                                            'CAL', // MR in Canada, AccountID: 6466356
                                        ])
                                        && $skipBalance === false
                                    ) {
                                        // refs #25086
                                        ...
                                        */
                                        ...
                                        if (
                                            ...
                                            $this->sendNotification("refs #16147. Need to check summary (program_code: {$program_code})",
                                                "awardwallet");
                                        }
                                    }
                                }// if ($detectedCard['CardDescription'] == C_CARD_DESC_ACTIVE)
                    ...
                    if ((isset($line_of_business_type) && in_array($line_of_business_type,
                                ['COMPANY_CARD', 'CORPORATE']) && count($accounts) == 1)
                        ...
                        if (
                            ...
                            $detectedCard['CardDescription'] = str_replace(['[Program]', '[Program_ID]'],
                                ['Air Canada Aeroplan (Altitude)', 2], C_CARD_DESC_UNIVERSAL);
                        }
...
//                            return false;
                    ...
                    // refs #20852
                    $originalCard['HistoryRows'] = $this->parseSubAccHistory($originalCard['Code'],
                        array_merge(["account_token" => $account_token], $headers), $originalCard['Code'],
                        $originalCard['DisplayName'], true);
                    $this->AddSubAccount($originalCard);
                }// $this->logger->notice("balance not found");
...
//                    $this->logger->info("Airline Fee Credit: {$displayName} *{$code}", ['Header' => 3]);
                    ...
                    $this->http->GetURL("https://global.americanexpress.com/card-benefits/view-all?account_key={$account_key}",
                        $headers);
                    /*
                    $headers = [
                    ...
                    */
                    $airlineFeeCredit = true;
                    $benefitSubAccounts = array_merge($benefitSubAccounts,
                        $this->parseCreditCards($displayName, $code, $account_token, $airlineFeeCredit));
                ...
                // FICO links refs#25647
                if (empty($ficoScoreURL)) {
                    $headers = [
                        "Accept" => "application/json",
                        "Accept-Language" => "en-US,en;q=0.5",
                        "Accept-Encoding" => "gzip, deflate, br",
                    ...
                    $this->http->PostURL("https://functions.americanexpress.com/ReadLinksForAccount.v1", $data,
                        $headers);
                    $readLinks = $this->http->JsonLog();
                    foreach ($readLinks as $readLink) {
                        if (isset($readLink->url, $readLink->label)) {
                            if (stristr($readLink->label, 'Your FICO® Score')) {
                                $ficoScoreURL = $readLink->url;
                                break;
                            }
                        }
                    }
        ...
        if (!empty($this->detectedCards)) {
            ...
            foreach ($linked_card as $key => $value) {
                ...
                // refs #21658
                if (array_search($linked_card[$key]["DisplayName"],
                    array_column($this->Properties['SubAccounts'], 'DisplayName'))) {
...
//                    }// if (isset($history[$this->Properties['SubAccounts'][$i]['Code']]))
        ...
        try {
            if (isset($ficoScoreURL)) {
                //if ($this->AccountFields['Login'] == 'veresch80') {
                // https://pieventscapture.americanexpress.com/personalization/v1/treatments/customers/events?client_id=MYCA&channel_id=WEB&locale=en-US&page_id=Dashboard&name=USUSCSBenefitAwarenessAwarenessUSCS_MCGBAULink_Q22024_UsefulLinks_Charge_CR0000022371300c&event_name=Clicked&placement_id=UsefulLinks&pub_guid={$blueboxpublic}&locationURL=https://freecreditscore.americanexpress.com/mycreditguide/enroll.do
                $this->logger->debug("[FICO® Score URL]: $ficoScoreURL");
                $result = $this->ficoSelenium($ficoScoreURL);
                /*$this->sendNotification('FICO® Score // MI');
                /*$this->http->RetryCount = 0;
                ...
                $this->http->GetURL("https://pieventscapture.americanexpress.com/personalization/v1/treatments/customers/events?client_id=MYCA&channel_id=WEB&locale=en-US&page_id=Dashboard&name=USUSCSBenefitAwarenessAwarenessUSCS_MCGBAULink_Q22024_UsefulLinks_Charge_CR0000022371300c&event_name=Clicked&placement_id=UsefulLinks&pub_guid={$blueboxpublic}&locationURL=https://freecreditscore.americanexpress.com/mycreditguide/enroll.do", $headers);

                $this->http->GetURL("https://mycreditguide.americanexpress.com/amex/auth?code=6d0ec15ff3c045fa996088ad708818a5&authCode=6d0ec15ff3c045fa996088ad708818a5&state=ca65039c-3cc8-4fff-98a9-f48581963a62");*/
                if ($result) {
                    $headers = [
                        "Accept" => "*/*",
                        "Referer" => "https://mycreditguide.americanexpress.com/",
                    ];
                    $this->http->RetryCount = 0;
                    $this->http->GetURL("https://mycreditguide.americanexpress.com/api/v1/credit/scores?exclude=ExcludeAll&filter=All&values=",
                        $headers);
                    $this->http->RetryCount = 2;
                    $scores = $this->http->JsonLog();
                    if (is_array($scores)) {
                        //$this->sendNotification('FICO® Score // MI');

                        $date = 0;
                        foreach ($scores as $score) {
                            $ficoUpdatedOn = strtotime($this->http->FindPreg('/(\d{4}-\d+-\d+)T/', false,
                                $score->score_date ?? null), false);
                            if ($ficoUpdatedOn > $date) {
                                $date = $ficoUpdatedOn;
                                $this->AddSubAccount([
                                    "Code" => "amexFICO",
                                    "DisplayName" => "FICO® Score 8 (Experian)",
                                    "Balance" => $score->score,
                                    "FICOScoreUpdatedOn" => date('m/d/Y', $ficoUpdatedOn),
                                ]);
                            }
                        }
                    }
...
//            $this->AddSubAccount($amexOffer);
    ...
    public function ficoSelenium($url = null)
    {
        $this->logger->notice(__METHOD__);
        $allCookies = array_merge($this->http->GetCookies(".americanexpress.com"), $this->http->GetCookies(".americanexpress.com", "/", true));
        $allCookies = array_merge($allCookies, $this->http->GetCookies("www.americanexpress.com"), $this->http->GetCookies("www.americanexpress.com", "/", true));
        $selenium = clone $this;
        //$selenium->seleniumOptions->recordRequests = true;
        ...
        try {
            ...
            $selenium->useCamoufox();

            if (isset($this->State['fingerprint'], $this->State['setUserAgent'])) {
                $selenium->seleniumOptions->fingerprint = $this->State['fingerprint'];
                $selenium->http->setUserAgent($this->State['setUserAgent']);
            }

            $selenium->http->start();
            $selenium->Start();
            $selenium->http->GetURL("https://www.americanexpress.com/us/homepage/error.html");


            //$selenium->http->GetURL("https://pieventscapture.americanexpress.com/personalization/");
            foreach ($allCookies as $key => $value) {
                $selenium->driver->manage()->addCookie(['name' => $key, 'value' => $value, 'domain' => '.americanexpress.com']);
                $this->logger->debug("Set-Cookie: {$key} => {$value};");
            }
            /*$selenium->http->GetURL("https://global.americanexpress.com/dashboard");
            sleep(10);*/
            $this->savePageToLogs($selenium);

            $selenium->http->GetURL($url);

            $selenium->waitForElement(WebDriverBy::xpath('//h1[contains(.,"Score for FREE is a click away")]
                | //iframe[@id="main-iframe"]'), 15);


            $terms = $selenium->waitForElement(WebDriverBy::xpath('//h1[contains(.,"Score for FREE is a click away")]'), 0);
            $this->savePageToLogs($selenium);

            if ($terms) {
                $this->logger->warning('accept terms');
                return false;
            }

            sleep(3);
            $iframeParent = $selenium->waitForElement(WebDriverBy::xpath('//iframe[@id="main-iframe"]'), 3);
            if ($iframeParent) {
                $this->sendNotification('FICO® Score -> captcha // MI');
                $selenium->driver->switchTo()->frame($iframeParent);
                $iframe = $selenium->waitForElement(WebDriverBy::xpath('//iframe[contains(@title,"hCaptcha")]'), 0);
                if ($iframe) {
                    $selenium->driver->switchTo()->frame($iframe);
                    $selenium->waitForElement(WebDriverBy::xpath('//div[@id="checkbox"]'), 5)->click();
                    // wait for green mark
                    $selenium->waitForElement(WebDriverBy::xpath('//img[@alt="Check mark"]'), 10);
                    sleep(10);
                    $this->savePageToLogs($selenium);
                }
...
//                $this->sendNotification("refs #20852 need to check history // RR");
            ...
            case 'Avios to be transferred':
                $this->logger->notice(">>> Skip British card");
                $detectedCard['CardDescription'] = str_replace(['[Program]', '[Program_ID]'], ['British Airways (Club)', 31], C_CARD_DESC_UNIVERSAL);
                break;
...
//                $subAccount['HistoryRows'] = $this->parseSubAccHistory($subAccount['Code'], array_merge(["account_token" => $account_token], $headers), $subAccount['Code'], $subAccount['DisplayName'], true);
        ...
        if (!empty($response) && $this->http->Response['code'] != 400) {
            foreach ($response as $nightInfo) {
                ...
                && isset($nightInfo->remaining)
                    ...
                    $displayName = "Spent until a Free Hilton Night Reward *{$code}";

                    if (in_array($nightInfo->name, [
                        'Earn a $100 Delta Flight Credit',
                    ])) {
                        $displayName = "Spent until a $100 Delta Flight Credit *{$code}";
                    }
    ...
    // refs #17046
    public function parseCreditCards($displayName, $code, $account_token, $airlineFeeCredit)
    {
        $this->logger->notice(__METHOD__);
        ...
        if ($responseEntertainments) {
            foreach ($responseEntertainments as $responseEntertainment) {
                ...
                foreach ($responseEntertainment->trackers as $tracker) {
                    if (
                        (
                            !in_array($tracker->benefitId, [
                                ...
                                "cons-gold-resy-restaurant-credit", // refs #25354
                                "cons-gold-dunkin-credit", // refs #25354
                                "hilton-statement-credit", // refs #25354
                                "walmart-platinum-tracker", // refs #25428
                            ])
                            ...
                            && !strstr($tracker->benefitName, "Delta Sky Club Access") // refs #25428
                        )
                    ...
                    $benefitDisplayName = str_replace("hilton-statement-credit", '$200 Hilton Statement Credit', $benefitDisplayName);
                    $benefitTargetBalance =
                        $this->http->FindPreg("/([\d.,]+) (?:CLEAR Credit|Equinox Credit)/", false, $benefitDisplayName)
                    ...
                    if (
                        ...
                        || strstr($benefitDisplayName, "Digital Entertainment Credit")// refs #24899, refs #25250
                        || $tracker->benefitName == "cons-gold-dunkin-credit" // refs #25354
                        || $tracker->benefitName == "walmart-platinum-tracker" // refs #25428
                        || strstr($benefitDisplayName, "0 Hilton Credit") // refs #25532
                        || strstr($benefitDisplayName, "0 Hilton Statement Credit") // refs #25532
                        || strstr($benefitDisplayName, "Dunkin' Credit") // refs #25354
                    ) {
                    ...
                    $this->logger->debug("benefitBalance: $benefitBalance");

                    if ($benefitBalance > 0
                        || stristr($benefitDisplayName, "Centurion Lounge Complimentary Guest Access")
                        || stristr($benefitDisplayName, "Saks Credit")
                        || stristr($benefitDisplayName, "Dell Statment Credit")
                        || stristr($benefitDisplayName, "Flight Credit")
                        || stristr($benefitDisplayName, "Hilton Resort Credit")
                        || stristr($benefitDisplayName, "Earn Free Night Rewards")
                        || strstr($benefitDisplayName, "Walmart+ Monthly Membership Credit")) {
                        // refs #22815
                        ...
                        */
                        // refs #24984
                        $expirationDate = $trackerDuration === 'CalenderYear' ? strtotime("31 Dec") :
                            strtotime("first day of next month", strtotime($tracker->periodEndDate));
                        $benefitDisplayName = preg_replace('/^\$\d+\s+/', '', $benefitDisplayName);
                        $displayNameHash = "$benefitDisplayName - card ending in {$code}";
                        $displayName = "$benefitDisplayName *{$code}";
                        $spentSoFar = $remainingSpend = $totalSpendRequired = $spendDeadline = $remainingVisits = $accessThrough = $available = null;
                        $allowNAInBalance = false;
                        $currency = '$';

                        // refs#24984, #25255
                        if (stristr($benefitDisplayName, "Saks Credit")
                            || stristr($benefitDisplayName, "Dell Statment Credit")
                            || stristr($benefitDisplayName, "Flight Credit")
                            || stristr($benefitDisplayName, "Hilton Resort Credit")
                            || stristr($benefitDisplayName, "Hilton Statement Credit")
                        ) {
                            /*if (strstr($benefitDisplayName, "Flight Credit")) {
                                $this->sendNotification('check expDate for Flight Credit // MI');
                            }
                            if (strstr($benefitDisplayName, "Hilton Resort Credit")) {
                                $this->sendNotification('check expDate for Hilton Resort Credit // MI');
                            }*/

                            $benefitBalance = $tracker->tracker->remainingAmount;
                            if ($expDate = $this->http->FindPreg('/^(\d+-\d+-\d+)/', false,
                                $tracker->periodEndDate ?? null)) {
                                $this->logger->debug('expDate: ' . $expDate);
                                // 2025-06-30 to 2025-07-01
                                if ($expDate = strtotime($expDate)) {
                                    $this->logger->debug('expDate: ' . date("Y-m-d H:i:s", $expDate));
                                    $expirationDate = strtotime("first day of next month", $expDate);
                                }
                                $this->logger->debug('expirationDate: ' . date("Y-m-d H:i:s", $expirationDate));
                                /*if (date('m') == '07')
                                    $this->sendNotification('refs#24984, #25255 check expDate for Credit // MI');*/
                            }
                        }
                        elseif (
                            stristr($benefitDisplayName, "Centurion Lounge Complimentary Guest Access")
                            || stristr($benefitDisplayName, "Delta Sky Club Unlimited Access")
                            || stristr($benefitDisplayName, "Earn Free Night Rewards")
                            || stristr($benefitDisplayName, "Unlimited Delta Sky Club Access")
                        ) {
                            ...
                            if ($tracker->tracker->spentAmount >= $tracker->tracker->targetAmount) {
                                ...
                                if ($expDate = strtotime($this->http->FindPreg('/^(\d+-\d+-\d+)T/', false,
                                    ...
                                    $this->logger->debug('ExpirationDate1 '. date('Y-m-d', $expDate));
                                    $expirationDate = strtotime('+1 day', $expDate);
                                    $this->logger->debug('ExpirationDate2 '. date('Y-m-d', strtotime('+1 day', $expirationDate)));
                                    $this->sendNotification('check ExpirationDate +1 day // MI');
                                }
                                $available = 'Available';
                                // https://redmine.awardwallet.com/issues/25466#note-2
                                if(stristr($benefitDisplayName, "Earn Free Night Rewards")) {
                                    // Free Night Reward *51008: 1
                                    $displayName = "Free Night Reward *$code";
                                    $benefitBalance = 1;
                                    $currency = "";
                                    $this->sendNotification('Free Night Reward 1 // MI');
                                }
                                ...
                                // refs#25412
                                $displayNameHash = "$benefitDisplayName *$code";
                            }
                        ...
                        // refs #25428
                        elseif (strstr($tracker->benefitName, "Delta Sky Club Access"))
                        {
                            //$this->sendNotification('Delta Sky Club Access // MI');
                            $expirationDate = null;
                            if ($expDate = strtotime($this->http->FindPreg('/^(\d+-\d+-\d+)T/', false, $tracker->periodEndDate ?? null))) {
                                $expirationDate = strtotime('first day of next month', $expDate);
                            }
                            $accessThrough = strtotime($this->http->FindPreg('/^(\d+-\d+-\d+)T/', false, $tracker->periodEndDate?? null));
                            // <b>10 of 10</b> visits available through Jan 31, 2026.</br></br>Spend $75,000 in eligible purchases by  Dec 31, 2025 to earn unlimited access through Jan 31, 2027. Terms apply.
                            $remainingVisits = strtotime($this->http->FindPreg('#<b>(\d+) of \d+</b> visits available through#', false, $tracker->progress->message));
                            $benefitBalance = number_format($tracker->tracker->remainingAmount, 2);
                            $currency = "";
                            $displayName = "$benefitDisplayName *$code";
                            // refs#25412
                            $displayNameHash = "$benefitDisplayName *$code";
                        ...
                        // refs #25428
                        elseif (strstr($benefitDisplayName, "Walmart+ Monthly Membership Credit"))
                        {
                            $expirationDate = null;
                            if ($expDate = strtotime($this->http->FindPreg('/^(\d+-\d+-\d+)T/', false, $tracker->periodEndDate ?? null))) {
                                $expirationDate = strtotime('first day of next month', $expDate);
                            }
                            $benefitBalance = number_format($tracker->tracker->targetAmount, 2);
                            $displayNameHash = $displayName = "Walmart+ Membership Credit *$code";
                        }

                        // refs #24984
                        $benefitSubAccounts[] = [
                            'Code'                   => 'amex' . ucfirst(str_replace(' ', '', $tracker->benefitName)) . md5($displayNameHash) . $code,
                            'DisplayName'            => $displayName,
                            'Balance'                => $benefitBalance,
                            //$propertyKey             => "$" . $totalSavingsYearToDate,
                            'Available'              => $available,
                        ...
                        if ($responseEntertainments) {
                            foreach ($responseEntertainments as $responseEntertainment) {
                                ...
                                foreach ($responseEntertainment->trackers as $tracker) {
                                    ...
                                    // refs #24984
                                    if (strstr($benefitDisplayName, "Centurion Lounge Complimentary Guest Access")
                                        || strstr($benefitDisplayName, "Delta Sky Club Unlimited Access")) {
                                        //$this->sendNotification('Guest Access // MI');
                                        $benefitBalance = null;
                                        if ($tracker->tracker->spentAmount >= $tracker->tracker->targetAmount) {
                ...
                if ($benefitName == "dining-credit") {
                    ...
                    if (isset($airlineNameInfo->data->enrollmentContent->enrollmentStatusById->code)
                        ...
                        if (
                            ...
                            || !isset($airlineNameInfo[0]->trackCenturion Lounge Complimentaryers[0]->status)
