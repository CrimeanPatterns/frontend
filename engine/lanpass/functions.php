...
//        $this->http->GetURL("https://www.latamairlines.com/us/en/my-trips");
    ...
    public function CheckConfirmationNumberInternal($arFields, &$it)
    {
        ...
        $url = $this->seleniumRetrieve($this->ConfirmationNumberURL($arFields), $arFields);
...
//        }
       /* $this->State["x-latam-app-session-id"] = 'd2d533e4-c525-43af-bb98-78d6ed5ec2db';
        ...
        $orderId = $this->http->FindPreg('/orderId=(\w+)&/', false, $url);
        $this->lastName = $arFields['LastName'];
        $message = $this->parseItinerary($orderId, urlencode($arFields['LastName']));
...
//            $selenium->useCache();
            ...
            sleep(1);
            $code = $selenium->waitForElement(WebDriverBy::xpath('//input[@name="code"]'), 10);
            $lastname = $selenium->waitForElement(WebDriverBy::xpath('//input[@name="lastName"]'), 0);
            $btn = $selenium->waitForElement(WebDriverBy::xpath('//button[contains(@id,"submit-")]'), 0);
            ...
            $cookiesPoliticsButton = $selenium->waitForElement(WebDriverBy::xpath('//button[@id="cookies-politics-button"]'), 5);
            if ($cookiesPoliticsButton) {
            ...
            $mover->sendKeys($code, $arFields['ConfNo'], 5);
            ...
            $selenium->driver->executeScript(
                ...
                "}\n" .
                "triggerInput('input[id*=searchbox-code]', '{$arFields['ConfNo']}');\n" .
                "triggerInput('input[id*=searchbox-lastname]', '{$arFields['LastName']}');"
            );*/
            sleep(2);
            $this->savePageToLogs($selenium);
            $btn->click();

            $selenium->waitForElement(WebDriverBy::xpath('//span[contains(text(),"Order number:")]'), 5);
            $this->savePageToLogs($selenium);
            ...
            foreach ($requests as $n => $xhr) {
                ...
                if (strpos($xhr->request->getUri(), '/mytrips/v1/validate') !== false) {
                    $data2 = $xhr->response->getBody();
            ...
            $selenium->driver->executeScript('window.stop();');
            $this->logger->debug("xhr data: $data2");
            $this->logger->debug("xhr data: $data");
    ...
    private function parseItinerary($orderId, $lastname)
    {
        $this->logger->notice(__METHOD__);
        $this->State["x-latam-app-session-id"] = 'd2d533e4-c525-43af-bb98-78d6ed5ec2db';
        $headers = [
            'Accept' => 'application/json, text/plain, */*',
            'X-Latam-Action-Name' => 'my-trips-second-level.order.initload-getitinerary',
            'X-Latam-App-Session-Id' => 'a741bbc1-98a2-4579-aa10-adf8418fd9e6',
            'X-Latam-Application-Country' => 'us',
            'X-Latam-Application-Lang' => 'en',
            ...
            'X-Latam-Country' => 'us',
            'X-Latam-Lang' => 'en',
            'X-Latam-Request-Id' => '261d78cb-09d2-4886-b3de-8b5b93ef53ea',
            'X-Latam-Track-Id' => '1edae799-0f56-4285-bfca-b5e514fee7ea',
        ];

        // https://www.latamairlines.com/us/en/my-trips/second-detail?orderId=LA0454855NJIE&lastname=IMBERTI
        $this->http->RetryCount = 0;
        $this->http->GetURL("https://www.latamairlines.com/bff/ordermanagement/v1/order/id/$orderId/lastname/{$lastname}?origin=second-detail", $headers);
        $this->http->RetryCount = 2;
        $response = $this->http->JsonLog();
        ...
        if (!$conf) {
            $this->logger->info("Failed Order #{$$orderId}", ['Header' => 3]);
            $message = $response->data->message ?? null;

            if ($message == 'Not Found') {
                $this->logger->notice("We couldn't find your trip {$$orderId}");
