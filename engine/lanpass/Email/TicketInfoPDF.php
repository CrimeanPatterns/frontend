...
class TicketInfoPDF extends \TAccountChecker
{
    ...
    protected function parseEmail(Email $email, string $textPdf)
    {
        ...
        $confName = $this->pdf->FindSingleNode('(//b[' . $this->eq($this->t('Reservation code')) . '])[1]');

        if (empty($nodes)){
            $nodes = array_values(array_unique(array_filter($this->pdf->FindNodes('//p[' . $this->eq($this->t('Reservation code')) . ']/following-sibling::p[1]', null, '/^\s*([A-Z\d]{5,})\s*( {3,}|$)/'))));
            $confName = $this->pdf->FindSingleNode('(//p[' . $this->eq($this->t('Reservation code')) . '])[1]');
        }

        foreach ($nodes as $value) {
            $f->general()
                ->confirmation($value, $confName);
        }
        ...
        $accounts2 = $this->pdf->FindNodes('//p[' . $this->starts($this->t('LATAM Pass CP')) . ']', null, "/^{$this->opt($this->t('LATAM Pass CP'))}[ ]*([0-9\-]+)$/u");

        if (!empty($accounts2)) {
            $f->program()
                ->accounts($accounts2, false);
        }
        ...
        if (empty(array_filter($totTexts))) {
            $totTexts = [$this->pdf->FindSingleNode("//p[normalize-space()='Total pago']/following::text()[normalize-space()][1]")];
        }
        ...
        } elseif (preg_match("/^\s*(\d[\d\., ]*\s*{$this->opt($this->t('points'))})\s*$/i", $cost, $m)) {
            $f->price()
                ->spentAwards($m[1]);
        }
        ...
        $taxesNode = $this->pdf->FindSingleNode("//p[{$this->eq($this->t('Amount'))}]/following-sibling::p[{$this->starts($this->t('('))} and {$this->contains($this->t(':'))}]", null, true, "/^[ ]*\([0-9]+\)[ ]*(\w+\:[ ]*\D{1,3}[ ]*\d[\d\., ]*.+)$/us");

        if ($taxesNode !== null){
            $taxesArray = preg_split("/([ ]+)(?=\w+\:)/", $taxesNode);

            foreach ($taxesArray as $tax) {
                if (preg_match("/^(?<name>\w+)\:[ ]*(?<currency>\D{1,3})[ ]*(?<amount>\d[\d\., ]*)$/u", $tax, $m)){
                    $f->price()
                        ->fee($m['name'], $this->getSum($m['amount'], $this->normalizeCurrency($m['currency'])));
                }
            }
        }
