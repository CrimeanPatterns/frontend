...
//                $answerInputs[$i]->sendKeys($answer[$i]);
    ...
    public function Parse()
    {
        ...
        $this->waitForElement(WebDriverBy::xpath('//div[@class = "eggpoints-activity-score"]/strong'), 25);
        $this->saveResponse();
    ...
    private function loginSuccessful()
    {
        ...
        if ($this->http->FindSingleNode('
                ...
                | (//div[contains(@class, "header2021-nav-subtitle") and not(contains(., "address")) and not(contains(., "Sign In")) and not(contains(., "Orders"))])[1]
            ')
            || $this->waitForElement(WebDriverBy::xpath('
                //div[@class = "nav-complex-title"]
                | //a[@title="My Account"]//div[contains(@class, "header2021-nav-subtitle") and not(contains(., "address")) and not(contains(., "Sign In")) and not(contains(., "Orders"))]
            '), 0)
        ) {
            return true;
        }
