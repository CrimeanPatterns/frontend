...
//        $this->http->GetURL('https://www.singaporeair.com/en_UK/sq-corporate/travel-programme/');
    ...
    public function Parse()
    {
        ...
        $response = $this->http->JsonLog($this->http->FindPreg('#<script id="__NEXT_DATA__" type="application/json">(.+?)</script>#'), 3);
        $corpUserProfile = $response->props->pageProps->initialData->pageData->userData->corpUserProfile ?? null;
        // Balance - HIGHFLYER points
        $this->SetBalance($response->props->pageProps->initialData->pageData->userData->totalCorporateMiles ?? $this->http->FindSingleNode('//p[contains(text(), "HIGHFLYER")]/following-sibling::p[1]', null, true, "/(.+)\s*point/ims"));
        // Name
        $name = ($corpUserProfile->firstName ?? '') . ' '. ($corpUserProfile->lastName ?? '');
        $this->SetProperty('Name', beautifulName($this->http->FindSingleNode('//p[contains(text(), "HIGHFLYER")]/preceding-sibling::p[2]') ?? $name));
        // Company Name
        $this->SetProperty('CompanyName', $corpUserProfile->corporateName ?? $this->http->FindSingleNode('//p[contains(text(), "HIGHFLYER")]/preceding-sibling::p[1]'));
        // Corporate Code
        $this->SetProperty('CorporateCode', $corpUserProfile->corporateCode ?? $this->http->FindSingleNode('//p[contains(., "Corporate Code:")]/following-sibling::p[contains(., "Corporate Code:")]', null, true, "/:\s*(.+)/"));
        // Account created on
        $this->SetProperty('MemberSince', $this->http->FindSingleNode('//p[contains(., "Corporate Code:")]/following-sibling::p[contains(., "Account created on:")]', null, true, "/:\s*(.+)/"));

        $this->logger->info('Expiration Date', ['Header' => 3]);
        // Expiration Date   // refs #4370, #12555
        $this->http->GetURL("https://www.singaporeair.com/corporate/expiring-points");
...
//        return $this->recognizeAntiCaptcha($this->recognizer, $postData);
    ...
    private function getCookiesFromSelenium(): void
    {
        ...
        try {
            ...
            /** @var Fingerprint $fp */
            $fp = $this->services->get(FingerprintFactory::class)->getOne([FingerprintRequest::mac()]);
