...
class TAccountCheckerGolair extends TAccountChecker
{
    ...
    public function InitBrowser()
    {
        ...
        if (
            ...
            if ($this->attempt > 0) {
                $array = ['nz', 'fi', 'dk', 'cz', 'fr', 'it', 'es', 'de', 'pt', 'be'];
                $targeting = $array[array_rand($array)];
            }
...
//            $_csrf = $this->http->getCookieByName('_csrf', null, '/usernamepassword/login');
            ...
            $data = [
                "client_id"      => $clientId,
                "redirect_uri"   => "https://www.smiles.com.br/logincb?dest=iss%3Dhttps%253A%252F%252Flogin.smiles.com.br%252F",
                "tenant"         => "smiles-prod",
                "response_type"  => "code",
                "scope"          => "openid profile email",
                "audience"       => "https://smiles.api",
                "_csrf"          => 'AT3Slx0O-qyeJUDU6tx1IjuGFFwqily4CTgs',
                "state"          => $state,
    ...
    public function Login()
    {
        ...
        if ($message) {
            ...
            if (strstr($message, 'Something went wrong')) {
                $this->DebugInfo = 'ip has been blocked';
                $this->markProxyAsInvalid();
...
//        $this->sendNotification("check incapsula // ZM");
    ...
    private function selenium()
    {
        ...
        try {
            ...
            try {
                $selenium->http->GetURL('https://www.smiles.com.br/login');
//                $selenium->http->GetURL("https://www.smiles.com.br/group/guest/minha-conta?p_p_id=smileswidgetmydataportlet_WAR_smileswidgetmyaccountportlet&p_p_lifecycle=2&p_p_state=normal&p_p_mode=view&p_p_resource_id=doWidgetMyData&p_p_cacheability=cacheLevelPage&p_p_col_id=column-2&p_p_col_count=2");
            ...
            $loginInput = $selenium->waitForElement(WebDriverBy::xpath("//input[@id = 'identifier']"), 0);
            ...
            $loginInput->sendKeys($loginInput, $this->AccountFields['Login']);
            sleep(1);
