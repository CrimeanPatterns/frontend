...
{
    public $mailFiles = "easyjet/it-137086637.eml, easyjet/it-141576457.eml, easyjet/it-742792067.eml, easyjet/it-746832370.eml, easyjet/it-749223124.eml, easyjet/it-892391390.eml";

    public $detectSubjects = [
        'en' => [
            'easyJet holidays booking',
            "it's time to check-in for your holiday",
        ],
    ];
    ...
    public static $dictionary = [
        'en' => [
            ...
            'Online check-in is now open!' => 'Online check-in is now open!',
        ],
    ];

    public function detectEmailFromProvider($from)
    {
        return preg_match('/[@.]easyjet\.com$/', $from) > 0;
    }

    public function detectEmailByHeaders(array $headers)
    {
        // detect Provider
        if (empty($headers['from']) || $this->detectEmailFromProvider($headers['from']) === true) {
            return false;
        }
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            // detect Provider for pdf
            if ($detectProvider === false && stripos($textPdf, 'easyJet holidays') === false) {
                continue;
            }

            // detect Format for pdf
            if ($this->assignLang($textPdf)) {
                return true;
            }
        }

        // detect Provider for html
        if ($this->http->XPath->query("//a/@href[{$this->contains('.easyjet.com')}]")->length < 3) {
            return false;
        }

        // detect Format for html
        foreach (self::$dictionary as $dict) {
            if (!empty($dict['Holiday reference']) && !empty($dict['Total price of your holiday'] && !empty($dict['Online check-in is now open!']))
                ...
                && ($this->http->XPath->query("//text()[{$this->contains($dict['Total price of your holiday'])}]")->length > 0
                    || $this->http->XPath->query("//text()[{$this->contains($dict['Online check-in is now open!'])}]")->length > 0)
            ) {
                return true;
            }
        }
    ...
    {
        $patterns = [
            'travellerName' => "(?:{$this->opt(['Miss', 'Mrs', 'Mr', 'Ms'])}\.?\s*)?([[:alpha:]][-.\/\'’[:alpha:] ]*[[:alpha:]])",  // use with flags "ui"
        ];
        ...
        $travellers = $this->http->FindNodes("//text()[{$this->eq($this->t('Your details'))}]/ancestor::*[not({$this->eq($this->t('Your details'))})][1]/descendant::text()[normalize-space()][position() > 1]", null, "/^\s*{$patterns['travellerName']}\s*$/ui");
        ...
        foreach ($fNodes as $fRoot) {
            ...
            if (preg_match($re, $text, $m)) {
                ...
                if (!empty($m['op'])) {
                    $s->airline()->operator($this->re("/^\s*(?:{$this->opt($this->t('Operated by'))}\s+)?(.+)\s*$/", $m['op']));
                }
    ...
    private function parsePdf(Email $email, $text): void
    {
        ...
        foreach ($guestNames as $gName) {
            if (preg_match("/^\s*{$patterns['travellerName']}\s*$/ui", $gName, $m)) {
                $travellers[] = $m[1];
            } else {
                $travellers = [];
