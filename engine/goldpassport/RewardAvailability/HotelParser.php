...
//        $selenium->useCache();
    ...
    private function parseDataJson($fields, $data, $referer)
    {
        ...
        foreach ($data->hotelData->hotelSummaries as $i => $root) {
            ...
            $referer = "https://www.hyatt.com/shop/rooms/{$root->hotelDetail->spiritCode}?checkinDate={$checkIn}&checkoutDate={$checkOut}&rooms=1&adults={$fields['Adults']}&kids={$fields['Kids']}&rate=Standard&rateFilter=woh&hpesrId=" . $root->hpesrId;
            $browser->GetURL($referer);
            $url = "https://www.hyatt.com/shop/service/rooms/roomrates/{$root->hotelDetail->spiritCode}?spiritCode={$root->hotelDetail->spiritCode}&rooms={$fields['Rooms']}&adults={$fields['Adults']}&checkinDate={$checkIn}&checkoutDate={$checkOut}&kids={$fields['Kids']}&rate=Standard";
            $browser->GetURL($url);
        ...
        foreach ($browser->asyncResponses as $response) {
            ...
            if (isset($response)) {
                $allRoomRates[] = $this->http->JsonLog(null, 0, true);
            }
        }
...
//            if ($this->withExtenedeCheck && $this->notAcceptingPointsAwards($fields, $browser, $root->hotelDetail->spiritCode,
//                    $root->hpesrId)) {
//                $this->logger->debug("skip hotel. notAcceptingPointsAwards");
//
//                continue;
//            }
            $this->roomRates = $allRoomRates[$i];
    ...
    private function notAcceptingPointsAwards($fields, $browser, $spiritCode, $hpesrId): bool
    {
        $this->roomRates = [];

        return $this->notAcceptingPointsAwardsUrl($fields, $browser, $spiritCode, $hpesrId);
    ...
    private function notAcceptingPointsAwardsUrl($fields, $browser, $spiritCode, $hpesrId): bool
    {
        try {
