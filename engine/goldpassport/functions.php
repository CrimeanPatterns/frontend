...
class TAccountCheckerGoldpassport extends TAccountChecker
{
    ...
    private const CONFIGS = [
        'chrome-extension-103-mac' => [
            'browser-family'  => SeleniumFinderRequest::BROWSER_CHROME_EXTENSION,
            'browser-version' => SeleniumFinderRequest::CHROME_EXTENSION_103,
            'os'              => SeleniumFinderRequest::OS_MAC,
        ],
        'chrome-extension-103-random-os' => [
            'browser-family'  => SeleniumFinderRequest::BROWSER_CHROME_EXTENSION,
            'browser-version' => SeleniumFinderRequest::CHROME_EXTENSION_103,
        ],
        'chrome-extension-104-mac' => [
            'browser-family'  => SeleniumFinderRequest::BROWSER_CHROME_EXTENSION,
            'browser-version' => SeleniumFinderRequest::CHROME_EXTENSION_104,
            'os'              => SeleniumFinderRequest::OS_MAC,
        ],
        'chrome-extension-104-random-os' => [
            'browser-family'  => SeleniumFinderRequest::BROWSER_CHROME_EXTENSION,
            'browser-version' => SeleniumFinderRequest::CHROME_EXTENSION_104,
        ],
        'camoufox-default' => [
            'browser-family'  => SeleniumFinderRequest::BROWSER_CAMOUFOX,
            'browser-version' => SeleniumFinderRequest::CAMOUFOX_DEFAULT,
        ]
    ];
...
//                $mover->click();
            ...
            if (!$retry && !$this->http->FindSingleNode("//span[contains(@class, 'error-message')]")) {
                ...
                $this->logger->info("body: " . substr($selenium->http->Response['body'], 0, 1000));

                if ($selenium->http->Response['body'] === '<html><head><link rel="stylesheet" href="resource://content-accessible/plaintext.css"></head><body><pre></pre></body></html>') {
                    $this->logger->info("seems like block");
...
//            if (strstr($e->getMessage(), 'Connection refused (Connection refused)'))
    ...
    private function checkErrors()
    {
        ...
        if(
            $this->http->FindSingleNode('//title[not(text())]')
            || $this->http->FindSingleNode('//pre[not(text()) and not(@id) and not(@style)]')
            || count($this->http->FindNodes('//*[not(text()) and @id and @style="display: none;"]')) === 2
        ) {
            $this->logger->debug('Blocked by a blank page');
            $this->DebugInfo = 'Blocked by a blank page';
            /*
    ...
    private function parseOneItineraryFromHtml($hotelNode): bool
    {
        ...
        foreach ($hotelInfoNodes as $hotelInfoNode) {
            ...
            foreach ($roomNodes as $roomNode) {
                ...
                $address = implode(', ',
                    array_filter($this->http->FindNodes('.//button[@data-js = "cancel-button"]/ancestor::div[1]/preceding-sibling::div[1]',
                        $hotelInfoNode)));
...
//        }
    ...
    private function seleniumSettings(self $selenium)
    {
        ...
        */
        ...
        $successfulConfigs = array_filter(array_keys($configs), function (string $key) {
            return Cache::getInstance()->get('goldpassport_config_' . $key) === 1;
        });

        $neutralConfigs = array_filter(array_keys($configs), function (string $key) {
            return Cache::getInstance()->get('goldpassport_config_' . $key) !== 0;
        });
        ...
        */
        ...
        if (isset(self::CONFIGS[$this->config]['os'])) {
            $selenium->seleniumRequest->setOs(self::CONFIGS[$this->config]['os']);
        }
    ...
    private function seleniumSettingsOld(self $selenium)
    {
        $this->logger->notice(__METHOD__);
        $resolutions = [
            [1152, 864],
...
//        $selenium->setScreenResolution($resolution);
    ...
    //    if (isset($this->AccountFields["UserID"]) && $this->AccountFields["UserID"] == 7) {
            $selenium->useCamoufox(SeleniumFinderRequest::CAMOUFOX_DEFAULT);
//        }
...
//        $checker2->waitForElement(WebDriverBy::xpath('//p[@class="bw"]'), 10);
    ...
    private function markConfigAsBadOrSuccess($success = true): void
    {
        if ($success) {
            ...
            Cache::getInstance()->set('goldpassport_config_' . $this->config, 1, 60 * 20);
        } else {
            $this->logger->info("marking config {$this->config} as bad");
            Cache::getInstance()->set('goldpassport_config_' . $this->config, 0, 60 * 20);
        }
    }
}
