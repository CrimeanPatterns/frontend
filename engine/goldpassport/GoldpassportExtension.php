    ...
    implements LoginWithIdInterface, ParseInterface, ParseItinerariesInterface, ParseHistoryInterface
{
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        $this->attempt++;
        if ($this->attempt > 2) {
            return LoginResult::providerError('Something went wrong. Please try again.');
        }

        //Blank page. May be a block.
        $mayBeBlock = $this->waiter->waitFor(function() use ($tab) {
            return $tab->evaluate('//a | //form', EvaluateOptions::new()->allowNull(true)->timeout(30));
        });

        if (!$mayBeBlock){
            return LoginResult::providerError('Something went wrong. Please try again later.');
        }
        ...
        sleep(1);
        $errorOrTitle = $tab->evaluate('//div[contains(@class,"MemberCard_container_")] | //span[@data-e2e = "errorText"] | //span[contains(@class,"error-message")] | //p[contains(normalize-space(), "unexpected error")]');
        ...
        if (stristr($errorOrTitle->getInnerText(), 'Something went wrong. Please try again.')) {
            ...
            if (str_contains($error, "An unexpected error has occurred while attempting to process your request")) {
    ...
    public function parseItineraries(Tab $tab, Master $master, AccountOptions $options, ParseItinerariesOptions $parseOptions) : void
    {
        ...
        $reservation = null;
        try {
            $reservation = $tab->fetch("https://www.hyatt.com/profile/api/stay/reservation?locale=en-US&firstName={$this->firstName}&lastName={$this->lastName}", $options);
            $this->logger->info($reservation->status);
            $this->logger->info($reservation->body);
        } catch (Exception $e) {
            $this->logger->error($e->getMessage());
            if (stristr($e->getMessage(), 'HTTP error! Status: 404')) {
                $master->setNoItineraries(true);
                return;
            }
            return;
        }
        $upcomingStays = json_decode($reservation->body)->reservations ?? [];
    ...
    private function parseOneItineraryFromHtml(Tab $tab, Master $master, $hotelNode): bool
    {
        ...
        foreach ($hotelInfoNodes as $hotelInfoNode) {
            ...
            foreach ($roomNodes as $roomNode) {
                ...
                // ConfirmationNumber
                $h->general()->confirmation($tab->findText('//p[contains(text(), "Confirmation:")]', FindTextOptions::new()->preg('/([\w\-]+)\s*$/')->timeout(3)));
                // Cancelled
                ...
                $address = implode(', ',
                    array_filter($tab->findTextAll('.//button[@data-js = "cancel-button"]/ancestor::div[1]/preceding-sibling::div[1]',
                        FindTextOptions::new()->contextNode($hotelInfoNode))));
                ...
                if (!in_array($roomInfo, ['(1)'])) {
                    if ($type = $this->findPreg('/^\s*\(\d+\)\s*([^<]+)/i', $roomInfo)) {
                        $r = $h->addRoom();
                        $r->setType($type);
                    }
                }
