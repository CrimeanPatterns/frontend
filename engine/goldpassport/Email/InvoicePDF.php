...
class InvoicePDF extends \TAccountChecker
{
    ...
    private static $detectors = [
        ...
        'en' => ["INVOICE", "Receipt for ", "Stay Charges:", "World of Hyatt Stay Summary", "Folio:", "Membership", "www.marriottrewards.com", "www.ihgrewardsclub.com", "As a Marrio", "Marriott", 'Thank you for choosing Hilton', 'United States 805-900-8388', 'under license of IHG Hotels Limited', 'Four Seasons', 'www.fourseasons.com/', '@fourseasons.com',
            'www.loewshotels.com', "Thank you for staying with us at 70 Park Avenue Hotel !", "www.ibisba.com.au", "I agree my liability for this bill is not waived and agree to be held personally l", "On behalf of the ladies and gentlemen of The Ritz-Carlton", ],
    ];
    private static $dictionary = [
        ...
        'en' => [
            "Booking #"         => ["Booking #", "Confirmation #", "Conf No.", "Confirmation No.", "CRS Number", "Conf No", "Conf. No.", "Conf.", "Confirmation Number", "Conf. Number", 'REFERENCE', 'Reg. No.', 'Reservation No.', 'Conf.No.', 'Conf. No.:', 'Guest Number', 'Client No', 'Client'],
            "Arrival"           => ["Arrival", "ARRIVAL DATE", "Arrival Date", "Arrive Date", "Arrivee"],
            "Departure"         => ["Departure", "DEPARTURE DATE", "Depart Date", "Depart"],
            ...
            "Room No:"          => ["Room No:", "Room No", "Room No.:", "Room No.", "Room:", "Room Number:", 'ROOM NO:', 'ROOM', 'Room Number', 'Chambre No', 'Chambre/Room'],
            "Guest Name"        => ["Guest Name", "Invoice For", "Receipt for"],
            'No of Guests'      => ['No of Guests', 'No. of Adults', 'Adult/Child', 'Number of guest(s)', 'Persons per room', 'Guests', 'Number of Adults', 'No. Of Guest', 'No de Client', 'Nb. Client'],
            "Mr/Mrs"            => ["Mr. and Mrs.", "Miss", "MISS", "Mrs", "MRS", "Mr", "MR", "Ms", "MS"],
            "Membership"        => ["Membership Number", "Membership", "Marriott Bonvoy No.", "Loyalty No.", "Loyalty Number", "Membership No.", "WyndhamRewards #:", "Rewards No.", "Membership No:MR #", "Rewards No:", "Bonvoy No.", "Marriott Bonvoy Number"],
            "Total"             => ["Total", "TOTAL:"],
            "Cost"              => "Vatable Amount",
            "Tax"               => ["VAT Amount", "VAT"],
    ...
    public function detectEmailByHeaders(array $headers)
    {
        ...
        if ($this->detectEmailFromProvider($headers['from']) !== true
            ...
            && stripos($headers['from'], 'wynnlasvegas.com') === false && stripos($headers['from'], 'langhamhotels.com') === false
        ) {
            return false;
        }
    ...
    public static function getEmailProviders()
    {
        return ['marriott', 'loews', 'goldpassport', 'rosewood', 'ichotelsgroup', 'omnihotels',
            ...
            'uniorlres', 'dorchester', 'montage', 'yotel', 'iberostar', 'langham', ];
    }
    ...
    private function assignProvider($headers, string $text): bool
    {
        ...
        if (stripos($headers['from'], 'langhamhotels') !== false
            || strpos($text, ' The Langham') !== false
        ) {
            $this->providerCode = 'langham';
    ...
    private function parseEmailPdf(Email $email, $text): void
    {
        ...
        if (preg_match("/^\s*{$this->opt($this->t('Guest Name'))}\s+({$patterns['travellerName']})[ ]*(?:Reference|TA Locator|Ref. agence)/m", $text, $m)
            ...
            || preg_match("/({$patterns['travellerName']})[ ]{10,}Page Number/i", $text, $m) && stripos($m[1], 'GUESTS') === false
        ) {
            $guestName = $m[1];
        }
        ...
        if (preg_match("/(?:^|[ ]{2}){$this->opt($this->t("Membership"))}[: ]+(?:LP|PC|SG)?\s*([A-Z\d]{4,12})(?:[ ]{2}|$)/im", $text, $m)) {
            if (stripos($m[1], 'Number') === false && stripos($m[1], 'Page') === false && strlen(trim($m[1])) > 4) {
        ...
        if (preg_match("/" . $this->opt($this->t("Arrival")) . "\s+(\d{4}[\s\/.-](?:[A-z]{3}|\d{1,2})[\s\/.-]\d{2,4})\b/ims", $text, $m)
            ...
            if (!empty($m[1]) && preg_match("#{$m[1]}\s*(\d+\:\d+\s*A?P?M?)#su", $text, $matches)) {
                $r->booked()
                    ->checkIn(strtotime($matches[1], $r->getCheckInDate()));
            }
        }
        ...
        } elseif (!empty($this->arrDate) && preg_match("#{$this->arrDate}\s*(\d+\:\d+\s*A?P?M?)#", $text, $matches)) {
            $r->booked()
                ->checkOut(strtotime($matches[1], $r->getCheckOutDate()));
        }
        ...
        if (preg_match("/\n {30,}(?<name>The Langham.+)\n (?<address>.+?) T ?(?<phone>[\d\- \(\)\+]{5,}) F(?<fax>[\d\- \(\)\+]{5,})?\n *.*www\.langhamhotels\.com/", $text, $match)) {
            $hotelName = trim($match['name']);
            $address = trim($match['address']);

            if (!empty($match['phone']) && strlen(preg_replace('/\D+/', '', $match['phone'])) > 5) {
                $r->hotel()->phone($match['phone']);
            }

            if (!empty($match['fax']) && strlen(preg_replace('/\D+/', '', $match['fax'])) > 5) {
                $r->hotel()->fax($match['fax']);
            }
        }

        if (empty($hotelName)
            && preg_match("/\n (?<address>.+?)\s+T ?(?<phone>[\d\- \(\)\+]{5,}) F(?<fax>[\d\- \(\)\+]{5,})?\n *.*www\.langhamhotels\.com/", $text, $match)) {
        ...
        if (empty($hotelName) && empty($address)
            ...
            if (stripos($hotelName, 'The The Ritz-Carlton') !== false) {
        ...
        //AMERICINN
        if (empty($hotelName) && stripos($this->subjectEmail, 'AMERICINN') !== false) {
            $hotelName = $this->re("/^(?:Fwd:\s+)?Reservation\s+Folio\s+From\s+(\D+)\s?-?$/", $this->subjectEmail);
        }

        if (empty($hotelName) && preg_match("/Shangri[\s\-]*La/iu", $this->subjectEmail)) {
            $hotelName = $this->re("#\n+\s*(.+)\s+\d+\/\d+\/\d{2}\s+\d+\:\d+\n+\s*DATE#ui", $text);
        ...
        if ((empty($hotelName) || empty($address))
            ...
            if (empty($r->getPhone()) && !empty(trim($m['phone']))) {
                $r->hotel()
                    ->phone($m['phone']);
            }
        ...
        if (!empty($roomNumber = $this->re("/({$this->opt($this->t('Room No:'))}\s*(?:\:\s+)?\d+)/", $text))) {
            ...
            if (preg_match("/(?:Number of Rooms)\s*(?:\:\s+)?(\d+)\s*(.+)[ ]{4,}/u", $text, $m)) {
                ...
                if (preg_match("/Room Rate\s+\D\s+([\d\.\,\'])+\s*per night/", $text, $m)) {
                    $room->setRate($m[1] . ' / night');
                }
            }
            ...
            if (preg_match("/{$this->opt($this->t('No. of rooms'))}\s*(\d+)/u", $text, $m)) {
                ...
                if (preg_match("/Price per room[\/\s]+night\s*([A-Z]{3}\s*[\d\.]+)/", $text, $m)
                    ...
                    || preg_match("/Room Rate\s+\D\s+([\d\.\,\'])+\s*per night/u", $text, $m)
                ) {
                    $room = $r->addRoom();
                    $room->setRate($m[1] . ' / night');
                }
