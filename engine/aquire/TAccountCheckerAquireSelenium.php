...
//                throw new CheckException($message, ACCOUNT_PROVIDER_ERROR);
    ...
    public function Login()
    {
        ...
        while ((time() - $startTime) < $sleep) {
            ...
            if ($choice = $this->waitForElement(WebDriverBy::xpath("//*[self::p or self::label][contains(text(), 'Answer a security question and email')]"), 0)) {
...
//                $this->driver->executeScript("document.querySelector('[value=\"EMAIL\"]').click()");
            ...
            if ($this->waitForElement(WebDriverBy::xpath("//div[contains(text(), 'Please answer the security question to verify your identity.')]/following-sibling::label"), 0)) {
                $this->markProxySuccessful();
                return $this->processSecurityCheckpoint();
            }
            sleep(1);
    ...
    public function processSecurityCheckpoint()
    {
        ...
        $question = $this->waitForElement(WebDriverBy::xpath("//div[contains(text(), 'Please answer the security question to verify your identity.')]/following-sibling::form/div"), 5);
        $securityAnswer = $this->waitForElement(WebDriverBy::xpath('//input[@name = "esq"]'), 0);
        $this->saveResponse();
        ...
        $error = $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "The answer you\'ve entered is incorrect.")]'), 0);
        $this->saveResponse();
    ...
    {
        $this->logger->notice(__METHOD__);
        $questionCode = $this->waitForElement(WebDriverBy::xpath("//div[contains(text(), 'Please enter the code we sent')]"), 10);
