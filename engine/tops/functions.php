...
class TAccountCheckerTops extends TAccountChecker
{
    ...
    {
        if (!$href = $this->http->FindSingleNode("(//a[contains(@title, 'BonusPlus') and contains(@href, 'GasBonusPoints')]/@href)[1]")) {
            $this->http->GetURL('https://www.topsmarkets.com/');
            $href = $this->http->FindSingleNode("(//a[contains(@title, 'BonusPlus') and contains(@href, 'GasBonusPoints')]/@href)[1]");
        }
