...
class TripSummary extends \TAccountChecker
{
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($nodes as $root) {
            ...
            $h->hotel()
                ->name($hotelName)
                ->address($this->http->FindSingleNode("//text()[{$this->eq($this->t('Trip summary'))}]/following::text()[normalize-space()={$hotelNameXpath}][1]/following::img[contains(@alt, 'star') or contains(@altx, 'star') or contains(@id, 'imgHotelRatingStar')][1]/following::text()[normalize-space()][1]"));
