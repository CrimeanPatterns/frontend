...
class BookingConfirmed extends \TAccountChecker
{
    ...
    private $reSubject = [
        ...
        // sl
        'Potrditev rezervacije pri Agoda – ID rezervacije:',
    ];
    private $reBody = [
        'en' => [
            ...
            ['Your bookings are confirmed!', 'Reservation'],
        ...
        'sl' => [
            ['rezervacija je zdaj potrjena', 'Prijava'],
        ],
    ];
    ...
    private static $dictionary = [
        ...
        "th" => [
            'Hi'                             => ['เรียน คุณ'],
            'booking ID is'                  => ['หมายเลขการจองของท่าน คือ', 'หมายเลขการจองคือ'],
            ...
            'Total price'                    => ['ราคารวมทั้งสิ้น', 'ราคาขณะนี้'],
        ...
        "sl" => [
            'Hi'            => ['Zdravo '],
            'booking ID is' => ['ID številka vaše rezervacije je'],
            'Reservation'   => 'Rezervacija',
            'room'          => 'soba',
            'Room type'     => 'Vrsta sobe',
            'Check in'      => 'Prijava',
            'Check out'     => 'Odjava',
            'Lead guest'    => 'Glavni gost',
            'Occupancy'     => 'Zasedenost',
            'adult'         => ["Št. odraslih"],
            // 'children'      => ["çocuk"],
            //            'Special request' => 'Forespørsler',
            'Total price'     => 'Skupaj zaračunano',
            //            'Cancellation and Change policy' => 'Termini di Cancellazione e Modifica',
    ...
    private $bookingIDs = [], $PDFsTexts = [];

    private $patterns = [
        'phone' => '[+(\d][-+. \d)(]{5,}[\d)]', // +377 (93) 15 48 52  |  (+351) 21 342 09 07  |  713.680.2992
    ];

    public function parseHotel(Email $email, ?\DOMNode $root = null): void
    {
        $this->logger->debug(__FUNCTION__ . '()');

        // it-899646532.eml
        $confirmation = $this->http->FindSingleNode("descendant::text()[{$this->starts($this->t('booking ID is'))}]", $root, true, "/^{$this->opt($this->t('booking ID is'))}[:\s]*(\d[-A-Z\d]+)$/u")
            ?? $this->http->FindSingleNode("preceding::text()[normalize-space()][1]/ancestor::*[ preceding-sibling::*[normalize-space()] or following-sibling::*[normalize-space()] ][1]", $root, true, "/^{$this->opt($this->t('booking ID is'))}[:\s]*(\d[-A-Z\d]+)$/u");
        ...
        if (empty($confirmation)) {
            $confirmation = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Occupancy'), true)}]/preceding::text()[{$this->contains($this->t('booking ID is'))}][1]/ancestor::tr[1]", $root, true, "/{$this->opt($this->t('booking ID is'))}\s*(\d{5,})(?:\b|이며|입니)/u");
        }
        ...
        if (in_array($confirmation, $this->bookingIDs)) {
            $this->logger->debug('This hotel reservation is a duplicate. Skip.');

            return;
        }

        if ($confirmation) {
            $this->bookingIDs[] = $confirmation;
        }
        ...
        if (empty($traveller)) {
            $traveller = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Reservation'), true)}]/preceding::text()[{$this->starts($this->t('Hi'))}][1]", $root, true, "/{$this->opt($this->t('Hi'))}[,，\s]*(\D+[^,，\d\s])/u");
        }

        if (empty($traveller)) {
            $traveller = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Reservation'), true)}]/preceding::text()[{$this->contains($this->t('Hi'))}][1]", $root, true, "/^[,，\s]*([^,，\d]+?)[,，\s]*{$this->opt($this->t('Hi'))}/u");
        }
        ...
        if (empty($cancellation)) {
            $cancellation = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Free Cancellation'))}]/ancestor::div[1]", $root, true, "/^\s*{$this->opt($this->t('Free Cancellation'))}.+/su");

            if (strlen($cancellation) > 2000) {
                $cancellation = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Free Cancellation'))}]/ancestor::tr[1]", $root, true, "/^\s*{$this->opt($this->t('Free Cancellation'))}.+/su");
            }
        }

        if (empty($cancellation)) {
            $cancellation = $this->http->FindSingleNode("descendant::tr[{$this->eq($this->t('Cancellation Policy'))}]/following-sibling::tr[normalize-space()][1][{$this->eq($this->t('Important Information'))}]/following-sibling::tr[normalize-space()][1]", $root);
        }
        ...
        if (preg_match('/^(?<currency>[^\-\d)(]+?)[ ]*(?<amount>\d[,.‘\'\d ]*)$/u', $totalPrice, $matches)) {
            ...
            $currency = ZAllHotelPdf::normalizeCurrency($matches['currency']);
        ...
        $this->detectDeadLine($h, $h->getCancellation(), $root);

        // Get info from PDF

        $targetTexts = [];

        foreach ($this->PDFsTexts as $textPdf) {
            if ($confirmation && preg_match("/^[ ]*Booking ID[: ]*:[:\s]*{$this->opt($confirmation)}(?:[ ]{2}|$)/m", $textPdf)) {
                $targetTexts[] = $textPdf;
            }
        }

        if (count($targetTexts) !== 1) {
            $this->logger->debug('Target PDF not found!');

            return;
        }

        $text = array_shift($targetTexts);

        if (preg_match("/^[ ]*(Booking Reference No)[: ]*:[:\s]*([A-z\d]{4,})(?:[ ]{2}|$)/m", $text, $m)) {
            $h->general()->confirmation($m[2], $m[1]);
        }

        if (preg_match("/^[ ]*(Member ID)[: ]{0,15}:[: ]{0,40}(\d{7,})(?:[ ]{2}|$)/m", $text, $m)) {
            $h->program()->account($m[2], false, $traveller, $m[1]);
        }

        if (preg_match("/^[ ]*(?:Hotel Contact Number|Property Contact Number)[: ]{0,15}?:[: ]{0,40}?({$this->patterns['phone']})(?:[ ]{2}|$)/im", $text, $m)) {
            $h->hotel()->phone(preg_replace('/[+]+/', '+', $m[1]), false, true);
        }
    }
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (empty($textPdf) || !preg_match("/^[ ]*Booking ID[: ]*:[:\s]*\d{7,}(?:[ ]{2}|$)/m", $textPdf)) {
                continue;
            }

            $this->PDFsTexts[] = $textPdf; // examples: it-756421500.eml
        }

        $xpath = "//tr/*[ descendant::text()[normalize-space()][1][{$this->eq($this->t('Check in'), true)}] ]/ancestor::*[{$this->contains($this->t('Total price'))}][1]";
    ...
    private function detectDeadLine(Hotel $h, ?string $cancellationText, ?\DOMNode $root = null): void
    {
        ...
        $freeCancellation = '';
        $values = $this->http->FindNodes("descendant::*[contains(@style,'border-left:') and contains(@style,'solid')][ descendant::*[{$this->eq($this->t('Free Cancellation'))}] ]/descendant::tr[not(.//tr[normalize-space()]) and normalize-space()]", $root);
        ...
        if (!empty($freeCancellation)) {
            ...
            if (
                ...
                // Do 23 april 11:59pop. po lokalnem času mesta Page (AZ)
                preg_match("/^\D*\b(?<day>\d{1,2})\b\s*(?<month>[[:alpha:]\x{0E00}-\x{0E7F}\.]{1,20})[,.]?\s*(?:\bเวลา|\balle|à|\()?\s*(?<timeH>\d{1,2})[:.](?<timeM>\d{2})(?<time2> ?[ap]m| ?ก่อนเที่ยง| *หลังเที่ยง| ?dop\.| ?pop\.|\b)?/iu", $freeCancellation, $m)
                ...
                $m['time'] = str_replace(['odp.', 'pop.', 'หลังเที่ยง', 'nachm.', 'da tarde', 'p. m.', 'fm'], 'PM', $m['time']);
                // a.m. -> am
