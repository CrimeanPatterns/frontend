...
class Flight2024 extends \TAccountChecker
{
    ...
    public static $dictionary = [
        ...
        'it' => [
            // 'otaConfNumber'     => ['Buchungsnummer:'],
            'otaConfNumberInText' => 'Il numero di riferimento della prenotazione è',
            'statusPhrases'       => ['La prenotazione del volo è stata'],
            'statusVariants'      => ['confermata'],
            'cabinValues'         => ['ECONOMY'],
            'Airline Reference'   => 'Codice di prenotazione della compagnia aerea',
            'Passenger details'   => 'Informazioni del passeggero',
            'Adult'               => 'Adult',
            ...
            // 'Seat selection' => '좌석 선택',
            'Total'      => 'Totale:',
            'feeHeaders' => ['Tasse e supplementi:'],
            'FREE'       => 'GRATIS',
        ],
    ];
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if ($otaConfNumbers->length === 0 && empty($email->getTravelAgency()->getConfirmationNumbers())) {
            $confs = $this->http->FindNodes("//a/@href[contains(., '.agoda.com')][{$this->contains(['/bookingdetails?BookingId=', '%2Fbookingdetails%3FBookingId%3D'])}]",
                null, "/bookingdetails(?:\?|%3F)BookingId(?:=|%3D)(\d{5,})\s*(?:&|$|__;|%26)/");
        ...
        */
        ...
        foreach ($segments as $root) {
            ...
            $duration = $this->http->FindSingleNode("*[2]", $root, true, '/^(?:\s*\d{1,3}\s*(?:[hm]|ชม|นาที|時間|分|시간|분|小时|分钟)[.\s]*)+$/iu');
    ...
    private function normalizeDate(?string $text): ?string
    {
        ...
        if (isset($day, $month, $year)) {
            ...
            $primaryLangs = [$this->lang, 'nl'];

            foreach (array_merge($primaryLangs, array_diff(array_keys(self::$dictionary), $primaryLangs)) as $lang) {
            ...
            // $this->logger->debug('$month(translated) = ' . print_r($month, true));
