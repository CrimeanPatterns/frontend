...
class TAccountCheckerSafeway extends TAccountChecker
{
    ...
    public $responseDataProfile;
    public $responseDataAuth;
    public $responseDataRewards;
    private $headers = [
    ...
    {
        require_once __DIR__ . "/TAccountCheckerSafewaySelenium.php";

        return new TAccountCheckerSafewaySelenium();
    ...
    private function selenium()
    {
        ...
        try {
            ...
            $selenium->waitForElement(WebDriverBy::id('label-email'), 30);
            $this->savePageToLogs($selenium);

            $this->logger->debug('checking for page crush');

            if (
                !$selenium->http->FindSingleNode('//*[@id="label-email"]')
                || !$selenium->http->FindSingleNode('//*[@id="label-password"]')
                || !$selenium->http->FindSingleNode('//*[@id="btnSignIn"]')
            ) {
                /*
                $selenium->http->GetURL("https://www.{$this->AccountFields['Login2']}.com/account/sign-in.html");
                */
                $selenium->driver->executeScript('window.location.reload();');
                $selenium->waitForElement(WebDriverBy::id('label-email'), 10);
                $this->savePageToLogs($selenium);
            }

            $login = $selenium->waitForElement(WebDriverBy::id('label-email'), 0);
