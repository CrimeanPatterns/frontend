...
//        $arg['CookieURL'] = 'https://www.colorline.no/ibe/profile/login.do';
    ...
    public function Login()
    {
        ...
        if ($message = $this->http->FindSingleNode('//div[contains(@class, "errormsg")]//li[1]')) {
            ...
            if (
                strstr($message, 'Bare agenter og organisasjoner kan logge seg inn via denne siden.')
                || strstr($message, 'Vi finner ingen profil som stemmer med opplysningene du har oppgitt under.')
            ) {
                throw new CheckException($message, ACCOUNT_INVALID_PASSWORD);
            }
