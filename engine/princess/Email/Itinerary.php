...
//                            $this->logger->alert('pdf2');
    ...
    private function parseEmailPdf_1($textPDF, Email $email)
    {
        ...
        $this->logger->debug($textInfo);

        $r->general()
            ->confirmation($this->re("#^\s*(?:BOOKING CONFIRMATION|FINAL PAYMENT NOTICE|FINAL PAYMENT REMINDER|UPGRADE NOTICE|DEPOSIT CONFIRMATION|Itinerary Change|).*\s*BOOKING\s+(\w+)#", $textInfo))
            ->date(strtotime($this->re("#^\s*(?:BOOKING CONFIRMATION|FINAL PAYMENT NOTICE|FINAL PAYMENT REMINDER|UPGRADE NOTICE|DEPOSIT CONFIRMATION|Itinerary Change|).*\s*BOOKING\s+\w+.*[ ]{2,}(.+?)\n#", $textInfo)));
    ...
    private function parseEmail_1(Email $email)
    {
        ...
        if ($rows->length === 0) {
            $xpath = "//*[contains(text(), 'CANCELLATION') and contains(text(), 'SCHEDULE')]/ancestor::table[1]/following-sibling::table[1]/descendant::tr[1]/following::tr[1]/td//tr[not(.//*[position()=1 and contains(text(), 'Date')])]";
            $rows = $this->http->XPath->query($xpath);
        }
...
//        $this->logger->alert('email 1');
        ...
        foreach ($this->http->FindNodes("//*[contains(text(), 'PASSENGERS')]/ancestor::table[1]/descendant::tr//*[contains(text(),'Member Number:')]/ancestor-or-self::td[1]/following-sibling::td") as $key => $name) {
            if ($name) {
                ++$key;
                $firstName = $this->http->FindSingleNode("//*[contains(text(), 'PASSENGERS')]/ancestor::table[1]/descendant::tr//*[contains(text(),'Last')]/ancestor-or-self::td[1]/following-sibling::td[{$key}]");
                $lastName = $this->http->FindSingleNode("//*[contains(text(), 'PASSENGERS')]/ancestor::table[1]/descendant::tr//*[contains(text(),'First/Middle Name:')]/ancestor-or-self::td[1]/following-sibling::td[{$key}]");
                $paxName = preg_replace("/^(?:Mrs\s|Mr\s|Ms\s|Miss\s)/", "", $firstName . ' ' . $lastName);

                if (!empty(trim($paxName))) {
                    $r->program()
                        ->account($name, false, $paxName);
                } else {
                    $r->program()
                        ->account($name, false);
                }
        ...
        foreach ($rows as $i => $row) {
            ...
            if (empty($arrTime) && !empty($depTime)) {
                if (!isset($s)) {
                    ...
                    $dtAboard = strtotime($date . " $year, " . $depTime);

                    if (!empty($this->lastDate) && $dtAboard < $this->lastDate) {
                        $dtAboard = strtotime('+1 year', $dtAboard);
                        $year = date('Y', $dtAboard);
                    }

                    $s->setAboard($dtAboard);
                    $this->lastDate = $dtAboard;
                    $overnight = false;
                } else {
                    $s = $r->addSegment();
                    $this->logger->debug('check type of all segments');
                ...
                if (preg_match("#\bOvernight\b#i", $name)
                    ...
                    $dtAshore = strtotime($date . " $year, " . $arrTime);

                    if (!empty($this->lastDate) && $dtAshore < $this->lastDate) {
                        $dtAshore = strtotime('+1 year', $dtAshore);
                        $year = date('Y', $dtAshore);
                    }

                    $s->setAshore($dtAshore);
                    $this->lastDate = $dtAshore;
                    $overnight = true;
                } elseif ($isCruise === true) {
