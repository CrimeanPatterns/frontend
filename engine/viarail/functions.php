...
//        $this->http->SetInputValue('rememberMe', "on");
    ...
    {
        $this->waitForElement(WebDriverBy::xpath("//p[contains(text(), 'Membership #:')] | //span[contains(@class, 'alert-message') or contains(@class, 'error-')] | //strong[contains(text(), 'I want to become a VIA Préférence member.')] | //legend[contains(text(), 'Get a code from your authenticator app.')]"), 10);
        $this->saveResponse();
    ...
    {
        $question = $this->http->FindSingleNode("//legend[contains(text(), 'Get a code from your authenticator app.')]");

        if (!$question) {
            return false;
        }

        $input = $this->waitForElement(WebDriverBy::xpath("//input[@id = 'verifyCodeFormCode']"), 0);
        $btn = $this->waitForElement(WebDriverBy::xpath("//button[contains(., 'Submit')]"), 0);
        $this->saveResponse();
