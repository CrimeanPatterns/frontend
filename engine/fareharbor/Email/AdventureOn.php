...
class AdventureOn extends \TAccountChecker
{
    ...
    public static $dict = [
        'en' => [
            ...
            'Google Link Map'                 => ['Google Link Map', 'Google Map Link', 'Click here for a map of the location',
                ...
                'Get directions ›', 'Click here for directions!', 'View in Google Maps', 'Click here for a map link',
                'See pickup location on a map »',
            ],
            'We are located at'       => ['We are located at', 'Our address is'],
            'Meeting Location'        => ['Meeting Location', 'Meeting Location:', 'Meeting location:', 'Address', 'Driving Directions', 'MEETING LOCATION:', 'Please check-in at our', 'Train Station Address / Check-In Location', 'Directions:'],
            'Directions'              => ['Directions', 'Directions:', 'DIRECTIONS:', 'IMPORTANT NOTE:', 'Directions to the'],
            'Activity Location'       => ['Activity Location', 'Location', 'Check-In Location', 'Where to meet:', 'Physical Address:'],
            'addressPrefix'           => ['Address:', 'Check-in at the', 'Please meet us at the', 'Our boat is located in the', 'The tour meets at the', 'Pickup will be at', 'Location', 'Check-In Location', 'WHEN AND IF is located at', 'located at', 'tour office (', 'End at', 'We are located in', 'All our tours start at our headquarters (', 'TOUR IS HELD AT', 'Located at', 'The park address is', 'Limited parking is available at the', 'Our physical address is'],
            'addressPostfix'          => ['boat ramps located', 'Please call', 'Click for directions', 'in front of the', 'You may park anywhere under the trees', ') at least', 'and arrive', 'Our driveway is', 'There is plenty of', '). We kindly ask you to', 'Phone:', 'We recommend typing', 'for your cruise', 'When using'],
            'Taxes & Fees'            => ['Taxes & Fees', 'Fees', 'Taxes'],
            'The address is'          => ['The address is', 'The address of the event is', 'MEETING AND ENDING LOCATION:'],
            'The starting address is' => ['The starting address is', 'Meeting point will be at', 'We are located at the', 'Departure spot is located at', 'Our GPS address is', 'Driving Directions:', 'We are located at', 'Check in address:', 'Located one', 'Directions to our Pick Up and Launch Zone -', 'The departure is from', 'Meet us at', 'Maps and directions to', 'Parking address:'],
            'departs from'            => ['departs from', 'depart from', 'trip begins and ends at', 'Please meet us at'],
            'Our Address is'          => ['is located at', 'We are located at', 'Located at', 'Google Place ID', 'Our Address is', 'Our address', 'Boat Address', 'the address for the parking lot is'],
        ],
        'fr' => [
    ...
    private function parseEmail(Email $email, $roots): void
    {
        ...
        if (empty($order)) {
            $order = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Confirmation for Order #'))}]",
                null, true, "/^\s*{$this->opt($this->t('Confirmation for Order #'))}\s*([A-Z]+)\b/");
        }
        ...
        $userEmails = array_unique(array_filter($this->http->FindNodes("//*[ (count(tr) = 2 or count(tr) = 3) and tr[last()][{$this->contains('@')}] ]/tr[last()][normalize-space()]",
            null, "/^\s*(\S+@\S+)\s*$/")));

        if (empty($userEmails)) {
            $userEmails = array_unique(array_filter($this->http->FindNodes("//text()[{$this->eq($this->t('Email:'))}]/following::text()[normalize-space()][1]",
                null, "/^\s*(\S+@\S+)\s*$/")));
        }

        if (empty($userEmails)) {
            $userEmails = array_unique(array_filter($this->http->FindNodes("//text()[{$this->eq($this->t('Email'))}]/preceding::text()[normalize-space()][1]",
                null, "/^\s*(\S+@\S+)\s*$/")));
        }

        if (count($userEmails) === 1) {
            $email->setUserEmail(end($userEmails));
        }
        ...
        foreach ($roots as $rootKey => $root) {
            ...
            // travellers
            $travellers = $this->http->FindNodes("//h2[{$this->eq($this->t('Details'))}]/following::table[normalize-space()][1]{$condition}//text()[{$this->eq($this->t('Full Name:'))}]/following::text()[normalize-space()][1]", null, "/^\s*([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])\s*$/");
            ...
            if (empty($travellers)) {
                $traveller = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Contact Name'))}]{$condition}/preceding::text()[normalize-space()][1]", null, true, "/^\s*([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])\s*$/");
            ...
            // Ventura -->Painted Cave Excursion   =>   Ventura - Painted Cave Excursion
            ...
            if ($type == 'rental') {
                ...
                if (!empty($ev['endDate'])) {
                    $r->dropoff()->date($ev['endDate']);
                }

                // no end date
                if (!empty($ev['startDate']) && empty($ev['endDate'])) {
                    $r->dropoff()->noDate();
                }
    ...
    private function getAddressEn($condition = ''): array
    {
        ...
        if (empty($address)) {
            $address = $this->http->FindSingleNode("//text()[{$this->starts('Directions')}]/following::text()[normalize-space()][1]{$condition}", null, false, "/^(\d+\-\d+.+\s\d+)$/");
        }
        ...
        if (empty($address)) {
            $addr = array_unique(array_filter($this->http->FindNodes(".//text()[{$this->eq($this->t('Meeting Location'))}]/following::text()[normalize-space()!=''][position()<3]{$condition}", null, "#\d.{5,50} [A-Z][a-z]+, [A-Z]{2}(?:\.?|\s\d+)$#")));
        ...
        if (empty($address)) {
            ...
            if (empty($link)) {
                $link = $this->http->FindSingleNode("(//a[contains(@href, '/goo.gl/maps/') or contains(@href, 'https://www.google.com/maps/place/') or contains(@href, 'https://maps.app.goo.gl/')]{$condition})[1]/@href");
            }
            ...
            if (empty($link)) {
                $link = $this->http->FindSingleNode(".//*[{$this->eq($this->t('Pickup Location:'))}]/following-sibling::*[1]/descendant::a[{$this->contains('google.com/maps', '@href')}]/@href{$condition}");
            }
            ...
            if (!empty($link)) {
                ...
                if (empty($address) || stripos($address, 'Google Maps') !== false) {
                    ...
                    if (preg_match("/https:\/\/www\.google\.com(?:\.pr|\.au)?\/maps\/place\/([^\/]{5,})/", $url, $m)) {
                        // it-49337442.eml
                        ...
                        $address = urldecode($m[1]);
                    } elseif (preg_match("/^{$this->opt('https://www.google.com/maps/search/')}([^\/&=?]+?)(?:$|[\/&=?])/", $url, $m)) {
                        $address = preg_replace("/\s+/", ' ', urldecode($m[1]));
                    }
                }
            }
        }
        ...
        if (empty($address)) {
            $addressNodes = $this->http->FindNodes("(//text()[{$this->contains($this->t('Our Address is'))}]{$condition})/ancestor::p[1]",
                null, "/{$this->opt($this->t('Our Address is'))}[\s\:]+(.{10,70}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        ...
        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains('Address')}]{$condition})[1]/ancestor::p[1]",
                null, false, "/^\s*{$this->opt('Address')}[\s\:]+(.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        }

        if (empty($address)) {
            $addressNodes = $this->http->FindNodes("(//text()[{$this->contains('We are located at')}]{$condition})/ancestor::p[1]",
                null, "/{$this->opt('We are located at')}[\s\:]+(\d+.{10,50}?)\s*\-\s+/");
        ...
        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains('We are located at')}]{$condition})[1]/ancestor::h1[1]",
                null, false, "/{$this->opt('We are located at')}[\s\:]+(.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains($this->t('Google Link Map'))}]{$condition})[1]/ancestor::p[1]",
                null, false, "/\b(\d+.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\s*{$this->opt($this->t('Google Link Map'))}/i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains($this->t('departs from'))} and {$this->contains('at')}]{$condition})[1]/ancestor::p[1]",
                null, false, "/{$this->opt($this->t('departs from'))}\s+.{10,40}?\s+at\s+(\d+.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains('Locations for Tours and Rentals:')}]{$condition})[1]/following::text()[normalize-space()][1]",
                null, false, "/^\s*(\d+.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains('See pickup location on a map')}]{$condition})[1]/preceding::text()[normalize-space()][1]",
                null, false, "/^\s*(.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//a[{$this->contains(['maps', 'g.page'], '@href')}]{$condition})[1]",
                null, true, "/^\s*(\d+.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\s*$/i");
        }

        // with precondition
        // Where would you like us to pick you up?: Cruise Ship Docks
        if (empty($address) && !empty($this->http->FindSingleNode("//text()[{$this->starts('Where would you like us to pick you up?')}]/following::text()[normalize-space()][1][{$this->contains('Dock')}]"))) {
            $address = $this->http->FindSingleNode("(//text()[{$this->starts(['Dock Address'])}]{$condition})[1]",
                null, false, "/^\s*{$this->opt('Dock Address')}[\s\:]+(\d+.{10,50}?[\s\,]+[a-z]{2}\s+\d{5})\b/i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->contains('across the street from')}]{$condition})[1]",
                null, false, "/\s*{$this->opt('across the street from')}\s*(.+?[\s,]+[a-z]{2}[,\s]+\d{5})\s*\./i");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("(//text()[{$this->starts('Thank you for registering for covered seating at')}]{$condition})[1]",
                null, false, "/^\s*Thank you for registering for covered seating at\s+(.+?)\s+for\s+/");
        }

        if (empty($phone)) {
            $phone = $this->http->FindSingleNode("//text()[{$this->eq('•')}]/ancestor::*[ count(./*[normalize-space()]) <= 5 and ./*[normalize-space()][2][{$this->eq('•')}] and ./*[normalize-space()][3][{$this->contains(['@', '.'])} and not({$this->contains(' ')})] ][1]/*[normalize-space()][1]",
                null, true, "/^\s*({$this->patterns['phone']})\s*$/");
        }
