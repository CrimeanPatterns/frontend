...
class TAccountCheckerAce extends TAccountChecker
{
    ...
    public function Parse()
    {
        ...
        $this->http->PostURL('https://www.acehardware.com/coupons/rewardsOffers', null, $this->headers);
        $coupons = $this->http->JsonLog() ?? [];

        if (count($coupons) > 0) {
            $this->sendNotification('refs #25366 ace - need to check coupons // IZ');
        }

        foreach ($coupons as $coupon) {
            $code = $coupon->Barcode ?? null;
            $displayName = $coupons->DisplayOffer ?? null;
            $expirationDate = $coupon->BarcodeEndDate ?? null;

            if (
                !isset($code, $displayName, $expirationDate)
