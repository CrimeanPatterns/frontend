...
class TicketEMDPdf extends \TAccountChecker
{
    ...
    private $airports = [
        ...
        'NICE COTE D AZUR',
        'BASTIA PORETTA',
        'FORT DE FRANCE MARTINIQUE A.CESAIRE',
        ...
        'TABUK TABUK/SA',
        // vietnam
        'HO CHI MINH CITY TAN SON NHAT INTL, VN',
        'HO CHI MINH CITY TAN SON NHAT',
        'BANGKOK SUVARNABHUMI INTL, TH',
        'CON DAO ISLAND CO ONG, VN',
        'DA NANG INTERNATIONAL, VN',
        'HANOI NOI BAI INTL, VN',
        // kenyaair
    ...
    private $detectLangHTML = [
        ...
        'vi' => ['Vui lòng xem thông tin hành trình và vé điện tử được gửi kèm email này.'],
    ];
    private $detectPdf = [
        'TICKET RECEIPT',
        '/ ELECTRONIC TICKET',
    ...
    private $terminalTitle = ['Terminal', 'Terminale', 'Nhà Ga', '航站', '终端'];
    ...
    private static $dictionary = [
        ...
        'en' => [
            ...
            "paymentEnd"            => ["ELECTRONIC MISCELLANEOUS DOCUMENT"],
        ...
        "fr" => [ // it-681693099.eml
            ...
            'Base Fare'             => 'Tarif',
            //            "cancelledTicket" => "",
            "Special Service Request" => "Demande de service spécial",
        ],
        "is" => [
            "Arrival"     => "Koma",
            "From"        => "Frá",
        ...
        "zh" => [
            ...
            "Frequent flyer number" => ["會員編號", "Loyalty programme number"],
            "paymentStart"          => ["票價明細", "票价详细信息"],
            "paymentEnd"            => ["其他雜費和附加費收據", "注意事項"],
            "Total Amount"          => "總額",
            'Base Fare'             => ['Prísur', '票面價'],
            //            "cancelledTicket" => "",
        ],
        "pt" => [
            "Arrival"     => "Para",
        ...
        "vi" => [
            ...
            "From"        => "Từ",
            "Class"       => "Hạng",
            "Operated by" => "Hãng vận chuyển",
            // 'Aircraft type' => '',
            "Duration"    => "Thời gian bay",
            "Seat"        => "Chỗ",
            // "Number of stops" => "",
            "Frequent flyer number" => "Mã số khách hàng thường xuyên",
            "paymentStart"          => ["CHI TIẾT THANH TOÁN", "CHI TIẾT GIÁ VÉ", "Hình thức thanh toán"],
            // "paymentEnd" => [""],
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if (count($pdfs)) {
            $this->airports = array_values(array_unique($this->airports));
        ...
        foreach ($pdfs as $pdf) {
            ...
            $isJunk[] = $this->pdfIsJunk($text) ? 'YES' : 'NO';

            if ($this->assignProvider(preg_replace('/\s+/', ' ', $text), $parser->getCleanFrom(), $parser->getSubject()) !== true) {
            ...
            $header = $this->findCutSection($text, null,
                ['ELECTRONIC TICKET RECEIPT', 'TICKET RECEIPT', 'ELECTRONIC TICKET', 'REÇU DE BILLET ÉLECTRONIQUE', 'Reçu du ticket électronique', 'TICKET CREDIT RECEIPT', '電子機票收據', 'Electronic ticket receipt', 'RECIBO DE BILHETE ELETRÓNICO', 'Ricevuta del biglietto elettronico', 'PHIẾU THU VÉ ĐIỆN TỬ', 'Recibo de Bilhete Eletrônico', 'TRAVEL DOCUMENT', 'Elektronischer Ticketbeleg', 'REISEDOKUMENT']);

            if (preg_match("/(?:Booking [Rr]ef\:?(?:erence)?|{$this->opt(['BOOKING REFERENCE', 'Código de reserva', 'Booking code', 'Numéro de réservation', 'Référence de la réservation', 'Référence de la', 'Reference du dossier', 'Bíleggingarnr.', 'ﺰﺠﺤﻟا ﻢﻗر', 'Numero di prenotazione', 'Numero di', 'Mã đặt chỗ', 'Referência da reserva', 'Bokningsnummer', 'Riferimento prenotazione', 'Buchungsnummer', '預訂參考編號', 'Buchungscode'], true)})(?:\s*\/\s*\S[^:\n]*?\s*[:]+)?\s*:?\s*(?:[A-Z\d]*\/)?([A-Z\d]{5,})\b/u", $header, $matches)) {
            ...
            if (preg_match('/(?:Passenger|Passager|Passageiro|Guest|Ferðandi|ﺐﻛاﺮﻟا ﻢﺳا|旅客|Passeggero|Hành khách|Passagerare|Gast|Passagier)(?:\s*\/\s*\S[^:\n]*?)?\s*[:]+\s*(.+?)(?: (?:Mrs|Mr|Miss|Ms))?(?:\s*\(.*?\))?(?:\s{3,}|Booking ref|Issuing|\n)/si', $header, $matches)
            ...
            if (preg_match("/{$this->opt(['Ticket number', 'Numéro de billet', 'Ferðaseðlanummar', 'Número do Bilhete', 'Numero Biglietto', 'Biljettnummer', 'E-ticketnummer', 'Số vé'], true)}(?:\s*\/\s*\S[^:\n]*?)?\s*[:]+\s*({$patterns['eTicket']})(?:[ ]{2}| ?\n)/i", $header, $matches)
            ...
            if (preg_match("/{$this->opt(['Issue date', 'Issuing date', 'Date of issue', 'Data di emissione', 'Ngày', 'Date', 'Data', 'Datum', 'Ausstellungsdatum'], true)}(?:\s*\/\s*\S[^:\n]*?)?\s*[:]+\s*([-A-z\d ,]{4,}\d+)(?: ?\n|$)/", $header, $matches)
            ...
            foreach (['Last check-in', 'Arrival', 'Arrivée', '抵達時間', '目的地', 'Para', 'Arrivo', 'Điểm đến', 'Till', 'Ankunft'] as $phrase) {
            ...
            if (preg_match_all("/[A-Z]{2,}.+?(?:\d+:\d+|∆∆:∆∆).+?(?:number:|Seat|Duration|Flight duration|Durée|{$this->opt($this->t('Seat'))}|{$this->opt($this->t('Duration'))}).+?(?:_|\n ?{$this->opt($this->t('Special Service Request'), true)})/us", $textSegment, $items, PREG_PATTERN_ORDER)
                || preg_match_all("/[A-Z]{2,}.+?(?:\d+:\d+|∆∆:∆∆).+?(?:number:|Seat|Duration|Flight duration|Durée|{$this->opt($this->t('Seat'))}|{$this->opt($this->t('Duration'))}).+?(?:_|\n\n)/us", $textSegment, $items, PREG_PATTERN_ORDER)) {
                $allSegments = $items[0];
        ...
        $this->logger->info('JUNK STATUSES (PDFs):');
        $this->logger->debug(print_r($isJunk, true));
        ...
        $typeParts[] = ucfirst($this->lang);
        $typeParts[] = ucfirst($this->lang2);
        $email->setType(implode('', $typeParts));
    ...
    public static function getEmailProviders()
    {
        return [
            ...
            'cairo', 'tunisair', 'vietnam', 'kenyaair', 'norwegian', 'aircorsica', 'uzair', 'asia',
        ];
    }
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if ($this->pdfIsJunk($text)) {
                return true;
            }

            if ($this->assignProvider($text, $parser->getCleanFrom(), $parser->getSubject()) !== true) {
                continue;
            }
    ...
    private function pdfIsJunk(string $textPdf): bool
    {
        if (preg_match('/^((?:\n*.+){1,20})/', $textPdf, $m)) {
            $startFragment = $m[1]; // get first lines only
        } else {
            return false;
        }
        // Supported Languages: en

        if (preg_match("/(?:^[ ]{0,15}|\/[ ]*)ELECTRONIC\s+MISCELLANEOUS\s+DOCUMENT(?:\s+RECEIPT|\s*\(\s*EMD\s*\))*(?:[ ]*\/|$)/im", $startFragment)
            && !preg_match("/{$this->opt(['ELECTRONIC TICKET RECEIPT', 'Flight duration', 'REASON FOR ISSUANCE'], true)}/i", $textPdf)
        ) {
            $this->junkReasons[] = 'flights not found';
    ...
    private function assignProvider($textPdf, ?string $from, string $subject): bool
    {
        if (preg_match('/[.@]vietnamairlines\.com$/i', $from)
            || $this->http->XPath->query('//*[contains(normalize-space(),"Cảm ơn Quý khách đã lựa chọn dịch vụ của Vietnam Airlines") or contains(normalize-space(),"Vietnam Airlines kính chúc Quý khách một chuyến bay tốt đẹp")]')->length > 0
            || stripos($textPdf, 'VIETNAM AIRLINES WEBSITE') !== false
            || stripos($textPdf, 'trên website của Vietnam Airlines') !== false
            || stripos($textPdf, 'Liên hệ trung tâm Chăm sóc khách hàng của Vietnam Airlines') !== false
        ) {
        ...
        if (stripos($from, '@flygbra.se') !== false
        ...
        if (stripos($from, '@kenya-airways.com') !== false
            || stripos($subject, 'Kenya Airways') !== false
        ...
        if (stripos($subject, 'Your Kuwait Airways electronic ticket') !== false
        ...
        if (strpos($textPdf, 'LOT.COM') !== false
            || stripos($textPdf, 'www.lot.com') !== false
            || preg_match('/Order offer on\s+(?-i)LOT(?i) Shop/i', $textPdf)
        ) {
        ...
        if (stripos($from, '@bangkokair.com') !== false
            || stripos($subject, 'Bangkok Airways: Ticketed') !== false
        ...
        if (stripos($from, 'CZECH AIRLINES ONLINE BOOKING OFFICE') !== false
        ...
        if (stripos($from, 'AIRCARAIBES.COM') !== false
        ...
        if (stripos($from, 'FLYBE LTD') !== false
            || stripos($from, '@flybe.com') !== false
        ...
        if (stripos($from, '@avianca.com') !== false
        ...
        if (stripos($subject, 'Your Luxair e-ticket receipt') !== false
        ...
        if (strpos($textPdf, 'HY INTERNET SITE') !== false // HY - is IATA code for Uzbekistan Airways
            || strpos($textPdf, 'UZBEKISTAN AIRWAYS WEB') !== false
            || stripos($textPdf, 'www.uzairways.com') !== false
            || stripos($textPdf, '+99878 1400200') !== false
        ) {
        ...
        if (
            stripos($from, 'booking@flyaircairo.com') !== false
        ...
        if (strpos($textPdf, 'TUNISAIR SIEGE') !== false) {
            $this->providerCode = 'tunisair';
        ...
        if (preg_match('/visit\s+qatarairways\.com/i', $textPdf)
            || stripos($textPdf, 'QATAR AIRWAYS') !== false
        ) {
        ...
        if (false !== stripos($textPdf, 'CATHAY PACIFIC AIRWAYS')) {
        ...
        if (stripos($from, 'eticket@amadeus.com') !== false
            || strpos($textPdf, 'or contact a Corsair') !== false
            || stripos($textPdf, '.corsair.fr/') !== false
            || strpos($textPdf, 'Thank you for choosing Corsair') !== false
    ...
    {
        $paymentDetails = $this->re("/\n(.*\b{$this->opt($this->t('paymentStart'), true)}\s[\s\S]+?)(?:\n+[ ]*(?i){$this->opt($this->t('paymentEnd'), true)}|$)/u", $text);
        ...
        if (preg_match("/(?:Taxes|Taxas|Tasse|Thuế|Steuern|稅額).*?:\s+(.+?[A-Z].+?)\s*(?i){$this->opt(['Montant total', 'Total Amount', 'Valor Total', 'Importo totale', 'Tổng tiền', 'Gesamtsumme', '總額'], true)}(?:\s*:|\s*\/|[ ]{2}|\s*\()/s", $paymentDetails, $m)) {
            // it-49583506.eml
    ...
    private function parseSegment(Flight $f, $text): void
    {
        ...
        // it-5116403.eml
        $text = preg_replace("/^\s*(.*\n+){1,2}{$this->opt($this->t('Special Service Request'))}\n/u", "", $text);
        ...
        $text = preg_replace("/^ *[A-Z]{4} - (?:[A-Z]+[ \/])+- (?:CONFIRMED|CONFIRMÉ|REQUESTED)\s*$/mu", "\n\n", $text);
        ...
        if (preg_match("/{$this->opt($this->t('Seat'))}[^:]*?\n*[:]*\s*(\d{1,3}[A-Z])\b/u", $text, $m)
            ...
            || preg_match("/\n\n(\d{1,2}[A-Z])\n[\s\n]+{$this->opt($this->t('Class'))}\s*:/u", $text, $m)
        ) {
            if (!empty($m[1])) {
                $s->extra()->seat($m[1]);
            }
