...
//        $this->logger->debug('HOTEL XPath = ' . print_r($hotelsXPath, true));
    ...
    private function parseCars(Email $email, \DOMNodeList $roots, array $totalTravellers): void
    {
        ...
        foreach ($roots as $root) {
            ...
            $pickUpPhone = $this->http->FindSingleNode(".//text()[" . $this->contains($this->t("Pickup Phone:")) . "]/following::text()[normalize-space()][1]/ancestor-or-self::*[not(" . $this->contains($this->t("Pickup Phone:")) . ")][1]",
                $root, true, "/^[\d\-\+ \-\(\)]{5,}$/");
            ...
            $pickOffPhone = $this->http->FindSingleNode(".//text()[" . $this->contains($this->t("Dropoff Phone:")) . "]/following::text()[normalize-space()][1]/ancestor-or-self::*[not(" . $this->contains($this->t("Dropoff Phone:")) . ")][1]",
                $root, true, "/^[\d\-\+ \-\(\)]{5,}$/");

            if (!empty($pickOffPhone)) {
                $r->dropoff()
                    ->phone($pickOffPhone);
            }
    ...
    private function normalizeDate($str, $dateFormat = null)
    {
        $in = [
            ...
            // 7:33 PM - Sun, 08/17/25
            "#^(\d{1,2}:\d+(?:\s*[ap]m)?)\s*-\s*\D+(\d{1,2})\/(\d{1,2})\/(\d{2})\s*$#ui",
        ];
