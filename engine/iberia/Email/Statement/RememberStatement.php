...
class RememberStatement extends \TAccountChecker
{
    ...
    private $reSubject = [
        ...
        'Iberia Plus is evolving with you',
        // de
    ...
    private $reBody = [
        'en' => [
            ...
            ['Balance at', 'DATA PROTECTION INFORMATION'],
        ],
        'fr' => [
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        Balance at
        */
        $preg1 = "/^\s*(?<name>[[:alpha:]\s\-\']{4,}\b)\s+(?<number>IB\s*\d+)\s+"
            . "(?<balance>-?\d[\d.,\s]*)(?:{$this->opt($this->t('Avios'))})?"
        ...
        Saldo a
         */
        $preg3 = "/^\s*(?<name>[[:alpha:]\s\-\']{4,}\b)\s+"
            . "(?<number>IB\s*\d+)\s+(?:Avios\s*)?{$this->opt($this->t('Balance at'))}\s*/us";

        $this->logger->debug('$preg1 = ' . print_r($preg1, true));
        //$this->logger->debug('$preg2 = '.print_r( $preg2,true));
        //$this->logger->debug('$preg3 = '.print_r( $preg3,true));

        if (preg_match($preg1, $root, $m) || preg_match($preg2, $root, $m)) {
            $this->logger->debug('preg1 or preg2');

            if (empty(trim($m['name']))) {
                $m['name'] = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Hello'))}]", null, true, "/{$this->opt($this->t('Hello'))}\s*(\D+)\,/");
            }
