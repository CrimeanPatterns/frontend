...
class IberiaExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface, ParseHistoryInterface, ParseItinerariesInterface, LoginWithConfNoInterface, RetrieveByConfNoInterface, ActiveTabInterface
{
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        ...
        if (!strstr($login, '@')) {
            $login = trim(preg_replace("/(?:^IB\s*|[^[:print:]\r\n])/ims", "", $login));
        }
        $tab->evaluate('//input[@id="loginPage:theForm:loginEmailInput"]')->setValue($login);
        ...
        $innerText = $result ? $result->getInnerText() ?? '' : '';
        $this->logger->notice("[Message]: {$innerText}");

        if (stristr($innerText, "Login has failed. Some of the details you entered may be incorrect")
            || stristr($innerText, "No hemos podido conectarte. Es posible que alguno de los datos ")) {
            return LoginResult::invalidPassword($innerText);
        }
        if (strtoupper($result->getNodeName()) === 'IMG') {
            return new LoginResult(true);
        }
        if (strstr($innerText, 'Sorry, an error has occurred. Please try again later.')) {
