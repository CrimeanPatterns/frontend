...
{
    public $mailFiles = "blade/it-637073097.eml, blade/it-922120018.eml";
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if (preg_match("/\b(Order\s*#)\s*([-A-Z\d]{5,})\b/i", $parser->getSubject(), $m)) {
            ...
            $orderIdValues = array_filter($this->http->FindNodes("//a[{$this->contains(['order_id=', 'order_id%3D'], '@href')}]/@href", null, "/{$this->opt(['order_id=', 'order_id%3D'], '@href')}([-A-Z\d]{5,})(?:&|%26|$)/"));
        ...
        $segments = $this->http->XPath->query("//tr[ count(*)=3 and *[1][normalize-space()] and *[2][normalize-space()='']/descendant::img and *[3][normalize-space()] ]");

        foreach ($segments as $root) {
            $s = $t->addSegment();

            $dateDep = $dateArr = $timeDep = $timeArr = null;
            $dateDepVal = $this->http->FindSingleNode("preceding::tr[not(.//tr[normalize-space()]) and normalize-space()][1][count(*[normalize-space()])=2]/*[{$this->starts($this->t('departs'))}]", $root, true, "/^{$this->opt($this->t('departs'))}[:\s]+(.{4,})$/");
            $dateArrVal = $this->http->FindSingleNode("preceding::tr[not(.//tr[normalize-space()]) and normalize-space()][1][count(*[normalize-space()])=2]/*[{$this->starts($this->t('arrives'))}]", $root, true, "/^{$this->opt($this->t('arrives'))}[:\s]+(.{4,})$/");
            ...
            $nameDep = $this->http->FindSingleNode("*[1]/descendant::text()[normalize-space()][1]/ancestor::*[ following-sibling::node()[normalize-space()] ][1]", $root);
            $addressDep = preg_replace($pattern = "/^(.{3,}?)[,(\s]+{$this->opt($this->t('view map'))}.*/i", '$1', implode(', ', $this->http->FindNodes("*[1]/descendant::text()[normalize-space() and not(ancestor::*[self::h1 or self::h2])]", $root, '/^[\s,;]*(.{2,}?)[\s,;]*$/')));
            ...
            $nameArr = $this->http->FindSingleNode("*[3]/descendant::text()[normalize-space()][1]/ancestor::*[ following-sibling::node()[normalize-space()] ][1]", $root);
            $addressArr = preg_replace($pattern, '$1', implode(', ', $this->http->FindNodes("*[3]/descendant::text()[normalize-space() and not(ancestor::*[self::h1 or self::h2])]", $root, '/^[\s,;]*(.{2,}?)[\s,;]*$/')));
            ...
            $vehicle = $this->http->FindSingleNode("following::tr[not(.//tr) and normalize-space()][1][ following::tr[not(.//tr)][normalize-space() or descendant::img][1][normalize-space()='' and descendant::img] ]", $root, true, "/^(.{2,}?)[:\s]*(?:{$this->opt($this->t('Operated by'))}|$)/i");
            $image = $this->http->FindSingleNode("following::tr[not(.//tr) and normalize-space()][1]/following::tr[not(.//tr)][normalize-space() or descendant::img][1][normalize-space()='']/descendant::img[@src]/@src", $root);
