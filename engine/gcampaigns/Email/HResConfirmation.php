...
{
    public $mailFiles = "gcampaigns/it-12135301.eml, gcampaigns/it-12191158.eml, gcampaigns/it-12191201.eml, gcampaigns/it-12191267.eml, gcampaigns/it-1645974.eml, gcampaigns/it-1786612.eml, gcampaigns/it-2422896.eml, gcampaigns/it-2976384.eml, gcampaigns/it-2987725.eml, gcampaigns/it-2999847.eml, gcampaigns/it-3.eml, gcampaigns/it-3017395.eml, gcampaigns/it-3017396.eml, gcampaigns/it-3542837.eml, gcampaigns/it-4.eml, gcampaigns/it-46126107.eml, gcampaigns/it-46206959.eml, gcampaigns/it-46316011.eml, gcampaigns/it-46318201.eml, gcampaigns/it-46340289.eml, gcampaigns/it-47165359.eml, gcampaigns/it-47225021.eml, gcampaigns/it-47545407.eml, gcampaigns/it-55549728.eml, gcampaigns/it-644232724.eml, gcampaigns/it-702618968.eml, gcampaigns/it-902997265.eml"; // +3 bcdtravel(html)[en]
    ...
    public static $dict = [
        'en' => [
            ...
            'your reservations at the'   => ['planning your stay at', 'your reservations at the', 'your reservations at', 'Your reservation at the', 'your reservation at', 'successfully booked your visit to', 'Your reservation at',
                'for a reservation at', 'confirm your reservation with the', 'We are pleased to confirm your upcoming reservation at the', ],
            ...
            'Arrival Date:'              => ['Arrival Date:', 'Arrival:', 'Arrival date:', 'Arrival Date', 'Check-in:', 'Check-In:', 'Check In', 'ARRIVAL:', 'Check-In Date', 'Check-In Date:'],
            ...
            'Room Type:'                 => ['Room Type:', 'Room type:', 'Room Type', 'Room', 'GUESTROOM TYPE:', 'Room Type', 'Room type', 'Room type assigned:', 'Room:'],
            'Nightly Rate'               => ['Night by Night Rate', 'Nightly Rate', 'Rate', 'Nightly Rates:', 'Single Occupancy Rate per Room:', 'Daily Room Rate', 'Rate:'],
            'Total Charge:'              => ['Total Charge:', 'Total Charges:', 'Estimated Total Charge:', 'Total Room Charge', 'Total Charge', 'Estimated Cost', 'Total Room Charge:', 'Total room charge:', 'Total Room Charges', 'Subtotal:', 'Total Charge including', 'Sub-Total:'],
    ...
    private function parseEmail(Email $email): void
    {
        ...
        if (/*empty($hotelName) && */ empty($address) && empty($phone)) {
            $addressText = implode("\n", $this->http->FindNodes("//text()[{$this->starts($this->t('Telephone:'))}]/ancestor::tr[1]/descendant::text()[string-length()>1]"));

            if (preg_match("/^(?<hotelName>\D+)\n(?<address>.+)\n{$this->opt($this->t('Telephone:'))}\:?\s*(?<phone>[+\d\-\s\.]+)\s*Fax:\s*(?<fax>[+\d\-\s\.]+)$/msu", $addressText, $m)) {
                $address = str_replace("\n", " ", $m['address']);
                $phone = $m['phone'];
                $fax = $m['fax'];
        ...
        if (empty($address) && empty($phone) && !empty($hotelName)) {
            $addressText = implode("\n", $this->http->FindNodes("//text()[{$this->contains($this->t('Telephone:'))}]/ancestor::table[1]/descendant::text()[normalize-space()]"));
        ...
        if (empty($checkin) && empty($checkout) && !empty($addressText)) {
            if (preg_match("/Number of Guests:\n(?<inDate>\w+\.?\s*\d+\,\s*\d{4})\n(?<outDate>\w+\.?\s*\d+\,\s*\d{4})\n/", $addressText, $m)) {
        ...
        if (empty($guests) && !empty($addressText)) {
            $guests = $this->re("/{$this->opt($this->t('Adults:'))}\s*(\d+)/", $addressText);
        }
        ...
        if ($kids === null && !empty($addressText)) {
            $kids = $this->re("/{$this->opt($this->t('Children:'))}\s*(\d+)/", $addressText);
        }

        $rooms = $this->re("/^(\d+)/", $this->nextTd($this->t('Number of Rooms:')));

        if (empty($rooms) && !empty($addressText)) {
            $rooms = $this->re("/^(\d+)\n{$this->opt($this->t('Adults:'))}/mu", $addressText);
        }
        ...
        if (empty($cancellation)) {
            $cancellation = $this->http->FindSingleNode("//img[contains(@alt, 'Cancellation Policy')]/following::text()[string-length()>2][1]");
        }
        ...
        if (!empty($roomType)) {
            ...
            if (count($roomRates) === 0) {
                $roomRates = $this->http->FindNodes("//text()[contains(normalize-space(), 'Rate:')]/ancestor::td[1]/following::td[1]/descendant::p/descendant::text()[normalize-space()][not(contains(normalize-space(), 'Rate'))]");
            }

            $roomRates = array_filter(preg_replace("/\s\d+\s*Confirmed\s*/", " - ", $roomRates));
    ...
    private function detectDeadLine(Hotel $h, string $cancellationText): void
    {
        ...
        if (
            preg_match("#^(?<time>\d+\s*a?p?m?)\s*(?<dayCount>\w+)\s*days\s*prior\s*to\s*arrival#i", $cancellationText, $m)
        ) {
            $m['dayCount'] = str_replace(['three'], ['3'], $m['dayCount']);
            $h->booked()->deadlineRelative($m['dayCount'] . ' day', $m['time']);
        }
    }
