...
class YourStatement extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            'AVAILABLE POINTS' => ['AVAILABLE POINTS', 'Available Points*', 'Available Points'],
        ],
    ];
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if ($this->http->XPath->query('//text()[starts-with(normalize-space(),"TIER:") or starts-with(normalize-space(),"CURRENT POINT BALANCE:")]')->length > 0) {
            ...
            $dateExpire = $this->http->FindSingleNode("//text()[{$this->contains($this->t('AVAILABLE POINTS'))}]/ancestor::td[1]", null, true, "/{$this->opt($this->t('Points Expire:'))}\s+(\d+\/\d+\/\d{4})/u");

            if (!empty($dateExpire)) {
                $st->setExpirationDate(strtotime($dateExpire));
            }
