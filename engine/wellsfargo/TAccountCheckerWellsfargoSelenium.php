...
//        $this->sendNotification("wellsfargo - Entering Advanced Access Code", 'awardwallet');
    ...
    public function Parse()
    {
        ...
        if ($rewardsPage) {
            ...
            // Name
            $this->SetProperty("Name", beautifulName(
                $this->http->FindSingleNode("//div[@id = 'gfr-sub-nav']//div[contains(@class, 'show-block')]//p[contains(@class, 'name')]")
                ?? $this->http->FindSingleNode('//span[contains(text(), "Welcome, ")]', null, true, "/Welcome, (.+)/")
            ));
            ...
            if (
                ...
                || $this->http->FindSingleNode('//div[contains(@class, "show-block")]//span[contains(@class, "rewardbal")]')
            ) {
                // Balance - GO FAR REWARDS
                $this->SetBalance($this->http->FindSingleNode('//span[@data-testid = "GO FAR REWARDS-balance" or @data-testid = "WELLS FARGO REWARDS-balance"] | //div[contains(@class, "show-block")]//span[contains(@class, "rewardbal")]'));

                if (strstr($this->Balance, '$')) {
                    $this->SetProperty("CurrencyType", 'CASH');
                }
            }
