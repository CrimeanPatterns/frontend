...
class It2 extends \TAccountChecker
{
    ...
    private function parseEmail(Email $email, string $text): void
    {
        ...
        if (empty($c->getConfirmationNumbers())
            && (preg_match("/^\s*{$this->opt(['ITINERARY CHANGE NOTIFICATION'])}\n/", $text)
                || preg_match("/^\s*{$this->opt(['IMPORTANT NOTIFICATION', 'VISA NOTIFICATION'])}\n/", $text)
                    && !preg_match("/(?:^[ ]*|[ ]{2}){$this->opt($this->t('Booking #'))}/im", $text)
            )
        ) {
