...
{
    public $mailFiles = "royalcaribbean/it-355697921.eml, royalcaribbean/it-51155282.eml, royalcaribbean/it-896377578.eml";
    ...
    private static $detectors = [
        ...
        'es' => ["Folleto de Pasaje del Huésped"],
    ];
    private static $dictionary = [
        ...
        'es' => [
            "RESERVATION ID:"                    => "NÚMERO DE RESERVA:",
            "THIS BOOKLET HAS BEEN PREPARED FOR" => "ESTE FOLLETO HA SIDO PREPARADO PARA",
            "MEMBERSHIP"                         => ["CROWN & ANCHOR AFILIACIÓN"],
            "SAILING FROM:"                      => ["SAILING FROM:"],
            "DECK #:"                            => "CUBIERTA:",
            "STATEROOM #:"                       => "CAMAROTE:",
            "CATEGORY:"                          => "CATEGORÍA:",
            "BOARDING DATE:"                     => "FECHA DE EMBARQUE:",
            'DAY'                                => 'DÍA',
            'DATE'                               => 'FECHA',
            'PORTS-OF-CALL'                      => 'PUERTOS DE LLAMADA',
            'DOCK OR'                            => 'MUELLE O',
            'ARRIVE'                             => 'LLEGADA',
            'DEPART'                             => 'SALIDA',
            'Travel Summary'                     => 'Resumen del Viaje',
            'Cruise Itinerary'                   => 'Itinerario del Crucero',
        ],
    ];
    private $from = "documentation@rccl.com";
    private $subject = ["Your Guest Vacation Documents are now ready for Reservation ID:", "Sus documentos de vacaciones de invitados ya están listos para la identificación de la reserva:"];
    ...
    private function parseEmailPdf(Email $email, $html, $text)
    {
        ...
        //Pax
        $rePax = "/{$this->opt($this->t('THIS BOOKLET HAS BEEN PREPARED FOR'))} +{$this->opt($this->t('MEMBERSHIP'))}.*(?:\n {30}.*)?" .
            "\n\s*([\s\S]+?)\n{3,}/";
        ...
        //Segment
        if (preg_match("/{$this->opt($this->t('Travel Summary'))}\n(?:[\s\S]*?\n)? *{$this->opt($this->t('Cruise Itinerary'))}((?:\n.*?)+?)\n *(?:Travel Documents|Port Directions|PARA CUALQUIER)/u", $text, $match)) {
            preg_match("/{$this->opt($this->t('DAY'))}\s*{$this->opt($this->t('DATE'))}\s*{$this->opt($this->t('PORTS-OF-CALL'))}\s*{$this->opt($this->t('DOCK OR'))}.*\s*{$this->opt($this->t('ARRIVE'))}\s*{$this->opt($this->t('DEPART'))}(?:\n {40,}.*)?((?:\n.*?)+?)\n\s*(?:\d{1}\s\n|Post Cruise Air Arrangements|For Any Day Of Travel Concerns|Post-Cruise Ground Arrangements|PARA CUALQUIER IMPREVISTO|$)/u",
                $match[1], $m);
            ...
            foreach ($segments as $segment) {
                ...
                if (isset($ts[5]) && !empty($ts[5])) {
                    ...
                    } elseif (preg_match("/^(?<week>\w+)\s+(?<day>\d+\s*\w+)\s*(?<name>.+)[ ]{2,}(\S)\s+(?<time>[\d\:]+\s*A?P?M?)$/", $segment, $m) && $this->aboard === false) {
                        $s = $r->addSegment();
                        $s->setName($m['name']);
                        $s->setAboard($this->normalizeDate($m['day'], $this->year, $m['week'], $m['time']));
                        $this->aboard = true;
                    } elseif (preg_match("/^(?<week>\w+)\s+(?<day>\d+\s*\w+)\s*(?<name>.+)[ ]{2,}(\S)\s+(?<time>[\d\:]+\s*A?P?M?)$/", $segment, $m) && $this->aboard === true) {
                        $s = $r->addSegment();
                        $s->setName($ts[2]);
                        $s->setAshore($this->normalizeDate($ts[1], $this->year, $ts[0], $ts[4]));
                        $this->aboard = false;
