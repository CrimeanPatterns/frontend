...
{
    public $mailFiles = "jetblue/it-30176369.eml, jetblue/it-30202841.eml, jetblue/it-36479639.eml, jetblue/it-888878973.eml";
    public $date;
    public $lang = '';
    public static $dict = [
        'en' => [
    ...
    private function parseEmail(Email $email)
    {
        ...
        foreach ($segments as $key => $segment) {
            ...
            foreach ($travellerRows as $row) {
                ...
                if (!empty($traveller) && !empty($ffNumber)) {
                    $ffNumbers[$traveller] = $ffNumber;
                }

                $seat = $this->http->FindSingleNode('./*[6]', $row, true, '/^\d{1,5}[A-Z]$/');

                if (!empty($traveller) && !empty($seat)) {
                    $seats[$traveller] = $seat;
                }
            }

            foreach ($seats as $traveller => $seat) {
        ...
        // accountNumbers
        foreach ($ffNumbers as $traveller => $ffNumber) {
            $f->program()->account($ffNumber, false, $traveller);
        }
        ...
        foreach ($hotels as $root) {
            ...
            $h->booked()->checkIn($this->normalizeDate($checkInText));

            $checkOutText = $this->http->FindSingleNode("following-sibling::tr[normalize-space()][1]/*[4]", $root);
            $h->booked()->checkOut($this->normalizeDate($checkOutText));
    ...
    private function normalizeDate(?string $date): ?int
    {
        ...
        $in = [
            '/^\s*(\w+),\s*(\w+)\s+(\d+)\s*$/iu', // Sun, Apr 09
            '/^\s*\w+,\s*(\w+)\s+(\d+)\s*(\d{4})$/iu', // Sat, Mar 07 2020
            '/^\s*(\w+),\s*(\w+)\s+(\d+)\s+(\d{1,2}:\d{2}(\s*[ap]m)?)\s*$/iu', // Sun, Apr 21 03:00 PM
        ];
        $out = [
            '$1, $3 $2 ' . $year,
            '$2 $1 $3',
