...
class Parser extends \TAccountChecker
{
    ...
    public function getRewardAvailabilitySettings()
    {
        ...
        return [
            ...
            'defaultCurrency'          => 'USD',
            'priceCalendarCabins'      => ["economy", "business", "unknown"],
        ];
    }
    ...
    public function ParseCalendar(array $fields): array
    {
        ...
        try {
            $this->url = $this->seleniumSearchFlights($fields);
        ...
        if (empty($this->calendar)) {
            $this->logger->error('No calendar found. Need check');

            throw new \CheckRetryNeededException(3, 0);
        }

        return ['fares' => $this->parseJsonCalendar($this->calendar, $fields)];
    }
...
//        https://www.jetblue.com/resp-magnoliapublic/.rest/jetblue/v4/dcInterlineMigration/customDelivery/dcInterlineMigration
        $list = [
            ...
            'GR' => 'Blue Extra',
            'J1' => 'Business',
            'J2' => 'Business',  //The data may be correct
            'MN' => 'Mint',
            'M1' => 'Business Lie',
            'MR' => 'Mint',
            'Z2' => 'First',
        ...
        if ($code === 'J2') {  //The data may be correct
            return $code;
        }
    ...
    private function getCabinClassFields($code)
    {
        $cabins = [
            ...
            'FIRST_REDEMPTION'          => 'firstClass', // operated by Qatar
            'FIRST'                     => 'firstClass', // operated byJapan Airlines International
            'BLUE_EXTRA'                => 'economy',
            'PREM._ECONOMY_REDEMPTION'  => 'premiumEconomy',
            'MINT'                      => 'business',
        ];

        if (!isset($cabins[$code])) {
            $this->sendnotification('check code ' . $code . ' //DM');
...
//            $selenium->useCache();
            ...
            try {
                ...
                if ($fields['ParseCalendar']) {
                    $this->getCalendarData($fields, $selenium, $url);
                }
            } catch (\UnrecognizedExceptionException | \Facebook\WebDriver\Exception\UnrecognizedExceptionException $e) {
                $this->logger->error("Exception: " . $e->getMessage());
...
//        $selenium->driver->executeScript($script);
    ...
    private function getCalendarData($fields, $selenium, $url): void
    {
        $this->logger->notice(__METHOD__);

        try {
            $date = date('Y-m-d', $fields['DepDate']);

            // Других кабин я пока не нашёл. в остальных случаях всегда выбирается economy.
            if ($fields['Cabin'] !== 'business') {
                $cabin = 'economy';
            } else {
                $cabin = 'business';
            }
            ...
            $script = "
                ...
                xhr.setRequestHeader('Api-Version', 'V3');
                xhr.setRequestHeader('Application-Channel', 'Desktop_Web');
                xhr.setRequestHeader('Booking-Application-type', 'NGB');
            ...
            $this->logger->debug('[Run script: ]');
            $this->logger->debug($script, ['pre' => true]);
            $request = $selenium->driver->executeScript($script);

            $this->calendar = $this->http->JsonLog($request);
        } catch (\Exception $e) {
            $this->logger->error($e->getMessage());
    ...
    private function interceptResponseDataStorage($url, $selenium, $limit = 10)
    {
        ...
        try {
            ...
            } catch (\UnexpectedJavascriptException | \Facebook\WebDriver\Exception\UnrecognizedExceptionException $e) {
    ...
    private function parseJsonCalendar($data, $fields): array
    {
        $this->logger->notice(__METHOD__);
        $result = [];

        if (isset($data->error)) {
            $this->SetWarning($data->error->message);

            return $result;
        }

        foreach ($data->dategroup[0]->group as $day) {
            if ($day->points === 'N/A') {
                continue;
            }
            ...
            if ($fields['Cabin'] !== 'business') {
                ...
                if (isset($data->programName) && strpos($data->programName, 'BASIC') !== false) {
                    $brandedCabin = 'BLUE';
                } else {
                    $brandedCabin = 'Economy';
                }
            } else {
                $cabin = 'business';
                ...
                if (isset($data->programName) && strpos($data->programName, 'BASIC') !== false) {
                    ...
                    $brandedCabin = 'Business';
                }
            }

            $result[] = [
                'date'        => $date,
                'redemptions' => ['miles' => $day->points],
                'payments'    => [
    ...
    private function parseJsonDataStorage($data)
    {
        ...
        if (empty($data->departingFlights->fareDetail)) {
            // Кажется бага, перелёты есть, а результата нет
            throw new \CheckRetryNeededException(5, 0);
        }
...
//"potential_matches": ["business", "econmy", "first", "premium"]
