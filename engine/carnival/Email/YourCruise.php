...
{
    public $mailFiles = "carnival/it-697444380.eml, carnival/it-698999795.eml, carnival/it-921458370.eml";
    ...
    private $xpath = [
        'segments' => "descendant::*/tr[1][count(*)>=4 and count(*[normalize-space()='' and descendant::img])>=4]/following-sibling::tr[normalize-space()]",
    ];
    ...
    {
        $patterns = [
            'date'          => '(?:[[:alpha:]]*\s*\b[[:alpha:]]+\/\d{1,2}\/\d{4}\b|[[:alpha:]]*\s*\b\d{1,2}\/[[:alpha:]]+\/\d{4}\b|\d+\/\d+\/\d{4})', // Apr/7/2025    |    7/Apr/2025
        ...
        */
        ...
        foreach ($segments as $root) {
            $columnCount = $this->http->XPath->query("*", $root)->length;
            ...
            if ($columnCount > 4) {
                ...
                $port = $this->http->FindSingleNode("*[2]", $root);
                $timeArr = $this->http->FindSingleNode("*[3]", $root, true, "/^{$patterns['time']}/");
                $timeDep = $this->http->FindSingleNode("*[4]", $root, true, "/^{$patterns['time']}/");
                $date = strtotime($this->normalizeDate($this->http->FindSingleNode("*[1]", $root, true, "/^{$patterns['date']}$/u")));
            }
    ...
    private function normalizeDate(?string $text): string
    {
        ...
        $in = [
            ...
            '/^(\d{1,2})\/([[:alpha:]]+)\/(\d{4})$/u',
            // Apr/7/2025
            '/^[[:alpha:]]+\s+([[:alpha:]]+)\/(\d{1,2})\/(\d{4})$/u',
            // Sunday 24/Aug/2025
            '/^[[:alpha:]]+\s+(\d{1,2})\/([[:alpha:]]+)\/(\d{4})$/u',
        ];
        $out = [
            '$2 $1 $3',
            '$1 $2 $3',
