...
class ItineraryText extends \TAccountChecker
{
    ...
    public function ParseCar(Email $email, $text)
    {
        ...
        if (preg_match("/Retirada\s*(?<pickUpDate>\d+\/\w+\s+[\d\:]+)\s+\-?\s*(?<pickUpLocation>.+)\s*\/\s+Devolução\s+(?<dropOffDate>\d+\/\w+\s+[\d\:]+)\s+\-?\s*(?<dropOffLocation>.+\((?:[A-Z]{3}|\d{3})\))\s+(?<carInfo>.+)Período/su", $text, $m)) {
            $r->pickup()
            ...
            //LOCALIZA || MOVIDA;
            if ((empty($m['carInfo']) || preg_match("/^\s*\w+\s*$/", $m['carInfo'])) && stripos($text, 'Observação:') !== false) {
                $m['carInfo'] = $this->re("/Pagamento.: .+\n\s*Observação\:\s+(.+)/", $text);
            }

            if (preg_match("/(?<carType>(?:[A-Z]{1,2}[\-\s]*)?(?:SUV|GRUPO|INTERMEDIÁRIO|ECONOMICO).+[A-Z]{2,})\n*\s+(?<carModel>.+\s(?:ou\s.*|LOCALIZA))/s", $m['carInfo'], $matches)
                || preg_match("/^\-?\s*(?<carType>[A-Z].+[A-Z]{3,})\s*(?:[0]*\*?LOCALIZA)/iu", $m['carInfo'], $matches)
                || preg_match("/^(?<carModel>.+\s+ou similar.*)(?:LOCALIZA|MOVIDA)/iu", $m['carInfo'], $matches)
                || preg_match("/^(?<carModel>.+SEDANSIMILAR)\s*$/iu", $m['carInfo'], $matches)
                || preg_match("/^(?<carType>\w+)\s+MOVIDA\s*$/iu", $m['carInfo'], $matches)
          ) {
                if (isset($matches['carType'])) {
                    $r->setCarType($matches['carType']);
                }

                if (!empty($matches['carModel'])) {
                    $r->setCarModel(preg_replace("/((?:LOCALIZA|MOVIDA|Loja de retirada).+)/iu", "", $matches['carModel']));
                }
            }
