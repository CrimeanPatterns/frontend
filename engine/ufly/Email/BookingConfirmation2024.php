...
{
    public $mailFiles = "ufly/it-710649372.eml, ufly/it-711229338.eml, ufly/it-712417220.eml, ufly/it-713941945-cancelled.eml, ufly/it-714225528.eml, ufly/it-717433606.eml, ufly/it-896247660.eml";
    ...
    public static $dictionary = [
        'en' => [
            ...
            'Previous Flight Schedule'  => ['Previous Flight Schedule'],
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        */
        ...
        foreach ($this->findSegments() as $root) {
            ...
            if (preg_match("/^{$this->opt($this->t('Previous Flight Schedule'))}\s*$/i", $statusText, $m)) {
                $f->removeSegment($s);
        ...
        $travellers = $this->http->FindNodes("//*[{$this->eq($this->t('Traveler Details'))}]/following::img[contains(@src, 'icon-pax.png')]/ancestor::td[1]", null, "/^{$this->patterns['travellerName']}$/u");

        if (empty($travellers)) {
            $travellers = $this->http->FindNodes("//*[{$this->eq($this->t('Traveler Details'))}]/following-sibling::table/descendant::tr[normalize-space() and not(.//tr[normalize-space()])][1][ descendant::node()[self::text()[normalize-space()] or self::img][1][self::img] ]",
                null, "/^{$this->patterns['travellerName']}$/u");
        }
        $areNamesFull = true;
