...
class API extends \TAccountCheckerAa
{
    ...
    public function getEmailType(\PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (strpos($pdfBody, 'Receipt') !== false && preg_match('/(?:\s*AA)+ (?:(?:\s*RECORD)+ (?:\s*LOCATOR)+|CONFIRMATION CODE:?)/', $pdfBody)) {
    ...
    protected function PrintTripAndReceiptPdf(\PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (strpos($text, 'Receipt') === false || !preg_match('/(?:\s*AA)+ (?:(?:\s*RECORD)+ (?:\s*LOCATOR)+|CONFIRMATION CODE:?)/', $text)) {
                continue;
            }
            $it = ['Kind' => 'T'];

            if (preg_match("/(?:\s*AA)+ (?:(?:\s*RECORD)+ (?:\s*LOCATOR:)+|CONFIRMATION CODE:?)\s+([A-Z\d]{5,8})\s+/", $text, $m)) {
                $it["RecordLocator"] = $m[1];
            }
            ...
            $tripText = $this->re("/\n{2}((?:.+\n+){2,4}[ ]*(?:\s*AA)+ (?:(?:\s*Record)+ (?:\s*Locator)+|Confirmation Code)[ ]{2,}(?:\s*Reservation)+ (?:\s*Name)+[\s\S]+?)\n+[ ]{0,10}(?:\s*Receipt)+\n/", $text);
