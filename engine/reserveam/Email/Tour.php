...
{
    public $mailFiles = "reserveam/it-883651304.eml, reserveam/it-886652385.eml, reserveam/it-887459002.eml";
    public $lang = 'en';
    ...
    public static $dictionary = [
        "en" => [
            'General Admission Adult' => ['General Admission:', 'General Admission Adult (Age 13 and Older):', 'Adult (15+):'],
            'General Admission Kid'   => ['General Admission (ages 5 and under):'],
        ],
    ];
    ...
    public function parseEvent(Email $email)
    {
        ...
        if (preg_match("/^\s*(?<desc>{$this->opt($this->t('Your Reservation Number:'))})\s+(?<confNumber>[\-\d]+)\s*$/", $confInfo, $m)) {
            $e->general()
        ...
        if (!empty($travellerName)) {
            $e->general()->traveller($travellerName);
        }
        ...
        $guestsCount = $this->http->FindSingleNode("//text()[{$this->eq($this->t('General Admission Adult'))}]/following::text()[normalize-space()][1]", null, true, "/^\s*{$patterns['guestCount']}\s*$/");
        $kidsCount = $this->http->FindSingleNode("//text()[{$this->eq($this->t('General Admission Kid'))}]/following::text()[normalize-space()][1]", null, true, "/^\s*{$patterns['guestCount']}\s*$/");
