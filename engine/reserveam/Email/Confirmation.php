...
{
    public $mailFiles = "reserveam/it-850485054.eml, reserveam/it-873251810.eml";
    public $lang = 'en';
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        if ($this->http->XPath->query("//text()[{$this->eq($this->t('*IMPORTANT BILLING INFORMATION:'))}]")->length > 0
            ...
            && $this->http->XPath->query("//text()[{$this->eq($this->t('Arrival Date:'))}]")->length > 0
            && $this->http->XPath->query("//text()[{$this->eq($this->t('Site Type:'))}]")->length > 0
        ) {
            return true;
        }
    ...
    public function parseHotel(Email $email)
    {
        ...
        if (preg_match("/\s*(?<desc>{$this->opt($this->t('Reservation #'))})\s+(?<confNumber>[\-\d]+)(?:\s+\((?<status>.+?)\))?\s*$/", $confInfo, $m)) {
            $h->general()
        ...
        // collect hotel name and address (park name)
        $parkName = $this->http->FindSingleNode("(//text()[{$this->eq($this->t('Campground:'))}])[last()]/following::text()[normalize-space()][1]")
