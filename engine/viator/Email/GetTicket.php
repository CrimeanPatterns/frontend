...
class GetTicket extends \TAccountChecker
{
    ...
    private function parseHtml(Email $email): void
    {
        ...
        foreach ($urls as $i => $url) {
            ...
            $http2->setDefaultHeader('Sec-Fetch-User', '?1');
            ...
            if ($http2->FindSingleNode("(//node()[starts-with(normalize-space(), 'Sorry, this is an old link')])[1]")) {
                $this->logger->debug('Your booking has been updated, so this ticket is no longer valid');
            ...
            $names = array_unique($http2->FindNodes("//div[@class = 'product-name' or @data-automation = 'voucher-product-name' or @data-automation = 'ticket-product-name']"));

            if (count($names) === 0) {
                $email->add()->event();
                $this->logger->debug('empty event name in tickets');

                return;
            } elseif (count($names) > 1) {
                $email->add()->event();
                // no examples
                $this->logger->debug('more then 1 events in tickets');
            ...
            if (empty($conf)) {
                $conf = $http2->FindSingleNode("(//a[contains(@href, 'download-pdf?code=')])[1]/@href", null, true,
            ...
            $eventConfTitle = trim($http2->FindSingleNode("(//text()[{$this->starts($this->t('Booking ref.:'))}])[1]",
                null, true, "/^\s*{$this->opt($this->t('Booking ref.:'))}/"), ':/');

            if (empty($eventConf)) {
                $eventConf = $http2->FindSingleNode("(//text()[{$this->eq($this->t('Booking ref.'))}])[1]/following::text()[normalize-space()][1]",
                    null, true, "/^\s*([A-Z\-\d]{5,})\s*$/");
                $eventConfTitle = $http2->FindSingleNode("(//text()[{$this->eq($this->t('Booking ref.'))}])[1]",
                    null, true, "/^\s*{$this->opt($this->t('Booking ref.'))}/");
            }
            $event->general()
                ->confirmation($eventConf, $eventConfTitle)
            ;
            ...
            if (empty($travellers)) {
                $travellers = array_filter(preg_replace("/\s*(?:\(\w+\))?\s+•\s+.+/i", '', $http2->FindNodes("//div[@data-automation = 'ticket-tour-pax' or @data-automation = 'voucher-traveller-name']",
                    null)));
            }
            $event->general()
            ...
            if (empty($conf)) {
                $conf = $http2->FindSingleNode("(//text()[{$this->eq($this->t('Tour operator confirmation no'))}])[1]/following::text()[normalize-space()][1]",
                    null, true, "/^\s*([A-Z\-\d]{5,})\s*$/");
            }
            ...
            $address = $http2->FindSingleNode("//text()[{$this->eq($this->t('Meeting and pickup'), "translate(.,':','')")}]/following::text()[position()<5]/ancestor::a[contains(@href,'maps.google.com')]/@href", null, true, "/[?&]q=(.{0,160}?)(?:&[_\w]+=|$)/i")
                ...
                ?? $http2->FindSingleNode("//text()[{$this->eq('Meeting Point')}]/following::text()[normalize-space()][1]/ancestor::a[contains(@href,'maps.google.com') or (contains(@href, '.google.com/maps/search'))]")
                ?? $http2->FindSingleNode("//text()[normalize-space()='Location Details']/following::text()[normalize-space()][1]/ancestor::a[contains(@href,'maps.google.com') or (contains(@href, '.google.com/maps/search'))]")
                ?? $http2->FindSingleNode("//text()[{$this->eq('Meeting Point')}]/following::text()[normalize-space()][1]/ancestor::a[contains(@href,'maps.google.com') or (contains(@href, '.google.com/maps/search'))]/@href", null, true, "/[?&]q=(.{0,160}?)(?:&[_\w]+=|$)/i")
            ;
            ...
            // Booked
            $event->booked()
                ->start($this->normalizeDate($http2->FindSingleNode("(//div[@class = 'tour-time' or @data-automation = 'ticket-tour-date-time' or @data-automation = 'voucher-tour-date-time'])[1]",
                    null, true, "/^\s*(.+?)(?:\\/|$)/")))
                ->noEnd()
            ;
            $guests = implode(" , ", $http2->FindNodes("//div[contains(@class, 'tour-pax') or contains(@data-automation, 'tour-pax') or @data-automation = 'voucher-traveller-name']"));
    ...
    private function normalizeDate($str)
    {
        ...
        $in = [
            ...
            // , May 27, 2025 • 01:30 PM
            "/^\s*[[:alpha:]\-]+[,.\s]\s*([[:alpha:]]+)\s+(\d{1,2})\s*[,\s]\s*(\d{4})\s*[,\s•]\s*(\d{1,2}:\d{2}(?:\s*[AP]M)?)?\s*$/ui",
        ];
        $out = [
            '$1 $2 $3, $4',
            '$1 $2 $3, $4 $5m',
