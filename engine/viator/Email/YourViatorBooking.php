...
class YourViatorBooking extends \TAccountChecker
{
    ...
    public static $dict = [
        'en' => [
            ...
            'Booking Reference'    => ['Booking Reference Number', 'Booking Reference', 'Booking reference', 'Confirmation Pending', 'Confirmation pending', 'Booking reference number', 'Reference Number'],
            'Adult'                => ['Number of Adults: ', 'Adult', 'Child', 'Infant', 'Youths', 'Adults', 'Youth'],
            ...
            'Get voucher'          => ['Get voucher', 'Get Voucher', 'Get ticket', 'Get Ticket', 'Get tickets', 'Get Tickets', 'Get Ticket'],
...
*/
    ...
    private function parseEmail(): array
    {
        ...
        */
        ...
        if ($itineraries->length === 0) {
            $xpath = "//a[{$this->containsUrl($subUrls)}]/ancestor::table[1]/following-sibling::table/{$xpathCellRight}[1]";
            $this->logger->debug('All itineraries6: ' . $xpath);
            $itineraries = $this->http->XPath->query($xpath);
            $this->logger->debug('length ' . $itineraries->length);
            $xpathType = 5;
        }
...
//        }
        foreach ($itineraries as $root) {
            ...
            if ($xpathType === 2) {
                ...
                if (empty($mainName)) {
                    $mainName = $this->http->FindSingleNode("./preceding::text()[normalize-space()][1][contains(normalize-space(), ':')]/preceding::text()[normalize-space()][1]", $root);
                }
            ...
            if (empty($it['StartDate'])) {
                $it['StartDate'] = $this->normalizeDate($this->http->FindSingleNode("./ancestor::table[1]/ancestor::tr[1]/following-sibling::tr[1]/descendant::text()[starts-with(normalize-space(), 'Travel Date:')]/ancestor::p[1]", $root, true, "/Travel Date\:\s*(.+\d{4})/"));
            }

            if (empty($it['StartDate'])) {
                $it['StartDate'] = $this->normalizeDate($this->http->FindSingleNode("./ancestor::table[1]/ancestor::tr[1]/following-sibling::tr[1]/descendant::text()[starts-with(normalize-space(), 'Travel Date:')]/ancestor::td[1]", $root, true, "/Travel Date\:\s*(.+\d{4})/"));
            }
            ...
            if (!isset($it['DinerName'])) {
                $pax = $this->http->FindSingleNode("//text()[starts-with(normalize-space(), 'Lead Traveler:')]/ancestor::tr[1]", null, true, "/Lead Traveler\:\s*([[:alpha:]][-.\/\'’[:alpha:] ]*[[:alpha:]])$/");
...
//            $url='';
    ...
    private function getAddressByURL(&$it, ?string $url, bool $hopTwo = false): ?string
    {
        ...
        $http2->setDefaultHeader('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7');
        $http2->setDefaultHeader('Accept-Encoding', 'gzip, deflate, br, zstd');
        $http2->setDefaultHeader('Sec-Fetch-Dest', 'document');
        $http2->setDefaultHeader('Sec-Fetch-Mode', 'navigate');
        $http2->setDefaultHeader('User-Agent', 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36');
        ...
        if (stripos($url, 'urldefense.proofpoint.com/v2') !== false) {
            if (preg_match("/(www\.viator\.com\_MptUrl.+)&d\=/u", $url, $m)) {
                $url = str_replace(['-25', '_', '-3F', '-3D'], ['%', '/', '?', '='], $m[1]);
                $http2->setMaxRedirects(5);
            }
        }

        //https://urldefense.com/v3
        if (stripos($url, 'urldefense.com/v3') !== false) {
            if (preg_match("/(www\.viator\.com\/MptUrl.+)\_\_/u", $url, $m)) {
                $url = str_replace(['*'], ['%'], $m[1]);
                $http2->setMaxRedirects(5);
            }
        }
        ...
        } elseif (!preg_match("/\.viator\.com\/ticket/", $http2->currentUrl())) {
            $this->logger->debug('Wrong URL or eventName! Abort getAddressByURL().');

            return '';
        }

        //$this->logger->warning('currentURL: '.$http2->currentUrl());
...
//            $this->logger->debug('$breadcrumbs = '.print_r( $breadcrumbs,true));
        ...
        if (empty($location)) {
            $this->logger->debug('Address: search attemp-16');
            $location = $http2->FindSingleNode("//text()[normalize-space()='Pickup Point']/following::text()[normalize-space()][1]");
        }

        if (empty($location)) {
            $this->logger->debug('Address: search attemp-17');
            $location = $http2->FindSingleNode("//a[contains(@href, 'google.com/maps')]");
        }
    ...
    private function normalizeDate($str)
    {
        ...
        $in = [
            ...
            "#^\w+\.\,\s*(\w+)\.?\s*(\d+)\,\s*(\d{4})$#", //sam., juin 07, 2025
        ];
        $out = [
            "$2 $1 $3",
            "$2 $1 $3, $4",
