...
class YourBooking extends \TAccountChecker
{
    ...
    public static $dictionary = [
        'en' => [
            ...
            'adult'               => ['adult', 'adults'],
            // 'child' => '',
            'roomHeader' => ['Room details', 'Cancelled room details'],
            // 'Telephone:' => '',
            // 'Room total:' => '',
            // 'Total price' => '',
            // 'What happens if I need to cancel or amend my booking' => '',
            'cancellationStart' => 'Your booking can be cancel',
            // 'cancellationContains' => '',
            // 'Booking' => '',
        ...
        'de' => [
            ...
            'cancellationStart' => ['Dies ist eine Buchung, die nicht storniert', 'Ihre Buchung ist bis'],
            'cancellationContains' => 'am Tag der Anreise storniert werden kann',
            'Booking'                                              => 'Buchung',
    ...
    private $detectors = [
        'en' => [
            ...
            'your upcoming stay has been cancelled', 'Your upcoming stay has been cancelled',
            'your upcoming stay has been canceled', 'Your upcoming stay has been canceled',
    ...
    private $patterns = [
        'date' => '(?<day>\d{1,2})[,.\s]*(?<month>[[:alpha:]]+)[,.\s]*(?<year>\d{2,4})', // 5 Jun 2019
    ...
    private function parseEmail(Email $email): void
    {
        ...
        $checkIn = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Check in'))}]/following::text()[normalize-space()][1]/ancestor::*[1]", null, true, "/^(?:{$this->opt($this->t('From'))}[:\s]+)?(.{6,})$/i");
        $h->booked()->checkIn2($this->normalizeDate($checkIn));

        $checkOut = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Check out'))}]/following::text()[normalize-space()][1]/ancestor::*[1]", null, true, "/^(?:{$this->opt($this->t('Before'))}[:\s]+)?(.{6,})$/i");
        ...
        $rooms = $this->http->XPath->query("//text()[{$this->eq($this->t('roomHeader'), "translate(.,'0123456789:','')")}]/ancestor-or-self::*[{$xpathBold}][1]/following::text()[normalize-space()][1]/ancestor::div[1]");

        foreach ($rooms as $root) {
            $roomText = implode("\n",
                $this->http->FindNodes('./descendant::text()[normalize-space()!=""][position()>1]', $root));

            $roomsArray = array_filter(preg_split("/^[ ]*{$this->opt($this->t('roomHeader'))}[ \d:]*$/m", $roomText));
            $roomCount = 0;
        ...
        if (!empty($h->getHotelName())) {
            $hotelNameVariants = [$h->getHotelName(), preg_replace('/^(?:hub by\s+)?Premier Inn\s+(.{3,})$/i', '$1', $h->getHotelName())];
            ...
            $h->hotel()->address($address ? rtrim($address, ', ') : null)->phone($phone, false, true);
        }

        $cancellation = $this->http->FindSingleNode("//text()[{$this->starts($this->t('What happens if I need to cancel or amend my booking'))}]/following::text()[string-length(normalize-space())>1][1][not(ancestor::*[{$xpathBold}] or ancestor::a)]/ancestor::*[1]")
            ?? $this->http->FindSingleNode("//text()[{$this->starts($this->t('cancellationStart'))}]/ancestor::p[1]")
            ?? $this->http->FindSingleNode("//text()[{$this->contains($this->t('cancellationContains'))}]/ancestor::p[1]")
        ;
    ...
    private function detectDeadLine(\AwardWallet\Schema\Parser\Common\Hotel $h): void
    {
        ...
        } elseif (preg_match('/Sie können diese Buchung bis (?<hour>\d{1,2})\s*Uhr am Anreisetag telefonisch ändern oder über/i', $cancellation, $m) // de
            || preg_match('/^Dies ist eine Flex-Tarif Buchung, die bis (?<hour>\d{1,2})\s*Uhr am Tag der Anreise storniert werden kann\s*(?:[.;!]|$)/i', $cancellation, $m) // de
        ) {
            $h->booked()->deadlineRelative('0 days', $m['hour'] . ':00');
        } else {
            $h->booked()
                ->parseNonRefundable('Dies ist eine Buchung, die nicht storniert oder geändert werden kann.') // de
    ...
    private function assignLang(): bool
    {
        ...
        foreach (self::$dictionary as $lang => $phrases) {
            ...
            if ($this->http->XPath->query("//*[{$this->contains($phrases['Check out'])}]")->length > 0
                && $this->http->XPath->query("//*[{$this->contains($phrases['Hotel details'])}]")->length > 0
            ) {
                $this->lang = $lang;
...
//        $this->logger->debug('$text = '.print_r( $text,true));
        ...
        $in = [
            ...
            "/^\s*(\d{1,2})\s*Uhr\s+[-]+\s+(?:[-[:alpha:]]+[,.\s]*)?{$this->patterns['date']}\s*$/iu",
            // 12pm - Wed 5 Jun 2019
            "/^\s*({$this->patterns['time']})(?:\s*Uhr)?\s+[-]+\s+(?:[-[:alpha:]]+[,.\s]*)?{$this->patterns['date']}\s*$/iu",
            // So 8 August 2021 - bis 15 Uhr
            "/^\s*(?:[-[:alpha:]]+[,.\s]*)?{$this->patterns['date']}\s+(?:[-]+\s+)?(?:[[:alpha:] ]+\s+)?(\d{1,2})\s*Uhr\s*$/iu",
            // Mon 12 Jun 2023 - before 12pm    |    Mon 12 Jun 2023 from 12pm
            "/^\s*(?:[-[:alpha:]]+[,.\s]*)?{$this->patterns['date']}\s+(?:[-]+\s+)?(?:[[:alpha:] ]+\s+)?({$this->patterns['time']})(?:\s*Uhr)?\s*$/iu",
        ];
        $out = [
            '$2 $3 $4, $1:00',
            '$2 $3 $4, $1',
