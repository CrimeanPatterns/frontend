...
{
    public $mailFiles = "deltav/it-12630884.eml, deltav/it-303156381.eml, deltav/it-307611544.eml, deltav/it-307614048.eml, deltav/it-410673600.eml";
    public $lang = 'en';
    ...
    public function ParseFlightPDF(Email $email, $text)
    {
        ...
        if (!empty($travellerInfo)) {
            ...
            $travellers = explode("\n", preg_replace("/(?:Miss\s|Mrs\s|Mr\s|Ms\s|\s\(.+\))/", "", $this->re("/{$this->opt($this->t('Passenger Name'))}\s*\n(.+)/su", $travellerTable[1])));
            $f->general()
            ...
            foreach ($tickets as $ticket) {
                if (preg_match("/^\s+\d+\s+(?<pax>.+)\s+[A-Z\d]{6}\s+$ticket/mu", $travellerInfo, $m)) {
                    $f->addTicketNumber($ticket, false, preg_replace("/(?:Miss\s|Mrs\s|Mr\s|Ms\s|\s\(.+\))/", "", $m['pax']));
                } else {
            ...
            foreach ($accounts as $account) {
                if (preg_match("/^\s+\d+\s+(?<pax>.+)\s+[A-Z\d]{6}\s+\d{10,}\s*$account(?:\s|\n|$)/mu", $travellerInfo, $m)) {
                    $f->addAccountNumber($account, false, preg_replace("/(?:Miss\s|Mrs\sMr\s|Ms\s|\s\(.+\))/", "", $m['pax']));
                } else {
        ...
        if (!empty($flightsInfo)) {
            ...
            if (count($flightRows) > 0) {
                foreach (array_filter($flightRows) as $flightRow) {
                    ...
                    // it-12630884
                    if (preg_match("/.+\s+\([A-Z]{3}\)[ ]*(\d{1,2}\:\d{2}[ ]*[Aa]?[Pp]?[Mm]?)/u", $flightTable[6], $t)){
                        $flightTable[6] = $this->re("/(.+\s+\([A-Z]{3}\))[ ]*\d{1,2}\:\d{2}[ ]*[Aa]?[Pp]?[Mm]?/u", $flightTable[6]);
                        array_splice($flightTable, 7, 0, $t[1]);
                    }
    ...
    public function ParseHotelPDF(Email $email, $text)
    {
        ...
        if (!empty($hotels)) {
            ...
            foreach ($hotelRows as $hotelRow) {
                ...
                if (preg_match_all("/\(\d+\)\s*(?:Ms|Mrs|Mr|Miss)?\s*(\D+)(?:\s*\(age\s*\d+\))?\s*(?:\,|$)/", $travellersText, $m)) {
                    $h->general()
                        ->travellers($m[1], true);
                }
            }
