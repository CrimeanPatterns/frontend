...
//            $this->http->GetURL("https://www.samsung.com/us/support/account/");
        ...
        */
        ...
        $pass = $this->waitForElement(WebDriverBy::xpath('//input[@id = "iptLgnPlnPD"]'), 10);
        $this->saveResponse();
        ...
        if (!$pass) {
            ...
            $this->waitFor(function () {
                ...
                $result = !$this->http->FindSingleNode('//a[contains(text(), "Solving is in process...")]');

                if ($result) {
                    // seems like captcha progress disappear then appear again
                    sleep(10);
                    $this->saveResponse();
                    $result = !$this->http->FindSingleNode('//a[contains(text(), "Solving is in process...")]');
                }
        ...
        if (!$pass) {
            ...
            if ($message = $this->http->FindSingleNode('//div[contains(text(), "Unusual activity was detected, so we blocked access to your account for security.")]')) {
