...
class TAccountCheckerSj extends TAccountChecker
{
    ...
    private $accessToken;
    private $tokenDataResponse;
    private $subscriptionKey = 'd6625619def348d38be070027fd24ff6';
    ...
    public function Parse()
    {
        ...
        $pointExpirations = $membershipInfo['pointExpirations'] ?? [];
        uasort($pointExpirations, function ($a, $b) {
            if (strtotime($a['validityEnd']) == strtotime($b['validityEnd'])) {
                return 0;
            }

            return ($a['validityEnd'] < $b['validityEnd']) ? -1 : 1;
        });

        if (count($pointExpirations) > 0) {
            $this->SetProperty('ExpiringPoints', $pointExpirations[0]['points']);
            $this->SetExpirationDate(strtotime($pointExpirations[0]['validityEnd']));
        }
    }
    ...
    private function getRecordedRequests()
    {
        ...
        foreach ($requests as $xhr) {
            if (strstr($xhr->request->getUri(), 'token?client-request-id')) {
                $this->tokenDataResponse = $this->http->JsonLog(json_encode($xhr->response->getBody()));
                $this->accessToken = $this->tokenDataResponse->access_token ?? null;
