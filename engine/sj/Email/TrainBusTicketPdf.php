...
{
    public $mailFiles = "sj/it-694063823.eml, sj/it-697211140.eml, sj/it-890582543.eml, sj/it-893832089.eml";
    public $reFrom = ["noreply@ticket.sj.se"];
    public $reBody = [
        'en' => ['The ticket was sold by SJ AB', 'Ticket no:'],
        'sv' => ['Biljetten är såld av SJ AB', 'Biljettnr:'],
    ];

    public $reSubject = [
        '#Tickets for booking#',
        '#Change of tickets for booking#',
        // sv
        '#Biljetter till bokning#',
        '#Ändring av biljetter för bokning#',
    ];
    ...
    public function ParseTrain(Email $email, $textPDF)
    {
        ...
        if (preg_match("/\n*^(?<traveller>[[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])\n*(?:{$this->opt($this->t('Member'))}\:.*\n)?{$this->opt($this->t('Booking no:'))}\s*(?<conf>[A-Z\d\-]{5,10})\n{$this->opt($this->t('Ticket no:'))}\s*(?<ticket>[A-Z\d\-]{10,})/mu", $textPDF, $m)) {
            $conf = $m['conf'];
            $ticket = $m['ticket'];
        ...
        if (preg_match("/\s+{$this->opt($this->t('Train'))}\s+(?<number>\d+)\s+{$this->opt($this->t('Carriage'))}\s+(?<car>\d+)\s*{$this->opt($this->t('Seat'))}\s*(?<seat>\d+)\D*\n*(?<serviceName>SJ.+)\n/", $textPDF, $m)
            || preg_match("/\s+{$this->opt($this->t('Train'))}\\s+(?<number>\d+)\s+{$this->opt($this->t('Carriage'))}\s+(?<car>\d+)\s*Berth.*\n*(?<serviceName>SJ.+)\n/", $textPDF, $m)) {
        ...
        } elseif (preg_match("/\s+{$this->opt($this->t('Train'))}\\s*(?<number>\d+)\s*(?:No seat reservation|Ingen platsreservation)\n*(?<service>(?:S?J?\s*\w+\,|S?J?\s*\w+\n))?/u", $textPDF, $m)) {
            $number = $m['number'];
        ...
        $cabin = $this->re("/\s(\S+\s(?:class|klasse|klass))/iu", $textPDF);
    ...
    public function ParseBus(Email $email, $textPDF)
    {
        ...
        if (preg_match("/(?<date>\w+\,\s*\d+\s+\w+\s*\d{4})\n+\s+(?<depTime>\d+\:\d+)\s*(?<depName>.+)\b\s*(?:Barcode for scanning|Streckkod för skanning)?\n+\s+(?<arrTime>\d+\:\d+)\s+(?<arrName>.+)\n+/u", $textPDF, $m)) {
            ...
            if (preg_match("/\s+{$this->opt($this->t('Bus'))}\s*(?<number>.+)\s*(?:No seat reservation|Ingen platsreservation)\n*/", $textPDF, $m)) {
                $s->extra()
                    ->number($m['number']);
            } elseif (preg_match("/\s+{$this->opt($this->t('Bus'))}\s+(?<number>.+)\s+/", $textPDF, $m)) {
                $s->extra()
                    ->number($m['number']);
            }
        }

        $cabin = $this->re("/\s(\S+\s(?:class|klass|klasse))/iu", $textPDF);
    ...
    private function parseEmailPdf($textPDF, Email $email)
    {
        ...
        $textPDF = preg_replace("/[ ]+Biljett \d+ av \d+/", "", $textPDF);
        $textPDF = str_replace('Streckkod för skanning', '', $textPDF);
        $textPDF = str_replace('Barcode for scanning', '', $textPDF);

        $segments = $this->splitText($textPDF, "/^(.+\–.+\n*[[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]]\n*(?:{$this->opt($this->t('Member'))}\:.*\n+)?{$this->opt($this->t('Booking no:'))}\s*[A-Z\d]{5,10}\n*{$this->opt($this->t('Ticket no:'))}\s*[A-Z\d\-]{10,})/mu", true);

        foreach ($segments as $segment) {
            if (stripos($segment, 'Train') !== false
                || stripos($segment, 'Tåg') !== false) {
