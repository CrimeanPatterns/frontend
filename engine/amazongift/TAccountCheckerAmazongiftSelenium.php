...
class TAccountCheckerAmazongiftSelenium extends TAccountChecker
{
    ...
    protected function processCaptcha()
    {
        ...
        if ($contShopping = $this->waitForElement(WebDriverBy::xpath('//button[@alt="Continue shopping"]'), 0)) {
            $this->saveResponse();
            $contShopping->click();
        }
    ...
    private function checkProviderErrors()
    {
        ...
        if ($message = $this->http->FindSingleNode("//*[contains(text(), 'Your password is incorrect')]")) {
            throw new CheckException($message, ACCOUNT_INVALID_PASSWORD);
        }
        ...
        */
        ...
        if ($message = $this->http->FindSingleNode("//*[contains(text(), 'Your email or password was incorrect. Please try again') or contains(text(), 'Votre mot de passe est incorrect')]")) {
            throw new CheckException($message, ACCOUNT_INVALID_PASSWORD);
        }
        // Your password is incorrect
