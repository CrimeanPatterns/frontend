...
use AwardWallet\Engine\asia\TAccountCheckerAsiaSelenium;
...
class TAccountCheckerAsia extends TAccountChecker
{
    ...
    {
        require_once __DIR__ . '/TAccountCheckerAsiaSelenium.php';
        return new TAccountCheckerAsiaSelenium();
    }
...
//        if (!$this->http->ParseForm(null, "//div[contains(@class, 'signIn')]//form")) {
    ...
    public function Parse()
    {
        ...
        if (isset($response->clubMiles) && $status != 'AM') {
            /*
            $this->sendNotification('refs #25749 - need to check properties // IZ');
            */
            // Tier
        ...
        $milesToExpire = $details->activityBucketInfo->expiryMiles ?? null;
        $exp = $details->activityBucketInfo->expiryDate ?? null;
        ...
        $this->http->GetURL("https://api.cathaypacific.com/mpo-services/v3/non-statements", [
            'Accept' => 'application/json, text/plain, */*',
            'Origin' => 'https://www.cathaypacific.com'
        ]);

        $history = $this->http->JsonLog();
        $activityDetails = $history->activityDetails ?? [];

        if (count($activityDetails) > 1) {
            /*
            $this->sendNotification('refs #25749 - need to check last activity // IZ');
            */
            uasort($activityDetails, function ($a, $b) {
                $a = strtotime($a->activityDate);
                $b = strtotime($b->activityDate);
                if ($a == $b) {
                    return 0;
                }
            ...
            $this->SetProperty('LastActivity', strtotime($activityDetails[0]->activityDate));
        }
    }
...
//        $this->http->GetURL('https://www.cathaypacific.com/mb/#!/en_HK/hub');
//        sleep(2);
...
//        $this->http->GetURL("https://www.cathaypacific.com/cx/en_HK/frequent-flyers/my-account/latest-transactions.html");
    ...
    private function loginSuccessful()
    {
        $this->logger->notice(__METHOD__);
        $prelogin =
            $this->http->getCookieByName('mlc_prelogin')
            ?? $this->http->getCookieByName('cx_prelogin')
        ;
    ...
    private function getSensorDataFromSelenium()
    {
        ...
        try {
            ...
            */
            ...
            // login
            $login = $selenium->waitForElement(WebDriverBy::xpath("//input[@name='{$typeLogin}']"), 5);
            $this->savePageToLogs($selenium);
            ...
            if ($contBtn = $selenium->waitForElement(WebDriverBy::xpath('//button[contains(@data-tealium-event-action, "SIGN_IN::CONTINUE_BTN::")]'), 0)) {
                $this->savePageToLogs($selenium);
                $contBtn->click();
                $selenium->waitForElement(WebDriverBy::xpath("//input[@name='password']"), 5);
            }
...
//            $pwd->sendKeys($this->AccountFields['Pass']);
            sleep(2);
            $btn = $selenium->waitForElement(WebDriverBy::xpath("//button[@data-tealium-event-action = 'SIGN_IN::ACCOUNT_CHECK::SIGN_IN_WITH_PASSWORD_BTN::WITH_PASSKEY' or @data-tealium-event-action = 'SIGN_IN::ACCOUNT_CHECK::SIGN_IN_WITH_PASSWORD_BTN::NO_PASSKEY']"), 0);
            $this->savePageToLogs($selenium);
            $btn->click();

            sleep(5);

            if ($btn = $selenium->waitForElement(WebDriverBy::xpath("//button[@data-tealium-event-action = 'SIGN_IN::VERIFY_ID::SIGN_IN_WITH_OTP_BTN']"), 0)) {
                $this->savePageToLogs($selenium);
                $btn->click();
            }
            ...
            if ($msg) {
                ...
                if ($msg === 'Your sign-in details are incorrect. Please check your details and try again. [ Error Code: 2004 ]'
                    ...
                    || $msg == 'The password you entered is incorrect. Please try again or reset your password.'
                ) {
            ...
            if ($this->http->FindSingleNode("//span[contains(@class, 'welcomeLabel ')][starts-with(normalize-space(),'Welcome,')]")
                ...
                foreach ($cookies as $cookie) {
                    ...
                    if (in_array($cookie['name'], ['mlc_prelogin', 'cx_prelogin']) && $cookie['value'] == 1) {
