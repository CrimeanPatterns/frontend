...
//                $selenium->http->GetURL('https://auth.velocityfrequentflyer.com/as/authorization.oauth2?client_id=vff_web&redirect_uri=https%3A%2F%2Fwww.velocityfrequentflyer.com%2Fmy-velocity%2Factivity%3Fexplicit_login%3Dweb&response_type=code&scope=openid+vff_web_group_scopes&state=2cda103d695e4eca96e62289f88660f7&code_challenge=aENgrk8P8F81pTF7eySwdnHjkR9F7sLODgrdez28i6o&code_challenge_method=S256&response_mode=query&prompt=login&cctp=velocity%3Ahome%3AvffLoginCta');
    ...
    public function ParseItineraries()
    {
        ...
        /*//$this->http->GetURL('https://www.virginaustralia.com/');
        $accept = $this->waitForElement(WebDriverBy::xpath('//button[.//p[contains(text(),"Accept and close")]]'),
            10);
        ...
        $this->waitForElement(WebDriverBy::xpath('//header[@id="headerContainer"]'), 3);
        $menuBtn = $this->waitForElement(WebDriverBy::xpath('//button[@id="control-login-module-dropdown-id" or @id="pointsButton"]'), 3);
        if ($menuBtn) {
            $menuBtn->click();
            sleep(1);
            $this->saveResponse();
            $menuBtn = $this->waitForElement(WebDriverBy::xpath('//a[contains(@href,"https://auth.velocityfrequentflyer.com/idp/startSSO.ping")]'), 10);
            if ($menuBtn) {
                $this->saveResponse();
                $menuBtn->click();
            }
...
//        $this->saveResponse();
//        $this->http->GetURL('https://auth.velocityfrequentflyer.com/idp/startSSO.ping?cctp=vffld_virginaustralia&PartnerSpId=sabre-dx&TARGET=https%3A%2F%2Fbook.virginaustralia.com%2Fdx%2FVADX%2F%23%2Fdashboard%3Fchannel%3Dvff-mybookings-browser');
        sleep(15);
        $this->saveResponse();*/
        $this->velocity = $this->getVelocity();
