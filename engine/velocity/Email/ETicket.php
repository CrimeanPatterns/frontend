...
{
    public $mailFiles = "velocity/it-122390869.eml, velocity/it-132146509-pt.eml, velocity/it-896857165.eml, velocity/it-893862642-pl.eml";
    public $subjects = [
    ...
    public static $dictionary = [
        "en" => [
            'confNumber'                => ['Your Booking Reference is', 'Your Etihad Airways reference is',
                'Booking reference is', 'Booking reference', 'Reservation code',
                'Agency Reservation ID:', ],
            ...
            'phrasesFromSegments'       => ['Please verify flight times prior to departure', 'Please review this booking confirmation carefully',
                'This email contains your personalized itinerary', ],
        ...
        "pl" => [
            ...
            'Seat'                                          => 'Miejsce',
            // 'Your ticket' => '',
            'Frequent Flyer:'                               => 'Numer karty:',
            'Please verify flight times prior to departure' => 'Przed odlotem proszę sprawdzić aktualne godziny wylotów',
            ...
            'phrasesFromSegments'                           => 'Przed odlotem proszę sprawdzić aktualne godziny wylotów',
            'statusVariants'                                => ['POTWIERDZONO', 'Potwierdzono'],
        ],
    ];
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        if ($this->http->XPath->query(
                ...
                . " | //text()[{$this->contains(['Virgin Australia', 'LATAM Airlines', 'Thank you for choosing SCAT Airlines', 'Contact SNAP Afterhours'])}]"
            )->length > 0
        ) {
        ...
        } elseif ($this->http->XPath->query("//text()[contains(normalize-space(),'Brickell Travel Management')]")->length > 0) {
            return $this->http->XPath->query("//text()[contains(normalize-space(),'Reservation code:')]")->length > 0
                && $this->http->XPath->query("//text()[contains(normalize-space(),'Passenger(s):')]")->length > 0;
        }
    ...
    {
        if ($this->http->XPath->query("//node()[{$this->contains($this->t('Aircraft:'))}]")->length === 0
            && $this->http->XPath->query("//img[contains(@id, 'air') and contains(@id, 'segment-icon')]")->length === 0
        ) {
            return;
        }
        $patterns = [
        ...
        $conf = $this->http->FindSingleNode("//text()[{$this->starts($this->tPlusEn('confNumber'), "translate(.,':','')")}]/ancestor::tr[1]", null, true, "/{$this->opt($this->tPlusEn('confNumber'))}[:\s]*([A-Z\d]{4,})$/");

        if (empty($conf)) {
            $conf = $this->http->FindSingleNode("//text()[{$this->eq($this->tPlusEn('confNumber'))}]/following::text()[normalize-space()][1]",
        ...
        if ($nodes->length == 0) {
            $nodes = $this->http->XPath->query("//img[contains(@src, 'air-segment.png')]/ancestor::table[3]");
        }
        ...
        foreach ($nodes as $root) {
            ...
            $patterns['duration'] = "/^[-⋅\s]*((?:\s*\d{1,3}\s*(?:godz|min|h|m)\s*\.?){1,2})[-⋅\s]*$/i"; // 3h 35min  |  55 min  |  1godz. 25min
            $duration = $this->http->FindSingleNode("descendant::table[1]/descendant::text()[{$this->contains($this->t('Miles'))}]/preceding::text()[string-length()>2][2]", $root, true, $patterns['duration'])
                ?? $this->http->FindSingleNode("descendant::table[1]/descendant::text()[{$this->eq($this->tPlusEn('nonStop'), "translate(.,'-⋅ ','')")}]/preceding::text()[string-length()>2][1]", $root, true, $patterns['duration']);
            $s->extra()->duration($duration, false, true);

            if ($this->http->XPath->query("descendant::text()[{$this->eq($this->tPlusEn('nonStop'), "translate(.,'-⋅ ','')")}]", $root)->length === 1) {
                $s->extra()->stops(0);
            }
    ...
    {
        if ($this->http->XPath->query("//node()[{$this->contains($this->t('TRAIN:'))}]")->length === 0
            && $this->http->XPath->query("//node()[{$this->contains($this->t('Train:'))}]")->length === 0
            && $this->http->XPath->query("//img[contains(@id, 'ral') and contains(@id, 'segment-icon')]")->length === 0
        ) {
            return;
        }
        ...
        $traveller = $this->http->FindSingleNode("//td[{$this->starts($this->t('Passenger(s):'))}]/following-sibling::td[1]");
        $f->general()->traveller($traveller, true);

        $nodes = $this->http->XPath->query("//img[contains(@id, 'ral') and contains(@id, 'segment-icon')]/ancestor::table[3]");
        ...
        foreach ($nodes as $root) {
            ...
            $status = $this->http->FindSingleNode("descendant::tr[not(.//tr)][count(*)=3][1]/*[3]", $root, true, "/^{$this->opt($this->t('statusVariants'))}$/");
            $s->extra()->status($status, false, true);
            ...
            $depWeekDay = $this->http->FindSingleNode("descendant::tr[not(.//tr)][count(*)=3][1]/following::text()[normalize-space()][1]", $root, true, "/^(\w+\,)/u");
            ...
            $seats = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Remarks:'))}]/following::text()[normalize-space()][1]", $root);

            if (preg_match("/^\s*COACH ([A-Z\d]+) ([A-Z\d]+)\s*$/", $seats, $m)) {
                $s->extra()
                    ->car($m[1])
                    ->seat($m[2])
                ;
            } elseif (preg_match("/\bCOACH\b$/", $seats, $m)) {
                $this->logger->info('check seats, no example for 2 or more seats');
                $s->extra()
                    ->seat($m[2])
                ;
            }

            $service = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Train:'))}]/following::text()[normalize-space()][1]", $root);
