...
class BonusStatement extends \TAccountChecker
{
    ...
    private static $dictionary = [
        'en' => [
            'Member no.'      => ['Member no.', 'Membership no.', 'MEMBERSHIP NO.', 'Membership No.'],
            'Velocity points' => ['Velocity points', 'VELOCITY POINTS'],
        ],
    ];
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        $number = $this->http->FindNodes("//text()[{$this->contains($this->t('Velocity points'))}]/ancestor::table[4]/descendant::text()[normalize-space()]");
        ...
        if (empty($number)) {
            $number = $this->http->FindNodes("//text()[{$this->eq($this->t('Member no.'))}]/ancestor::table[1]/descendant::text()[normalize-space()]");
        }
        ...
        if (preg_match("/^(?<name>.+?)\s+(?<level>[[:alpha:]]{2,})\s+{$this->opt($this->t('Velocity points'))}\s*(?:(?<balance>[\d.,\s]+))?" .
            "\s*\(as at (?<dateBalance>.+?)\)\s+{$this->opt($this->t('Member no.'))}\s+(?<login>[\w\-]{5,})/", join("\n", $number), $m)) {
            $st->addProperty('Name', $this->niceName($m['name']));
         ...
         *  Mr Antony Clark
            ...
            2102155173
         * */
        if (preg_match("/^(?<name>.+?)\s*\n(?:(?<level>\w+)\s+)?{$this->opt($this->t('Member no.'))}\s+(?<number>[\w\-]{5,})$/", join("\n", $number), $m)) {
            $st->addProperty('Name', $this->niceName($m['name']));

            if (isset($m['level']) && !empty($m['level'])) {
                $st->addProperty('Level', $m['level']);
            }
    ...
    public function niceName(string $traveller): string
    {
        return preg_replace("/^(?:MRS|MS|MR)/iu", "", $traveller);
    }
