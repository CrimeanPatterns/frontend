...
//        $this->setProxyDOP();
    ...
    public function getRewardAvailabilitySettings()
    {
        return [
            ...
            'priceCalendarCabins'      => ["economy", "unknown"],
        ];
    }
    ...
    public function ParseCalendar(array $fields): array
    {
        ...
        $this->responseData = $this->selenium($url);

        $calendar = $this->getRewardCalendar($fields);
...
//        }
    ...
    private function ParseRewardNew($fields = [])
    {
        ...
        if (!isset($data['data']['bookingAirSearch']['originalResponse'])) {
            ...
            $this->sendNotification('check graphql Reward Flight // DM');
    ...
    private function getRewardCalendar($fields = [])
    {
        $this->logger->notice(__METHOD__);
        $this->logger->info("ParseCalendar [" . $fields['DepDate']['m-d-Y'] . "-" . $fields['DepCode'] . "-" . $fields['ArrCode'] . "]",
            ['Header' => 2]);
...
//        }
        ...
        $payload = '{"operationName":"bookingAirSearch","variables":{"airSearchInput":{"cabinClass":"First","awardBooking":true,"promoCodes":[""],"searchType":"CALENDAR30","itineraryParts":[{"from":{"useNearbyLocations":false,"code":"' . $fields['DepCode'] . '"},"to":{"useNearbyLocations":false,"code":"' . $fields['ArrCode'] . '"},"when":{"date":"' . $fields['DepDate']['Y-m-d'] . '"}}],"passengers":{"ADT":' . $fields['Adults'] . '}}},"extensions":{},"query":"query bookingAirSearch($airSearchInput: CustomAirSearchInput) {\n  bookingAirSearch(airSearchInput: $airSearchInput) {\n    originalResponse\n    __typename\n  }\n}"}';
        $this->http->RetryCount = 0;
        $this->http->PostURL("https://book.virginaustralia.com/api/graphql", $payload, $headers, 30);
        ...
        if (!isset($data['data']['bookingAirSearch']['originalResponse'])) {
            ...
            $this->sendNotification('check graphql Calendar // DM');

            throw new \CheckException('other format', ACCOUNT_ENGINE_ERROR);
        }
        $data = $data['data']['bookingAirSearch']['originalResponse'];

        if (isset($data['unbundledAlternateDateOffers'][0]) && is_array($data['unbundledAlternateDateOffers'][0]) && empty($data['unbundledAlternateDateOffers'][0])) {
            if (!isset($this->twice)) {
                $payload = str_replace('BRANDED', 'CALENDAR30', $payload);
                $this->http->PostURL("https://dc.virginaustralia.com/v4.3/dc/products/air/search?jipcc=VADX", $payload,
                    $headers);
        ...
        return $this->parseRewardCalendar($data);
    }
    ...
    private function parseRewardCalendar($date)
    {
        $this->logger->notice(__METHOD__);
        $calendar = [];

        foreach ($date['unbundledAlternateDateOffers'][0] as $item) {
            if ($item['status'] !== 'AVAILABLE') {
                continue;
            }

            $calendar[] = [
                'date'        => $item['departureDates'][0],
                'redemptions' => ['miles' => $item['fare']['alternatives'][0][0]['amount']],
                'payments'    => [
                    'currency' => $item['taxes']['alternatives'][1][0]['currency'],
                    'taxes'    => $item['taxes']['alternatives'][1][0]['amount'],
                    'fees'     => null,
                ],
                'cabin'            => $this->convertToStdCabin($item['cabinClass']),
                'brandedCabin'     => null,
            ];
        }

        $this->logger->debug(var_export($calendar, true), ['pre' => true]);
...
//            $selenium->seleniumRequest->setOs(\SeleniumFinderRequest::OS_MAC);
            ...
            $cookieBtn = $selenium->waitForElement(\WebDriverBy::xpath("//button[normalize-space()='I accept']"), 5);
            ...
            $selenium->waitFor(function () use ($selenium) {
                return $selenium->waitForElement(\WebDriverBy::xpath("
                    ...
                    |// span[@data-translation='flightNotFound.title']
            ...
            if ($msg = $selenium->waitForElement(\WebDriverBy::xpath("//span[normalize-space()='Flight not found']"), 0)) {
                ...
                if ($this->useCalendar) {
                    ...
                    $this->http->SetBody($this->body = $selenium->driver->executeScript('return document.documentElement.innerHTML'));
...
no XHR cause its fetch
            ...
            $this->logger->error("TypeError exception: " . $e->getMessage());
            $this->DebugInfo = "TypeError";
            $retry = true;
        } finally {
