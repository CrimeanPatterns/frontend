...
//        $this->logger->debug(var_export($this->http->Response['headers'], true), ['pre' => true]);
        ...
        if (!$digitalToken) {
            ...
            if (
                ...
                $this->DebugInfo = 'px-captcha';

                throw new CheckRetryNeededException(2, 1, self::PROVIDER_ERROR_MSG, ACCOUNT_PROVIDER_ERROR);
            }
    ...
    public function checkErrors()
    {
        ...
        if (
            $this->http->FindPreg('/blockScript/')
            || $this->http->FindPreg('/altBlockScript/')
        ) {
            $this->DebugInfo = 'px-captcha';
...
//            ];
            ...
            $postData = [
                ...
                */
                'websiteURL'   => 'https://www.budget.com/en/loyalty-profile/fastbreak/login' ?? $currentURL ?? $this->http->currentUrl(),
                "websiteKey"   => $key,
                "minScore"     => 0.3,
                "pageAction"   => "login",
                "isEnterprise" => true,
    ...
    */
    ...
    private function selenium()
    {
        ...
        try {
            ...
            $selenium->http->GetURL('https://www.budget.com/en/loyalty-profile/fastbreak/login');

            $selenium->waitForElement(WebDriverBy::xpath('//input[@name="username"]'), 15);
