...
class It2088170 extends \TAccountChecker
{
    ...
    public static $detectProvider = [
        'perfectdrive' => [
            'from'          => '@e.budget.com',
            'uniqueSubject' => 'Budget Rent A Car:',
            'bodyText'      => [
                'Thank you for not smoking. Budget maintains a 100% smoke-free fleet',
                'budget@e.budget.com',
                'Budget does not accept digital driver',
            ],
            'bodyUrl' => ['.e.budget.com'],
        ],
        'avis' => [
            'from'          => '@e.avis.com',
            'uniqueSubject' => 'Avis Rent a Car:',
            'bodyText'      => [
                'Avis Preferred Program',
                'Avis Rent A Car System, LLC',
                'Thank you for not smoking. Avis maintains a 100% smoke-free fleet',
            ],
            'bodyUrl' => ['.e.avis.com'],
        ],
    ];
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        if (empty($this->providerCode)) {
            foreach (self::$detectProvider as $code => $pDetect) {
                if (stripos($parser->getCleanFrom(), $pDetect['from']) === false
                    && stripos($parser->getSubject(), $pDetect['uniqueSubject']) === false
                ) {
                ...
                if ($this->http->XPath->query("//node()[{$this->contains($pDetect['bodyText'])}]")->length === 0
                    && $this->http->XPath->query("//a/@href[{$this->contains($pDetect['bodyUrl'])}]")->length === 0
                ) {
                    $this->providerCode = $code;
    ...
    {
        return preg_match('/[@\.e]{1,2}\.budget\.com/i', $from) > 0;
    }

    public function detectEmailByHeaders(array $headers)
    {
        foreach (self::$detectProvider as $code => $pDetect) {
            if (stripos($headers['from'], $pDetect['from']) === false
                && stripos($headers['subject'], $pDetect['uniqueSubject']) === false
            ) {
                continue;
            }
