...
//    }
    ...
    private function auth($selenium): ?bool
    {
        ...
        if ($selenium->waitForElement(\WebDriverBy::xpath("//span[contains(normalize-space(), 'Login temporarily unavailable.') or contains (normalize-space(), 'Login temporariamente indisponível')]"), 10)) {
            $counter = \Cache::getInstance()->get('ra_tapportugal_failed_auth');
        ...
        while (((time() - $startTime) < $sleep) && !$login) {
            ...
            if ($message = $selenium->waitForElement(WebDriverBy::xpath("//h5[contains(.,'Login Error')]/following-sibling::div[1][contains(.,'Algo correu mal. Tente mais tarde.')]"), 0)) {
                ...
                if ($message = $selenium->waitForElement(WebDriverBy::xpath("//span[contains(.,'Login temporarily unavailable')]"),
                    0)) {
                    throw new \CheckException($message->getText(), ACCOUNT_PROVIDER_ERROR);
                }
            ...
            if ($message = $selenium->waitForElement(WebDriverBy::xpath("//*[contains(normalize-space(text()),'Login temporariamente indisponível"), 0)) {
                $msg = $message->getText();
                $this->logger->error($msg);
                $this->sendNotification('check msg // ZM');
