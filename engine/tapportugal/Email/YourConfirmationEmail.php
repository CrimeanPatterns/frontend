...
class YourConfirmationEmail extends \TAccountChecker
{
    ...
    public static $dictionary = [
        ...
        'pt' => [
            'hello' => 'Olá',
            'Booking reference'  => ['Referência da reserva', 'Referência de reserva', 'Código de reserva', 'Código de Reserva'],
    ...
    private $detectSubject = [
        ...
        ' is waiting for you', // Nicholas, LONDON is waiting for you!
    ];
    ...
    private $detectBody = [
        'pt' => [
            ...
            'Veja o que está incluso em sua viagem',
            'Veja tudo o que está incluído na sua viagem',
        ],
        'de' => 'Ihre Flüge',
        'en' => [
            ...
            'New Flight',
            'See everything that is included in your trip', 'See what is included in your trip',
    ...
    {
        // $this->logger->debug('$dateIn = '.print_r( $date,true));
        ...
        // $this->logger->debug('$dateOut = '.print_r( $date,true));

        if (preg_match('/^(?<week>[-[:alpha:]]+), (?<date>\d+ \w+ .+)/u', $date, $m)) {
            $weeknum = WeekTranslate::number1(
                WeekTranslate::translate($m['week'], $this->lang)
                ?? WeekTranslate::translate($m['week'], 'en')
            );
