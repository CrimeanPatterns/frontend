...
class TAccountCheckerSlh extends TAccountChecker
{
    ...
    public function Parse()
    {
        ...
        // Current club level
        $tierImngUrl = $this->http->FindSingleNode('//img[contains(@src, "slh_club") and contains(@src, "logo")]/@src');
        $tierName = $this->http->FindPreg('/slh-club\/slh_(.+?)_/', false,$tierImngUrl);
        $tierCode = $this->http->FindPreg('/_(..)_logo/', false, $tierImngUrl);
        $tier = strtoupper($tierName) . " " . $tierCode;

        $this->logger->debug("[tier name]: {$tierName}");
        $this->logger->debug("[tier code]: {$tierCode}");
        $this->logger->debug("[tier]: {$tier}");
        if (isset($tier)) {
            $this->SetProperty('Status', $tier);
        }
        // Account Number
        $number = $this->http->FindSingleNode('//img[contains(@src, "slh_club") and contains(@src, "logo")]/following-sibling::p', null, true, '/\d+/');
        if (isset($number)) {
            $this->SetProperty('MembershipNumber', $number);
        }
        // Club Expiry
        $clubExpiry = $this->http->FindSingleNode('//p[contains(@class ,"tier") and contains(@class, "expiration")]/strong', null, true);
        if (isset($clubExpiry)) {
            $this->SetProperty('TierExpiryDate', $clubExpiry);
        }
