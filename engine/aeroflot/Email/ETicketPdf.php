...
{
    public $mailFiles = "aeroflot/it-10705830.eml, aeroflot/it-11715974.eml, aeroflot/it-12618821.eml, aeroflot/it-2760661.eml, aeroflot/it-27815929.eml, aeroflot/it-2820622.eml, aeroflot/it-33694676.eml, aeroflot/it-44621234.eml, aeroflot/it-5587431.eml, aeroflot/it-6721468.eml, aeroflot/it-6996591.eml";
    ...
    public static $dictionary = [
        ...
        'fr' => [
            ...
            'ISSUE DATE'                  => ['DATE D\'ÉMISSION DU BILLET'],
            'Total Amount'                => ['Montant total'],
            //			'Fare' => '',
            // 'Equivalent'                  => '',
            'Taxes/Fees/Carrier' => 'Taxes / charges / frais',
            'Itinerary Details'  => ['Détails De L\'itinéraire'],
            'Allowances'         => ['Informations De Paiement'],
            //			'Operated by:' => '',
            'Time'        => 'Heure',
            'Class'       => 'Cabine',
    ...
    {
        $pdfs = $parser->searchAttachmentByName("(?:Recibo de pasaje electrónico, .+ para |Electronic ticket receipt, .+ for |Reçu de billet électronique\, .+ pour ).+\.pdf");
        // example: name="Recibo de pasaje electrónico, 21 octubre para NICOLE AMERENA SERRANO.pd"
        // "Electronic ticket receipt, May 12 for MR OCTAVIAN CUCU.pdf"
    ...
    private function parsePdf(&$itineraries): bool
    {
        ...
        //it-12618821.eml
        $segments = $this->split("#\n(?:(\s{0,10}\d|\s{0,5}{$this->opt($this->t('Departure'))}\s+\d))#", $flights);

        $trains = $flights = [];

        foreach ($segments as $stext) {
            //it-12618821.eml
            //Départ: 02 juin 25 AEROMEXICO        Mexico               Paris, France        Cabine AFFAIRES / O
            //$stext = preg_replace("/({$this->opt($this->t('Departure'))}\s+\d+\s*\w+\s*\d+)\s(\w+.+)\s([ ]{5,}[A-Z]+.+[ ]{5,}[A-Z]+.+{$this->opt($this->t('Class'))}.+)/", "$1  $2$3", $stext);
            $stext = preg_replace("/({$this->opt($this->t('Departure'))}\s+\d+\s*\w+\s*\d+)\s(\w+.+)\s([ ]{5,}[A-Z]+.+[ ]{5,}[A-Z]+.+{$this->opt($this->t('Class'))}.+)/", "$1  $2$3", $stext);
    ...
    private function parseSegments(array $segments, array &$reservation, ?bool $isTrain = false): void
    {
        foreach ($segments as $table) {
            ...
            //it-12618821.eml
            if (preg_match_all("/\:(\d.+)/", $dateText, $m)) {
                $dateValues = $m[1];
            }
