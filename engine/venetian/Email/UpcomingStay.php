...
class UpcomingStay extends \TAccountChecker
{
    ...
    public function ParseHotel(Email $email)
    {
        ...
        if (preg_match("/^\D*\d{4}\s*\D+(?:Rights Reserved\.|under license\.?\s*)\s*(?:.+? \s+)?(.+)$/", $hotelNameText, $m)
            ...
            || preg_match("/^\D*\d{4}\s*.+(?:Rights Reserved\.|under license\.?\s*)\s*The Venetian (.*\d.+)$/", $hotelNameText, $m)
        ) {
            ...
            if (empty($name) && preg_match("/^\s*©\s*\d{4} (The Venetian [^\.]+?)\./", $hotelNameText, $m2)) {
                $name = $m2[1];
            }
            $h->hotel()
                ->name($name)
        ...
        $account = $this->http->FindSingleNode("//text()[{$this->contains($this->t('Venetian Rewards ID:'))}]", null, true,
            "/{$this->opt($this->t('Venetian Rewards ID:'))}\s*(\d{5,})\./");
