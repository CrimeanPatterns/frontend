...
//        $this->http->GetURL("https://onlinebanking.usbank.com/auth/login/");
        ...
        $login = $this->waitForElement(WebDriverBy::xpath("//input[@name = 'Username' or @name = 'aw-personal-id']"), 20);
        $pass = $this->waitForElement(WebDriverBy::xpath("//input[@name = 'Password' or @name = 'aw-password']"), 0);
    ...
    public function Login()
    {
        try {
            ...
            if ($this->waitForElement(WebDriverBy::xpath('//div[
                ...
                if ($differentMethod = $this->waitForElement(WebDriverBy::xpath('//span[contains(text(), "Verify with a different method") or (@data-testid="usb-link-" and not(@aria-describedby="espanol-desc"))]'), 0)) {
                ...
                if (!$asqSQ) {
                    $asqSQ =
                        $this->waitForElement(WebDriverBy::xpath('//span[@id = "emailradio0--text"] | //input[@class = "usb--radio-input" and contains(@value, "@")]'), 0)
                        ?? $this->waitForElement(WebDriverBy::xpath('//span[@id = "smsradio0--text"] | //input[@class = "usb--radio-input" and contains(@value, "******")]'), 0)
                    ;
