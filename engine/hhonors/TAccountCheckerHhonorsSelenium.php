...
class TAccountCheckerHhonorsSelenium extends TAccountChecker
{
    ...
    public function InitBrowser()
    {
        ...
        if ($this->attempt == 0) {
            ...
            $this->useCamoufox(SeleniumFinderRequest::CAMOUFOX_DEFAULT);
...
//                throw new CheckException($message, ACCOUNT_PROVIDER_ERROR);
    ...
    private function loginSuccessful()
    {
        ...
        foreach ($cookies as $cookie) {
            $this->logger->debug("\$cookie['name']: {$cookie['name']}");
//            if ($cookie['name'] == 'wso2AuthToken') {
            if ($cookie['name'] == 'webGuestMetadata') {
                $this->wso2AuthToken = $this->http->JsonLog(urldecode($cookie['value']));
            }
        }
...
//        $auth = "Bearer {$this->wso2AuthToken->accessToken}";
        ...
        if (empty($result) || isset($message) && in_array($message, ['Gateway Timeout', 'Service Unavailable'])) {
            sleep(5);
            $this->logger->error("[Retrying]: {$message}");
            $this->sendNotification("Gateway Timeout / Service Unavailable");
            $this->sendReservationRequest($auth, $confNumber, $lastName, $arrivalDate, $guestId);
            $result = $this->http->JsonLog(null, 3, true);
        }
