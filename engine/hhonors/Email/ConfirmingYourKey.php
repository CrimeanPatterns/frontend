...
class ConfirmingYourKey extends \TAccountChecker
{
    ...
    {
        if (isset($headers['from']) && stripos($headers['from'], '@h1.hilton.com') !== false or stripos($headers['from'], '@h6.hilton.com') !== false) {
    ...
    {
        return $this->http->XPath->query("//text()[contains(normalize-space(), 'Hilton Honors')]")->length > 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('Confirming your Digital Key Request'))}]")->length > 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('We are preparing a Digital Key for your current stay at the'))}]")->length > 0;
    }
