...
class YourRewards extends \TAccountChecker
{
    ...
    private $subjects = [
        'en' => ['Your Free Night Reward has arrived', ' earning ', ' earn '],
    ];
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        $balance = $this->http->FindSingleNode("//text()[{$this->starts(['#', 'Member #'])}]/ancestor::table[2]/descendant::text()[{$this->contains($this->t('Points'))}]", null, true, "/^([\d\.\,\']+)\s*Points/su");
        ...
        if (empty($number)) {
            $number = $this->http->FindSingleNode("//text()[{$this->starts(['#', 'Member #'])}]", null, true, "/[#]\s*(\d{5,})$/");
        }
        $st->setNumber($number)
            ->setLogin($number);

        $userEmail = $this->http->FindSingleNode("//text()[{$this->starts(['This email advertisement was delivered to', 'This email was delivered to'])}]/following::text()[normalize-space()][1]", null, true, "/^(\S+[@]\S+)$/");
    ...
    {
        return preg_match('/[@.](?:h4|h5|h6)\.hilton\.com/i', $from) > 0;
    }
