...
{
    public $mailFiles = "ratehawk/it-665267684.eml, ratehawk/it-665552248.eml, ratehawk/it-740163756.eml, ratehawk/it-745994579.eml, ratehawk/it-748364460.eml, ratehawk/it-920142316.eml";
    public $lang = '';
    ...
    public $detectLang = [
        ...
        'de' => ['Buchungsbestätigung ref', 'Anreise'],
    ];
    ...
    public static $dictionary = [
        ...
        "de" => [
            //HTML
            'Guest name'                                                               => 'Gastname',
            // 'As per your request, booking ref'                                      => '',
            // 'has been cancelled'                                                    => '',
            // 'was canceled automatically due to non-compliance with payment conditions' => '',
            'Check-in'                                                                 => 'Anreise',
    ...
    public function ParseHotelHTML(Email $email)
    {
        ...
        $h->general()
            ->traveller($this->http->FindSingleNode("//text()[{$this->eq($this->t('Guest name'))}]/following::p[normalize-space()][1]"));

        $conf = $this->http->FindSingleNode("//text()[{$this->eq($this->t('As per your request, booking ref'))}]/ancestor::tr[normalize-space()][1]", null, true, "/{$this->opt($this->t('As per your request, booking ref'))}\s*[№]\s*(\d{5,})\s/u");
        ...
        $h->booked()
            ->checkIn($this->normalizeDate($this->http->FindSingleNode("//text()[{$this->eq($this->t('Check-in'))}]/following::p[normalize-space()][1]")))
            ->checkOut($this->normalizeDate($this->http->FindSingleNode("//text()[{$this->eq($this->t('Check-out'))}]/following::p[normalize-space()][1]")));

        $h->hotel()
            ->name($this->http->FindSingleNode("//text()[{$this->eq($this->t('Guest name'))}]/preceding::p[normalize-space()][2]"))
            ->address($this->http->FindSingleNode("//text()[{$this->eq($this->t('Guest name'))}]/preceding::p[normalize-space()][1]"));

        if ($h->getCancelled() !== true){
            $roomsRoots = $this->http->XPath->query("//text()[{$this->eq($this->t('order number'))}]/ancestor::tr[normalize-space()][3]/preceding-sibling::tr[normalize-space()][1]/descendant::td[normalize-space()][1]/child::table[{$this->contains($this->t('for'))}]");

            $guestsCount = 0;

            foreach ($roomsRoots as $roomsRoot){
                $r = $h->addRoom();

                $r->setType($this->http->FindSingleNode("./descendant::text()[normalize-space()][1]", $roomsRoot));

                $guestsCount += $this->http->FindSingleNode("./descendant::text()[normalize-space()][2]", $roomsRoot, false, "/^{$this->opt($this->t('for'))}[ ]*([0-9]+)[ ]*{$this->opt($this->t('adults'))}$/u");
            }
            ...
            $totalPrice = $this->http->FindSingleNode("//td[{$this->eq($this->t('Amount'))}][1]/following-sibling::td[normalize-space()][1]");
            ...
            $cancellation = $this->http->FindSingleNode("//td[{$this->eq($this->t('Cancellation conditions'))}][1]/following-sibling::td[normalize-space()][1]");
    ...
    private function detectDeadLine(Hotel $h)
    {
        ...
        if (preg_match("/Kostenlose Stornierung vor dem[ ]*(\d+[ ]+\w+[ ]*\d{4}\,\s*\d+\:\d+)\s*\(?UTC/", $cancellationText, $m)) {
            $h->setDeadline($this->normalizeDate($m[1]));
        }
    }
