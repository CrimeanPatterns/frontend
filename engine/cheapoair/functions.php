...
//    }
    ...
    public function LoadLoginForm()
    {
        ...
        $this->http->GetURL("https://www.cheapoair.com/profiles/#/user-signin", [
...
//        $this->http->SetInputValue('txtPassword', $this->AccountFields['Pass']);
        ...
        return $this->getAbckFromSelenium();
        ...
        $data = [
            "EmailExpressOtherInfo" => [
                ...
                "PageReferrer"         => "https://www.cheapoair.com/profiles/#/user-signin",
...
//        $this->http->setDefaultHeader("X-EncUserId", "null");
        ...
        $this->http->PostURL("https://www.cheapoair.com/clubmiles-auth/publicapi/v5/Authentication/RecaptchaSignIn", json_encode($data));
    ...
    private function getAbckFromSelenium()
    {
        ...
        try {
            ...
            $selenium->http->GetURL("https://www.cheapoair.com/profiles/#/user-signin");
            $loginField = $selenium->waitForElement(WebDriverBy::xpath('//input[@name = "email"]'), 10);
            $btn = $selenium->waitForElement(WebDriverBy::xpath('//button[contains(text(), "Next")]'), 0);
            ...
            $password = $selenium->waitForElement(WebDriverBy::xpath('//input[@name = "password"]'), 10);
            ...
            $selenium->waitForElement(WebDriverBy::xpath('//span[contains(text(), "Hi, ")]'), 10);
            $this->savePageToLogs($selenium);
            ...
            foreach ($requests as $n => $xhr) {
                ...
                if (strstr($xhr->request->getUri(), 'Authentication/RecaptchaSignIn')) {
                    $this->logger->notice("xhr request {$n}: {$xhr->request->getVerb()} {$xhr->request->getUri()} " . json_encode($xhr->request->getHeaders()));
    ...
    {
        $this->http->GetURL("https://www.cheapoair.com/clubmiles/api/v1/traveler/userdetails");
        ...
        $this->http->GetURL("https://www.cheapoair.com/clubmiles/api/v1/Loyalty/LoyaltyPointsWithTierStatus");
        $response = $this->http->JsonLog(null, 3, true);
        // Balance - available points
        $this->SetBalance(ArrayVal($response, 'TotalPoint'));
