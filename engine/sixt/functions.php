    ...
    public function Login()
    {
        ...
        if ($this->http->FindSingleNode("//p[
            ...
            | //*[self::span or self::div][
                    ...
                    or contains(text(), 'Complete your account')
                    or contains(text(), 'Vervollständigen Sie Ihr Konto')
                ]
        ")) {
        ...
        if ($message = $this->http->FindSingleNode('//div[contains(text(), "Ihr Konto ist gesperrt")]')) {
...
//            $selenium->useGoogleChrome();
            ...
            try {
                $selenium->http->GetURL("https://www.sixt.{$this->domain}/#/?page=loginregister&redirectPath=%2Faccount%2F%23%2Fbookings");
            ...
            $loginInput = $selenium->waitForElement(WebDriverBy::xpath('//input[@id = "username" or @id = "email" or @name = "email"]'), 15);
            $this->savePageToLogs($selenium);
            ...
            try {
                ...
                $this->savePageToLogs($selenium);
                $loginInput = $selenium->waitForElement(WebDriverBy::xpath('//input[@id = "username" or @id = "email" or @name = "email"]'), 0);
                $loginInput->sendKeys($this->AccountFields['Login']);
            }

            $this->acceptCookies($selenium);

            if (!$this->clickByXpath($selenium, '//button[(normalize-space() = "Continue" or normalize-space() = "Weiter" or contains(., "Next") or normalize-space(.) = "Continue with email" or normalize-space(.) = "Weiter mit E-Mail" or normalize-space(.) = "Weiter mit E-Mail-Adresse") and not(@disabled)]')) {
                return false;
            }
            ...
            $this->clickByXpath($selenium, '//*[self::p or self::div][contains(text(), "with password") or contains(text(), "Mit Passwort")] | //span[contains(text(), "Passwort verwenden") or contains(text(), "with password") or contains(normalize-space(text()), "Use password instead") or contains(text(), "Stattdessen Passwort verwenden")]');

            $passwordInput = $selenium->waitForElement(WebDriverBy::xpath('//input[@id = "password" or @name = "password"]'), 5);
            $this->savePageToLogs($selenium);

            if (!$passwordInput) {
                if ($selenium->waitForElement(WebDriverBy::xpath('//span[contains(., "Error when attempting to fetch resource.")]'), 0)) {
                    throw new CheckException(self::PROVIDER_ERROR_MSG, ACCOUNT_PROVIDER_ERROR);
                }
            ...
            if (!$this->clickByXpath($selenium, '//button[(normalize-space() = "Log in" or normalize-space() = "Sign in" or normalize-space(.) = "Einloggen" or normalize-space(.) = "Login" or normalize-space(.) = "Anmelden") and not(@disabled)]')) {
                $this->savePageToLogs($selenium);
    ...
    private function clickByXpath($selenium, $xpath): bool
    {
        $this->logger->notice(__METHOD__);

        $btn = $selenium->waitForElement(WebDriverBy::xpath($xpath), 3);
        $this->savePageToLogs($selenium);
    ...
    {
        $this->logger->notice(__METHOD__);
        $selenium->driver->executeScript("try { var popup = document.querySelector('#usercentrics-root').style = 'display: none;'; if (popup) popup.style = \"display: none;\"; } catch (e) {}");
    }
}
