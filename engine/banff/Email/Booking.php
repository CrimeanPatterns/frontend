...
{
    public $mailFiles = "banff/it-245588660.eml, banff/it-248378931.eml, banff/it-520761792.eml, banff/it-889352954.eml, banff/it-918407894.eml";
    ...
    private function parseEmailHtml(Email $email)
    {
        ...
        foreach ($nodes as $root) {
            ...
            $s->setDepDate($this->normalizeDate($this->http->FindSingleNode("*[normalize-space()][1]/descendant::text()[normalize-space()][2]", $root)));

            // collect departure name
            $depNameText = $this->http->FindSingleNode("*[normalize-space()][1]/descendant::text()[normalize-space()][not({$this->starts('Your driver')})][4]", $root);

            if (preg_match("/^\s*(?<firstPart>.+?)(?:\s*\({$this->opt($this->t('Pickup at'))}\s+(?<secondPart>.+?)\))?\s*$/", $depNameText, $m)) {
                $depName = $m['firstPart'];

                if (!empty($m['secondPart'])) {
                    $depName .= ', ' . $m['secondPart'];
                }

                $s->setDepName($depName);
            }
