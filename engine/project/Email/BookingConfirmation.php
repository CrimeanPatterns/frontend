...
class BookingConfirmation extends \TAccountChecker
{
    ...
    {
        return stripos($from, '@projectexpedition.com') !== false;
    }

    public function detectEmailByHeaders(array $headers)
    {
        $detectProv = false;
        $fromHeader = array_key_exists('from', $headers) ? rtrim($headers['from'], '> ') : '';
        $subjectHeader = array_key_exists('subject', $headers) ? $headers['subject'] : '';

        if ($this->detectEmailFromProvider($fromHeader) === true
            || preg_match('/@(?:ntmllc|smartflyer)\.com$/i', $fromHeader)
            || $this->stripos($subjectHeader, 'Project Expedition')
        ) {
            $detectProv = true;
        }

        foreach ($this->reSubject as $reSubject) {
            if ($detectProv && stripos($subjectHeader, $reSubject) !== false) {
                return true;
            }
        }
    ...
    private function parseItinerary(Email $email, \DOMNode $root): void
    {
        ...
        $operatorPhoneVal = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Tour Operator:'))}][1]/ancestor::*[../self::tr][1]/descendant::text()[{$this->eq($this->t('Contact Info:'))}][1]/following::text()[normalize-space()][1]", $root) ?? '';

        if ($operatorPhoneVal && trim($operatorPhoneVal) === '.'
            || preg_match('/^\S+@\S+$/', $operatorPhoneVal)
            || preg_match('/^.*:$/', $operatorPhoneVal)
        ) {
            // skiping bad phone value
        } else {
            ...
            $r->program()->phone(
                $this->re("/(?:^|[A-Z:]\)?)(?:​)?[\s‪‬]*({$this->patterns['phone']})[\s‪‬]*(?:[,;\|\/]| or |[A-Z(]{2}|[\s(]{2}|$)/mu", $operatorPhoneVal),
                $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Tour Operator:'))}][1]/following::text()[normalize-space()][1]", $root, false, '/^.*[^:]$/')
            );
        }
    ...
    private function findAddress(\DOMNode $root, string $notes = ''): ?string
    {
        ...
        */
        ...
        if (!empty($specialAddress)) {
            $this->logger->info('Collected special address!');
