...
{
    public $mailFiles = "finnair/it-814188266.eml, finnair/it-914319623-fi.eml";

    public $lang = '';

    public static $dictionary = [
        'fi' => [
            'confNumber' => ['Varaustunnuksesi'],
            // 'cabinValues' => [''],
            'your journey to' => 'matkasi kohteeseen',
            // 'Your' => '',
        ...
        'en' => [
            ...
            'cabinValues' => ['Economy', 'Business', 'Premium Economy'],
            // 'your journey to' => '',
            // 'Your' => '',
            // 'Operated by' => '',
            // 'Total duration' => '',
    ...
    public function detectEmailByHeaders(array $headers)
    {
        ...
        return array_key_exists('subject', $headers)
            && preg_match('/[,\s](?:your journey to|matkasi kohteeseen) .{2,} (?:is in|alkaa)\s+\d/i', $headers['subject']) > 0;
    }
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        $patterns = [
            'date' => '(?:\b\d{1,2}[,. ]+[[:alpha:]]+[,. ]+\d{4}\b|\b\d{1,2}\.\d{1,2}\.\d{4}\b)', // 22 December 2024  |  29.05.2025
        ...
        if (preg_match("/(?:^|:\s*)({$patterns['travellerName']})\s*,\s*{$this->opt($this->tPlusEn('your journey to'))}/iu", $parser->getSubject(), $m)) {
        ...
        foreach ($segments as $root) {
            ...
            */
            ...
            if (preg_match("/^(?<cabin>{$this->opt($this->tPlusEn('cabinValues'))})[\s(]+(?<bookingCode>[A-Z]{1,2})[\s)]*$/im", $segHeader, $m)) {
                // Economy (I)
                $s->extra()->cabin($m['cabin'])->bookingCode($m['bookingCode']);
            } elseif (preg_match("/^(?<cabin>{$this->opt($this->tPlusEn('cabinValues'))})(?:\s*\(|$)/im", $segHeader, $m)) {
                // Business
            ...
            */
            ...
            if (preg_match("/{$this->opt($this->t('Total duration'))}[:\s]+((?:\d{1,3}[ht min]+)+)/i", $flightInfo, $m)) {
