...
class BookingConfirmation extends \TAccountChecker
{
    ...
    private function parseEmail(Email $email): void
    {
        ...
        foreach ($segments as $segment) {
            ...
            $seatsNodes = $this->http->XPath->query("./following-sibling::tr[{$this->contains($this->t('Purchase seats'))}][1]/following-sibling::tr/td[2]/descendant::text()[normalize-space(.)!=''][1]",
                $segment);

            foreach ($seatsNodes as $sRoot) {
                if (preg_match("/^\s*(\d{1,3}[A-Z])\s*$/u", $sRoot->nodeValue, $m)) {
                    $pax = $this->http->FindSingleNode("preceding::text()[normalize-space()][1]", $sRoot, true, "/^(.+)\s*\(/");
            ...
            if ($seatsNodes->length === 0) {
                ...
                foreach ($seatsNodes as $sRoot) {
                    if (preg_match("/^\s*[A-Z]{3}\s*[A-Z]{3}\s*$/", $sRoot->nodeValue)) {
                        break;
                    } elseif (preg_match("/^\s*[[:alpha:]]+\s+(\d{1,3}[A-Z])\s*$/u", $sRoot->nodeValue, $m)) {
                        $pax = $this->http->FindSingleNode("preceding::text()[normalize-space()][1]", $sRoot, true,
                            "/^(.+)\s*\(/");
