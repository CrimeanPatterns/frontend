...
class InvoiceAgentGuestPdf extends \TAccountChecker
{
    ...
    private static $dict = [
        ...
        'es' => [
            ...
            'Reservation ID'                 => ['Clave de reservación', 'Reservation ID'],
            'Ship'                           => ['Barco', 'Ship'],
            'Itinerary'                      => ['Itinerario', 'Itinerary'],
            'Stateroom'                      => ['Camarote', 'Stateroom'],
            'Sailing Date'                   => ['Inicio de servicios', 'Sailing Date'],
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        if (count($pdfs) === 0) {
            $pdfs = $parser->searchAttachmentByName('(?:guest_copy|Copia_Cliente)\.pdf');
        }
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (stripos($textPdf, 'www.celebritycruises.com') === false
                ...
                && strpos($textPdf, 'RCL Cruises Ltd') === false
    ...
    private function parsePdf(Email $email, $text)
    {
        ...
        $this->logger->debug("/ {2}{$this->opt($this->t('Departure Date'))}[: ]{2,}(.+?)(?: {2}|$)/m");
        ...
        // travellers
        $guestInformation = preg_match("/^[ ]*({$this->opt($this->t('Guest Information'))}.+?{$this->opt($this->t('Guest'))}\b.+?)\s*{$this->opt($this->t("Captain's Club Numbe"))}/msu", $text, $m) ? $m[1] : '';

        if (empty($guestInformation)) {
            $guestInformation = preg_match("/^[ ]*({$this->opt($this->t('Guest Information'))}.+?{$this->opt($this->t('Guest'))}\b.+?)\n\n{$this->opt($this->t("IMPORTANT NOTICE TO GUESTS:"))}/msui", $text, $m) ? $m[1] : '';
        }
        ...
        if ($guestInformation) {
            ...
            $this->logger->error(var_export($tableTraveller, true));
            ...
            $c->general()->travellers(preg_replace("/^\s*(?:MRS|MR|MS)/", "", $travellers));
        }
