...
{
    public $mailFiles = "blane/it-113049766.eml, blane/it-150519579.eml, blane/it-155911952.eml, blane/it-156905248.eml, blane/it-157511867.eml, blane/it-20945262.eml, blane/it-25717908.eml, blane/it-32994509.eml, blane/it-35737650.eml, blane/it-444820959.eml, blane/it-702694082.eml, blane/it-920001030.eml, blane/it-926316451.eml";
    ...
    public static $dict = [
        'en' => [
            ...
            'phonesHeader' => ['Call our 24/7 Service Hotlines', 'call our 24/7 Service Hotlines'],
            'USA,'         => ['USA,', ', D.C.'],
        ],
        'de' => [
            //            'Dear '  => '',
            ...
            'Passenger:'      => ['Fahrgast:'],
            // 'badServices' => [''],
            'phonesHeader' => ['Rufen Sie unsere rund um die Uhr verfügbaren Service-Hotlines', 'rufen Sie rund um die Uhr unsere Service-Hotlines'],
            // 'USA,' => '',
        ],
        'fr' => [
            'Dear '               => 'Cher',
            ...
            'Passenger:'      => ['Passager:'],
            // 'badServices' => [''],
            'phonesHeader' => "Appelez notre service d'assistance 24h/24, 7j/7",
            // 'USA,' => '',
        ],

        'es' => [
            //'Dear '               => '',
            "confNo"              => "Número de reserva:",
            ...
            'Passenger:'      => ['Pasajero:'],
            // 'badServices' => [''],
            'phonesHeader' => "Llame a nuestras líneas de asistencia 24/7:",
            // 'USA,' => '',
        ],
    ];
    ...
    private function parseEmail(Email $email, ?string $providerCode): void
    {
        ...
        foreach ($nodes as $root) {
            ...
            //it-702694082.eml
            $confirmations = array_filter(explode(' ', $this->http->FindSingleNode(".//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1][not(.//tr) and {$this->eq($this->t('confNo'))}] ]/*[normalize-space()][2]/descendant::*[contains(@style, 'bold')]", $root)));

            if (count($confirmations) === 0) {
                $confirmations = array_filter(explode(' ', $this->http->FindSingleNode(".//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1][not(.//tr) and {$this->eq($this->t('confNo'))}] ]/*[normalize-space()][2]", $root)));
            }
            ...
            //it-702694082.eml
            $node = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Date and time:'))}]/ancestor::tr[1]/descendant::td[2]/descendant::*[contains(@style, 'bold')]")
                ?? $this->nextTD($this->t('Date and time:'), $root)
            ...
            if (preg_match("/^(.{3,})\s*\-\s*(All terminals\,.+)/", $departure, $m)) {
                $departure = $m[1];
                $r->setNotes('From: ' . $m[2]);
            }
            ...
            if ($this->http->XPath->query("//text()[{$this->contains($this->t('USA,'))}]")->length > 0) {
                $s->departure()
            ...
            if (empty($arrival)) {
                ...
                if (preg_match("/back\s*to\s*([A-Z]{3})\s*.+\s+(\d+\s*a?p?m)/", $arrival, $m)) {
                ...
                } elseif (preg_match("/[+]\s*return\s*to\s*cruise\s*terminal$/", $arrival, $m)) {
                    // it-920001030.eml
                    $s->arrival()
                        ->address($departure);
                    ...
                    if (!empty($duration)) {
                        $s->arrival()
                            ->date(strtotime('+' . $duration, $s->getDepDate()));
                    }
                }
            ...
            $traveller = preg_replace("/^\s*(M\.|Mme\.|Ms\.|Herr|Frau|Mr\.|Sr\.) /", '', $traveller);
            ...
            foreach ($this->parsePhones($root) as $phone) {
                $r->program()->phone($phone['number'], $phone['desc']);
            }
        }
    }

    private function parsePhones(?\DOMNode $root = null): array
    {
        $phones = [];

        $textPhones = implode("\n", $this->http->FindNodes("descendant::text()[{$this->contains($this->t('phonesHeader'))}]/ancestor::*[../self::tr][1]/descendant::text()[normalize-space() and not({$this->contains($this->t('phonesHeader'))})]", $root));

        preg_match_all("/^(?<desc>[^:\n\d]+?)\s*[:]+\s*(?<number>{$this->patterns['phone']})$/m", $textPhones, $phoneMatches, PREG_SET_ORDER);

        foreach ($phoneMatches as $m) {
            if (in_array($m['number'], array_column($phones, 'number'))) {
                continue;
            }

            $phones[] = [
                'desc' => $m['desc'], 'number' => $m['number'],
            ];
        }
