...
class PieologyPunchhDotComExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    ...
    private $name;
    private $domain = 'iframe.punchh.com';

    public function getStartingUrl(AccountOptions $options): string
    {
        if ($this->code == 'tgifridaysfranchisor') {
            $this->domain = 'iframe.us-west.punchh.com';
        }

        return "https://{$this->domain}/customers/sign_in.iframe?slug=" . $this->code;
    }
    ...
    {
        $tab->gotoUrl("https://{$this->domain}/customers/edit.iframe?slug=" . $this->code);
    ...
    public function logout(Tab $tab): void
    {
        ...
        $tab->gotoUrl('https://' . $this->domain . '/customers/sign_out.iframe?slug=' . $this->code);
    ...
    public function parse(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
        ...
        if (
            $tab->getUrl() != 'https://' . $this->domain . '/customers/edit.iframe?slug=' . $this->code . '/secure_migration'
            && $tab->getUrl() != 'https://' . $this->domain . '/whitelabel/' . $this->code
        ) {
            $tab->gotoUrl('https://' . $this->domain . '/whitelabel/' . $this->code);
        }
        ...
        // Name
        if ($tab->getUrl() != 'https://' . $this->domain . '/customers/edit.iframe?slug=' . $this->code) {
            $tab->gotoUrl('https://' . $this->domain . '/customers/edit.iframe?slug=' . $this->code);
        }
        $name = beautifulName(trim($tab->findText('//input[@id="user_first_name"]/@value', FindTextOptions::new()->timeout(10)->allowNull(true)) . ' ' . $tab->findText('//input[@id="user_last_name"]/@value', FindTextOptions::new()->timeout(10)->allowNull(true))));
        ...
        */
        ...
        // Rewards
        $tab->gotoUrl('https://' . $this->domain . '/whitelabel/' . $this->code . '/offers');
        ...
        if ($tab->evaluate('//strong[
            ...
            && $tab->getUrl() == 'https://' . $this->domain . '/customers/edit.iframe?slug=' . $this->code
