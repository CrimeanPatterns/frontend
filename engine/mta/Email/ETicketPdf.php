...
{
    public $mailFiles = "mta/it-16100089.eml, mta/it-164069646.eml, mta/it-16938561.eml, mta/it-27211209.eml, mta/it-27527828.eml, mta/it-679358756-train.eml, mta/it-908254554.eml";
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (
                (stripos($text, 'NATURALISTE TRAVEL') !== false
                    || stripos($text, 'AIR TICKETS NTC IATA:') !== false
                    || stripos($text, 'Endorsements/Airline Conditions:') !== false)
                && $this->assignLang($text)
            ) {
                return true;
            }
            ...
            if (
                (stripos($text, 'Issued on behalf of:') !== false
                    || stripos($text, 'THIS E-TICKET ITINERARY/RECEIPT MAY BE REQUIRED AT CHECK-IN AND') !== false
                    || stripos($text, 'Itinerary as at ') !== false)
                && $this->assignLang($text)
            ) {
                return true;
            }
    ...
    private function parseEmail($textPDF, Email $email): void
    {
        ...
        foreach ($pages as $page) {
            ...
            $segs = $this->splitter("/^( {0,15}(?:\S ?)+ {2,}.+ Baggage Allowance:)/m", $itBlock);
