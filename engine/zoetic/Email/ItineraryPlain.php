    ...
    private function parseEmail($textPDF, Email $email): bool
    {
        ...
        if (preg_match_all("/^[\r\t ]*FOR: *([\s\w\/]+?)\n[\r\t ]*\n/m", $textPDF, $m, PREG_SET_ORDER)) {
            foreach ($m as $v) {
                ...
                $pax = preg_replace('/\s+/', ' ', $pax);
                $pax = preg_replace(["/^(?:MRS|MR|MS|DR)\s+/i", "/\s+(?:MRS|MR|MS|DR)\s*$/i"], '', $pax);
                $pax = preg_replace("/^\s*(\S.+?)\s*\/\s*(\S.+?)\s*$/", '$2 $1', $pax);
...
//        $this->logger->debug('$table = '.print_r( $table,true));
    ...
    private function parseAir($textPDF, Email $email): bool
    {
        ...
        if (preg_match("#AIR +(.+) FLT: *[A-Z]*(\d+)(?: {3,}(.+?) {3,}(.+))?#", $textPDF, $m) || preg_match("#AIR +(.+) \n*\s*FLT: *[A-Z]*(\d+)\s*(?: {3,}(.+?)\n {3,}(.+))?$#m", $textPDF, $m)) {
            if (!empty(trim($m[1]))) {
                $s->airline()
                    ->name($m[1]);
            }
        ...
        if (!empty($s->getFlightNumber()) && empty($s->getDepName()) && empty($s->getDepDate())
            && empty($s->getArrName()) && empty($s->getArrDate())
            && preg_match("/AIR .* FLT:\d+\D*\bLV\s+\D+\s+\d+HR ?\d+MIN\s+AR\s+\D+$/", $textPDF)
        ) {
        ...
        if (preg_match("# *DEPART: +(.+\n.+)[ ]{10,}(\d+(?:H|M).+)\n#", $textPDF, $m)
            ...
            $s->extra()
                ->duration($this->re("#(?:^[ ]*| {3,})(\d{1,3}HR[ ]*\d.+|\d{1,4}MIN)#m", $textPDF), true, true);
        }

        if (preg_match("/ \bLV[\s\S]+\s*ARRIVE:[ ]+(.+?\n.+)[ ]{10,}(?:[ ]{3,}(REF.+)|[ ]{3,}.*)?\n/", $textPDF, $m)
           || preg_match("/\bLV[\s\S]+\s*ARRIVE:[ ]+(.+?)(?:[ ]{3,}(REF.+)|[ ]{3,}.*)?\n/", $textPDF, $m)) {
            $s->arrival()
                ->terminal(trim(preg_replace("#\bTERM\b#", '', str_replace("TERMINAL", '', $m[1]))));
        }
