...
class UpcomingTripHtml2016 extends \TAccountChecker
{
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        // detect Provider
        if ($this->http->XPath->query("//text()[{$this->contains('carlsonwagonlit.com')}]")->length === 0
            && $this->http->XPath->query("//a/@href[{$this->contains('carlsonwagonlit.com')}]")->length === 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('CWT Service Center'))}]")->length === 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('CWT Trip locator'))}]")->length === 0
        ) {
            return false;
        }
