...
// TODO: merge with parsers sotc/ElectronicTicket, ctraveller/ElectronicTicketDetails (in favor of yatra/ETicketDetails)

class ETicketDetails extends \TAccountChecker
{
    public $mailFiles = "yatra/it-197706503.eml, yatra/it-28251051.eml, yatra/it-49072666.eml, yatra/it-53733658.eml, yatra/it-914724632-cancelled.eml, yatra/it-913988934.eml, yatra/it-914088763.eml";
    private $subjects = [
        'en' => [
            'Electronic Ticket Details', 'Hotel Confirmation Voucher',
            'CANCELLED BOTH WAYS', 'CANCELED BOTH WAYS',
            'CANCELLED AS PER MAIL', 'CANCELED AS PER MAIL',
            'FLIGHT CHANGE',
        ],
    ];
    private $langDetectors = [
        'en' => ['Electronic Ticket Details', 'Booking Details', 'Hotel Confirmation Voucher', 'Cart Details', 'Amendment Comment'],
    ];
    private $lang = '';
    private static $dict = [
        'en' => [
            ...
            'Departure'        => ['Departure', 'D eparture'],
        ],
    ];
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        $amendmentComment = $this->http->FindSingleNode("//tr/*[normalize-space()][1][{$this->eq($this->t('Amendment Comment'), "translate(.,':','')")}]/following-sibling::*[normalize-space(translate(.,':',''))][1]");
        $this->logger->info('AMENDMENT COMMENT: ' . $amendmentComment);

        if ($this->http->XPath->query("//text()[{$this->eq('Hotel Confirmation Voucher')}]/ancestor::*[1]")->length > 0) {
            $this->parseHotel($email);
        } else {
            $this->parseFlight($email, $amendmentComment);
        }
    ...
    private function parseFlight(Email $email, ?string $amendmentComment): void
    {
        ...
        // cancelled
        if (preg_match("/^CANCELL?ED(?:\s+AS PER MAIL|\s+BOTH WAYS|\s+BOTH ONWARD AND RETURN)*$/i", $amendmentComment)
            || preg_match("/^as per mail$/i", $amendmentComment) && preg_match("/^Booking Cancell?ed$/i", $bookingStatus)
        ) {
            $f->general()->cancelled();
        } elseif ($amendmentComment) {
        ...
        foreach ($segments as $segment) {
            ...
            // airlineName
            // flightNumber
            $flight = $this->http->FindSingleNode("descendant-or-self::tr/*[normalize-space()][1][{$this->eq($this->t('Flight'), "translate(.,':','')")}]/following-sibling::*[normalize-space(translate(.,':',''))][1]", $segment, true, "/^(.*?)[A-z<>]*$/");
            ...
            // depCode
            // depTerminal
            $departure = $this->http->FindSingleNode("following-sibling::*[normalize-space()][1]/descendant-or-self::tr[ *[normalize-space()][1][{$this->eq($this->t('Departure'), "translate(.,':','')")}] ][1]", $segment, true, "/^{$this->opt($this->t('Departure'))}[:\s]*(.*)$/i");
            ...
            // arrCode
            // arrTerminal
            $arrival = $this->http->FindSingleNode("following-sibling::*[normalize-space()][2]/descendant-or-self::tr[ *[normalize-space()][1][{$this->eq($this->t('Arrival'), "translate(.,':','')")}] ][1]", $segment, true, "/^{$this->opt($this->t('Arrival'))}[:\s]*(.*)$/i");
            ...
            if (!empty($s->getDepCode()) && !empty($s->getArrCode())
                && preg_match("/^CANCELL?ED[:\s]+{$s->getDepCode()}[-\s]*{$s->getArrCode()}(?:\s+ONLY)?$/i", $amendmentComment)
                    ||
                !empty($s->getDepCode()) && !empty($s->getArrCode()) && !empty($s->getDepDate())
                && preg_match("/^CANCELL?ED[:\s]+{$s->getDepCode()}[-\s]*{$s->getArrCode()}\s+" . date('dM', $s->getDepDate()) . "(?:\s+ONLY)?$/i", $amendmentComment)
            ) {
                $unsupportedFormat = false;
                $s->extra()->cancelled();
            }
        ...
        $confirmationNumbers = array_values(array_unique($this->http->FindNodes($xpathFragment1 . '/*[3]', null, '/^(?:[A-Z\d]{5,8}|(?i)Pending)$/')));

        if (count($confirmationNumbers) === 1 && preg_match("/^Pending$/i", $confirmationNumbers[0])) {
            // it-914088763.eml
            $f->general()->noConfirmation();
        } else {
            foreach ($confirmationNumbers as $item) {
                $f->general()->confirmation(preg_match('/^[A-Z\d]{5,8}$/', $item) ? $item : null, $confirmationNumberTitle);
            }
        }
        ...
        if (preg_match('/^(?<amount>\d[,.\'\d]*)\s*(?<currency>[A-Z]{3})\b/', $payment, $matches)) {
            ...
            if (preg_match('/^-?(?<amount>\d[,.\'\d]*)\s*' . preg_quote($matches['currency'], '/') . '\b/', $discount, $m)) {
                $f->price()->discount(PriceHelper::parse($m['amount'], $matches['currency']));
            }
        }
        ...
        if ($unsupportedFormat && !preg_match("/^{$this->opt(['Confirmed'])}$/i", $bookingStatus)) {
            $this->logger->debug('Found unsupported format!');
