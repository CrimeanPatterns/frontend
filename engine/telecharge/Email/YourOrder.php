...
class YourOrder extends \TAccountChecker
{
    ...
    private $subjects = [
        'en' => ['Your order has been confirmed', 'order is confirmed! Order Number'],
    ];
    ...
    {
        if ($this->http->XPath->query('//a[contains(@href,".telecharge.com/") or contains(@href,"www.telecharge.com") or contains(@href,"www.telechargeoffers.com")]')->length === 0
    ...
    private function parseEvent(Email $email): void
    {
        ...
        if (empty($name)) {
            $name = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Order Status'))}]/preceding::div[normalize-space()][1]");
        }
        ...
        if (empty($event->getAddress())) {
            ...
            if ($address === null){
                $address = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Order Status'))}]/preceding::p[normalize-space()][1]");
            }

            if (!empty($address)) {
                $event->setAddress(preg_replace("/(View on map)/", '', $address));
            }
        }
        ...
        if (preg_match("/^(?:\s*{$this->opt($this->t('Ticket Details'))})?\s*[-[:alpha:]]+\s*,\s*(?<date>[[:alpha:]]+\s+\d{1,2}\s*,\s*\d{4})\b.+?(?<time>\d{1,2}[:：]\d{2}(?:[ ]*[AaPp](?:\.[ ]*)?[Mm]\.?)?)[ ]*\n/u", $detailsText, $m)) {
            ...
            $detailsText = $this->htmlToText($this->http->FindHTMLByXpath("//text()[{$this->contains($this->t('Ticket Details'))}]/ancestor::*[ descendant::text()[normalize-space()][2] ][1]"));
        ...
        if (preg_match("/^.*{$this->opt($this->t('Billing Information'))}.*\n+[ ]*([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])[ ]*(?:\n|$)/u", $billingText, $m)) {
            ...
            $billingText = $this->htmlToText($this->http->FindHTMLByXpath("//text()[{$this->contains($this->t('Ticket Details'))}]/ancestor::td[1]/following-sibling::td[1]/descendant::text()[normalize-space()][1]"));
            if (preg_match("/^[ ]*([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])[ ]*$/u", $billingText, $m)) {
                $traveller = $m[1];
            }
        }
