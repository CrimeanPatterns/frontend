...
class TAccountCheckerAireuropa extends TAccountChecker
{
    ...
    public function Login()
    {
        ...
        if ($message =
                ...
                ?? $this->http->FindSingleNode("(//div[contains(@class, 'alert-danger')])[1] | //*[contains(@id, 'mat-mdc-error-')]")
                ?? $this->http->FindSingleNode("//div[contains(text(), 'Please check your email and confirm your identity to log in to your account.')]")
                ?? $this->http->FindSingleNode("//h2[contains(text(), 'Sorry, there was an error')]")
                ?? $this->http->FindPreg("/id=\"mat-mdc-error-8\">([^<]+)/")
        ) {
    ...
    public function Parse()
    {
        ...
        // Expiration Date, refs #25745
        if (!isset($this->State['cmptoken'])) {
            return;
        }
        ...
        $headers = [
            'Authorization' => sprintf('Bearer %s', $this->State['cmptoken']),
            'Content-Type'  => 'application/json',
        ];

        /*
        $this->http->GetURL("https://loyalty.aireuropa.com/b2c/me/balances", $headers);
        $response = $this->http->JsonLog();
        */
        $this->http->GetURL("https://loyalty.aireuropa.com/b2c/me/points-expiration-forecast", $headers);
        $response = $this->http->JsonLog();
        $exp = null;
        $points = 0;
        ...
        foreach ($response as $row) {
            if (
                isset($row->expirationDate)
                && (!$exp || strtotime($row->expirationDate) <= $exp)
            ) {
                if ($exp == strtotime($row->expirationDate)) {
                    $points += $row->points;
                } else {
                    $points = $row->points;
                }
...
//                $this->savePageToLogs($selenium);
            ...
            $selenium->waitForElement(WebDriverBy::xpath("//div[contains(@class, 'passenger-name')] | //div[contains(@class, 'invalid-credentials')] | //div[contains(text(), 'Please check your email and confirm your identity to log in to your account.')] | //h2[contains(text(), 'Sorry, there was an error')]"), 10);
            $this->savePageToLogs($selenium);
            ...
            // refs #25745
            if ($janoToken) {
//                $this->http->GetURL("https://loyalty.aireuropa.com/en-GB/my-account");
                // open rewards site in the same window
                $selenium->driver->executeScript('document.querySelector(\'#user-details-button-desktop\').click();');
                $selenium->driver->executeScript('var windowOpen = window.open; window.open = function(url) { windowOpen(url, \'_self\'); }');
                sleep(1);
                $this->savePageToLogs($selenium);
                $selenium->driver->executeScript('document.querySelector(\'a[data-testid="my-account-redirect"]\').setAttribute[\'target\', \'_self\']');
                $selenium->driver->executeScript('document.querySelector(\'a[data-testid="my-account-redirect"]\').click();');

                $selenium->waitForElement(WebDriverBy::xpath("//div[contains(text(), 'My Suma number')]"), 20);
                $this->savePageToLogs($selenium);
                $cmptoken = $selenium->driver->executeScript("return sessionStorage.getItem('cmptoken')");
                $this->logger->info("[cmptoken]: " . $cmptoken);
                $this->State['cmptoken'] = $this->http->JsonLog($cmptoken)->access_token ?? null;
            }
