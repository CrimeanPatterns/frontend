...
//            return true;
    ...
    public function Parse()
    {
        ...
        $this->http->PostURL('https://prd-west.webapi.enterprise.com/enterprise-ewt/profile/login/EP', json_encode($data), $headers);
        $response = $this->http->JsonLog();
        ...
        $profileData = $response->gbo->profile->profile->basic_profile->loyalty_data ?? [];
        $number = $profileData->loyalty_number ?? null;
        $tier = $profileData->loyalty_tier ?? null;

        if (isset($number)) {
            // Member #
            $this->SetProperty("MemberNumber", $number);
        }

        if (isset($tier)) {
            // Tier Level
            $this->SetProperty("TierLevel", $tier);
        }

        $driverInfo = $response->gma->commit_request->driver_info ?? [];
        $firstName = $driverInfo->first_name ?? null;
        $lastName = $driverInfo->last_name ?? null;
        ...
        $rentalsData = $response->gbo->profile->profile->basic_profile->loyalty_data->year_to_date_rental_counts ?? [];
        $rentalsCount = $rentalsData->number_of_rentals ?? null;
        $rentalsDays = $rentalsData->number_of_rental_days ?? null;

        if (isset($rentalsCount)) {
            // Rentals
            $this->SetProperty('RentalsCount', $rentalsCount);
        }

        if (isset($rentalsDays)) {
            // Rentals Days
            $this->SetProperty('RentalDays', $rentalsDays);
        }

        // Expiration Date  // refs #5959   https://redmine.awardwallet.com/issues/5959#note-21
        $exp = $profileData->points_expiration_date ?? null;
        ...
        $this->SetBalance($profileData->points_to_date);
