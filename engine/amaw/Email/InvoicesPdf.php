...
class InvoicesPdf extends \TAccountChecker
{
    ...
    {
        $pdfs = $parser->searchAttachmentByName('Agent Copy\.pdf');

        if (count($pdfs) == 0) {
            $pdfs = $parser->searchAttachmentByName('Guest Copy\.pdf');
        }
    ...
    private function parsePdf(Email $email, $text): void
    {
        ...
        */
        ...
        if ($totalPriceIndex !== null && preg_match("/\n([ ]*{$this->opt($this->t('TOTAL VOYAGE CHARGES'))}[ ]{2}.*\d.*)/", $priceText, $m)) {
            ...
            if (preg_match("/\n([ ]*{$this->opt($this->t('Voyage Fare'))}[ ]{2}.*\d.*)/", $priceText, $m)) {
                $table = $this->splitCols($m[1], $tablePos);
                $cpCruise = trim($table[$totalPriceIndex]);
            }

            if ($cpCruise !== null && preg_match("/\n[ ]*{$this->opt($this->t('Voyage Fare'))}.+\n((?:.+\n){1,10})[ ]*{$this->opt($this->t('TOTAL VOYAGE CHARGES'))}/u", $priceText, $m)) {
                $feeRows = array_filter(explode("\n", $m[1]));

                foreach ($feeRows as $feeRow) {
                    $feeName = $this->re("/^\s*(\D+)\b[ ]{5,}/", $feeRow);
                    $table = $this->splitCols($feeRow, $tablePos);

                    $feeSumm = $table[$totalPriceIndex];

                    if (preg_match("/\.$/", $feeSumm)) {
                        $feeSumm = $table[$totalPriceIndex + 2];
                    }

                    $fpCruise = array_merge($fpCruise, [$feeName => $feeSumm]);
                }

                //Agent Copy.pdf
                if (preg_match("/{$this->opt($this->t('Charges'))}\s*{$this->opt($this->t('Commission'))}/", $priceText)) {
                    $feeText = $this->re("/({$this->opt($this->t('TOTAL VOYAGE CHARGES'))}.+)/", $priceText);
                    $table = $this->splitCols($feeText, $tablePos);
                    $fpCruise = array_merge($fpCruise, ['Commission' => $table[0]]);
                }
            }
        }
        ...
        */
        ...
        $this->parseCruise($email, $text, $bookingDate, $tpCruise, $cpCruise, $fpCruise, $cancelledBookings, $status);
    ...
    private function parseCruise(Email $email, $text, $bookingDate, ?string $totalPrice, ?string $costPrice, ?array $feesCruise, ?bool $cancelledBookings, ?string $status): void
    {
        if ($cancelledBookings) {
        ...
        if (preg_match("/^{$this->patterns['totalPrice']}$/", $costPrice, $matches)) {
            $cruise->price()
                ->currency($this->currencyCode ?? $matches['currency'])
                ->cost(PriceHelper::parse($matches['amount'], $this->currencyCode));
        }

        $discount = 0;

        foreach ($feesCruise as $feeName => $feeCruise) {
            if (preg_match("/\-/", $feeCruise) && preg_match("/^{$this->patterns['totalPrice']}$/", $feeCruise, $matches)) {
                $discount += PriceHelper::parse($matches['amount'], $this->currencyCode);
            } elseif (preg_match("/^{$this->patterns['totalPrice']}$/", $feeCruise, $matches)) {
                $cruise->price()
                    ->currency($this->currencyCode ?? $matches['currency'])
                    ->fee($feeName, PriceHelper::parse($matches['amount'], $this->currencyCode));
            }
        }
