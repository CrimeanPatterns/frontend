...
class YourUpcomingTrip extends \TAccountChecker
{
    ...
    {
        return $this->http->XPath->query("//text()[contains(normalize-space(), 'Allegiant Travel Company')]")->length > 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('Your Flight Details'))}]")->length > 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('Departure Airport'))}]")->length > 0;
    }
    ...
    public function parseFlightSegment(Flight $f, $root)
    {
        $s = $f->addSegment();
        ...
        $arrTerminal = $this->http->FindSingleNode("./following::text()[normalize-space()='Arrival'][1]/ancestor::td[1]/following::td[normalize-space()][1]", null, true, "/(\w+)\s*Terminal\s*\(/");
        ...
        if (!empty($s->getDepCode()) && !empty($s->getArrCode())) {
            $number = $this->http->FindSingleNode("//text()[normalize-space()='Your Flight Details']/preceding::text()[starts-with(normalize-space(), 'Confirmation #')][1]"
                . "/following::text()[starts-with(normalize-space(), 'Flight')]/ancestor::td[1]", null, true, "/Flight\s*(\d{1,4})\s*,\s*{$s->getDepCode()}\s*{$s->getArrCode()}\s*/");
