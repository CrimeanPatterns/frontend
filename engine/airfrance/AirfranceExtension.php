...
class AirfranceExtension
    ...
    ContinueLoginInterface, LoginWithConfNoInterface, RetrieveByConfNoInterface
{
    use TextTrait;
    ...
    public function getStartingUrl(AccountOptions $options): string
    {
        ...
        return "https://$this->host/exchange/deeplink?country=us&language=en&target=/";
    ...
    public function isLoggedIn(Tab $tab): bool
    {
        ...
        $tab->gotoUrl("https://$this->host/endpoint/v1/oauth/redirect?loginPrompt=&source=homepage.homepage&locale=US/en-US");
        sleep(3);
    ...
    public function getLoginId(Tab $tab): string
    {
        ...
        return $tab->findText('//p[@class="bw-fb-membership-card__number-text" and contains(text(),"Flying Blue number:")]/strong',
            FindTextOptions::new()->visible(false)->timeout(30));
    }
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        ...
        if ($loginPage) {
            $loginPage->click();
            sleep(1);
            $tab->evaluate('//button[@data-testid="logo-header-auth-menu-login"]')->click();
        }

        // Log in with your password instead?
        $tab->evaluate('//a[contains(@data-test,"prefer-password-")]')->click();
        sleep(2); // refs#https://redmine.awardwallet.com/issues/25269#note-12
        $login = $tab->evaluate('//input[@name="jloginId"]');
        $login->setValue($credentials->getLogin());

        $password = $tab->evaluate('//input[@name="jpassword"]');
        $password->setValue($credentials->getPassword());
        $keepMe = $tab->evaluate('//label[@id="mat-mdc-slide-toggle-1-label"]',
            EvaluateOptions::new()->allowNull(true));
        if ($keepMe) {
            $keepMe->click();
        }
        $tab->evaluate('//button[contains(@class,"login-form-continue-btn")]')->click();
...
//                $segment->flight->newArrivalDate, $segment->flight->newDepartureDate - not print on page(site)
    ...
    {
        return "https://$this->host/trip/flight-details";
    }

    public function loginWithConfNo(Tab $tab, array $fields, ConfNoOptions $options): LoginWithConfNoResult
    {
        $tab->evaluate('//input[@name="bookingCodeOrTicketNumber"]',
            EvaluateOptions::new()->timeout(20))->setValue($fields['ConfNo']);
        $tab->evaluate('//input[@name="lastName"]')->setValue($fields['LastName']);

        //$tab->evaluate('//button[@type="submit" and contains(.,"Search")]')->click();
        /*$loginResult = $tab->evaluate('//h1[contains(text(), "Your trip to:")]
        | //p[contains(text(),"t find your booking details. Please check the booking reference or ticket number and the last name linked to your booking.")]');

        if ($loginResult) {
            $error = $loginResult->getInnerText();
            if (stristr($error, "This booking code has not been found. Please check that it is correct and try again.")) {
    ...
    public function retrieveByConfNo(Tab $tab, Master $master, array $fields, ConfNoOptions $options): void {
        $reservation = $this->getReservationRetrieveByConfNo($tab, $fields['ConfNo'], $fields['LastName']);
        if (empty($reservation)) {
            $this->logger->error("Skipping reservation");
        ...
        $this->parseReservation($tab, $master, $fields['ConfNo'], $reservation);
    }

    private function getReservationRetrieveByConfNo(Tab $tab, string $conf, string $lastName)
    {
        $this->logger->notice(__METHOD__);
        ...
        $this->logger->notice("conf: $conf, lastName: $lastName");
        $payload = [
            'operationName' => 'TripReservationLogonQuery',
            'variables'     => [
                'bookingCode' => $conf,
                'lastName'    => $lastName,
                'recaptchaAssessment' => [
                    'expectedAction' => 'tripEntryPageLogon',
                    'token' => null,
                ]
            ],
            'extensions' => [
                'persistedQuery' => [
                    'version'    => 1,
                    'sha256Hash' => 'fdcc73b38c0966825f3fdba539041560f27d4c6539c55fe45e88b31e3661f487',
                ],
            ],
        ];
        $options = [
            ...
            'headers' => [
                'Accept'        => 'application/json, text/plain, */*',
                'Content-Type'  => 'application/json',
                'x-dtpc'               => '18$59681816_34h26vASDOODHMIFPNOAFMPGPNNUMKSHAPCHPB-0e0',
                'country'              => 'US',
                'language'             => 'en',
                'afkl-travel-country'  => 'US',
                'afkl-travel-host'     => $this->provider,
                'afkl-travel-language' => 'en',
                'x-aviato-host' => $this->host,
                'x-client-revision' => '0454edbd0e52fa463a89efe9d887ab09bc4f25d2'
            ],
            'body' => json_encode($payload),
        ];
        ...
        $message = $response->data->reservationLogon->messages[0]->name ?? null;
        ...
        return $response->data->reservationLogon ?? [];
    }
}
