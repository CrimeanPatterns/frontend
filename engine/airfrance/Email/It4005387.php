...
class It4005387 extends \TAccountChecker
{
    ...
    public static $dictionary = [
        ...
        'pt' => [ // it-125478099.eml
            ...
            'PASSENGER NAME'              => ['SOBRENOME DO PASSAGEIRO', 'SOBRENOME DOPASSAGEIRO'],
            // 'ffNumber' => '',
            'ISSUE DATE'                  => ['DATA DE EMISSÃO DO BILHETE', 'DATA DE EMISSÃO DOBILHETE', 'DATA DE EMISSÃO DO BILHET E'],
            // 'Classes'                     => '',
            // 'seatConfirmed' => '',
            // 'Operated by:'                => '',
            'Fare Basis:'                 => ['Base tarifária:', 'Base t arifária:'],
            'Receipt And Payment Details' => 'Detalhes E Recibo Do Pagamento',
            'Total Fare'                  => ['Tarifa total', 'Tarifatotal', 'Total com taxas'],
        ],
        'es' => [ // it-4005387.eml
            // 'cancelledPhrases' => [''],
    ...
    public function parsePDF(Email $email): void
    {
        ...
        $passengerName = $this->normalizeTraveller($this->pdf->FindSingleNode("//*[{$this->eq($this->t('PASSENGER NAME'))}]/following::text()[normalize-space()][1][not({$xpathBold})]/ancestor::*[1]"));

        $f->general()
            ->confirmation(str_replace(' ', '', $this->nextText($this->t('Booking Reference'))))
            ->traveller($passengerName, true);
        ...
        // ReservationDate
        $issueDate = $this->pdf->FindSingleNode("//*[{$this->eq($this->t('ISSUE DATE'))}]/following::text()[normalize-space()][1][not({$xpathBold})]/ancestor::*[1]");
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        $detectProvider = $this->assignProviderHtml($parser->getCleanFrom(), $parser->getSubject());
    ...
    {
        return ['airfrance', 'alitalia', 'lanpass', 'aerolineas', 'ethiopian', 'hawaiian', 'velocity', 'airserbia'];
    }
    ...
    private function assignProviderHtml(?string $from, string $subject): bool
    {
        if ($this->http->XPath->query('//a[contains(@href,"//www.airfrance.com")]')->length > 0
            || $this->http->XPath->query('//*[contains(.,"www.airfrance.com")]')->length > 0
        ) {
        ...
        if (stripos($from, '@alitalia.sabre.com') !== false || stripos($subject, 'ALITALIA ELECTRONIC TICKET RECEIPT') !== false
            || $this->http->XPath->query('//a[contains(@href,"//www.alitalia.com")]')->length > 0
            || $this->http->XPath->query('//*[contains(normalize-space(),"thank you for choosing Alitalia service") or contains(normalize-space(),"On Alitalia operated flight you may") or contains(.,"www.alitalia.com")]')->length > 0
        ) {
        ...
        if (preg_match('/[@.]latam\.com$/i', $from) > 0
            || $this->http->XPath->query('//*[contains(normalize-space(),"Thanks for choosing LATAM Airlines")]')->length > 0
        ) {
            $this->providerCode = 'lanpass';
    ...
    private function assignProviderPdf(): bool
    {
        ...
        if ($this->pdf->XPath->query('//a[contains(@href,".lan.com/") or contains(@href,"www.lan.com")]')->length > 0
            || $this->pdf->XPath->query('//*[contains(normalize-space(),"consulte o site: www.tam.com.br")]')->length > 0
        ) {
            $this->providerCode = 'lanpass';
