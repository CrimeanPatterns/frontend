...
{
    public $mailFiles = "croatia/it-110706629.eml, croatia/it-2674485.eml, croatia/it-2700567.eml, croatia/it-307127583-hr.eml, croatia/it-896373639-de.eml";

    public $reSubject = [
        "en"=> "croatiaairlines.com - FlyOnLine Confirmation",
        "de"=> "FlyOnLine Electronic Ticket",
            "- Elektronisches Ticket/Dokument",
    ...
    public static $dictionary = [
        ...
        "de" => [
            ...
            'Your Itinerary' => 'Ihr Flugplan',
            'Date' => 'Datum',
            "Departure"                  => "Abreise",
            'Flight' => 'Flug',
            'Fare Families' => 'Tarifklasse',
            'Documents' => 'Unterlagen',
            'Electronic Ticket' => 'Elektronisches Ticket',
            'Croatia Airlines Internet Sales' => 'Internetverkauf',
        ],
        "hr" => [
    ...
    public function parsePDF(Email $email, $text): void
    {
        ...
        if (preg_match("/{$this->opt($this->t('Payment information'))}\n[ ]*(\d[\d,.]*)[ ]*([A-Z]{3})\s*{$this->opt($this->t('Total for all travellers'))}/", $text, $m)) {
            $f->price()
    ...
    {
        return preg_match('/[.@]croatiaairlines\.hr$/i', $from) > 0;
    }

    public function detectEmailByHeaders(array $headers)
    {
        if ((!array_key_exists('from', $headers) || $this->detectEmailFromProvider(rtrim($headers['from'], '> ')) !== true)
            && (!array_key_exists('subject', $headers) || strpos($headers['subject'], 'Croatia Airlines') === false)
        ) {
            return false;
        }
