...
class Parser extends \TAccountChecker
{
    ...
    private const CONFIGS = [
        ...
        //        // mac - если сильно будет падать рейт  old
        'chrome-94-mac' => [
            'browser-family'  => \SeleniumFinderRequest::BROWSER_CHROME,
...
//        $this->setProxyGoProxies(null, $targeting, null, null, 'https://www.delta.com');
    ...
    public function ParseCalendar(array $fields)
    {
        ...
        return ['fares' => $this->parseCalendarJson($fields)];
    }
...
//        $this->sendNotification("check cabin data: {$cabin} // DM");
    ...
    private function parseCalendarJson($fields)
    {
        $this->logger->notice(__METHOD__);
        $result = [];

        if (isset($this->calendar['errors'])) {
            if ($this->calendar['errors'][0]['message'] == 'RetailOfferError') {
                $this->SetWarning('(Calendar) ' . $this->calendar['errors'][0]['extensions']['errors']['message']);

                return [];
            }
        }

        if (empty($this->calendar['data']) && empty($this->responses['data'])) {
            $this->logger->warning("No has data");

            throw new \CheckRetryNeededException(5, 0);
        }

        foreach ($this->calendar['data']['gqlSearchOffers']['gqlOffersSets'] as $data) {
            foreach ($data['offers'] as $offer) {
                ...
                $result[] = [
                    ...
                    'payments'    => [
                        'currency' => $this->calendar['data']['gqlSearchOffers']['offerDataList']['pricingOptions'][0]['pricingOptionDetail']['currencyCode'],
    ...
    private function jmLogicWithBookingCodes($brandId): ?string
    {
        ...
        switch ($brandId) {
            ...
            case 'FIRST':
                $cabin = 'business';  // point.me was told to remove firstClass
...
//                break;
    ...
    private function selenium($fields)
    {
        ...
        try {
            ...
            if ($this->config == 'chrome-94-mac' || $this->config == 'firefox-playwright-101-mac' || $this->config == 'chrome-puppeteer-103') {
                $selenium->http->setUserAgent(null);
                $this->seleniumOptions->userAgent = null;
                $selenium->seleniumRequest->setOs("mac");
            } elseif ($this->config == 'firefox-playwright-101') {
                $selenium->http->setUserAgent(null);
                $this->seleniumOptions->userAgent = null;
            ...
            $this->responses = $this->getDataAjax($selenium, $fields);

            if ($fields['ParseCalendar']) {
                sleep(5);
                $this->calendar = $this->getCalendarAjax($selenium, $fields);
            }
    ...
    private function clickElements($selenium, $fields)
    {
        ...
        if ($btn = $selenium->waitForElement(\WebDriverBy::xpath('//button[contains(.," - English ")]'), 5)) {
            $btn->click();
        }
        $inputs = [
    ...
    private function getCalendarAjax($selenium, $fields, $isRetry = false)
    {
        $pageNum = 1;
        $responses = [];
