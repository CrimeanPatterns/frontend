...
class DeltaExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    ...
    {
        $loginButton = $tab->evaluate('//a[contains(normalize-space(), "VIEW YOUR BENEFITS") and contains(@href, "skymiles/medallion-program")] | //div[@class="login-button"] | //button[normalize-space()="Log in"]', EvaluateOptions::new()->nonEmptyString());
    ...
    {
        $buttonLogIn = $tab->evaluate("//button[normalize-space()='Log in']", EvaluateOptions::new()->allowNull(true));

        if ($buttonLogIn) {
            $buttonLogIn->click();
        }

        $login = $tab->evaluate('//input[contains(@aria-label, "SkyMiles Number Or Username")]');
        $login->setValue($credentials->getLogin());

        $password = $tab->evaluate('//input[contains(@aria-label, "Password")]');
        $password->setValue($credentials->getPassword());

        $tab->evaluate('//button[contains(@data-cy, "button-Log")]')->click();

        $errorOrTitle = $tab->evaluate('//div[@class="personalProfileSelectorScreen"] | //p[@class="ng-star-inserted"] | //div[contains(@class, "pax-details")]', EvaluateOptions::new()->nonEmptyString());
