...
{
    public $mailFiles = "delta/statements/it-418129514.eml, delta/statements/it-925035426.eml";
    public $lang = 'en';
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        $currentMQDs = $this->http->FindSingleNode("//text()[contains(normalize-space(), 'MQDs to')]/preceding::text()[normalize-space()='MQDs']/preceding::text()[normalize-space()][1]", null, true, "/^(\D{1,3}\s*[\d\.\,]+)$/");

        if ($currentMQDs !== null) {
            $st->addProperty('MedallionDollarsYTD', $currentMQDs);
        }

        $MQDsToNextLevel = $this->http->FindSingleNode("//text()[contains(normalize-space(), 'MQDs to')]", null, true, "/^(\D{1,3}\s*[\d\.\,]+)\s*{$this->opt($this->t('MQDs to'))}/");

        if ($currentMQDs !== null) {
            $st->addProperty('MQDsToNL', $MQDsToNextLevel);
        }

        $MQDsPending = $this->http->FindSingleNode("//text()[contains(normalize-space(), 'MQDs Pending')]", null, true, "/^(\D{1,3}\s*[\d\.\,]+)\s*{$this->opt($this->t('MQDs Pending'))}/");

        if ($MQDsPending !== null) {
            $st->addProperty('MQDsPending', $MQDsPending);
        }

        $millionMiles = $this->http->FindSingleNode("//text()[normalize-space()='MILLION MILER STATUS'][1]/following::text()[normalize-space()][1]", null, true, "/^\D*([\d\,\.]+)\s*(?:{$this->opt($this->t('MILLION MILER STATUS'))})?$/");

        if ($millionMiles !== null) {
            $st->addProperty('MillionMiles', $millionMiles);
        }

        $drinkVouchers = $this->http->FindSingleNode("//text()[normalize-space()='Drink Vouchers']/preceding::text()[normalize-space()][1][not(contains(normalize-space(), 'Redeem'))]", null, true, "/^(\d+)$/");

        if ($drinkVouchers !== null) {
            $st->addProperty('DrinkVouchers', $drinkVouchers);
        }

        $certificatesANDeCredits = $this->http->FindSingleNode("//text()[normalize-space()='Certificates & eCredits']/preceding::text()[normalize-space()][1][not(contains(normalize-space(), 'Redeem'))]", null, true, "/^(\d+)$/");

        if ($certificatesANDeCredits !== null) {
            $st->addProperty('CertificatesANDeCredits', $certificatesANDeCredits);
        }

        $clubVisitsAvailable = $this->http->FindSingleNode("//text()[contains(normalize-space(), 'Delta Sky Club Visits available')]/preceding::text()[normalize-space()][1]", null, true, "/^(\d+)\s*of\s*\d+$/");

        if ($clubVisitsAvailable !== null) {
            $st->addProperty('DeltaSkyClubVisitsAvailable', $clubVisitsAvailable);
        }
