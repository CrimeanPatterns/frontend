...
class LastMinuteInformation extends \TAccountChecker
{
    ...
    public function parseHtml(Email $email): void
    {
        ...
        $confno = $this->getConfNo($this->http);

        if (!$confno
            && !empty($url = $this->http->FindSingleNode("descendant::a[{$this->starts(['View as a web page', 'VIEW AS A WEB PAGE'])} and normalize-space(@href)][1]/@href"))
        ) {
            $this->http2 = clone $this->http;
            $this->http2->GetURL($url);
            $this->logger->debug('Parsing confNumber by URL.');
            $confno = $this->getConfNo($this->http2);
        }
        ...
        $xpath = $preXpath . "text()[{$this->eq("FLIGHT/DATE")}]/ancestor::tr[2]/following-sibling::tr[.//td[3] and contains(.,':')]//td[3][not(contains(translate(@style,' ',''),'text-decoration:line-through'))]/..";
        $nodes = $this->http->XPath->query($xpath);

        if ($nodes->length === 0) {
            $xpath = $preXpath . "text()[{$this->eq("DEPART")}]/ancestor::tr[contains(.,'ARRIVE')][1]/following-sibling::tr[.//td[3] and contains(.,':') and not(contains(normalize-space(),'Upgrade Requested'))]//td[3]/..";
            $nodes = $this->http->XPath->query($xpath);
        }
        $lastDate = null;
        ...
        foreach ($nodes as $root) {
            ...
            if ($this->http->XPath->query(".//*[self::td or self::span][contains(translate(@style,' ',''),'text-decoration:line-through') and string-length(normalize-space())>10]", $root)->length > 1) {
                continue;
            }
            ...
            $date = $this->normalizeDate($this->http->FindSingleNode("*[normalize-space()][1]/descendant::text()[normalize-space()][2]", $root, true, '/^(?:([A-Z][a-z]{2}[,]\s[A-Z]\S+\s+\d+)|([A-Z][a-z]{2}[,]\s\d{2}[A-Z]{3}))/'));
            ...
            if ($this->http->XPath->query("*[normalize-space()][1]/descendant::tr[not(.//tr[normalize-space()]) and normalize-space()]", $root)->length > 1) {
                // it-10106309.eml
                $cell1 = implode("\n", $this->http->FindNodes("*[normalize-space()][1]/descendant::text()[normalize-space()]", $root));
            } else {
                // it-48699058.eml
                $cell1Html = $this->http->FindHTMLByXpath('*[normalize-space()][1]', null, $root);
            ...
            $node = $this->http->FindSingleNode(
                "*[normalize-space()][3]/descendant::text()[normalize-space()][3][starts-with(normalize-space(),'**')]/ancestor::td[1]",
                $root,
                false,
                "/\*{2}\s*(.+)/");
            ...
            // TODO: fix parsing seats for it-895293728.eml
            ...
            if (empty($seats)) {
                ...
                $seats = array_filter(explode(", ", join(", ",
                    $this->http->FindNodes("//text()[{$this->eq("SEAT")}]/ancestor::tr[{$this->starts('FLIGHT')}][1]/following-sibling::tr[ .//td[2] ][*[normalize-space()][1][{$this->ends(' ' . $s->getFlightNumber())} or {$this->ends(' ' . $s->getFlightNumber() . '*')}]]/*[normalize-space()][2]"))));
            }
    ...
    private function getConfNo(\HttpBrowser $http): ?string
    {
        $confno = $this->re("/^([A-Z\d]{5,})(?:\s*\[.*\]|$)/", $this->nextText($this->t("Confirmation #:"), null, $http));

        if (empty($confno)) {
            $confno = $this->re("/^(?:Error!\s*Filename\s*not\s*specified.)?([A-Z\d]{5,})(?:\s*\[.*\]|$)/", str_replace(" ", '',
                $http->FindSingleNode("(.//text()[normalize-space()='Your Trip Confirmation #:' or normalize-space()='Delta Confirmation #:'])[1]/following::*[normalize-space(.)!=''][1]")));
        }

        if (empty($confno)) {
            $confno = $http->FindSingleNode("descendant::a[{$this->contains(['?confirmationNo=', '&confirmationNo='], '@href')}][1]/@href", null, true, "/confirmationNo=([A-Z\d]{5,8})(?:[&#]|$)/")
                ?? $http->FindSingleNode("descendant::a[{$this->contains(['%3FconfirmationNo%3D', '%26confirmationNo%3D'], '@href')}][1]/@href", null, true, "/confirmationNo%3D([A-Z\d]{5,8})(?:%26|#|$)/");
        }

        if (empty($confno)) {
            $confno = $http->FindSingleNode("descendant::a[{$this->contains(['?recordLocator=', '&recordLocator='], '@href')}][1]/@href", null, true, "/recordLocator=([A-Z\d]{5,8})(?:[&#]|$)/")
                ?? $http->FindSingleNode("descendant::a[{$this->contains(['%3FrecordLocator%3D', '%26recordLocator%3D'], '@href')}][1]/@href", null, true, "/recordLocator%3D([A-Z\d]{5,8})(?:%26|#|$)/");
        }

        if (empty($confno)) {
            $confno = $http->FindSingleNode("descendant::a[{$this->contains(['DL_REC_LOC='], '@href')}][1]/@href", null, true, "/DL_REC_LOC=([A-Z\d]{5,8})(?:[&#]|$)/")
                ?? $http->FindSingleNode("descendant::a[{$this->contains(['DL_REC_LOC%3D'], '@href')}][1]/@href", null, true, "/DL_REC_LOC%3D([A-Z\d]{5,8})(?:%26|#|$)/");
        }

        if (empty($confno)) {
            $confno = $http->FindSingleNode("//text()[normalize-space()='Your upcoming flight, Trip Confirmation']/following::*[normalize-space(.)!=''][1]", null, true, "/^\s*#([A-Z\d]{5,7})\s*$/");
        }

        if (empty($confno)) {
            $confno = $http->FindSingleNode("(//text()[contains(., 'Trip Confirmation')])[1]/ancestor::*[count(.//text()) > 1][1]", null, true, "/\bTrip Confirmation\s*#([A-Z\d]{5,7})\b/");
        }

        if (empty($confno)) {
            $confno = $http->FindSingleNode('(//text()[starts-with(normalize-space(), "CONFIRMATION #:")])[1]/ancestor::td[1]', null, true, '/CONFIRMATION\s*#\s*:\s*([A-Z\d]{6})\s*$/');
        }
...
//        }
    ...
    private function nextText($field, ?\DOMNode $root = null, ?\HttpBrowser $http = null): ?string
    {
        if ($http === null) {
            $http = $this->http;
        }
        return $http->FindSingleNode("descendant::text()[{$this->eq($field)}][1]/following::text()[normalize-space()][1]", $root);
    }
