...
class ETicket extends \TAccountChecker
{
    ...
    {
        return $this->http->XPath->query("//text()[{$this->contains(['FLYBONDI', 'SERVICIO AEREO REGIONAL REGAIR CIA . LTDA.'])}]")->length > 0
    ...
    {
        if (stripos($headers['from'], '@kiusys.com') !== false) {
            $this->providerCode = 'kiu';

            return true;
        }

        if (stripos($headers['from'], '@FLYBONDI.COM') !== false) {
            $this->providerCode = 'bondi';
