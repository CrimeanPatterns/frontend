...
{
    public $mailFiles = "viking/it-148879890-oceania.eml, viking/it-153210529-oceania.eml, viking/it-177809857-oceania.eml, viking/it-20548342.eml, viking/it-213750628.eml, viking/it-21582261.eml, viking/it-43242805.eml, viking/it-564086959.eml, viking/it-805523164.eml, viking/it-895510650.eml";

    public static $dictionary = [
        "en" => [
            'Oceania Club Number' => ['Oceania Club Number', 'Seven Seas Society#', 'Reward Cruises'],
        ],
    ];
    ...
    public function parsePdf(Email $email, string $text): void
    {
        ...
        if (preg_match("/Cruise Name/", $text) > 0 || preg_match("/Embarkation Date/", $text) > 0) {
            ...
            //$this->logger->debug($itineraryText);
            ...
            if (preg_match("/\n[ ]*Date[ ]+Port Description[ ]+Arrive[ ]+Depart\b.*\n+(?<body>.+[\s\S]*?(?:\n+----newPage\n *[[:alpha:]]{3} \d{1,2} {2,}[\s\S]+?)*)\n\n/m",
                ...
                foreach ($tablePages as $i => $page) {
                    if (preg_match("/^( {0,10}[[:alpha:]]{3} \d{1,2} {1,}.+? {2})[[:alpha:]]{3} \d{1,2} {2,}/", $page, $m)) {
                ...
                foreach ($tables as $tableKey => $table) {
                    $table = preg_replace("/(\w+\s*\d+\s*\D+)\n\s+(\s\d{1,2}:\d{2}\s*A?P?M\s*\d{1,2}:\d{2}\s*A?P?M)/", "$1$2", $table);
                    $table = preg_replace("/^(\w+\s*\d+\s+.+)\n(\s+\d{1,2}:\d{2}\s*A?P?M)$/m", "$1$2", $table);
                    $tableRows = explode("\n", trim($table));
                    ...
                    foreach (array_filter($tableRows) as $key => $rowText) {
                        ...
                        if (preg_match("/^[ ]*(?<date>[[:alpha:]]{3} \d{2})[ ]+(?<port>\S.*?\S)(?: *(?<time1>{$this->patterns['time2']}))?(?:[ ]+(?<time2>{$this->patterns['time2']}))?$/u",
                            ...
                            if (preg_match("/^(Cruising .+|Cross The International Date Line|(?:Panama|Suez|Kiel) Canal(?: Daylight)? Transit|Crossing The)/i", $m['port']) > 0
                                && !preg_match("/\s\d{1,2}:\d{2}/", $rowText)
                            ) {
                                continue;
                            }
                ...
                $textCruise = $this->re("/^([ ]+[A-Z]{3}\s+\w+\s*\d+,\s*\d{4}\s+Embark In.+Disembark In\D{5,20}(?:\s+\d{1,2}:\d{2}\s*A?P?M[ ]{3,}\d{1,2}:\d{2}\s*A?P?M|$))\n/msu", $text);
                $segmentsRows = explode("\n", $textCruise);

                foreach ($segmentsRows as $rowText) {
                    if (preg_match("/(Cruising .+|Cross The International Date Line|(?:Panama|Suez) Canal(?: Daylight)? Transit|Crossing The|Penthouse Veranda|Sail The Bering Sea|Scenic Sailing|Sail The |Crossing International Dateline)/i", $rowText)
                        && !preg_match("/\d{1,2}:\d{2}/", $rowText)
                    ) {
                        continue;
                    }
                    ...
                    if (preg_match("/[ ]{5,}(?<date>\w+\s*\d+\,\s*\d{4})[ ]{5,}Embark In\s+(?<port>.+)\b[ ]{2,}(?<timeStart>[\d\:]+\s*A?P?M)/u", $rowText, $m)) {
                        $s = $c->addSegment();
                        $s->setName(preg_replace('/(?: on Viking Vela|\d{1,2}:\d{2}\s*A?P?M)/', '', $m['port']));
                    ...
                    } elseif (preg_match("/[ ]{5,}(?<date>\w+\s*\d+\,\s*\d{4})[ ]{5,}(?<port>.+)\b[ ]{2,}(?<time>[\d\:]+\s*A?P?M)$/", $rowText, $match)) {
                        //it-895510650.eml
                        if ($this->shortSegment === false) {
                            $s = $c->addSegment();
                            $s->setName($match['port']);
                            $s->setAshore($this->normalizeDate($match['date'] . ', ' . $match['time']));
                            $this->shortSegment = true;
                        } else {
                            $s = $c->addSegment();
                            $s->setName($match['port']);
                            $s->setAboard($this->normalizeDate($match['date'] . ', ' . $match['time']));
                            $this->shortSegment = false;
                        }
                    }
                }
            } else {
            ...
            if (stripos($text, 'Guest Cancellation Statement') !== false) {
                $c->general()
                    ->cancelled();
            }
            ...
            $c->general()
                    ->noConfirmation()
                    ->travellers(preg_replace("/^\s*(?:MRS|MR|MS)\s+/u", "", $travellers), true);
//            }
        }
        ...
        // Price
        if (preg_match("/\s+Gross\b.*\n+[ ]*Grand Total[ ]{10,}(?:[$] ?)?(\d[\d,. ]*?)(?:[ ]{2}|\n)/", $text, $m)) {
            if (preg_match_all("/^[ ]*Currency[:\s]+(?-i)([A-Z]{3})$/im", $text, $currencyMatches)) {
                $currency = count(array_unique($currencyMatches[1])) === 1 ? $currencyMatches[1][0] : null;
            } else {
                $currency = 'USD';
            }
