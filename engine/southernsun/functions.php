...
class TAccountCheckerSouthernsun extends TAccountChecker
{
    ...
    {
        // Name
        $this->SetProperty("Name", $this->http->FindSingleNode("//div[@class = 'personal-details']//div[contains(@class, 'title')]"));
        // Card number
        $this->SetProperty("MemberNo", $this->http->FindSingleNode("//div[contains(text(), 'Membership Number')]/following-sibling::div"));
        // Card Status
        $this->SetProperty("TierStatus", $this->http->FindSingleNode("//div[contains(text(), 'Membership Status')]/following-sibling::div/text()[1]"));
        // Card Expiry
        $this->SetProperty("MembershipExpiry", $this->http->FindSingleNode("//div[contains(text(), 'Membership Expiry')]/following-sibling::strong[1]"));
        // Balance - SunRands Balance
        $this->SetBalance($this->http->FindSingleNode("//div[contains(text(), 'SunRands Balance')]/following-sibling::div"));
        // Status Points - Status Points earned in a rolling 12-month period
        $this->SetProperty("StatusPoints", $this->http->FindSingleNode("//span[contains(text(), 'Earned in a rolling 12-month period')]/parent::div/text()[1]"));
    ...
    private function loginSuccessful()
    {
        ...
        if ($this->http->FindSingleNode("//div[@class = 'personal-details']//div[contains(@class, 'title')]")) {
            return true;
        }
