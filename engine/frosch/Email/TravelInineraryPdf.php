...
//            $this->logger->debug('Pdf text = ' . print_r($text, true));
    ...
    public function parseFlight(Flight $f, $text)
    {
        ...
        // Arrival
        if (preg_match("/\n *" . $this->opt($this->t("To:")) . " *(?<code>[A-Z]{3}) *- *(?<name>.+) +" . $this->opt($this->t("Arrive:")) . " *(?<time>\d{1,2}:\d{2}(?: *[ap]m)?)(?<date> +\d{1,2} *\w+)(?<terminal> +.*\b(?:terminal|term|AEROGARE|INTL)\b.*)?\n/i", $text, $m)) {
        ...
        if (preg_match_all("/\n+\s+[[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]]\s+(\d{1,3}[A-Z])\s+/", $text, $m)) {
            foreach ($m[1] as $seat) {
                $pax = $this->re("/\s+([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])\s+$seat/", $text);
...
//        $this->logger->debug('$segments = ' . print_r($segments, true));
        ...
        if (isset($flight)) {
            if (preg_match_all("/\n\s*" . $this->opt($this->t("Ticket Number:")) . " *[A-Z\d]{2}-(\d{8,})/u", $textPdf, $m)) {
                foreach ($m[1] as $ticket) {
                    $pax = $this->re("/[ ]{2,}([[:alpha:]][-.\/\'’[:alpha:] ]*[[:alpha:]])\n{$this->opt($this->t("Ticket Number:"))}\s*[A-Z]+\-$ticket/ui", $textPdf);
