...
//            $email->setProviderCode('marriottvacationclub');
    ...
    private function parseEmail(Email $email)
    {
        ...
        if (!empty($account)) {
            ...
            $accounts = array_filter($this->http->FindNodes("//tr[{$this->eq($this->t('Account'))}]/preceding-sibling::tr[normalize-space()][1]", null, "/^[Xx]*\d{4,}$/"));
        }

        foreach (array_unique($accounts) as $accNumber) {
            $h->program()->account($accNumber, preg_match("/^[Xx].+$/", $accNumber) > 0);
        }
