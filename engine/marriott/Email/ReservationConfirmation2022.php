...
class ReservationConfirmation2022 extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            'PHONE:'        => ['PHONE:', 'MAIN:'],
            '# Of Guests:'  => ['# Of Guests:', '# of Guests:'],
            'usage details' => ['usage details', 'USAGE DETAILS'],
        ],
    ];
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        if ($this->http->XPath->query("//text()[contains(normalize-space(), 'Marriott')]")->length > 0) {
            return $this->http->XPath->query("//text()[{$this->contains($this->t('STAY PREFERENCES'))}]")->length > 0
                && $this->http->XPath->query("//text()[{$this->contains($this->t('GUEST PROFILE'))}]")->length > 0
                && $this->http->XPath->query("//text()[{$this->contains($this->t('usage details'))}]")->length > 0;
        }
    ...
    public function ParseHotels(Email $email): void
    {
        ...
        $h->hotel()
            ->name($this->http->FindSingleNode("//text()[{$this->contains($this->t('PHONE:'))}]/preceding::text()[normalize-space()][2]"))
            ->address($this->http->FindSingleNode("//text()[{$this->contains($this->t('PHONE:'))}]/preceding::text()[normalize-space()][1]"))
            ->phone($this->http->FindSingleNode("//text()[{$this->contains($this->t('PHONE:'))}]/following::text()[normalize-space()][1]", null, true, "/^{$patterns['phone']}/"))
        ;
        ...
        $h->booked()->guests($this->http->FindSingleNode("//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1][{$this->eq($this->t('# Of Guests:'))}] ]/*[normalize-space()][2]", null, true, '/^(\d{1,3})\b/'));
