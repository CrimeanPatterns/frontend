...
class TAccountCheckerMarriott extends TAccountChecker
{
    ...
    public function InitBrowser()
    {
        ...
        /*if ($this->attempt == 1) {
            $array = ['us', 'ca', 'br', 'cl', 'mx', 'gb', 'no', 'fi', 'ee', 'dk', 'cz', 'fr', 'gr', 'it', 'es', 'de', 'pt', 'be', 'bl', 'ru'];
            $targeting = $array[array_rand($array)];
            $this->setProxyGoProxies(null, $targeting);
        } else {
            $array = ['uk', 'no', 'fi', 'ee', 'dk', 'cz', 'fr', 'gr', 'it', 'es', 'de', 'pt', 'be', 'bl'];
            $targeting = $array[array_rand($array)];
...
//                $mover->click();
            ...
            if (
                ...
                $buttonXpath = "//form[@name = 'directLoginForm']//button[contains(., 'Sign In')]";
            } else {
                ...
                $buttonXpath = "//form[@name = 'signInForm']//button[contains(text(), 'Sign In')] | //div[@data-testid=\"account-SignIn\" or contains(@class, 'StyledSignInContainerDiv')]//button[contains(., 'Sign In')]";
            }
...
//                $mover->moveToElement($button);
    ...
    public function seleniumAuthV2()
    {
        {
            $this->logger->notice(__METHOD__);
            $selenium = clone $this;
            ...
            try {
                ...
                if ($this->attempt == 0) {
                    $selenium->useChromePuppeteer(SeleniumFinderRequest::CHROME_PUPPETEER_104);
                }
                if ($this->attempt == 1) {
                ...
                $selenium->driver->executeScript("window.scrollBy(0, 200)");
                sleep(random_int(0,3));
                $selenium->driver->executeScript("window.scrollBy(0, -100)");
                sleep(random_int(0,2));
                // save page to logs
                //$this->savePageToLogs($selenium);
                $selenium->driver->executeScript('var c = document.querySelector("button[aria-label=\'Sign In or Join\']"); if (c) c.click();');


                /*$loginButton = $selenium->waitForElement(WebDriverBy::xpath('//span[contains(text(),"Sign In or Join")]'),
                    15);
                if (!$loginButton) {
                    return $this->checkErrors();
                }
                ...
                $loginInput = $selenium->waitForElement(WebDriverBy::xpath('//input[@name="input-text-Email or Member Number"]'), 0);
                $passwordInput = $selenium->waitForElement(WebDriverBy::xpath('//input[@name="input-text-Password"]'), 0);
                $buttonInput = $selenium->waitForElement(WebDriverBy::xpath('//button[@id="signInButton"]'), 0);
                ...
                try {
                    sleep(random_int(1,3));
                    $selenium->driver->executeScript('var c = document.querySelector("button#signInButton"); if (c) c.click();');
                ...
                $this->waitFor(function () use (&$signOut, $selenium) {
                    ...
                    if (in_array($this->seleniumURL, [
                        ...
                        || strstr($this->seleniumURL,
                            'https://www.marriott.com/loyalty/myAccount/send-otp-challenge.mi')
                    ) {
                        $selenium->waitForElement(WebDriverBy::xpath("//h1[contains(text(), 'Confirm your identity')] | //h2[contains(text(), 'Confirm Your Identity')]"),
                            20);

                        if ($sendCodeChoice =
                            $selenium->waitForElement(WebDriverBy::xpath('//label[@for="email"]'),
                                3)
                            ?? $selenium->waitForElement(WebDriverBy::xpath('//label[@for="number_MT"]'),
                            0)
                        ) {
                            $this->savePageToLogs($selenium);
                            $sendCodeChoice->click();
                            $sendCode = $selenium->waitForElement(WebDriverBy::xpath('//button[@data-testid="send-code-btn"]'), 0);
                            $sendCode->click();
                            ...
                            | //p[contains(text(), 'We sent a verification code via ')]
                            | //div[contains(text(), 'We sent a six-digit code via email to')]
                            "), 10);
                            $this->savePageToLogs($selenium);
                            ...
                            if (!isset($question)) {
                                ...
                                if ($message = $this->http->FindSingleNode('//div[contains(text(), "There is no valid contact method on your account to send a verification code.")]')) {
                                    throw new CheckException($message . " For assistance, contact the customer support center.",
                        ...
                        if ($selenium->waitForElement(WebDriverBy::xpath('//div[contains(@class, "skeleton-loader")]'),
                            0)) {
                            $this->logger->error("site to long loading");
                            return false;
                        }
                ...
                if ($this->seleniumURL == 'https://www.marriott.com/default.mi') {
                    ...
                    $signOut = $selenium->waitForElement(WebDriverBy::xpath(self::LOGOUT_XPATH), 5, false);
                    // save page to logs
                    $this->savePageToLogs($selenium);
                }

                if (!$signOut
                    && !stristr($this->seleniumURL, '/loyalty/myAccount/send-otp-challenge.mi')
                ...
                foreach ($cookies as $aCookie) {
                    $this->http->setCookie($aCookie['name'], $aCookie['value'], $aCookie['domain'], $aCookie['path'],
                        $aCookie['expiry'] ?? null);
                }
                ...
                if ($retry && ConfigValue(CONFIG_SITE_STATE) != SITE_STATE_DEBUG) {
                    ...
                    throw new CheckRetryNeededException(2, 7, self::PROVIDER_ERROR_MSG, ACCOUNT_PROVIDER_ERROR);
                }
            }
    ...
    public function seleniumAuth()
    {
        ...
        try {
            ...
            if (in_array($this->AccountFields["Login"], ["veresch@gmail.com", "veresch80@yahoo.com"])) {
                $config = 12;
            }
...
//            $selenium->disableImages();
            ...
            if (
                ...
                throw new CheckRetryNeededException(3, 7, self::PROVIDER_ERROR_MSG, ACCOUNT_PROVIDER_ERROR);
            }
        }
    ...
    public function parseQuestion()
    {
        ...
        if (!$this->http->ParseForm("otpActionForm") || (!isset($email) && !isset($phone))) {
            ...
            $metadata = "UjBrtT8UzWQ4hlFVBXlykbvVzzGyrgoq223fANyvTkfsOhJRUaf2t%2BZNVOWMjFbLOXXWqVwOHNGBXAoLAnO7qI8PvoJ6YN9xRzRYUXn0GOrRWADe7oS3ghApQGX%2F81RNUrFD%2BD5U%2BXG4KpI14gRwZQ%3D%3D%2CEINQbGdrufWV6TP3NDgY3Q%3D%3D";// TODO: hard code
...
//                $proxy = "direct:" . $selenium->http->getProxyAddress();
                $selenium->setProxyBrightData(null, "static", "ca");
                $proxy = "lumunati-ca:" . $selenium->http->getProxyAddress();
...
//                $proxy = "nn-us:" . $selenium->http->getProxyAddress();
...
//                $proxy = "lumunati-fi:" . $selenium->http->getProxyAddress();
                ...
                $proxy = "go-ca:" . $selenium->http->getProxyAddress();
