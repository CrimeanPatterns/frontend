...
class SephoraExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    ...
    {
        $userData = $tab->getFromLocalStorage('UserData');
        return json_decode($userData)->data->profile->profileId ?? '';
    }
    ...
    {
        $accessToken = json_decode($tab->getFromLocalStorage('accessToken'))->data ?? null;
        $userData = json_decode($tab->getFromLocalStorage('UserData'))->data ?? null;
        $profileId = $userData->profile->profileId ?? null;
        $profileId = $userData->profile->profileId ?? null;
        $preferredZipCode = $userData->profile->preferredZipCode ?? null;
        $biAccountId = $userData->profile->beautyInsiderAccount->biAccountId ?? null;
        if (!isset($accessToken, $profileId, $preferredZipCode, $biAccountId)) {
            $this->logger->error('ids not found');
            return;
        }

        $options = [
            'method' => 'get',
            'headers' => $this->headers + ['seph-access-token' => $accessToken,],
        ];
        $data = $tab->fetch("https://www.sephora.com/gapi/users/profiles/$profileId/current/full?&includeApis=profile,loves,shoppingList,smsStatus,tax,beautyPreference,biPoints,targetedPromotion,personalizedPromotions,availableRRCCoupons,smsStatus,segments&skipApis=targetersResult&cb=".time()."&biAccountId=$biAccountId&storeId=1208",
            $options)->body;
        ...
        $options = [
            'method' => 'get',
            'headers' => $this->headers + ['seph-access-token' => $accessToken, 'x-requested-source' => 'web'],
        ];
        $body = $tab->fetch("https://www.sephora.com/gway/v1/lxs/$biAccountId/points?source=profile&redeemedPoints=0&netBeautyBankPointsAvailable={$data->biPoints->beautyBankPoints}",
            $options)->body;
    ...
    public function parseOld(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
