...
//        $this->saveResponse();
    ...
    public function ProcessStep($step)
    {
        ...
        $this->sendNotification("ProcessStep -> $step // MI");
...
//                $this->driver->executeScript("$('a:contains(\"View/Redeem ThankYou® Points\")').attr('target', '_self').get(0).click()");
        ...
        if ($viewMoreDetails = $this->waitForElement(WebDriverBy::xpath('//a[contains(text(), "View More Details")]'), 0)) {
            $this->logger->info("View More Details", ['Header' => 3]);
        ...
        if (isset($this->Properties['SubAccounts']) && count($this->Properties['SubAccounts']) > 0) {
            ...
            for ($i = 0; $i < $countSubAccounts; $i++) {
                ...
                $arrayBalances[] = floatval(str_replace([',', '.'], ['', ','], $this->Properties['SubAccounts'][$i]['Balance']));
                $this->Properties['SubAccounts'][$i]['BalanceInTotalSum'] = true;
            }
            $arrayBalances = array_unique($arrayBalances);
            if (count($arrayBalances) > 2) {
                //$this->sendNotification('check balance // MI');
                $this->logger->info(var_export($arrayBalances, true), ['pre' => true]);
                $subAccountBalance = 0;
                for ($i = 0; $i < $countSubAccounts; $i++) {
                    if (
                        !isset($this->Properties['SubAccounts'][$i]['Balance'])
                ...
                $this->SetBalance(array_sum($arrayBalances));
            }
        }// if (isset($this->Properties['SubAccounts']) && count($this->Properties['SubAccounts']) > 0)
...
//        $this->logger->debug(var_export($result, true), ['pre' => true]);
    ...
    public function parseSubAccHistoryFromMainPage($code, $startDate, $allTransactionsList, $prestigeCard, $account)
    {
        ...
        foreach ($allTransactionsList as $allTransactionsInfo) {
            ...
            // refs #17468
            if (
                stristr($allTransactionsInfo->columns[1]->activityColumn[0] ?? $allTransactionsInfo->transactionColumns[1]->displayValue ?? $allTransactionsInfo->transactionColumns[2]->displayValue, 'cruise')
                && stristr($allTransactionsInfo->columns[1]->activityColumn[0] ?? $allTransactionsInfo->transactionColumns[1]->displayValue ?? $allTransactionsInfo->transactionColumns[2]->displayValue, 'boat')
            ) {
                if (isset($allTransactionsInfo->columns[0]->activityColumn[0])) {
                    $this->sendNotification("citybank - refs #17468. {$allTransactionsInfo->columns[0]->activityColumn[0]}: Cruise may be was found");
                } else {
            ...
            if ($prestigeCard
                && (
                    strstr($allTransactionsInfo->columns[1]->activityColumn[0] ?? $allTransactionsInfo->transactionColumns[1]->displayValue ?? $allTransactionsInfo->transactionColumns[2]->displayValue, 'Congrats! Citi Air Travel Credit')
                    || Html::cleanXMLValue($allTransactionsInfo->columns[1]->activityColumn[0] ?? $allTransactionsInfo->transactionColumns[1]->displayValue ?? $allTransactionsInfo->transactionColumns[2]->displayValue) == 'Travel Credit'// refs #17620
                )
            ) {
                $airTravelCreditTransactions[] = array_merge(
                    [
                        "Date"             => $this->http->FindPreg("/([^\|]+)/", false, $allTransactionsInfo->columns[0]->activityColumn[0] ?? $allTransactionsInfo->transactionColumns[0]->actualValue ?? $allTransactionsInfo->transactionColumns[2]->displayValue),
