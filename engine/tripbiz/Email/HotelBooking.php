...
{
    public $mailFiles = "tripbiz/it-230208071.eml, tripbiz/it-620666029.eml, tripbiz/it-922801448.eml, tripbiz/it-923968829.eml";
    ...
    public static $dictionary = [
        "en" => [
            ...
            '[t.ctrip.cn]' => ['[t.ctrip.cn]', '[m.ctrip.com]'],
        ],
        "zh" => [
            'bookingNo'           => ['订单号:', '酒店确认号'],
            ...
            'Limited-time Cancellation' => '限时取消',
            'Room Type'                 => '房型',
            'Price Summary'             => '费用明细',
            '/Room'                     => '/间',
            'Total:'                    => '总价:',
    ...
    private function parseHtml(Email $email): void
    {
        ...
        // Travel Agency
        $bookingNo = null;
        $bookingNumbers = array_filter($this->http->FindNodes("//text()[{$this->eq($this->t('Booking Information'))}]/preceding::text()[{$this->starts($this->t('bookingNo'))}]/following::text()[normalize-space()][1]", null, "/^\s*([A-Z\d]{5,})(?:\s*{$this->opt($this->t('[t.ctrip.cn]'))})?\s*$/"));
        ...
        if (empty($cancellation)) {
            $cancellation = implode("\n", $this->http->FindNodes("(//*[{$this->eq($this->t('Limited-time Cancellation'))}])[1]/following-sibling::*[normalize-space()][position() <= 2]/descendant::tr[not(.//tr)][normalize-space()]", null, "/^\s*(.*?\d{4}.*?)\s*$/"));
        }
        ...
        // replace '酒店优惠券' by english phrase
        $discountText = $this->http->FindSingleNode("//text()[{$this->eq('酒店优惠券')}]/following::text()[normalize-space()][2]");

        if (preg_match('/^\-?(?<currency>[A-Z]{3})[ ]*(?<amount>\d[,.\'\d ]*)$/', $discountText, $m)
            || preg_match('/^\-?(?<amount>\d[,.\'\d ]*?)[ ]*(?<currency>[A-Z]{3})$/', $discountText, $m)
            || preg_match('/^\-?(?<currency>[^\s\d]{1,5}) ?(?<amount>\d[,.\'\d ]*)$/', $discountText, $m)
            || preg_match('/^\s*\-?(?<amount>\d[,.\'\d ]*?) ?(?<currency>[^\s\d]{1,5})\s*$/u', $discountText, $m)
        ) {
            $h->price()
                ->discount(PriceHelper::parse($m['amount'], $this->currency($m['currency'])));
        }
    ...
    private function detectDeadLine(\AwardWallet\Schema\Parser\Common\Hotel $h)
    {
        ...
        if (preg_match("/Before (?<time>\d+:\d+), (?<date>\w+\s+\d+\,\s+\d{4}) Free Cancellation/ui", $cancellationText, $m)) {
            $h->booked()
                ->deadline(strtotime($m['time'], $this->normalizeDate($m['date'])));
        }

        if (preg_match("/(?<date>\d{4}.+?)(?<time>\d+:\d+).+? 免费取消/ui", $cancellationText, $m)) {
            $h->booked()
                ->deadline(strtotime($m['time'], $this->normalizeDate($m['date'])));
        }
    }
