...
{
    public $mailFiles = "tripbiz/it-231406212.eml, tripbiz/it-619473753.eml, tripbiz/it-619474220.eml, tripbiz/it-619484247.eml, tripbiz/it-620347947.eml, tripbiz/it-919039296.eml, tripbiz/it-923543095.eml, tripbiz/it-924228611.eml";
    ...
    public function parseHtml(Email $email)
    {
        ...
        foreach ($segments as $root) {
            ...
            $s->airline()
                ...
                // replace '共享航班，实际承运：' by english phrase
                ->operator($this->http->FindSingleNode("descendant::text()[{$this->starts($this->t('共享航班，实际承运：'))}]", $root, null, "/^\s*{$this->opt($this->t('共享航班，实际承运：'))}\s*(.+?)\s*(?:[A-Z\d][A-Z]|[A-Z][A-Z\d])\d{1,5}\s*/u"), true, true)
                ->carrierName($this->http->FindSingleNode("descendant::text()[{$this->starts($this->t('共享航班，实际承运：'))}]", $root, null, "/^\s*{$this->opt($this->t('共享航班，实际承运：'))}\s*.+?\s*([A-Z\d][A-Z]|[A-Z][A-Z\d])\d{1,5}\s*/u"), true, true)
                ->carrierNumber($this->http->FindSingleNode("descendant::text()[{$this->starts($this->t('共享航班，实际承运：'))}]", $root, null, "/^\s*{$this->opt($this->t('共享航班，实际承运：'))}\s*.+?\s*(?:[A-Z\d][A-Z]|[A-Z][A-Z\d])(\d{1,5})\s*/u"), true, true)
            ;
            ...
            if (preg_match("/^{$s->getAirlineName()}{$s->getFlightNumber()}\s+\|\s+(?<aircraft>.+)\s+\|\s+(?<cabin>.+?)\s*(?:[\d\.]+|$)/", $cabinText, $m)) {
                $s->extra()
                    ->aircraft($m['aircraft'])
                    ->cabin($m['cabin']);
            }
    ...
    private function normalizeDate($str)
    {
        ...
        $in = [
            // 2022年 4月 30日 12:35
            '/^\s*(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日\s*[[:alpha:]\s,]*\s+(\d{1,2}:\d{2})\s*$/u',
