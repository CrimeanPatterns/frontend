...
//    private const REWARDS_PAGE_URL = "https://portal.discover.com/dfs/accounthome/portal?flowExecutionSnapshotId=e1s1&flowStateName=beforeSummary";
    ...
    public function InitBrowser()
    {
        ...
        */
        ...
        //$this->http->setUserAgent('Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:97.0) Gecko/20100101 Firefox/97.0');
    }
    ...
    public function uniqueStateKeys()
    {
        ...
        try {
            ...
            if ($this->attempt == 2) {
                $selenium->useFirefoxPlaywright(SeleniumFinderRequest::FIREFOX_PLAYWRIGHT_103);
            }
...
//            $selenium->useCache();
            ...
            if ($loginFormButton = $selenium->waitForElement(WebDriverBy::xpath('//button[contains(.,"modal dialog")]'), 10)) {
...
//            $passwordInput->sendKeys($this->AccountFields['Pass']);
            ...
            */
            ...
            $message = $selenium->waitForElement(WebDriverBy::xpath("
            ...
            if ($message && strstr($message->getText(), 'Your account cannot currently be accessed.')) {
...
//        $this->http->SetInputValue("ssid", "8a84d652-f4c7-4df1-880d-a438179f33b-".time().date("B"));
    ...
    public function Login()
    {
        ...
        */
        ...
        if ($message =
                $this->http->FindSingleNode("//h2[contains(text(), 'We are unable to allow access into your account at this time.')]")
                ?? $this->http->FindSingleNode("//h4[contains(text(), 'Your Discover account is currently unavailable.')]")
        ) {
            throw new CheckException($message, ACCOUNT_PROVIDER_ERROR);
        }
        /*
...
//                $this->sendNotification("Discover. Code was sent");
        ...
        if ((stristr($question, 'address') || stristr($question, 'mail')) && $this->getWaitForOtc()) {
            $this->sendNotification("refs #25603 - mailbox was found // MI");
        }
    ...
    public function Parse()
    {
        ...
        if ($this->http->currentUrl() == "https://portal.discover.com/enterprise/portal/customeraccountinfo/v1/summary") {
            ...
            foreach ($cardAccounts as $cardAccount) {
                ...
                if (isset($number, $displayName, $balance)) {
                    if (count($cardAccounts) == 1 || $cardAccount->rewardsSummary->earningMiles == true) {
                        $this->logger->notice('We are not collecting a sub-account');
            ...
            foreach ($cardAccounts as $cardAccount) {
                ...
                if (isset($number, $displayName, $balance)) {
//                    if (!in_array($displayName, ['Discover it Card'])) {
//                        $this->sendNotification("Detected Card -> $displayName // MI");
//                    }
                    if ($cardAccount->cardInfo->studentCard ?? false) {
                        $this->sendNotification("#25603 Detected Card -> studentCard // MI");
                    }
                    $this->addDetectedCard([
                        "Code" => 'discover' . $number,
                        "DisplayName" => "$displayName ...$number",
                        "Number" => $number,
                        "CardDescription" => $cardAccount->cardTypeIndicator,
                    ]);
                }
            }
