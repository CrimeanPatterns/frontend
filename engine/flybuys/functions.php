    ...
    public function LoadLoginForm()
    {
        ...
        switch ($this->AccountFields['Login2']) {
            ...
            default:
                ...
                sleep(rand(1, 5));
                $elem = $this->waitForElement(WebDriverBy::xpath('//input[@id = "username"]'), $this->timeout * 2);
                $this->saveResponse();
                ...
                if (!$elem) {
                    ...
                    if (!($elem = $this->waitForElement(WebDriverBy::xpath('//input[@id = "username"]'), $this->timeout * 2))) {
                        $this->saveResponse();
            ...
            case 'Australia': default:
                ...
                try {
                    while ($time < $sleep) {
                        ...
                        || $this->waitForElement(WebDriverBy::xpath('//h2[contains(text(), "Verify your ") or contains(text(), "Verify Your ")]'), 0)) {
                            return $this->processSecurityCheckpoint();
                        }
...
//                $this->http->GetURL("https://www.flybuys.co.nz/myflybuys");
        ...
        case 'Australia': default:
            $this->waitForElement(WebDriverBy::xpath('//div[contains(@class, "desktop-header-menu")]//span[contains(text(), "point") and not(contains(text(), "0 points"))]'), 5);
            $this->saveResponse();
            // Balance - MY POINTS
            $this->SetBalance(
                $this->http->FindSingleNode('//div[contains(@class, "fb-me-account-summary-section")]//h2')
                ?? $this->http->FindSingleNode('//div[contains(@class, "desktop-header-menu")]//span[contains(text(), "point")]')
            );
            // Name
