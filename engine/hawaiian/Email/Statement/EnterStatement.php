...
{
    public $mailFiles = "hawaiian/it-540414400.eml, hawaiian/statements/it-116370919.eml, hawaiian/statements/it-116491836.eml, hawaiian/statements/it-61935502.eml, hawaiian/statements/it-74470896.eml, hawaiian/statements/it-74600173.eml, hawaiian/statements/it-914869012.eml, hawaiian/statements/it-916245106.eml, hawaiian/statements/it-916381999.eml, hawaiian/statements/it-918010002.eml, hawaiian/statements/it-918598282.eml";
    private $lang = '';
    private $reFrom = ['hawaiianairlines.'];
    private $reProvider = ['Hawaiian Airlines'];
    private $reSubject = [
        'Enter our #',
        'off just for you',
        'HawaiianMiles Statement',
        "let's celebrate our year together",
        "earn additional miles when you shop or dine HawaiianMiles Buy & Fly",
        "Earn Bonus HawaiianMiles with Champion",
    ];
    private $reBody = [
        'ja' => [
            'アカウント',
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        if (empty($text)) {
            $text = $this->http->FindSingleNode("//text()[{$this->contains($this->t('| Account'))}]/ancestor::td[1]");
        }
        ...
        // it-916381999
        $balance = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Current miles balance'))}]/ancestor::td[1]/following-sibling::td[normalize-space()][1]", null, false, '/^([0-9\,\.]+)$/u');
        ...
        if (!empty($text)) {
            ...
            if (preg_match("/^\s*(?<name>.+?)\s*\|\s*(?:{$this->opt($this->t(' Acct'))}| Account)\s+(?<number>[\w\-]+)/u", $text, $m)) {
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        if ($this->http->XPath->query("//text()[normalize-space()='HawaiianMiles number']")->length > 0
            ...
            || $this->http->XPath->query("//div[contains(normalize-space(), 'EXCLUSIVE OFFER')]")->length > 0
            || $this->http->XPath->query("//text()[normalize-space()='to view your mileage statement.']")->length > 0
            || $this->http->XPath->query("//text()[normalize-space()='You went the distance with the Hawaiian Airlines']")->length > 0
            || $this->http->XPath->query("//text()[normalize-space()='You went the distance with the Hawaiian Airlines']")->length > 0
