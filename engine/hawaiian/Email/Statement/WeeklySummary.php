...
class WeeklySummary extends \TAccountChecker
{
	public $mailFiles = "hawaiian/statements/it-917998847.eml";
	private $lang = '';
    private $reFrom = ['partners.hawaiianairlines.com'];
    private $reProvider = ['HawaiianMiles'];
    private $reSubject = [
        'Your weekly bonus HawaiianMiles summary',
    ];
    ...
    private static $dictionary = [
        'en' => [
            ...
            'detectBody' => ['Your weekly summary'],
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        $name = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Congratulations'))}]", null, false, "/^{$this->opt($this->t('Congratulations'))}[\,\s]+([[:alpha:]][-.\'[:alpha:] ]*[[:alpha:]])(?:[\!\,\.\s]|$)/u");
        $number = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Account ending:'))}]", null, false, "/^{$this->opt($this->t('Account ending:'))}[ ]*([\d\-]+)$/u");
        $mail = $this->http->FindSingleNode("//text()[{$this->eq($this->t('This email has been sent to you at'))}]/following::text()[normalize-space()][1]", null, false, "/^(.+\@.+\.[\D\d]+)$/u");

        $email->setUserEmail($mail);
