    ...
    private function parseEmail(Email $email): void
    {
        ...
        if (isset($nodes)) {
            foreach ($nodes as $root) {
                ...
                $airlineText = $this->http->FindSingleNode("descendant::tr[normalize-space() and not(.//tr)][position()<3][{$this->contains($this->t(' to '))}][contains(translate(.,'0123456789', 'dddddddddd'), 'dd')]", $root);

                if (empty($airlineText)) {
                    $airlineText = $this->http->FindSingleNode("descendant::text()[normalize-space()][{$this->contains($this->t(' to '))}][3][contains(translate(.,'0123456789', 'dddddddddd'), 'dd')]", $root);
                }

                if (empty($airlineText)) {
                    $airlineText = $this->http->FindSingleNode("preceding::text()[normalize-space()][1][{$this->contains($this->t(' to '))}][contains(translate(.,'0123456789', 'dddddddddd'), 'dd')]", $root);
                }

                if (empty($airlineText)) {
                    $airlineText = $this->http->FindSingleNode("./descendant::text()[normalize-space()][1][contains(translate(.,'0123456789', 'dddddddddd'), 'dd')]", $root);
                }
...
//            sex, 20 de ago, 23h59 - 6h35 (sem escalas) +1
        ...
        foreach ($hotels as $hRoot) {
            if ($this->http->XPath->query("./descendant::text()[{$this->contains($this->t('Print activity voucher'))}]")->length === 0) {
                ...
                //it-898871182.eml
                $firstRow = $this->http->FindSingleNode('.', $hotelRows[0]);

                if (preg_match("/^(?:\w+\,\s*\w+\s*\d+\s*\-\s*\w+\,\s*\w+\s*\d+|\w+\,\s*\d+\s*\w+\s*\-\s*\w+\,\s*\d+\s*\w+$)$/", $firstRow)) {
                    $hotelRows = $this->http->XPath->query("descendant-or-self::text()[normalize-space()]", $hRoot);
                }
        ...
        foreach ($rentals as $rRoot) {
            if ($this->http->XPath->query("./following::text()[normalize-space()][4]/ancestor::div[1][{$this->contains($this->t('Your ride'))}]", $rRoot)->length === 0
                ...
                //You have until 11:59pm PT on the date of booking to cancel this flight for free.
                $rentalContent = preg_replace("/You have until.+\n/", "", $rentalContent);
                ...
                */
                ...
                /*Hertz
                Pick-up
                6301 Toronto Pearson Intl Apt, Silver Dart Drive, Toronto, Ontario, CAN, YYZ
                Hours of operation: Midnight - 1:00 a.m.
                Fri, Mar 14, 10:00 p.m.
                Drop-off
                6301 Toronto Pearson Intl Apt, Silver Dart Drive, Toronto, Ontario, CAN, YYZ
                Hours of operation: 12:01 a.m. - Midnight
                Sat, Mar 15, 10:00 p.m.
                Car details Managers Special or similar Economy Automatic transmission
                Unlimited mileage
                More information available about this rental’s*/

                $regexp4 = "/^(?<company>.{2,})\n+\s*(?:Pick\-up|Pick up)\s*\n\s*(?<pickup>[\s\S]+?)\s*\n+\s*(?:Drop\-off|Drop off)\s*(?<dropoff>[\s\S].+\n(?:Hours of operation)[:：\s].+\n\w+\,.+(?:a|p)\.?m\.?)\n(?:Car details)\s+(?<model>.+or similar)\s+(?<type>(?:Economy\s+.*))/u";

                if (preg_match($regexp4, $rentalContent, $m) || preg_match($regexp, $rentalContent, $m) || preg_match($regexp2, $rentalContent, $m) || preg_match($regexp3, $rentalContent, $m)) {
                    // Pick Up
