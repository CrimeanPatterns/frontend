...
{
    public $mailFiles = "expedia/it-305720129.eml, expedia/it-364657875.eml, expedia/it-667275046.eml, expedia/it-672825790.eml, expedia/it-672828262.eml, expedia/it-894637087.eml";
    private $detectLang = [
        'en' => ['Traveler Details', 'Refund Summary', 'Price details'],
        'fr' => ['Détails sur le ou les voyageurs'],
    ];
    private $subjects = [
        // en
    ...
    private static $dictionary = [
        ...
        'fr' => [
            'confNoInBody'                       => ['Voyage nᵒ'],
            'confNoInSubject'                    => ['voyage nᵒ'],
            //'confNoByCompany'                    => [''],
            'Traveler Details'                   => ['Détails sur le ou les voyageurs'],
            'Pick up'                            => ['Prise en charge du véhicule'],
            ...
            //'All set,'                           => [''],
            'Free cancellation up until pick-up' => ['Annulation et non-présentation'],

            'You earned'             => 'Vous avez gagné',
            'Expedia Rewards points' => 'points Expedia Rewards',
        ],
    ];
    ...
    private function parseCar(Email $email, ?\DOMNode $root = null, $subject): void
    {
        ...
        if (trim($typeCar) === 'Car details' || $r->getCompany() === trim($typeCar)) {
            $typeCar = $this->http->FindSingleNode("descendant::text()[{$this->contains($this->t('or similar'))}]/following::text()[string-length()>2][1]", $root);
        }
    ...
    private function normalizeDate($date)
    {
        ...
        $res = [
            ...
            '/^\s*(?<week>[-[:alpha:]]+)\.?\s*(?<day>\d+)\s*(?<month>\w+)\.?[\sà]+(?<hour>\d+)[\sh]+(?<min>\d+)$/us',
        ];
        ...
        foreach ($res as $re) {
            if (preg_match($re, $date, $m)) {
                ...
                if (!empty($m['hour']) && !empty($resultDate)) {
                    $resultDate = strtotime($m['hour'] . ':' . $m['min'], $resultDate);
                }
