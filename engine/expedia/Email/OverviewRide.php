    ...
    private function parseHtml(Email $email): void
    {
        ...
        foreach ($nodes as $root) {
            ...
            if (!empty($this->lastArrivePoint) && stripos($this->lastArrivePoint, $name) !== false) {
                $s->departure()
                    ->name(str_replace(' - All Inclusive', '', $this->lastArrivePoint));
            } else {
                $s->departure()
                    ->name(str_replace(' - All Inclusive', '', $name));
            }
...
//                $s->departure()->date($this->normalizeDate($node));
            ...
            if (!empty($country)) {
                $s->arrival()
                    // ->name($country . ', ' . $name);
                    ->name(str_replace(' - All Inclusive', '', $name . ', ' . $country));
                $this->lastArrivePoint = str_replace(' - All Inclusive', '', $country . ', ' . $name);
            } else {
                $s->arrival()
                    ->name($name);
