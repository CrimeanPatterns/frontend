...
class HotelConfirmed extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            ...
            'Expedia itinerary:' => ['Expedia itinerary:', 'Hotels.com itinerary:', 'Itinerary no.'], // only after hotel name
            ...
            'adult'            => 'adult',
            'room'             => 'room',
            'feeNames'         => ['Taxes and fees', 'Resort fee', 'Extra guest fee', 'Taxes & Fees', 'Locally collected mandatory fees/taxes*', 'Taxes'],
            'discount'         => ['Coupon applied', 'OneKeyCash used'],
            'Traveler details' => ['Traveler details', 'Traveler Details', 'Traveller details'],
            'badPhrases'       => ['We have included all charges', 'Late arrival instructions'],
        ],
    ];
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        if ($this->http->XPath->query("//text()[normalize-space()='View full itinerary']/following::text()[{$this->eq($this->t('Traveler details'))} or {$this->contains($this->t('Traveler details'))}]/following::text()[normalize-space()='Accommodation details']")->length > 0) {
            return true;
        }
    ...
    public function ParseHotel(Email $email): void
    {
        ...
        $cancellation = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Free cancellation until'))}]");
        ...
        if (empty($cancellation)) {
            $cancellation = $this->http->FindSingleNode("//text()[{$this->starts($this->t('There is no charge for cancellations made before'))}]/following::text()[normalize-space()][1]");
        }
        ...
        */
        ...
        // Hotel
        $hotelName = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Expedia itinerary:'))}]/preceding::text()[normalize-space()][1][not(contains(normalize-space(), 'Your booking is'))]");
        ...
        if (empty($address)) {
            $address = $this->http->FindSingleNode("//text()[normalize-space()='Check-in']/preceding::a[1]/ancestor::tr[1]/descendant::img[contains(@src, 'lob_hotels_color') or contains(@src, 'cid')]/ancestor::tr[1][not(contains(normalize-space(), 'View full itinerary'))]");
        }

        if (empty($address)) {
            $address = $this->http->FindSingleNode("//text()[normalize-space()='Check-in']/preceding::text()[normalize-space()][1]/ancestor::a[1]");
        }
        ...
        $fax = $this->http->FindSingleNode("//text()[{$this->contains('contact ' . $hotelName)}]/following::text()[normalize-space()][1][{$this->starts($this->t('Tel:'))}]", null, true, "/{$this->opt($this->t('Fax:'))}\s*([\d\(\)\s\-]+)\s*(?:$|\,)/");
        ...
        if (preg_match("/(?<night>\d+)\s*nights?\,\s+(?<adult>\d+)\s*adults?\,(?:\s*(?<kids>\d+)\s*child(?:ren)?\,)?\s*(?<room>\d+)\s*room/", $bookedInfo, $m)) {
            ...
            $guestsInfo = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Traveler details'))}]/following::text()[normalize-space()][1][contains(normalize-space(), 'Adult')]");

            if (preg_match("/{$this->opt($this->t('Adults'))}\,?\s*(?<guests>\d+)\;?(?:\s*{$this->opt($this->t('Children'))}\,\s*(?<kids>\d+))?/", $guestsInfo, $m)) {
                $h->booked()
                    ->guests($m['guests']);
        ...
        $roomType = $this->http->FindSingleNode("//text()[normalize-space()='Check-in']/following::img[contains(@src,'icon__lob_hotels')][1]/following::text()[contains(normalize-space(), 'room') and contains(normalize-space(), 'night')][1]/following::text()[normalize-space()][1][not({$this->contains($this->t('badPhrases'))})]");

        if (empty($roomType)) {
            // it-672608247.eml
            $roomType = $this->http->FindSingleNode("//text()[starts-with(normalize-space(), 'You booked')]/following::text()[string-length() > 2][not(contains(normalize-space(), 'room'))][1]");
        }
        ...
        if (empty($hotelName) && !empty($address)) {
            $hotelName = $this->http->FindSingleNode("//text()[{$this->eq($address)}]/preceding::text()[normalize-space()][1]");
        }
        $h->hotel()
            ->name($hotelName)
            ->address($address)
        ...
        if (preg_match('/^(?<currency>[^\-\d)(]+?)[ ]*(?<amount>\d[,.‘\'\d ]*)(?:$|\s+\()/u', $price, $matches)) {
            // COP 3.288.600.00
        ...
        if (empty($pointInfo)) {
            $pointInfo = $this->http->FindSingleNode("//text()[starts-with(normalize-space(), 'You will earn')]/ancestor::*[1]");
        }

        if (preg_match("/\s+(?<earn>\D[\d\.]+)\s*in\s*OneKeyCash/u", $pointInfo, $m)
         || preg_match("/You will earn\s+(?<earn>[\d\.]+\s*Expedia Rewards points)/", $pointInfo, $m)) {
    ...
    private function detectDeadLine(\AwardWallet\Schema\Parser\Common\Hotel $h): void
    {
        ...
        } elseif (preg_match("/There is no charge for cancellations made before\s+(?<time>[\d\:]+a?p?m?)\s+\(property local time\) on\s+(?<day>\d+\s+\w+)\.\s+(?<year>\d{4})\./iu", $cancellationText, $m)
            || preg_match("/Cancellations or changes made before\s+(?<time>[\d\:]+a?p?m?)\s+\(property local time\) on\s+(?<day>\d+\s+\w+)\s+(?<year>\d{4})/iu", $cancellationText, $m)
            || preg_match("/Cancellations or changes made (?:before|after)\s+(?<time>[\d\:]+a?p?m?)\s+\(property local time\) on\s+(?<day>\w+\s+\d+)\,?\s+(?<year>\d{4})/iu", $cancellationText, $m)) {
            $h->booked()->deadline(strtotime($m['day'] . ' ' . $m['year'] . ', ' . $m['time']));
        } elseif (preg_match("/Free cancellation until\s+(?<date>\d+.*\d{4})\s+at\s+(?<time>[\d\:]+)\s+\(property local time\)/iu", $cancellationText, $m)) {
