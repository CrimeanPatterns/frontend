...
{
    public $mailFiles = "hotels/it-220158363.eml, hotels/it-220158497.eml, hotels/it-267414225.eml, hotels/it-461443600.eml, hotels/it-913702133.eml";
    public $lang = '';
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (empty($this->detectProvider($text))) {
                return false;
            }

            if (strpos($text, $this->t('Reservation details')) !== false
    ...
    public function detectProvider($text)
    {
        if (strpos($text, $this->t('Hotels.com itinerary')) !== false) {
            return 'hotels';
        }

        if (strpos($text, $this->t('Expedia itinerary:')) !== false) {
            return 'expedia';
        }
    ...
    public function ParseHotelPDF2(Email $email, $text)
    {
        ...
        $email->ota()
            ->confirmation($this->re("/{$this->opt($this->t('Hotels.com itinerary'))}\:?\s*(\d+)\n/", $text) ?? $this->re("/{$this->opt($this->t('Expedia itinerary'))}\:?\s*(\d+)\n/", $text));

        $travellersText = $this->re("/{$this->opt($this->t('Reserved for'))}\s*(.+)\s*{$this->opt($this->t('Payment details'))}/s", $text);

        if (preg_match_all("/^\s*([[:alpha:]][-&.\'’[:alpha:] ]*[[:alpha:]])\,?\s*\(?\d+\s*{$this->opt($this->t('adult'))}\)?\n$/mu", $travellersText, $m)) {
            $travellers = $m[1];
        }
        ...
        $priceText = $this->re("/{$this->opt($this->t('Total'))}\s*(\D{1,5}[\d\.\,\s]+)\n/u", $text);

        if (empty($priceText)) {
            $priceText = $this->re("/{$this->opt($this->t('Total'))}\s*([\d\.\,\s]+\D{1,5})\n/u", $text);
        }

        if (preg_match("/^\s*(?<currency>\D{1,3})\s*(?<total>[\d\.\,]+)/u", $priceText, $m)
        || preg_match("/^\s*(?<total>[\d\.\,]+)\s*(?<currency>[A-z\.]{3})/u", $priceText, $m)) {
            ...
            $tax = $this->re("/{$this->opt($this->t('Taxes & Fees'))}\s*\D{1,3}\s*([\d\.\,]+)\n/u", $text);
        ...
        $rewards = $this->re("/You earned\s*(\d+.*points)/", $text);

        if (!empty($rewards)) {
            $h->setEarnedAwards($rewards);
        }
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            $provCode = $this->detectProvider($text);
