...
class ItineraryPdf2017En extends \TAccountChecker
{
    ...
    private function parseFlightSegment(Flight $f, $text): void
    {
        ...
        if (preg_match('/[[:alpha:]] (?<date>\d{1,2} [[:alpha:]]+ \d{4}) at \d+\s+[-,.;!()\[\]\w\s]+\s+\(\s*(?<al>[A-Z][A-Z\d]|[A-Z\d][A-Z])\s*(?<fn>\d+)\s*\)(?:.*?Operated by\s*(?<operator>\S.+))?/u', $text, $matches)) {
            // Sun 25 May 2025 at 0630               LOT-POLISH AIRLINES (LO0286)
        ...
        if (preg_match_all("/^[ ]*Seat\s+Requested[ ]+[A-Z()\/ ]+?[ ]{2,}(?-i)(\d{1,4}[A-Z])$/im", $text, $seatMatches)) {
    ...
    private function parseHotel(Email $email, $text): void
    {
        ...
        $remarks = preg_match("/\n[ ]{0,35}Remarks[ ]*:[ ]*(\S.+?\S)(?:\n[ ]{0,35}Important Information[ ]*:|\n\n\n|$)/is", $text, $m) && !preg_match("/\n[ ]{0,35}\S.{1,23}\S[ ]*:/", $m[1]) ? $m[1] : null;

        if ($remarks) {
            $this->logger->info('[HOTEL] Remarks:');
            $this->logger->debug($remarks);
        }

        $importantInfo = preg_match("/\n[ ]{0,35}Important Information[ ]*:[ ]*(\S.+?\S)(?:\n\n\n|$)/is", $text, $m) && !preg_match("/\n[ ]{0,35}\S.{1,23}\S[ ]*:/", $m[1]) ? $m[1] : null;

        if ($importantInfo) {
            $this->logger->info('[HOTEL] Important Information:');
            $this->logger->debug($importantInfo);
        }

        // General
        $confirmation = $confirmationTitle = null;

        if (preg_match("/^[ ]*(Confirmation No)[ ]*[:]+\s*([^:\n]{4,})$/im", $text, $m)) {
            $confirmation = $m[2];
            $confirmationTitle = $m[1];
        }

        $confNoIsWebsite = preg_match("/^WEB[-\s]*SITE$/i", $confirmation) > 0;

        if ($confNoIsWebsite
            || empty($confirmation) && preg_match('/Confirmation No[ ]*[:]+\s*(?:\n+[^:\n]+:|$)/i', $text)
        ) {
            $h->general()->noConfirmation();
        } elseif (preg_match("/^[A-Za-z\d]{4,}$/", $confirmation)) {
        ...
        if (preg_match('/\n[ ]{0,35}Cancell?ation Information[ ]*:[ ]*((?:.*\S\n){1,5}?)(?:\n*[ ]{0,35}Remarks[ ]*:|\n\n|$)/i', $text, $m)
            || preg_match('/(?:^|\n)[ ]*Cancell?ation policy[ :]*\n?((?:\n[ ]*\S.*){1,5}?)(?:\n\n|$)/i', $remarks, $m)
        ) {
            $h->general()->cancellation($this->normalizeText($m[1]));
        }
        ...
        $importantInfoNormal = $this->normalizeText($importantInfo);

        if (preg_match("/^NO CANCELL?ATION CHARGE APPLIES PRIOR TO\s+(?<time>{$this->patterns['time']})(?:\s+LOCAL TIME)?\s*,\s*UP TO\s+(?<prior>\d{1,3}\s*DAYS?)\s+PRIOR TO ARRIVAL(?:\s*[.;!]|$)/i", $importantInfoNormal, $m)
        ) {
    ...
    {
        $GDSReference = $GDSReferenceTitle = null;

        if (preg_match("/^[ ]*(GDS Reference)[ ]*[:]+[ ]{0,60}([-A-z\d]{3,})(?:[ ]{2}|$)/im", $text, $m)) {
            $GDSReference = $m[2];
            $GDSReferenceTitle = $m[1];
            $email->ota()->confirmation($GDSReference, $GDSReferenceTitle);
        }

        if (preg_match("/^[ ]*(Booking #)[ ]*[:]+[ ]{0,60}([-A-z\d]{3,})(?:[ ]{2}|$)/im", $text, $m)
            && $m[2] !== $GDSReference
        ) {
