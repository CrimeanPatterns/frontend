...
{
    public $mailFiles = "ferryscan/it-361172975-el.eml, ferryscan/it-364207998.eml, ferryscan/it-372679468-fr.eml, ferryscan/it-924503883.eml";
    ...
    public static $dictionary = [
        ...
        'en' => [
            ...
            'Seat Class'                       => ['Seat Class', 'Seat class', 'Class'],
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($tripList as $root) {
            ...
            foreach ($passengerRows as $pRow) {
                ...
                if ($seatNumber) {
                    $seatParts[] = $seatNumber . ', ' . $traveller;
                }
            ...
            if ($shipType !== null && !preg_match("/({$this->opt($this->t('Baggage Allowance Per Passenger'))}|{$this->opt($this->t('Passenger'))})/u", $shipType)) {
            ...
            if (preg_match('/^(?<currency>\D{1,3})[ ]*(?<amount>\d[,.‘\'\d ]*)$/u', $priceFor, $matches)
                || preg_match('/^(?<amount>\d[,.‘\'\d ]*?)[ ]*(?<currency>\D{1,3})$/u', $priceFor, $matches)
            ) {
                // €182.96
        ...
        if (preg_match('/^(?<currency>\D{1,3})[ ]*(?<amount>\d[,.‘\'\d ]*)$/u', $totalPrice, $matches)
            || preg_match('/^(?<amount>\d[,.‘\'\d ]*?)[ ]*(?<currency>\D{1,3})$/u', $totalPrice, $matches)
        ) {
            // €365.92    |    148,30 EUR
            $currencyCode = $this->normalizeCurrency($matches['currency']);
            $email->price()
