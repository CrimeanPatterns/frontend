    ...
    public function LoadLoginForm()
    {
        ...
        if (is_string($result)) {
            ...
            if (
                ...
                || strstr($result, 'Our service is temporarily unavailable.')
            ) {
...
//            $selenium->http->removeCookies();
            ...
            if (!$panel) {
                ...
                if ($iframe = $selenium->waitForElement(WebDriverBy::xpath("//iframe[@id = 'wiz-iframe-intent']"), 5)) {
                    $selenium->driver->switchTo()->frame($iframe);
                    $this->savePageToLogs($selenium);

                    $error = $selenium->waitForElement(WebDriverBy::xpath("//div[contains(text(), 'Our service is temporarily unavailable.')]"), 0);
