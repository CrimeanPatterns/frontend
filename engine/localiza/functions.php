...
class TAccountCheckerLocaliza extends TAccountChecker
{
    ...
    private array $headers = [
        ...
        "client_id"       => "iA6b0TNSPXDCFj3deGbQIVrRU5eMgvJQUFAiKM7lbZfG9VfY",
    ...
    {
        if (!isset($this->State['Authorization']) || !isset($this->State['authorization_app'])) {
            return false;
        }
    ...
    public function LoadLoginForm()
    {
        ...
        $headers = ["Authorization" => "Basic aUE2YjBUTlNQWERDRmozZGVHYlFJVnJSVTVlTWd2SlFVRkFpS003bGJaZkc5VmZZOldXYnZIMUs5em1pc3Yyck5oQVdzTTJsUWZLNTNhUFFoeURhdnlBUlhUVGVoV1hxbThoeHZMb2RFQjJ4aU9PUmM="];
        $this->http->PostURL("https://api-gtw-prd.localiza.com/oauth/token/accesstoken?grant_type=client_credentials", "{}", $this->headers + $headers);
        ...
        $data = json_encode([
            'deviceId'    => '6f359952-2584-4e16-a0d1-0453d0c8bc35',
            'deviceName'  => '6f359952-2584-4e16-a0d1-0453d0c8bc35',
            'redirectUri' => 'https://www.localiza.com/autenticacao',
        ]);
        $this->headers += ["Authorization" => "Bearer {$response->access_token}"];
        $this->http->PostURL("https://canaisdigitais-api.localiza.com/sitelocaliza-api-netcore/v1/OAuth/grant-code", $data, $this->headers);
        $response = $this->http->JsonLog();
    ...
    public function Login()
    {
        ...
        elseif (isset($response->data->finalRedirectUri)) {
            ...
            $this->headers += [
                "X-dispositivo-id"   => "6f359952-2584-4e16-a0d1-0453d0c8bc35",
                "X-dispositivo-nome" => "6f359952-2584-4e16-a0d1-0453d0c8bc35",
            ];
            $this->http->RetryCount = 0;
            $this->http->PostURL("https://canaisdigitais-api.localiza.com/sitelocaliza-api-netcore/v1/OAuth/access-token", json_encode($data), $this->headers);
            $this->http->RetryCount = 2;
            $response = $this->http->JsonLog();

            if (isset($response->accessToken)) {
                $this->State['authorization_app'] = "Bearer {$response->accessToken}";
                $this->State['Authorization'] = $this->headers['Authorization'];
                $this->headers['authorization_app'] = $this->State['authorization_app'];
    ...
    public function Parse()
    {
        ...
        $this->http->GetURL("https://canaisdigitais-api.localiza.com/sitelocaliza-api-netcore/v1/Fidelidade/meus-dados", $this->headers);
        $response = $this->http->JsonLog();
    ...
    private function loginSuccessful()
    {
        ...
        $headers = [
            ...
            "authorization_app" => $this->State['authorization_app'],
        ];
        $this->http->RetryCount = 0;
        $this->http->GetURL("https://canaisdigitais-api.localiza.com/sitelocaliza-api-netcore/v1/Clientes/resumo-dados-pessoais", $this->headers + $headers);
        $this->http->RetryCount = 2;
        $response = $this->http->JsonLog();
