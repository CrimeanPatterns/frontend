    ...
    private function parseAirPdf2($text, Email $email): void
    {
        ...
        */
        ...
        foreach ($segments as $sText) {
            ...
            if (preg_match("/^(.+[ ]{2}){$this->preg_implode($this->t('Airline:'))}/mu", $tableText, $m)) {
                $td2Positions[] = mb_strlen($m[1]);
            }

            if (preg_match("/^(.+[ ]{2}){$this->preg_implode($this->t('Flight no:'))}/mu", $tableText, $m)) {
                $td2Positions[] = mb_strlen($m[1]);
            }

            if (preg_match("/^(.+[ ]{2}){$this->preg_implode($this->t('Operated by'))}[ ]*:/mu", $tableText, $m)) {
                $td2Positions[] = mb_strlen($m[1]);
            }

            if (preg_match("/^(.+[ ]{2}){$this->preg_implode($this->t('Duration:'))}/mu", $tableText, $m)) {
                $td2Positions[] = mb_strlen($m[1]);
            }
            ...
            if ((preg_match("/ +{$this->preg_implode($this->t('Carrier reservation number (PNR)'))}[: ]+(?<pnr>[A-Z\d]{5,})$/imu", $sText, $m)
                ...
                && (strlen($m['pnr']) < 11 || preg_match('/[A-z]/u', $m['pnr']) > 0)
            ) {
                $s->airline()->confirmation($m['pnr']);
            }

            if (preg_match("/{$this->opt($this->t('Airline:'))}\s*([A-z].*)/", $sText, $m)) {
                $s->airline()
                    ->operator($m[1]);
            }
