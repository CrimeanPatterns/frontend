...
//        curl_setopt($this->http->driver->curl, CURLOPT_SSL_VERIFYHOST, false);
        ...
        if (!$this->http->ParseForm(null, '//form[@id="loginForm"]')) {
            return $this->checkErrors();
        }
    ...
    public function parseQuestion()
    {
        ...
        if (!$this->http->FindSingleNode('//button[@class="btn-email-otp"]')) {
            $this->logger->debug('send email otp button not found');
        ...
        $params = [
            ...
            'sessionDataKey' => $this->http->FindPreg('/sessionDataKey=(.+?)&/', false, $this->http->currentUrl()),
            'multiOptionURI' => '/login/unity_login.jsp?loginsource=home',
            'response_mode'  => 'fragment',
            'state'          => $this->http->FindPreg('/state=(.+?)&/', false, $this->http->currentUrl()),
            'x-client-SKU'   => 'ID_NET461',
            'x-client-ver'   => '5.3.0.0',
            'authenticators' => 'SMSOTP:SMSOTP;EmailOTP:EmailOTP',
            'inputType'      => 'idf',
        ];

        $this->http->GetURL('https://unity.login.hardrock.com/commonauth?' . http_build_query($params));

        $question = $this->http->FindSingleNode('
            //label[contains(text(), "A new 6-digit code has been sent to your email address at")]
            | //label[contains(text(), "A 6-digit code was sent to the email address associated with your account")]
        ');
