...
class TAccountCheckerSaudisrabianairlinSelenium extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        $this->waitForElement(WebDriverBy::xpath('//button[@id="login-btn"] | //iframe[@id="main-iframe"]'), self::WAIT_TIMEOUT * 4);
    ...
    public function Login()
    {
        ...
        if (
            ...
            if (
                strstr($message, 'Incorrect Login ID or password')
                || strstr($message, 'Invalid credentials')
            ) {
    ...
    public function Parse()
    {
        ...
        // Name
        $this->SetProperty("Name", beautifulName($this->http->FindSingleNode('//div[@class="member-id-wrapper"]/span[text() and not(text()="") and position() = 1]')));
