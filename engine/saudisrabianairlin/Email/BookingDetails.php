...
{
    public $mailFiles = "saudisrabianairlin/it-641080497.eml, saudisrabianairlin/it-644023069.eml, saudisrabianairlin/it-644023118.eml, saudisrabianairlin/it-696580849.eml, saudisrabianairlin/it-835773222.eml, saudisrabianairlin/it-844504502.eml, saudisrabianairlin/it-914707676.eml";
    public $subjects = [
        '/(?:^|:\s*)(?:Booking|Rebooking|Flight)\s+Confirmation(?:\s+ETicket_|$)/i',
        '/التذكرة الإلكترونية لتأكيد الرحلة_إيصال_/iu',
        '/Important\:\s*Your\s*booking\s*details\s*have\s*changed/iu',
    ];
    ...
    public static $dictionary = [
        'en' => [
            ...
            'Booking details' => ['Booking details', 'Updated booking details', 'Your itinerary', 'Itinerary details', 'Other flight remains unchanged'],
    ...
    public function ParseFlight(Email $email): void
    {
        ...
        $seats = [];
        $arrayForSeats = $this->http->FindNodes("//text()[{$this->starts($this->t('Routes'))}][./ancestor::td[normalize-space()][2][not({$this->contains($this->t('Original flight'))})]]");
        ...
        $nodes = $this->http->XPath->query("//text()[{$this->starts($this->t('Routes'))}][./ancestor::td[normalize-space()][2][not({$this->contains($this->t('Original flight'))})]]");
        ...
        foreach ($nodes as $key => $root) {
            ...
            if (preg_match("/^(?<arrName>.+?)\s*{$this->opt($this->t('Terminal'))}\s*(?<arrTerminal>[A-Z0-9]+)\s*(?:\+[ ]*\d+[ ]*{$this->opt($this->t('day'))}|$)/is", $arrInfo, $m)
                || preg_match("/^(?<arrName>.+?)(?:\s+\+\s*\d+\s*{$this->opt($this->t('day'))}|\s*)$/s", $arrInfo, $m)
            ) {
                $s->arrival()
                    ->name(trim($m['arrName'], '-'));
