...
class TAccountCheckerAirnewzealand extends TAccountChecker
{
    ...
    private function parseItinerary()
    {
        $fareConstants = [
            ...
            'FLEXIPAY_FEE'     => 'Flexipay Fee',
            'TAX'              => 'Tax',
            'SEATS'            => 'Seat select',
            'PERCENTAGE_CARD_PAYMENT_FEE' => 'Card payment fee'
        ];
        ...
        $spentAwards = 0;
        $airpointsPayments = $travelDetails->payments->paymentBreakup->airpointsPayments ?? [];

        /*
        if (count($airpointsPayments) > 1) {
            $this->sendNotification('refs #25144 airnewzealand - need to check airpoints payments // IZ');
        }
        */

        foreach ($airpointsPayments as $payment) {
            $currencyCodes[] = $payment->amount->currencyCode;
            $spentAwards += $payment->amount->amount;

            if (!isset($fareConstants[$payment->paymentLineItemType])) {
                $this->sendNotification('refs #25144 airnewzealand - new fare contant was found // IZ');
        ...
        if ($spentAwards > 0) {
            $f->price()->spentAwards("{$spentAwards} Airpoints Dollars");
        }

        $total = 0;
        $taxes = $travelDetails->payments->taxes ?? [];
        ...
        */
        ...
        foreach ($taxes as $tax) {
            $currencyCodes[] = $tax->amount->currencyCode;
            $total += $tax->amount->amount;
            $f->price()->fee($tax->description, $tax->amount->amount);
        }

        $otherPayments = $travelDetails->payments->paymentBreakup->otherPayments ?? [];

        /*
        if (count($otherPayments) > 1) {
        ...
        */
        ...
        if (count(array_unique($currencyCodes)) > 1) {
            $this->sendNotification('refs #25144 airnewzealand - need to check currency code // IZ');
        }
        ...
        foreach ($travelDetails->itinerary as $itineraryCounter => $itinerary) {
            foreach ($itinerary->flights as $flightCounter => $flight) {
                ...
                $s->extra()->aircraft($flight->aircraft->name ?? null, false, true);
                $s->extra()->cabin($flight->cabin->name);
                $s->extra()->bookingCode($flight->cabin->bookingClass);
                $s->extra()->duration($flight->duration ?? null, false, true);
    ...
    private function checkErrors()
    {
        ...
        if ($message = $this->http->FindSingleNode("//h1[
            ...
            throw new CheckException($message, ACCOUNT_PROVIDER_ERROR);
            */
            throw new CheckRetryNeededException(1, 0, $message, ACCOUNT_PROVIDER_ERROR);
        }
