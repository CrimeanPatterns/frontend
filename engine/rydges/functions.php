...
//        $this->http->SetInputValue("loginpass", $this->AccountFields['Pass']);
    ...
    private function seleniumAuth()
    {
        ...
        try {
            ...
            $logout = $selenium->waitForElement(WebDriverBy::xpath('//a[contains(., "Logout")] | //span[contains(@class, "user-name") and normalize-space(.) != ""]'), 20, false);

            if ($button =
                $selenium->waitForElement(WebDriverBy::xpath("//button[contains(text(), 'Remind me later')]"), 0)
                ?? $selenium->waitForElement(WebDriverBy::xpath("//button[contains(text(), 'Continue')]"), 0)
            ) {
                $this->savePageToLogs($selenium);
                $button->click();
                $logout = $selenium->waitForElement(WebDriverBy::xpath('//a[contains(., "Logout")] | //span[contains(@class, "user-name") and normalize-space(.) != ""]'), 20, false);

                if (!$logout && ($button = $selenium->waitForElement(WebDriverBy::xpath("//button[contains(text(), 'Continue')]"), 0))) {
                    $this->savePageToLogs($selenium);
                    $button->click();
                    $logout = $selenium->waitForElement(WebDriverBy::xpath('//a[contains(., "Logout")] | //span[contains(@class, "user-name") and normalize-space(.) != ""]'), 20, false);
                }
