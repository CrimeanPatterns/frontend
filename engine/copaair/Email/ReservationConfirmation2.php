...
{
    public $mailFiles = "copaair/it-152357585-pt.eml, copaair/it-167796455.eml, copaair/it-186255100.eml, copaair/it-186891332.eml, copaair/it-636815446.eml, copaair/it-88293378.eml, copaair/it-88515978.eml, copaair/it-916431101.eml";
    public $subjects = [
        // en
    ...
    public function ParseFlight(Email $email): void
    {
        ...
        $travellers = array_filter($this->http->FindNodes("//text()[{$this->eq($this->t('Passengers in this reservation'))}]/ancestor::tr[1]/following-sibling::tr/descendant::tr/descendant::text()[normalize-space()][1]", null, "/^\s*(?:{$this->opt(['Miss', 'Mrs', 'Mr', 'Ms'])}\.?\s*)?([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])\s*$/"));
        ...
        if ($newItinerary !== true) {
            foreach ($travellers as $traveller) {
                if ($this->http->FindSingleNode("//text()[{$this->eq($traveller)}]/following::text()[normalize-space()][1][{$this->eq($this->t('Infant'))}]")) {
                    $f->addInfant($traveller);
                } else {
        ...
        foreach ($f->getTravellers() as $traveller) {
            $accountText = $this->http->FindSingleNode("//text()[{$this->eq($traveller[0])}]/ancestor::th[1][{$this->contains('#')}]");

            if (preg_match("/^\s*{$this->opt($traveller[0])}\s*(?<desc>.+?)\s*\#\s*(?<number>[A-Z\d]+)\s*$/", $accountText, $m)) {
                $f->addAccountNumber($m['number'], false, $traveller[0], $m['desc']);
            }
        }
...
//        $this->logger->debug('$xpath = '.print_r( $xpath,true));
        ...
        foreach ($nodes as $i => $root) {
            ...
            $operated = $this->http->FindSingleNode("./descendant::text()[{$this->contains($this->t('Operated by'))}]/following::text()[normalize-space()][1][not({$this->eq($this->t('Fare Family'))})]", $root);
            ...
            if (preg_match("/^(?<time>{$patterns['time']})\s*(?<airport>\S.{3,})?$/", $arrInfo, $m)) {
                ...
                if (!empty($m['airport'])) {
                    $s->setArrName($m['airport']);
                }
            }

            foreach ($f->getTravellers() as $traveller) {
                $seats = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Seats'))}]/ancestor::tr[1][{$this->contains($traveller[0])}]/descendant::text()[normalize-space()][last()]");
                $seatArray = preg_split("/[\s\,]+/", $seats);

                if (!empty($seatArray[$i]) && preg_match("/^(\d{1,2}[A-Z])$/", $seatArray[$i])) {
                    $s->addSeat($seatArray[$i], false, false, $traveller[0]);
                }
            }
