...
{
    public $mailFiles = "copaair/it-187556565-es.eml, copaair/it-292756328.eml, copaair/it-643170306-fr.eml, copaair/it-645060176.eml, copaair/it-717843355-pt.eml, copaair/it-720213614-es.eml, copaair/it-724585641-es.eml, copaair/it-910893997.eml";
    public $detectSubjects = [
        ...
        'Pequeno ajuste no seu itinerário de viagem',
        // es
        'Check-In Pendiente',
    ...
    public static $dictionary = [
        ...
        "pt" => [
            ...
            'previousItinerary'      => 'Itinerário anterior',
        ...
        "es" => [
            ...
            'detectBody'            => ['Tu itinerario actualizado', 'Detalles de itinerario', 'Detalles del itinerario', 'Su itinerario actualizado'],
            'previousItinerary'     => ['Itinerario anterior', 'Itinerário anterior'],
            'Frequent flyer number' => ['Número de viajero frecuente'],
            'Seat'                  => ['Asiento(s)', 'Asientos', 'Asiento'],
            'Operated by'           => 'Operado por',
        ],
    ...
    public function ParseFlight(Email $email): void
    {
        ...
        // segments: type-1
        $xpath = "//tr[ count(*)=3 and *[1][{$xpathTime}] and *[2][normalize-space()='' and descendant::img] and *[3][{$xpathTime}] ][not(preceding::*[{$this->eq($this->t('previousItinerary'))}])]";
        ...
        foreach ($travellersRows as $tRow) {
            $nonNormalizeTraveller = $this->http->FindSingleNode("*[normalize-space()][1]/descendant::text()[normalize-space()][1]/ancestor::p[1]", $tRow, true, "/^{$patterns['travellerName']}$/u");
            $passengerName = $this->normalizeTraveller($nonNormalizeTraveller);
            $f->general()->traveller($passengerName, true);

            $accountText = $this->http->FindSingleNode("//text()[{$this->eq($nonNormalizeTraveller)}]/ancestor::th[1][{$this->contains(['#', ':'])}]");

            if (preg_match("/^\s*{$this->opt($nonNormalizeTraveller)}\s*(?<desc>.+?)[\#\:\s]+(?<number>[A-Z\d]+)\s*$/", $accountText, $m)) {
                $f->addAccountNumber($m['number'], false, $passengerName, $m['desc']);
            }
    ...
    private function normalizeDate(?string $date): ?int
    {
        ...
        $in = [
            '/^\s*(\w+),\s+(\d+)[,.\s]*([[:alpha:]]+)[,.\s]*(\d{4})\s*$/ui', // Thu, 17Nov, 2022   |   jue, 26 sep, 2024
            '/^\s*(\w+),\s+([[:alpha:]]+)\s+(\d+),\s+\d{3}\s*$/ui', // Thu, Nov 17, 202
            '/^\s*(\w+),\s+(\d+)\s+([[:alpha:]]+),\s+\d{3}\s*$/ui', // Thu, 17 Nov, 202
            '/^\s*(\w+),\s+([[:alpha:]]+)\s+(\d+)\s*$/ui', // Thu, Nov 17
            '/^\s*(\d+)\s+(\w+)\s+(\d{4})\s*$/ui', // 17 Nov 2022
            '/^\s*(\d+)\s*([[:alpha:]]+)\s*$/ui',  // 17Nov   |   17 Nov
            '/^\s*([[:alpha:]]+)\s*(\d+)\s*$/ui',  // Nov17   |   Nov 17
        ];
        $out = [
            '$1, $2 $3 $4',
            "$1, $3 $2 $year",
        ...
        if (preg_match("/^(?<week>\w+), (?<date>\d+ \w+ .+)/u", $date, $m)) {
            ...
            // $this->logger->debug('date after week translate = ' . print_r( $date, true));
        } elseif (preg_match("/\b\d{4}\b/", $date)) {
            $date = strtotime($date);
        } else {
            if (!empty($this->year)) {
                $date = strtotime($date . ' ' . $this->year);
            } elseif (!empty($this->emailDate)) {
                $date = EmailDateHelper::parseDateRelative($date, $this->emailDate);
            } else {
                $date = null;
            }
        }
