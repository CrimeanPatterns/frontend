...
{
    public $mailFiles = "msccruises/it-153731887-pt.eml, msccruises/it-155094570.eml, msccruises/it-262417833-pt.eml, msccruises/it-265162861-it.eml, msccruises/it-267315596.eml, msccruises/it-267491008-nl.eml, msccruises/it-270364993-fr.eml, msccruises/it-350509990.eml, msccruises/it-702202905-pt.eml, msccruises/it-756458967.eml, msccruises/it-758407644-pt.eml, msccruises/it-904376826-pt.eml";
    ...
    public static $dictionary = [
        'pt' => [
            ...
            'AT SEA'           => 'Navegando',
            'Deck'             => 'Andar',
            'totalAmount'      => ['Total devido em', 'Tota em'],
            ...
            'checkInBegin'        => 'Seu check-in começa às',
        ],
        'it' => [
            // 'CRUISE TICKET' => '',
        ...
        'fr' => [
            ...
            'Currency' => 'Devise',
            'Disembarkation Date' => 'Date de débarquement',
            'Embarkation port' => 'Port',
            'Disembarkation Port' => 'Port',
            // 'checkInBegin' => '',
        ],
        'de' => [
            // 'CRUISE TICKET' => '',
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if ($detectProvider === false && stripos($textPdf, 'w.msccruisesusa.com') === false && stripos($textPdf, 'w.msccruzeiros.com') === false && stripos($textPdf, 'www.msccruises.be') === false
                && strpos($textPdf, 'www.msccruises.ca') === false
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($pdfs as $pdf) {
            $textPdf = \PDF::convertToText($parser->getAttachmentBody($pdf), false, \PDF::MODE_TEXT_PAGEBREAK);
            $textPdfClean = str_replace("\f", '', $textPdf);

            if (!$textPdfClean || !$this->assignLang($textPdfClean)) {
                continue;
            }
            ...
            if ($segmentsText) {
                $this->parsePdf($email, $textPdfClean, $segmentsText);
            } else {
    ...
    private function parsePdf(Email $email, $text, ?string $segmentsText): void
    {
        ...
        if (preg_match_all("/{$this->opt($this->t('Card Number'))}[:\s]+(\d{5,})$/m", $text, $accountMatches)) {
        ...
        if (preg_match_all("/^.+ (?:\d{1,2}:\d{2}.*?|--:--)[ ]+(?:\d{1,2}:\d{2}.*?|--:--)$/m", $segmentsText, $rowMatches)) {
            // it-702202905-pt.eml
            $lenValues = array_map(function ($item) {
                return mb_strlen($item);
            }, $rowMatches[0]);
            rsort($lenValues);
            
            $segmentsText = preg_replace("/^(.{" . $lenValues[0] . ",}?)[ ]{5,}\S.+/m", '$1', $segmentsText);
    ...
    {
        $this->logger->debug(__FUNCTION__ . '()');
        // examples: it-267315596.eml, it-262417833-pt.eml, it-904376826-pt.eml

        $re = [
            'dateEmbark' => "/^[ ]*{$this->opt($this->t('embarkationDate'))}[: ]{1,50}(\b.{3,}?)(?:[ ]{2}|$)/im",
            'dateDisembark' => "/(?:^[ ]*|[ ]{2}){$this->opt($this->t('Disembarkation Date'))}[: ]{1,50}(\b.{3,}?)(?:[ ]{2}|$)/im",
            'portEmbark' => "/[ ]{2}{$this->opt($this->t('Embarkation port'))}[: ]{1,50}(\S.{2,})\n/im",
            'portDisembark' => "/[ ]{2}{$this->opt($this->t('Disembarkation Port'))}[: ]{1,50}(\S.{2,})\n/im",
        ];
        ...
        foreach ($pages as $pageText) {
            $dateEmbark = $this->re($re['dateEmbark'], $pageText);
            $dateDisembark = $this->re($re['dateDisembark'], $pageText);
            $portEmbark = $this->re($re['portEmbark'], $pageText);
            $portDisembark = $this->re($re['portDisembark'], $pageText);

            if ($dateEmbark === null || $dateDisembark === null || $portEmbark === null || $portDisembark === null) {
                $buffer[] = $pageText;

                continue;
            }

            $id = $dateEmbark . ' -> ' . $dateDisembark . ' | ' . $portEmbark . ' -> ' . $portDisembark;

            if (count($buffer) > 0) {
                $idLast = $idLast ?? $id;
                $pagesByCruise[$idLast] = array_merge(array_key_exists($idLast, $pagesByCruise) ? $pagesByCruise[$idLast] : [], $buffer);
                $buffer = [];
            }
            
            $pagesByCruise[$id] = array_merge(array_key_exists($id, $pagesByCruise) ? $pagesByCruise[$id] : [], [$pageText]);
            $idLast = $id;
        }

        foreach ($pagesByCruise as $cruisePages) {
            $cruiseText = implode("\n", $cruisePages);
            $this->parsePdfShortCruise($email, $cruiseText, $re);
        }
    }

    private function parsePdfShortCruise(Email $email, string $text, array $re): void
    {
        $cr = $email->add()->cruise();
        ...
        if (preg_match_all("/[ ]{2}{$this->opt($this->t('Booking status'))}[ ]*[:]+[ ]*(.{2,}?)\n/i", $text, $statusMatches)
            ...
            && preg_match("/^{$this->opt($this->tPlusEn('statusVariants'))}$/i", $statusMatches[1][0])
        ) {
            $cr->general()->status($statusMatches[1][0]);
        }

        if (preg_match_all("/ (?:{$this->opt($this->t('Stateroom'))}[ ]*[:]+[ ]*| {$this->opt($this->t('Stateroom'))}[: ]+)(\d+)(?:[ ]+[-−]|\n)/iu", $text, $roomMatches)
        ...
        if (preg_match_all($re['dateEmbark'], $text, $embarkationDateMatches)
            && count(array_unique($embarkationDateMatches[1])) === 1
            && preg_match_all($re['dateDisembark'], $text, $disembarkationDateMatches)
            && count(array_unique($disembarkationDateMatches[1])) === 1
            && preg_match_all($re['portEmbark'], $text, $embarkationPortMatches)
            && count(array_unique($embarkationPortMatches[1])) === 1
            && preg_match_all($re['portDisembark'], $text, $disembarkationPortMatches)
    ...
    {
        if (preg_match("/^[ ]*{$this->opt($this->tPlusEn('totalAmount'))}[: ]{1,6}(?:\n?(?<currency>[A-Z]{3})[* ]+)?\n?.*?\b(?<amount>\d[,.‘\'\d]*)$/mu", $text, $matches)) {
            // USD*        12620.72
            if (empty($matches['currency'])
                && preg_match("/^[ ]*{$this->opt($this->tPlusEn('totalAmount'))}[: ]{1,6}.+\n+[ ]*(?<currency>[A-Z]{3})[* ]*$/mu", $text, $m)
            ) {
