...
class Booking extends \TAccountChecker
{
    ...
    public function ParseSegments(\AwardWallet\Schema\Parser\Common\Flight $f)
    {
        ...
        foreach ($nodes as $ri => $root) {
            if ($ri != 0 && stripos(trim($nodes->item($ri - 1)->nodeValue), trim($root->nodeValue)) !== false) {
                continue;
            }
        ...
        foreach ($segments as $parts) {
            ...
            // Airline
            $airline = implode("\n", $this->http->FindNodes("descendant::td[not(.//td)][.//img][normalize-space()][1]/ancestor::td[1]//text()[normalize-space()]", $parts['dep']));
