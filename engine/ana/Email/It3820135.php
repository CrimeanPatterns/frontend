            ...
            foreach ($segments as $segment) {
                if ($segment->getId() !== $seg->getId()) {
                    if (($segment->getAirlineName() == $seg->getAirlineName()
                        ...
                        && $segment->getDepName() == $seg->getDepName()
                        && $segment->getArrName() == $seg->getArrName()
                        && strtotime('00:00', $segment->getDepDate()) == strtotime('00:00', $seg->getDepDate()))
                    ) {
                        if (!empty($seg->getSeats())) {
                            $segment->extra()->seats(array_unique(array_merge($segment->getSeats(),
                                $seg->getSeats())));
                        }
                        $flight->removeSegment($seg);
