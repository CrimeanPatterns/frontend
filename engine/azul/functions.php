...
//        }
    ...
    public function Login()
    {
        ...
        if ($this->http->FindNodes('//p[contains(text(), "Pontos Qualificáveis") or normalize-space(text()) = "Saldo de Pontos"]')) {
            return true;
        }
    ...
    {
        $response = $this->http->JsonLog(urldecode($this->http->getCookieByName("TudoAzul", ".www.voeazul.com.br")));
        $returnObject = $response->comarch_token->ReturnObject ?? $response;
        // Balance - Você possui ... pontos
        $this->SetBalance(
            $returnObject->RedeemablePoints
            ?? $response->programInfo->redeemable->points
            ?? $this->http->FindSingleNode('//p[normalize-space(text()) = "Pontos Azul" or normalize-space(text()) = "Saldo de Pontos"]/following-sibling::p[1]')
        );
        // Name
        $this->SetProperty("Name", beautifulName(
            ($returnObject->Name ?? $response->name->first) . " " .
            ($returnObject->LastName ?? $response->name->last)
        ));
        // Status
        $this->SetProperty("Status",
            $returnObject->LoyaltyLevel
            ?? $response->programInfo->name
            ?? $this->http->FindSingleNode('//p[contains(text(), "Você é")]/b')
        );
        ...
        $this->SetProperty("AccountNumber",
            ...
            ?? $this->http->FindPreg("/\{\"Id\":\"([^\"]+)/", false, $this->http->getCookieByName("ComarchToken", ".voeazul.com.br"))
        );
        // Qualifying points - Pontos qualificáveis
        $this->SetProperty("CreditQualifying",
            $returnObject->QualifyingPoints
            ?? $response->programInfo->qualifyingPoints
            ?? $response->programInfo->flights
            ?? $this->http->FindSingleNode('//p[contains(text(), "Pontos Qualificáveis")]/following-sibling::p[1]')
        );
        // Trechos
        $this->SetProperty("QualifyingFlights",
            $response->programInfo->flights
            ?? $this->http->FindSingleNode('//p[contains(text(), "Trechos")]/following-sibling::p[1]')
        );
        // Saldo Azul
        $this->SetProperty("CashBalance", $this->http->FindSingleNode('//p[contains(text(), "Saldo Azul") or contains(text(), "Crédito Azul")]/following-sibling::p[1]'));
    ...
    private function getSensorDataFromSelenium()
    {
        ...
        try {
            ...
            try {
                ...
                if ($this->http->FindSingleNode('//h1[contains(text(), "Access Denied")] | //body[contains(text(), "Im a teaPot")]')) {
...
//                $selenium->driver->executeScript('try { window.stop(); } catch (e) {}');
            ...
            $loginBtn = $selenium->waitForElement(WebDriverBy::xpath('//div/div/div/button[@aria-label="Log in" or @aria-label="Login"] | //button[contains(., "Login")]'), 15);
            $this->savePageToLogs($selenium);
            ...
            $login = $selenium->waitForElement(WebDriverBy::xpath('//input[@data-test-id="modal-login-document" or @aria-label="Informe o CPF ou número Azul" or @aria-label="CPF ou Nº Azul"]'), 10);
            $pwd = $selenium->waitForElement(WebDriverBy::xpath('//input[@data-test-id="modal-login-password" or @aria-label="Senha"]'), 0);
...
//            $pwd->sendKeys($this->AccountFields['Pass']);
            ...
            $btn = $selenium->waitForElement(WebDriverBy::xpath('//button[@data-test-id="login-submit-button"] | //button[contains(text(), "Entrar") and not(@disabled)]'), 0);
...
//                $selenium->driver->executeScript('try { window.stop(); } catch (e) {}');
            ...
            $this->savePageToLogs($selenium);
            $name = $selenium->waitForElement(WebDriverBy::xpath('//p[contains(@class, "name__text") or contains(@class, "gnedik") and not(contains(text(), "Login"))]'), 25) ?? $this->http->FindNodes('//p[contains(@class, "name__text") or contains(@class, "gnedik")]');
            $this->logger->debug(var_export($this->http->FindNodes('//p[contains(@class, "name__text") or contains(@class, "gnedik") and not(contains(text(), "Login"))]'), true), ['pre' => true]);
...
//                $selenium->http->GetURL("https://www.voeazul.com.br/perfil/br/pt/home");
                $selenium->waitForElement(WebDriverBy::xpath('//p[contains(text(), "Pontos Qualificáveis") or normalize-space(text()) = "Saldo de Pontos"]/following-sibling::p[1] | //h2[contains(text(), "Parece que tivemos um problema para carregar a página")]'), 40);
