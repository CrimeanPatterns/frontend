...
//            $selenium->useCache();
            ...
            $logIn = $selenium->waitForElement(WebDriverBy::xpath("//button[contains(@class, 'hub_menu_ui__menu_button')]/p[contains(., 'Log in')]"), 5);
...
//            }, 180);
            ...
            if ($loginSuccess && $this->http->FindSingleNode("//div[contains(@class, 'AccountActivityCard_userId')]")) {
                $selenium->http->GetURL("https://www.lifemiles.com/account/elite/benefits");
                $selenium->waitForElement(WebDriverBy::xpath('//div[@class="account-ui-VipBenefits_entry"]/div[contains(text(), "entries")]'), 20);
                $this->savePageToLogs($selenium);

                $aviancaLoungesEntries = $this->http->FindSingleNode('//div[@class="account-ui-VipBenefits_entry"]/div[contains(text(), "entries")]/following-sibling::div');
                $cabinUpgrade = $this->http->FindSingleNode('//div[div[div[div[contains(text(), "Cabin upgrade")]]]]//div[contains(text(), "available")]/following-sibling::div');
                $premiumSeats = $this->http->FindSingleNode('//div[div[div[div[contains(text(), "Premium seats")]]]]//div[contains(text(), "available")]/following-sibling::div');
                $this->logger->debug('aviancaLoungesEntries: ' . $aviancaLoungesEntries);
                $this->logger->debug('cabinUpgrade: ' . $cabinUpgrade);
                $this->logger->debug('premiumSeats: ' . $premiumSeats);

                if (isset($aviancaLoungesEntries) && $aviancaLoungesEntries > 0) {
                    $this->AddSubAccount([
                        'Code'        => 'aviancaLoungesEntries',
                        'DisplayName' => "Avianca lounges entries",
                        'Balance'     => $aviancaLoungesEntries,
                    ]);
                }

                if (isset($cabinUpgrade) && $cabinUpgrade > 0) {
                    $this->AddSubAccount([
                        'Code'        => 'cabinUpgrade',
                        'DisplayName' => "Cabin upgrade",
                        'Balance'     => $cabinUpgrade,
                    ]);
                }
