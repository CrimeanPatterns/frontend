...
class LifemilesAwardTicketRedemption extends \TAccountChecker
{
    ...
    private function flight(Email $email)
    {
        ...
        if (empty($account)) {
            $account = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Status:'))}]/preceding::text()[normalize-space()][1]", null, true, "#(?:^|\:)\s*(\d{5,})\s*$#");
        }
        ...
        $status = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Status:'))}]", null, true, "#{$this->opt($this->t('Status:'))}\s*(\w+)$#");

        if (!empty($status) && !empty($account)) {
            $st = $email->add()->statement();
            $st->setNumber($account);
            $st->addProperty('EliteStatus', $status);
        ...
        foreach ($nodes as $root) {
            ...
            // Airline
            $s->airline()
                ->name($this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][not(contains(normalize-space(),'lifemiles'))][1]/td[normalize-space()][last()]", $root, true, "#^\s*([A-Z][A-Z\d]|[A-Z\d][A-Z])\d{1,5}#"))
                ->number($this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][not(contains(normalize-space(),'lifemiles'))][1]/td[normalize-space()][last()]", $root, true, "#^\s*(?:[A-Z][A-Z\d]|[A-Z\d][A-Z])(\d{1,5})\b#"));
