...
class ParserSMR extends \TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        if ($message = $this->http->FindSingleNode("//p[contains(., 'Actualmente, nuestro sitio web y app están temporalmente fuera de servicio debido a un mantenimiento programado')] | //h1[contains(., '¡Estamos trabajando para ti!')]")) {
            throw new \CheckException($message, ACCOUNT_PROVIDER_ERROR);
        }

        if (!$this->waitForElement(\WebDriverBy::xpath($resXpath),
                40)) {
        ...
        if ($this->waitForElement(\WebDriverBy::xpath('//p[contains(@class, "hub_overview_ui__milesBold")]'), 5)) {
...
//        $this->mover->click();
    ...
    public function Login()
    {
        ...
        $loginSuccessXpath = "
            //div[contains(@class, 'AccountActivityCard_userId')]
            | //span[contains(@class, 'hub_menu_ui__username')]
    ...
    public function ParseCalendar(array $fields): array
    {
        ...
        $this->logger->info("Parse Calendar Availability", ['Header' => 2]);
        ...
        $calendar = $this->searchFormForCalendar($fields);

        return ['fares' => $this->parseRewardCalendarJson($calendar, $fields)];
    }
    ...
    public function ParseRewardAvailability(array $fields)
    {
        ...
        if (empty($this->bearer)) {
            if (!$fields['ParseCalendar']) {
                $this->waitForElement(\WebDriverBy::xpath('//p[contains(@class, "hub_overview_ui__milesBold")]'),
                    10);
            }
            $this->saveResponse();
...
//        // no range
    ...
    private function getIP(): void
    {
        ...
        $browser->GetURL('https://ipinfo.io/ip');
        $ip = $browser->Response['body'];
        $ip = $browser->FindPreg("/(\d+\.\d+\.\d+\.\d+)/", false, $ip);
    ...
    private function getXHRPost($url, array $headers, $payload)
    {
        ...
        try {
            return $this->driver->executeScript($script);
        } catch (\WebDriverCurlException | \UnexpectedJavascriptException | \WebDriverException $e) {
            $this->logger->error($e->getMessage());
...
//                    $conn = $this->getConnections($segments, $itOffers);
    ...
    private function ParseRewardAirlines($fields, $idCoti, $schHcfltrc, $searchTypePrioritize)
    {
        ...
        foreach ($validAirlines as $validAirline) {
            $this->logger->critical('searchTypePrioritize: ' . $searchTypePrioritize);
            $res = $this->findFlight($fields, $searchTypePrioritize, $validAirline, $headers, $idCoti, $schHcfltrc);
    ...
    private function findFlight($fields, $typeSearch, $validAirline, $headers, $idCoti, $schHcfltrc)
    {
        $this->logger->notice(__METHOD__);

        if ($validAirline == "SSA") {
            $payload = '{"internationalization":{"language":"en","country":"us","currency":"usd"},"currencies":[{"currency":"USD","decimal":2,"rateUsd":1}],"passengers":' . $fields['Adults'] . ',"od":{"orig":"' . $fields['DepCode'] . '","dest":"' . $fields['ArrCode'] . '","departingCity":"' . $fields['DepCity'] . '","arrivalCity":"' . $fields['ArrCity'] . '","depDate":"' . $fields['DepDate'] . '","depTime":""},"filter":false,"codPromo":null,"idCoti":"' . $idCoti . '","officeId":"","ftNum":"","discounts":[],"promotionCodes":["DBEP21"],"context":"D","ipAddress":"' . $this->ip . '","channel":"COM","cabin":"' . $fields['Cabin'] . '","itinerary":"OW","odNum":1,"usdTaxValue":"0","getQuickSummary":false,"ods":"","searchType":"SSA","searchTypePrioritized":"SSA","sch":{"schHcfltrc":"' . $schHcfltrc . '"},"posCountry":"US","odAp":[{"org":"' . $fields['DepCode'] . '","dest":"' . $fields['ArrCode'] . '","cabin":' . $fields['Cabin'] . '}],"suscriptionPaymentStatus":""}';
        } else {
            $payload = '{"internationalization":{"language":"en","country":"us","currency":"usd"},"currencies":[{"currency":"USD","decimal":2,"rateUsd":1}],"passengers":' . $fields['Adults'] . ',"od":{"orig":"' . $fields['DepCode'] . '","dest":"' . $fields['ArrCode'] . '","departingCity":"' . $fields['DepCity'] . '","arrivalCity":"' . $fields['ArrCity'] . '","depDate":"' . $fields['DepDate'] . '","depTime":""},"filter":false,"codPromo":null,"idCoti":"' . $idCoti . '","officeId":"","ftNum":"","discounts":[],"promotionCodes":["DBEP21"],"context":"D","ipAddress":"' . $this->ip . '","channel":"COM","cabin":"' . $fields['Cabin'] . '","itinerary":"OW","odNum":1,"usdTaxValue":"0","getQuickSummary":false,"ods":"","searchType":"' . $validAirline . '","searchTypePrioritized":"' . $typeSearch . '","sch":{"schHcfltrc":"' . $schHcfltrc . '"},"posCountry":"US","odAp":[{"org":"' . $fields['DepCode'] . '","dest":"' . $fields['ArrCode'] . '","cabin":' . $fields['Cabin'] . '}],"suscriptionPaymentStatus":""}';
        }
    ...
    private function searchFormForCalendar($fields)
    {
        $this->logger->notice(__METHOD__);

        try {
            $this->http->GetURL('https://www.lifemiles.com/fly/find');
        } catch (\Exception | \UnknownServerException  $e) {
            $this->logger->error($e->getMessage());

            throw new \CheckRetryNeededException(3, 0);
        }

        $oneWayBtn = $this->waitForElement(\WebDriverBy::xpath('//button[contains(., "One-Way")]'), 20);
        $from = $this->waitForElement(\WebDriverBy::xpath('//input[@id="ar_bkr_fromairport"]'), 0);
        $to = $this->waitForElement(\WebDriverBy::xpath('//input[@id="ar_bkr_toairport"]'), 0);

        if (!$oneWayBtn || !$from || !$to) {
            $this->logger->error('No load page');
        ...
        try {
            ...
            $down = $this->waitForElement(\WebDriverBy::xpath('//p[@class="waru-AirportsDropdown_airportsDropdownCode"]'),
                5);

            if (!$down && $this->waitForElement(\WebDriverBy::xpath('//p[@class="waru-MetropolisAirportsDropdown_metropolisName"]'), 1)) {
                $this->SetWarning('Only airports Code');
        ...
        $dateInput = $this->waitForElement(\WebDriverBy::xpath('//button[contains(., "Departure")]'), 0);
        $dateInput->click();
        $this->saveResponse();

        $m = date('M', $fields['DepDate']);
        $y = date('Y', $fields['DepDate']);
        $d = (int) date('d', $fields['DepDate']);

        while (!$this->waitForElement(\WebDriverBy::xpath("//div[@class='waru-Month_wrapper']/div[contains(., '{$y}') and contains(.,'{$m}')]"), 10)) {
            $this->waitForElement(\WebDriverBy::xpath("//div[@class='waru-Month_imgWrapper']/img[@alt='next month']"), 5)->click();
            $this->saveResponse();
        }

        $this->saveResponse();

        $this->waitForElement(\WebDriverBy::xpath("(//div[@class='waru-Month_wrapper'][contains(., '{$m}') and contains(.,'{$y}')]//div[@class='waru-Day_dayNumberNrm' and contains(., '{$d}')])[1]"), 5)->click();
        $this->saveResponse();
        $this->waitForElement(\WebDriverBy::xpath('//button[@class="waru-BookerContent_selectingButton"][contains(.,"Adult")]'), 0)->click();

        while ($this->waitForElement(\WebDriverBy::xpath('(//p[@class="waru-TravelersDropdown_travelersPassengersCount"])[1]'), 10)->getText() != $fields['Adults']) {
            $this->saveResponse();
            $this->waitForElement(\WebDriverBy::xpath('(//button[@class="waru-TravelersDropdown_travelersPassengersIncrement"])[1]'), 0)->click();
        }

        $this->waitForElement(\WebDriverBy::xpath('//div[@class="waru-TravelersDropdown_buttonTravels"]'), 0)->click();
        $this->waitForElement(\WebDriverBy::xpath('//button[@class="waru-BookerContent_bookerActionButtonLarge"]'), 0)->click();

        if ($viewDate = $this->waitForElement(\WebDriverBy::xpath('//button[@class="waru-AirRedemptionFlightPicker_buttonFindMore"]'), 20)) {
            $viewDate->click();
        }

        if (!$this->waitForElement(\WebDriverBy::xpath('(//div[@class="waru-FlightCarousel_wrapper"]/button[contains(@class, "FlightCarousel_sevenDaysDay")]//span[@class="waru-FlightCarousel_price"])[1][contains(., "From") or contains(.,"Not available") ]'), 20)) {
            $this->saveResponse();
        ...
        do {
            sleep(5);
            $this->saveResponse();
            $days = $this->http->FindNodes('//div[@class="waru-FlightCarousel_wrapper"]/button[contains(@class, "FlightCarousel_sevenDaysDay")][contains(., "From") or contains(.,"Not available")]');
        } while (count($days) < 5);

        $this->logger->debug(var_export($days, true), ['pre' => true]);
    ...
    private function parseRewardCalendarJson($data, $fields): array
    {
        $result = [];
        $tic = round(0 - (count($data) / 2), 0, PHP_ROUND_HALF_DOWN);

        foreach ($data as $num => $day) {
            if (strpos($day, 'Not available') !== false) {
                $tic++;

                continue;
            }

            $date = date('Y-m-d', strtotime("+{$tic} day", $fields['DepDate']));
            preg_match("/\d{1,3}(?:,\d{3})/", $day, $matches);
            $this->logger->debug(var_export($matches, true), ['pre' => true]);

            $result[] = [
                'date'             => $date,
                'redemptions'      => ['miles' => ucwords(strtolower(str_replace(',', '', $matches[0])))],
                'payments'         => [
                    'currency'     => 'USD',
                    'taxes'        => null,
                    'fees'         => null,
        ...
        if (empty($result) && $this->ErrorCode !== 9) {
            $this->SetWarning('There are no flights for the 7 days');
        }
