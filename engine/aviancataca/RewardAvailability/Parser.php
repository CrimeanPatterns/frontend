...
class Parser extends \TAccountChecker
{
    ...
    public static function GetAccountChecker($accountInfo)
    {
        ...
        if (self::$isNew) {
//        if ($debugMode) {
            require_once __DIR__ . "/ParserSMR.php";

            return new ParserSMR();
        }
    ...
    public function LoadLoginForm()
    {
        ...
        if (!$this->waitForElement(\WebDriverBy::xpath("//div[@class='homepage-ui-Homepage_landingJoin']//button[1][@class='homepage-ui-Homepage_landingLoginButton']/span[contains(., 'Log in')]"), 40)) {
            $this->logger->error("Page not loading");

            throw new \CheckRetryNeededException(5, 0);
        }

        if ($cookieBtn = $this->waitForElement(\WebDriverBy::xpath("//button[contains(@class, 'CookiesBrowserAlert_acceptButtonNO')]"), 5)) {
            sleep(2);
            $this->mover->moveToElement($cookieBtn);
            $this->mover->click();

            if ($this->waitForElement(\WebDriverBy::xpath("//button[contains(@class, 'CookiesBrowserAlert_acceptButtonNO')]"), 2)) {
        ...
        $logIn = $this->waitForElement(\WebDriverBy::xpath("//div[@class='homepage-ui-Homepage_landingJoin']//button[1][@class='homepage-ui-Homepage_landingLoginButton']/span[contains(., 'Log in')]"), 0);
...
//                $loginWithUsername->click();
        ...
        */
        ...
        if (!$login || !$pass) {
            ...
            if (!$this->waitForElement(\WebDriverBy::xpath("//p[contains(text(), 'our activity and behavior on this site made us think that you are a bot')] | //p[contains(text(), 'activity and behavior on our site made us think you could be a robot')]"),
                0)) {
                $this->http->GetURL("https://www.lifemiles.com");
            }
            $this->saveResponse();
...
//        $login->sendKeys($this->AccountFields['Login']);
        $this->mover->sendKeys($login, $this->AccountFields['Login'], 30);
...
//        $pass->sendKeys($this->AccountFields['Pass']);
        $this->mover->sendKeys($pass, $this->AccountFields['Pass'], 30);
        ...
        if ($this->waitForElement(\WebDriverBy::xpath("//div[@id = 'sec-container']"), 10)) {
            $this->saveResponse();
        ...
        // may be too long loading
        if ($conditions) {
            $loginSuccess = $this->waitForElement(\WebDriverBy::xpath($loginSuccessXpath), 60);
        }
        $this->saveResponse();
    ...
    public function ParseRewardAvailability(array $fields)
    {
        ...
        try {
            ...
            //"{\"cabin\":\"{$fields['Cabin']}\",\"ftNum\":\"\",\"internationalization\":{\"language\":\"en\",\"country\":\"co\",\"currency\":\"usd\"},\"itineraryName\":\"One-Way\",\"itineraryType\":\"OW\",\"numOd\":1,\"ods\":[{\"id\":1,\"origin\":{\"cityName\":\"Armenia\",\"cityCode\":\"AXM\"},\"destination\":{\"cityName\":\"Washington\",\"cityCode\":\"IAD\"}}],\"paxNum\":1,\"selectedSearchType\":\"SMR\"}";
            $headers = [
