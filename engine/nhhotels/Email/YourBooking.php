...
{
    public $mailFiles = "nhhotels/it-12623907.eml, nhhotels/it-33841933.eml, nhhotels/it-33893965.eml, nhhotels/it-35196921.eml, nhhotels/it-47909176.eml, nhhotels/it-48480570.eml, nhhotels/it-54191532.eml, nhhotels/it-57037871.eml, nhhotels/it-62049852.eml, nhhotels/it-8228739.eml";

    public $reBody = [
        'en'   => ['Reservation number', 'Room'],
        'en2'  => ['Reservation number', 'Booker'],
        'en3'  => ['Booking Details', 'Occupancy:'],
        'en4'  => ['Booking details', 'Your Details'],
    ...
    public static $dict = [
        'en' => [
            ...
            'Price'     => ['Price', 'Room', 'Rooms'],
            //			'VAT' => '',
            'Final price' => ['Final price', 'Total price'],
            'Guests'      => ['Guests', 'Guest details', 'Guest'],
            //			'Rate includes:' => '',
...
//            return null;
    ...
    private function parseEmail(Email $email, $subject)
    {
        ...
        if (empty($nodes)) {
            $nodes = array_filter($this->http->FindNodes("//text()[{$this->eq($this->t('Booking Details'))}]/following::text()[{$this->eq($this->t('Occupancy:'))}][1]/ancestor::tr[1]/following-sibling::tr[1]"));
        }
        ...
        if ($types) {
            ...
            ?? $this->http->FindSingleNode("//text()[{$this->eq($this->t('Rooms'))}]/ancestor::tr[1][{$this->eq($this->t('Rooms'))}]/following-sibling::tr[1]")) {
            $r = $h->addRoom();
            $r->setType($type);
        ...
        if ($guestRows->length === 0) {
            $guestRows = $this->http->XPath->query("//text()[{$this->starts($this->t('Guests'))}]/ancestor::span[1]/following-sibling::strong[normalize-space()]");
        }
    ...
    private function detectDeadLine(\AwardWallet\Schema\Parser\Common\Hotel $h, string $cancellationText)
    {
        ...
        if (preg_match('/Ihre Reservierung kann kostenfrei bis (\d{1,2}\-\w+\-\d{2,4})\s+(\d{1,2}:\d{2})(?::\d{2})?\s*storniert werden/i', $cancellationText, $m) //de
            ...
            || preg_match('/Your reservation can be modified or cancelled free of charge until (\d{4}\-\d+\-\d{1,2}) (\d{1,2}:\d{2})\:/', $cancellationText, $m) //en
