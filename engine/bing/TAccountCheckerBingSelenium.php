...
class TAccountCheckerBingSelenium extends TAccountChecker
{
    ...
    public $verifyEmail = 'Please enter your full email address ';
    ...
    private const XPATH_VERIFY_BUTTON = '//input[@id = "idSubmit_SAOTCC_Continue" or @id = "iVerifyIdentityAction" or @aria-label="Verify"] | //button[@aria-label="Verify"] | //button[contains(text(), "Next")]';
    ...
    public function LoadLoginForm()
    {
        ...
        $login = $this->waitForElement(WebDriverBy::xpath('//input[@name = "loginfmt" or @id = "usernameEntry"]'), 20);
        ...
        try {
            if ($next = $this->waitForElement(WebDriverBy::xpath('//button[@data-testid="primaryButton" and contains(text(), "Next")]'), 0)) {
                $next->click();
            } else
        ...
        if ($this->waitForElement(WebDriverBy::xpath('//div[
                ...
                or contains(., "We\'ll send a code to ")
            ]
            | //span[contains(text(), "Select this number in the sign-in request on you")]
            | //h1[contains(text(), "We didn\'t get an approval")]
        '), 4)
            && ($usePassword = $this->waitForElement(WebDriverBy::xpath('//*[
                @id = "idA_PWD_SwitchToPassword"
                or @id = "idA_PWD_SwitchToCredPicker"
            ]
            | //span[contains(text(), "get a code in a different way") or contains(text(), "Other ways to sign in")]
            '), 0))
//                or @id = "idA_PWD_SwitchToFido"
            ...
            if ($usePassword2 = $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "Use my password")] | //span[contains(text(), "Use your password")]'), 3)) {
                $usePassword2->click();
                $this->saveResponse();
            }
        }

        $passwordInput = $this->waitForElement(WebDriverBy::xpath('//input[@name = "passwd"]'), 16);
        $this->saveResponse();
        ...
        if (!$passwordInput) {
            ...
            if ($appApproving = $this->waitForElement(WebDriverBy::xpath('//div[contains(., "Approve a request on my Microsoft Authenticator app") or contains(., "Approve a request using my Microsoft app") or contains(., "Check your Authenticator app") or contains(., "Check your OutlookDev app") or contains(., "Check your Outlook app")] | //span[contains(text(), "Approve sign-in with a mobile app")]'), 0)) {
                $appApproving->click();
                sleep(3);
                $this->saveResponse();

                $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "We sent a sign in request to your ") or contains(text(), "We sent a sign-in request to ")] | //span[contains(text(), "We sent a sign-in request to your")]'), 20);
                $this->saveResponse();

                if ($message = $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "We sent a sign-in request to ")] | //span[contains(text(), "We sent a sign-in request to your")]'), 0)) {
        ...
        try {
            ...
            if (
                $error = $this->waitForElement(WebDriverBy::xpath('
                    ...
                    | //span[contains(text(), "That password is incorrect for your Microsoft account.")]
                    | //span[contains(text(), "We couldn\'t find a Microsoft account")]
                    | //span[contains(text(), "This username has been turned-off for sign in.")]
                    | //span[contains(text(), "That doesn\'t match the alternate email associated with your account.")]
                '), 0)
            ) {
    ...
    public function Login()
    {
        ...
        if ($nextBtn = $this->waitForElement(WebDriverBy::xpath("//button[contains(text(), 'Next')]"), 0)) {
            $nextBtn->click();
            sleep(5);
            $this->saveResponse();
        }
        ...
        if ($this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "Your data will be processed outside of your country or region")]'), 0)) {
        ...
        if ($this->waitForElement(WebDriverBy::xpath('
                    ...
                    | //h1[contains(text(), "Approve sign-in request")]
                    ...
                    | //h1[contains(text(), "We didn\'t get an approval")]
                    | //span[contains(text(), "Just one more step to verify it’s you.")]
            '), 5)
        ) {
            $this->logger->debug("Click 'Having trouble?'");

            if ($havingTroubleLink = $this->waitForElement(WebDriverBy::xpath('//a[@id = "iHavingTroubleLink"] | //a[@id = "idA_SAOTCAS_Trouble"] | //a[@id = "signInAnotherWay"] | //span[contains(text(), "get a code in a different way") or contains(text(), "Other ways to sign in")]'), 0)) {
        ...
        if ($this->waitForElement(WebDriverBy::xpath('
                ...
                | //span[contains(text(), "Send a code to ") and contains(text(), "@")]
            '), 0)
        ) {
            $emailText = $this->waitForElement(WebDriverBy::xpath("(//*[self::div or self::span][contains(text(), 'Email')])[1] | //span[contains(text(), 'Send a code to ') and contains(text(), '@')]/../../.."), 0);
            ...
            $email = $this->http->FindPreg("/(?:Email\s*|Send a code to )(.+)/", false, $emailText->getText());
            $this->logger->debug('Click "Send a code to ' . $email . '"');
            $emailText->click();

            // We will send a verification code to ja*****@pyleiton.net. To verify that this is your email address, enter it below.
            // We'll send a verification code to ja*****@pyleiton.net. To verify this is your email address, enter it here.
            if ($questionText = $this->waitForElement(WebDriverBy::xpath('
                //div[contains(text(), "We will send a verification code to ")]
                | //span[contains(text(), "ll send a verification code to ")]
            '), 10)
            ) {
                $email = $this->http->FindPreg('/code to (.+?)\. To verify/', false, $questionText->getText());
                $this->logger->debug("Email -> {$email}");

                if ($email) {
                    $this->holdSession();
                    $this->logger->debug("Question -> " . $this->verifyEmail . $email);

                    $this->AskQuestion($this->verifyEmail . $email, null, 'verifyEmail');
                }
        ...
        if ($this->waitForElement(WebDriverBy::xpath('//div[@id = "idDiv_SAOTCC_Description" and contains(., "Please type in the code displayed on your authenticator app from your device")] | //h1[contains(text(), "Enter the code generated by your authenticator app")]'), 0)) {
            $this->holdSession();
        ...
        if ($error = $this->http->FindPreg('#(You\s+need\s+to\s+use\s+a\s+security\s+code\s+to\s+verify\s+your\s+identity)\.\s+How\s+would\s+you\s+like\s+to\s+receive\s+your\s+code\?#i')) {
            ...
            || $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "Your Skype account is now a Microsoft account")]'), 0)
        ...
        if ($error = $this->waitForElement(WebDriverBy::xpath('//div[
                ...
                or contains(text(), "We sent a sign-in request to ")
                or contains(text(), "Approve sign-in request")
            ]
            | //span[contains(text(), "We sent a sign-in request to your")]
            | //h1[contains(text(), "Approve sign-in request")]
        '), 0)) {
...
//            $this->driver->executeScript('if (document.querySelector("input[id = '.$id.']")) document.querySelector("input[id = '.$id.']").value = "'.$variantValue.'";');
    ...
    public function twoStepAuth($providerMessage = null)
    {
        ...
        if (isset($this->Answers[$this->twoStepAuthQuestion])) {
            ...
            $input = $this->waitForElement(WebDriverBy::xpath('//input[@id = "iOttText" or @id = "idTxtBx_SAOTCC_OTC" or @id = "otc-confirmation-input"] | //input[contains(@aria-describedby, "__validationMessage")]'), 0);
...
//        if ($this->isNewSession())
    ...
    private function verifyEmail() {
        $questionText = $this->waitForElement(WebDriverBy::xpath('
                //div[contains(text(), "We will send a verification code to ")]
                | //span[contains(text(), "ll send a verification code to ")]
            '), 10);
        ...
        $question = $this->verifyEmail . $email;
        if (!isset($this->Answers[$question])) {
            $this->logger->error('Not found answer -> ' . $question);
            return false;
        }
        $answer = $this->Answers[$question];
        unset($this->Answers[$question]);

        $input = $this->waitForElement(WebDriverBy::xpath('//input[contains(@id,"proof-confirmation")]'), 3);
        $input->sendKeys($answer);

        $button = $this->waitForElement(WebDriverBy::xpath('//button[@data-testid="primaryButton" or @aria-label="Send code"]'), 3);
        $button->click();


        // If jason@pyleiton.net matches the email address on your account, we'll send you a code.
        $result = $this->waitForElement(WebDriverBy::xpath('//*[contains(text(),"matches the email address on your account, we")]'), 10);
        $this->saveResponse();
        if (isset($result) && stristr($result->getText(),'matches the email address on your account')) {
            $this->holdSession();
            $this->AskQuestion($result->getText(), null, 'verifyEmailCode');
            return false;
        }
        return true;
    }
    ...
    private function verifyEmailCode() {
        $this->saveResponse();
        $question = $this->waitForElement(WebDriverBy::xpath('//*[contains(text(),"matches the email address on your account, we")]'), 3);
        if (!$question) {
            $this->saveResponse();
            return false;
        }
        ...
        $this->logger->notice('Question -> ' . $question);



        $answer = $this->Answers[$question];
        unset($this->Answers[$question]);
        $this->logger->notice('Answer -> ' . $answer);

        $input = $this->waitForElement(WebDriverBy::xpath('//input[@id="otc-confirmation-input"]'), 0);
        if ($input) {
            $input->sendKeys($answer);
            $btn = $this->waitForElement(WebDriverBy::xpath('//button[@id="oneTimeCodePrimaryButton"]'), 0);
            $btn->click();
        } else {
            for ($i = 0; $i < strlen($answer) && $i < 6; $i++) {
                $securityAnswer = $this->waitForElement(WebDriverBy::xpath("//input[
                @id = 'codeEntry-$i'
            ]"), 0);

                $securityAnswer->clear();
                $securityAnswer->sendKeys($answer[$i]);
                usleep(100);
            }
        }


        $result = $this->waitForElement(WebDriverBy::xpath('//span[contains(text(),"t work. Check the code and try again.")]'), 10);
        // That code didn't work. Check the code and try again.
        if (isset($result) && stristr($result->getText(),"'That code didn't work. Check the code and try again.'")) {
            $this->saveResponse();
            $this->AskQuestion($answer, $result->getText(), 'verifyEmailCode');
        }
        $this->saveResponse();
        return true;
    }
