...
{
    public $mailFiles = "checkmytrip/it-372164012-fr.eml, checkmytrip/it-376473964.eml, checkmytrip/it-380825922.eml, checkmytrip/it-384586391.eml, checkmytrip/it-385152794-pt.eml, checkmytrip/it-862229939-de.eml, checkmytrip/it-928365384.eml";
    ...
    public function detectEmailByBody(\PlancakeEmailParser $parser)
    {
        ...
        $this->assignLang();
        return $this->findTripSegments()->length > 0;
    }
    ...
    private function findTripSegments(): \DOMNodeList
    {
    ...
    {
        $types = [];
        $tripSegments = $this->findTripSegments();

        $transferAddFlightNI = null;

        foreach ($tripSegments as $ni => $root) {
        ...
        $xpathT = "//tr[ *[1][{$this->eq($this->t('Traveler'))}] and *[2][{$this->eq($this->t('Agency Information'))}] ]/following::tr[normalize-space()][1]";
        $travellersAll = $this->http->FindNodes($xpathT . "/*[1]/descendant::text()[normalize-space()]", null, "/^[.\d\s]*(.*)$/");

        if (count($travellersAll) === 0) { // it-928365384.eml
            $travellersAll = $this->http->FindNodes("//tr[ *[1][{$this->eq($this->t('Traveler'))}] and *[2] ]/following::tr[normalize-space()][1][starts-with(normalize-space(),'1')]/*[1]/descendant::text()[normalize-space()]", null, "/^[.\d\s]*(.*)$/");
        }
    ...
    private function parseFlight(Email $email, $root): void
    {
        foreach ($email->getItineraries() as $it) {
            if ($it->getType() == 'flight') {
                /** @var Flight $it */
        ...
        if (!isset($f)) {
            ...
            $accounts = [];
            $ffNumberValues = $this->http->FindNodes("//text()[{$this->starts($this->t('Frequent Flyer number'))}]/ancestor::td[1]", null, "/{$this->opt($this->t('Frequent Flyer number'))}[\s:]*([A-Z\d]{7,}(?:\s*,\s*[A-Z\d]{7,})*)$/u");

            foreach ($ffNumberValues as $ffNumVal) {
                $accounts = array_merge($accounts, preg_split('/(?:\s*,\s*)+/', $ffNumVal));
            }
    ...
    private function parseTrain(Email $email, $root): void
    {
        foreach ($email->getItineraries() as $it) {
            if ($it->getType() == 'train') {
                /** @var Train $it */
    ...
    private function parseRental(Email $email, $root): void
    {
        $r = $email->add()->rental();
    ...
    private function getParam($field, $root = null, $regexp = '(.+)'): ?string
    {
    ...
    private function assignLang(): bool
    {
        ...
        foreach (self::$dictionary as $lang => $phrases) {
            ...
            if (!empty($phrases['Your trip']) && $this->http->XPath->query("//*[{$this->eq($phrases['Your trip'])}]")->length > 0
                || !empty($phrases['Airline Booking Reference']) && $this->http->XPath->query("//text()[{$this->starts($phrases['Airline Booking Reference'])}]")->length > 0
            ) {
                $this->lang = $lang;
                return true;
            }
    ...
    private function normalizeDateRelative($date, $relativeDate): ?int
    {
        ...
        if (preg_match('/^\d{1,2}\s+([[:alpha:]]+)(?:\s+\d{4}\b|$)/u', $date, $m)) {
            if ($en = MonthTranslate::translate($m[1], $this->lang)) {
                $date = str_replace($m[1], $en, $date);
            }
        }
