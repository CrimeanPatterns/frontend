...
class PDF extends \TAccountChecker
{
    ...
    public static $dict = [
        ...
        'zh' => [
            ...
            'for'                          => ['對於'],
            "Meal"                         => ['機上餐點'],
            "Airline Booking Reference(s)" => "訂位代號",
            // 'Total Amount' => '',
            // HOTEL
            // "Confirmation number" => "",
            // "Check-in" => "",
    ...
    public function ParsePlanEmailLocal(\PlancakeEmailParser $parser, Email $email): void
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (stripos($pdfBody, 'Scan for check-in. Not to be used') !== false) {
                // it-12287505.eml
                $pdfBody = preg_replace(['/[ ]{2,}Scan for check-in\. Not to be used[- ]*$/im', '/[ ]{2,}as boarding pass[.?! ]*$/im'], '', $pdfBody);
                $pdfBody = preg_replace('/^[ ]{50,}[A-Z](?: ?[-A-Z])+\n+[ ]{50,}(?i)Scan for check-in\. Not to be used as boarding pass[.?! ]*$/m', '', $pdfBody);
                $pdfBody = preg_replace('/(\n)(?:\n+[ ]+Scan for check-in\. Not to be used as boarding pass[.?! ]*\n+)/i', '$1', $pdfBody);
            }

            if (stripos($pdfBody, 'Scan for check-in. Not to be used') !== false) {
                $pdfBody = preg_replace('/\n+[ ]*Scan for check-in\. Not to be used as boarding pass[.?! ]*/i', '', $pdfBody);
            }
...
//                                            Free Cancellation Till 6.00Pm 17 N
    ...
    private function parseEmailTransfer(Email $email, string $text, ?string $textPrev, ?string $textNext): void
    {
        ...
        $patterns['time'] = '\d{1,2}[:：]\d{2}(?:[ ]*[AaPp](?:\.[ ]*)?[Mm]\.?)?'; // 4:19PM  |  2:00 p. m.
        ...
        $locationByMisc = preg_replace([
            "/\s+\d{6,}(?:\s+(?:Pu|Do) ?{$patterns['time']})?(?:\s*\*{2}[\s\S]+|\s+Rqst\s+For\s+[\s\S]*)*$/i",
            "/\s+/",
            "/[\/.\w]+\s*[ ]*\d{2}:?\d{2} ?Hrs\s*/",
            "/Tel\s*\d*/",
            "/^[[:alpha:]]{1,4}\.[ ]*/u", // Bne.Haymarket Hotel  ->  Haymarket Hotel
        ], ['', ' ', '', '', ''], $misc);

        $timeDep = $this->re("/\bPu[ ]*({$patterns['time']})(?:\s*\*{2}[\s\S]+|\s+Rqst\s+For\s+[\s\S]*)*$/i", $misc);

        if (empty($timeDep) && preg_match("/P\/U\s*(\d{2}):?(\d{2}) ?Hrs/", $misc, $m)) {
            $timeDep = $m[1] . ':' . $m[2];
        }
        $timeArr = $this->re("/\bDo[ ]*({$patterns['time']})(?:\s*\*{2}[\s\S]+|\s+Rqst\s+For\s+[\s\S]*)*$/i", $misc);

        $dateTimeDep = $timeDep !== null ? strtotime($timeDep, $dateDep) : null;
        $dateTimeArr = $timeArr !== null ? strtotime($timeArr, $dateDep) : null;

        if (preg_match("/\bDeparture[ ]+.{3,}\n+(?:.+\n+){0,1}.*\bArrival[ ]+(?<date>.{3,} {$patterns['time']})[ ]+(?<airport>\D+[ ]*,.+\))\s*\(/", $textPrev, $m)) {
        ...
        if (preg_match("/\bDeparture[ ]+(?<date>.{3,} {$patterns['time']})[ ]+(?<airport>\D+[ ]*,.+\))\s*\(.+\n+(?:.+\n+){0,1}.*\bArrival[ ]+.{3,}[ ]+\D+[ ]*,.+\)\s*\(/", $textNext, $m)) {
        ...
        if (stripos($text, 'Chauffeur driven car For Flight Departure') !== false) {
            ...
            if (!$nameDep && $locationByMisc) {
                $nameDep = $locationByMisc;
            }
            ...
            if (!$nameArr && $locationByMisc) {
                $nameArr = $locationByMisc;
            }
        }
        ...
        if ($dateTimeDep) {
            ...
            // mta/it-916812180.eml (no time in Miscellaneous block)
            $s->departure()->noDate();
        }
