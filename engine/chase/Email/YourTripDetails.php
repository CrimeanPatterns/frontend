...
class YourTripDetails extends \TAccountChecker
{
    ...
    private $langDetectors = [
        'en' => ['Flight Confirmation #', 'Hotel Confirmation #', 'Car Confirmation #', 'Check with your airline for the most up-to-date flight', 'Activity Reservation', 'Confirmation#'],
    ];
...
//    ];
    ...
    private $xpath = [
        ...
        'time2' => '(contains(translate(normalize-space(),"0123456789：.Hh","∆∆∆∆∆∆∆∆∆∆::::"),"∆:∆∆") or contains(translate(normalize-space(),"0123456789：.Hh","∆∆∆∆∆∆∆∆∆∆::::"),"∆∆:∆∆"))',
    ];
    ...
    public function parseHtmlFlight2(Email $email, ?array $tripId): void
    {
        $f = $email->add()->flight();

        $confirmation = $this->http->FindSingleNode("//tr[*[normalize-space()][1][{$this->eq($this->t('Flights'))}] ]/following-sibling::tr[normalize-space()][1]/descendant::text()[normalize-space() = 'Reservation code:']/following::text()[normalize-space()][1]", null, false, "/^([A-Z\d]{5,7})$/");
        ...
        $passengers = array_filter($this->http->FindNodes('//text()[' . $this->eq($this->t("Passenger(s):")) . ']/ancestor::*[1]/following-sibling::*[normalize-space()][1]/descendant::text()'));

        if (count($passengers) > 0) {
            foreach (array_unique($passengers) as $passNode){
                if (preg_match("/^({$this->patterns['travellerName']})[ ]*(?:\-[ ]*e\-Ticket Number\:[ ]*([0-9A-Z\-]{5,})|$)/u", $passNode, $matches)){
                    $f->general()->traveller($matches[1], true);

                    if ($matches[2]){
                        $f->issued()->ticket($matches[2], false, $matches[1]);
                    }
                }
            }
        ...
        $xpath = "//tr[./descendant::table[normalize-space()][2][contains(normalize-space(), 'Confirmation#')] and ./descendant::text()[{$this->xpath['time2']}]][not({$this->starts($this->t('Itinerary'))})]";
        $segments = $this->http->XPath->query($xpath);
        ...
        foreach ($segments as $root) {
            ...
            $segConfirmation = $this->http->FindSingleNode("./descendant::table[normalize-space()][2]", $root, true, "/{$this->opt($this->t('Confirmation#'))}[ ]*\:[ ]*([A-Z\d]{5,})$/");

            if ($segConfirmation !== null) {
                $s->airline()->confirmation($segConfirmation);
            }

            $flight = $this->http->FindSingleNode("./descendant::table[normalize-space()][1]", $root);

            if (preg_match('/^.*\,?[ ]*([A-Z][A-Z\d]|[A-Z\d][A-Z])[ ]+(\d+)(?:\,[ ]*OPERATED BY[ ]*(.+)|$)/', $flight, $matches)) {
                $s->airline()->name($matches[1]);
                $s->airline()->number($matches[2]);

                if (isset($matches[3])){
                    $s->airline()->operator($matches[3]);
                }
            }

            $s->departure()
                ->code($this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][1]/descendant::text()[normalize-space()][1]", $root, null, "/^([A-Z]{3})$/"))
                ->name(preg_replace("/^[ ]*(\-)[ ]*/", "", $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][1]/descendant::text()[normalize-space()][2]", $root)))
                ->date(strtotime($this->normalizeDate($this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][1]/descendant::text()[normalize-space()][last()]", $root))));

            $depTerminal = $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][1]/descendant::text()[normalize-space()][3]", $root, false, "/^Terminal[ ](.+)$/");
            ...
            $s->arrival()
                ->code($this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][2]/descendant::text()[normalize-space()][1]", $root, null, "/^([A-Z]{3})$/"))
                ->name(preg_replace("/^[ ]*(\-)[ ]*/", "", $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][2]/descendant::text()[normalize-space()][2]", $root)))
                ->date(strtotime($this->normalizeDate($this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][2]/descendant::text()[normalize-space()][last()]", $root))));

            $arrTerminal = $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::td[1]/descendant::tr[count(./td) = 2]/child::td[normalize-space()][2]/descendant::text()[normalize-space()][3]", $root, false, "/^Terminal[ ](.+)$/");
            ...
            $aircraft = $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::table[{$this->contains($this->t('Aircraft'))}][1]/descendant::text()[normalize-space() = 'Aircraft:']/following::text()[normalize-space()][1]", $root);
            ...
            $duration = $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::table[{$this->contains($this->t('Time'))}][1]/descendant::text()[normalize-space() = 'Time:']/following::text()[normalize-space()][1]", $root);
            ...
            $cabin = $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::table[{$this->contains($this->t('Cabin'))}][1]/descendant::text()[normalize-space() = 'Cabin:']/following::text()[normalize-space()][1]", $root);
            ...
            $meal = $this->http->FindSingleNode("./descendant::table[normalize-space()][3]/descendant::table[{$this->contains($this->t('Meal'))}][1]/descendant::text()[normalize-space() = 'Meal:']/following::text()[normalize-space()][1]", $root);
            ...
            $seats = $this->http->FindNodes("./descendant::table[normalize-space()][3]/descendant::table[{$this->contains($this->t('Seat(s)'))}][1]/descendant::text()[normalize-space() = 'Seat(s):']/following::td[normalize-space()][1]/descendant::text()[normalize-space()]", $root);

            if (!empty($seats)) {
                foreach ($seats as $seat){
                    if (preg_match("/^([0-9]{1,2}[ ]*[A-Z])$/", $seat)){
                        $s->extra()
                            ->seat($seat);
                    }
                }
    ...
    public function parseHtmlEvent(Email $email, ?array $tripId): void
    {
        ...
        foreach ($nodes as $root) {
            ...
            if (empty($address)) {
                ...
                $address = $http2->FindSingleNode("//text()[normalize-space()='Meeting Point']/following::text()[normalize-space()][1]");
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if ($this->http->XPath->query("//text()[{$this->contains($this->t('Confirmation#'))}]")->length > 0
            && $this->http->XPath->query("//text()[{$this->contains($this->t('Flights'))}]")->length > 0) {
            $this->parseHtmlFlight2($email, $tripId);
        }
    ...
    private function normalizeDate($str)
    {
        ...
        $in = [
            ...
            "#^(\w+)[ ]*(\d{1,2})\,[ ]*(\d+:\d+\s+[AP]M)$#", //JAN 31, 6:25 AM
        ];
        $out = [
            "$3 $2 $year, $1",
            "$2 $1 $year, $3",
