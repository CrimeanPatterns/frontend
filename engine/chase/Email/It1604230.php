...
class It1604230 extends \TAccountCheckerExtended
{
    ...
    public function ParseFlight(Email $email)
    {
        ...
        $f->general()
            ->travellers($this->http->FindNodes("//tr[count(./td) >= 2][./td[starts-with(normalize-space(), 'Passenger')]]/descendant::td[normalize-space()][2]"));
        ...
        foreach ($nodes as $root) {
            ...
            $arrDate = $this->http->FindSingleNode("./descendant::td[4]/descendant::text()[normalize-space()][not(contains(normalize-space(), 'Arrival:'))][1]", $root);
            $arrTime = $this->http->FindSingleNode("./descendant::td[4]/descendant::text()[normalize-space()][not(contains(normalize-space(), 'Arrival:'))][2]", $root);
            $s->arrival()
                ->date(strtotime($arrDate . ' ' . $arrTime))
                ->name($this->http->FindSingleNode("./descendant::td[4]/descendant::text()[normalize-space()][not(contains(normalize-space(), 'Arrival:'))][3]", $root, true, "/^(.+)\s\(/"))
                ->code($this->http->FindSingleNode("./descendant::td[4]/descendant::text()[normalize-space()][not(contains(normalize-space(), 'Arrival:'))][3]", $root, true, "/\(([A-Z]{3})\)/"));
        }
