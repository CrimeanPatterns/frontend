...
class ChaseExtension extends AbstractParser implements LoginWithIdInterface, ParseAllInterface, ActiveTabInterface, ContinueLoginInterface
{
    use TextTrait;

    private const OTC_QUESTION = "What is the one-time code that was sent to your phone?";
    ...
    private int $attemptLogin = -1;
    private const XPATH_SUCCESSFUL_AUTH = '//mds-navigation-bar[@id="primaryNavigationBar"] 
        | //div[@id="ovd-layout-container"] 
        | //div[@id="dynamic-layout-container"]';
    ...
    {
        $tab->showMessage('');
        $loginIn = $tab->evaluate($xpath = '//mds-text-input[@id="userId"] 
        | //iframe[@id="logonbox"] 
        | ' . self::XPATH_SUCCESSFUL_AUTH, EvaluateOptions::new()->timeout(30));

        if (strtoupper($loginIn->getNodeName()) === 'IFRAME') {
            $i = 0;
            $stop = false;
            do {
                sleep(1);
                ...
                if ($tab->evaluate(self::XPATH_SUCCESSFUL_AUTH,
                    EvaluateOptions::new()->timeout(0)->allowNull(true))) {
                    return true;
                } else {
                    $frame = $tab->selectFrameContainingSelector('//div[@id="logon-spin" and contains(@style, "none")]',
                        SelectFrameOptions::new()->method("evaluate")->visible(false)->allowNull(true)->timeout(0));
                    if ($frame) {
                        if ($frame->evaluate('//mds-text-input[@id="userId"]', EvaluateOptions::new()->allowNull(true)->timeout(0)) && $i < 2) {
                            $this->logger->notice('wait...');
                            ...
                            $response = $this->fetch($tab, '/svc/rr/profile/l4/v1/overview/list', $options);
                            if ($response) {
                                $this->logger->debug($response->body);
                                $this->logger->debug($response->status);
                                $this->notificationSender->sendNotification('check /overview/list // MI');
                            }
                            continue;
                        }
                        $loginIn = $tab->evaluate($xpath, EvaluateOptions::new()->timeout(0));
                        $stop = true;
                    }
                }
        ...
        $tab->logPageState();
        return strtolower($loginIn->getNodeName()) === 'div' || strtolower($loginIn->getNodeName()) === 'mds-navigation-bar';
    }
    ...
    {
        $barShadowRoot = $tab->querySelector("#globalBrandBar", QuerySelectorOptions::new()->visible(false))->shadowRoot();
        $logOutShadowRoot = $barShadowRoot->querySelector("#brand_bar_sign_in_out", QuerySelectorOptions::new()->visible(false))->shadowRoot();
        $logOutShadowRoot->querySelector("button", QuerySelectorOptions::new()->visible(false))->click();
        sleep(5);
        $xpath = '//button[@data-pt-name="hd_bb_sign-in-panel"] 
            | //main[@id="logon-content"] 
            | //a[contains(@class,"signInBtn")]
            | ' . self::XPATH_SUCCESSFUL_AUTH;
        $result = $tab->evaluate($xpath);
        if (in_array($result->getAttribute('id'), ['primaryNavigationBar', 'ovd-layout-container', 'dynamic-layout-container'])) {
            ...
            if (in_array($result->getAttribute('id'), ['primaryNavigationBar', 'ovd-layout-container', 'dynamic-layout-container'])) {
                ...
                $result = $tab->evaluate($xpath);
                if (in_array($result->getAttribute('id'), ['primaryNavigationBar', 'ovd-layout-container', 'dynamic-layout-container'])) {
                    $this->notificationSender->sendNotification('logout retry success // MI');
                } else {
                    $this->notificationSender->sendNotification('logout retry failed // MI');
                }
            }
        }
    }
    ...
    private function fetch(Tab $tab, $url, array $options = []): ?FetchResponse
    {
        ...
        && !stristr($currentUrl, 'https://www.chase.com')) {
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        try {
            ...
            if ($this->attemptLogin == 0) {
                $frame->evaluate('//mds-button[@id="signin-button"]')->click();
            } else {
                $tab->showMessage('Please press the "Sign in" button to log in to your Chase account. AwardWallet will do the rest.');
            }
            ...
            | //label[contains(text(),"How would you like to receive your one-time code?")]
            ...
            do {
                ...
                if ($tab->evaluate(self::XPATH_SUCCESSFUL_AUTH,
                    EvaluateOptions::new()->timeout(0)->allowNull(true))) {
                    $stop = true;
                } elseif ($tab->evaluate('//*[contains(text(),"It looks like this part of our site isn")]',
                    EvaluateOptions::new()->timeout(0)->allowNull(true)->visible(false))) {
                    //$tab->evaluate('//*[contains(@value,"Try again")] | //*[contains(.,"Try again")]')->click();
                    $stop = true;
                    //$tab->logPageState();
                    $tab->gotoUrl('https://secure.chase.com/web/auth/dashboard');
                    ...
                    try {
                        $frame = $tab->selectFrameContainingSelector($xpath, SelectFrameOptions::new()
                            ->method("evaluate")->visible(false)->timeout(0)->nonEmptyString());
                        if ($frame && $frame->evaluate('
                            //mds-text-input-secure[@error-message="Please tell us your password."]',
                                EvaluateOptions::new()->timeout(0)->allowNull(true))) {
                            $stop = true;
                            return $this->login($tab, $credentials);
                        } elseif ($frame && $frame->evaluate('//*[contains(text(),"It looks like this part")]',
                                EvaluateOptions::new()->timeout(0)->allowNull(true)->visible(false))) {
                            $stop = true;
                            $tab->evaluate('//*[contains(text(),"Try again")]')->click();
                            return $this->login($tab, $credentials);
                        }
                        if ($frame) {
                            $stop = true;
            ...
            try {
                $errorOrTitle = $frame->evaluate($xpath, EvaluateOptions::new()->timeout(0)->allowNull(true)->visible(false)->nonEmptyString());
            } catch (\Exception $e) {
                $this->logger->info($e->getMessage());
            }
            if (isset($errorOrTitle)) {
                $errorOrTitleText = $errorOrTitle->getInnerText();
                $this->logger->debug('[errorOrTitle]:' . $errorOrTitleText);
                if (stristr($errorOrTitleText, "How should we get in touch?")
                    || stristr($errorOrTitleText, "How would you like to receive your one-time code?")
                    || stristr($errorOrTitleText, "We sent a push notification to")
                    || stristr($errorOrTitleText, "We sent you a text message")
                    || stristr($errorOrTitleText, "Let's make sure it's you")) {

                    if ($this->context->isServerCheck()) {
                        $frame->evaluate('//div[contains(text(), "Contact me in a different way")]')->click();
                        $frame->evaluate('//input[@value = "Choose one"]')->click();
                        $frame->evaluate('//li[.//*[text() = "TEXT ME"]]/following-sibling::li[' . ($credentials->getLogin() === 'awardwallet04' ? 2 : 1) . ']/a')->click();
                        $frame->evaluate('//button[span[text() = "Next"]]')->click();
                        $errorOrSuccess = $frame->evaluate('//h3[text() = "We sent you a text message."] | //span[@id = "icon-alert-the-user"]');

                        if ($errorOrSuccess->getAttribute("id") == "icon-alert-the-user") {
                            return LoginResult::providerError($errorOrSuccess->getInnerText());
                        }

                        $this->stateManager->keepBrowserSession(true);

                        return LoginResult::question(self::OTC_QUESTION);
                    }

                    $tab->showMessage(Message::identifyComputerSelect('Next'));
                    $errorOrTitle = $tab->evaluate(self::XPATH_SUCCESSFUL_AUTH,
                ...
                if (stristr($errorOrTitleText, "We can't find that username and password. Try again.")) {
                    return LoginResult::invalidPassword("We can't find that username and password. Try again.");
                }

                if (stristr($errorOrTitleText, "It looks like this part of our site isn't working right now.")) {
                    return $this->login($tab, $credentials);
                }
            } elseif ($tab->evaluate(self::XPATH_SUCCESSFUL_AUTH,
                EvaluateOptions::new()->timeout(0)->allowNull(true))) {
                //$tab->logPageState();
                return new LoginResult(true);
            }
    ...
    public function parseAll(
        ...
        */
        ...
        $tab->evaluate(self::XPATH_SUCCESSFUL_AUTH);
        ...
        if (empty($st->getBalance()) && !empty($this->Properties['Name'])
            && (($st->getDetectedCards() !== null && count($st->getDetectedCards()) > 0)
                || $accountWithoutCards)) {
    ...
    public function parseCardDetails(Tab $tab, Statement $st, AccountOptions $accountOptions, ?ParseHistoryOptions $historyOptions, $accounts, $version, &$accountWithoutCards)
    {
        ...
        foreach ($accounts as $account) {
            ...
            if (empty($accountId) || $unavailable || $nickname == 'TOTAL CHECKING' || $closed || $skip
                ...
                if (!empty($accountId) && in_array($cardType, ['SOUTHWEST_PREMIER', 'SOUTHWEST_AIRLINES'])) {
                    ...
                    try {
                        ...
                        $rewardProgramNameSW = $rewardSW->rewardProgramName ?? null;
            ...
            $rewardsCardType = $reward->rewardsCardType ?? null;
...
//                            $description = " for <br> Gas stations and Internet, Cable, and Phone providers";
                ...
                if (stristr($rewardsCardType, 'AMAZON_PRIME')) {
                    $this->notificationSender->sendNotification('Amazon card // MI');
                }
                ...
                if (
                    ...
                    || stristr($rewardsCardType, 'AMAZON_PRIME')
                ) {
...
//                            }
                ...
                if ($tab->getUrl() != 'https://secure.chase.com/web/auth/dashboard#/dashboard/overview') {
                    ...
                    $result = $tab->evaluate(self::XPATH_SUCCESSFUL_AUTH,
                        EvaluateOptions::new()->timeout(10)->allowNull(true));
                    if (!$result) {
                        $tab->saveHtml();
                        $tab->gotoUrl('https://secure.chase.com/web/auth/dashboard#/dashboard/overview');
    ...
    private function addDetectedCard(Statement $st, array $card): void
    {
        $this->logger->notice(__METHOD__);
        foreach($st->getDetectedCards() as $detectedCard) {
            if ($detectedCard['Code'] == $card['Code']) {
                $this->logger->notice('duplicate card:' . json_encode($card));
                return;
            }
        }
        ...
        switch ($cardType) {
            ...
            case 'CHASE_INK_BUSINESS_PREFERRED':
                if (
                    ...
                    $this->notificationSender->sendNotification('Ink Unlimited // MI');
                    /*
                        'rewardProgramName' => 'Ink Business Unlimited SM',
            ...
            case 'UNITED':
                $kind = 'United';
                if ($rewardProgramName && strstr($rewardProgramName, 'United<sup>SM</sup> Explorer Card')) {
                    $kind = 'United Explorer Card';
                }
...
//                    $kind = "Ink Plus 521";// refs #15541
    ...
    {
        $tab->evaluate('//input[@id = "otpcode_input-input-field"]')->setValue($credentials->getAnswers()[self::OTC_QUESTION]);
        $tab->evaluate('//input[@id = "password_input-input-field"]')->setValue($credentials->getPassword());
        $tab->evaluate('//button[span[text() = "Next"]]')->click();

        $errorOrSignout = $tab->evaluate('//span[@id = "icon-alert-the-user"] | ' . self::XPATH_SUCCESSFUL_AUTH);

        if ($errorOrSignout->getAttribute("id") === "icon-alert-the-user") {
            return LoginResult::question(self::OTC_QUESTION, $errorOrSignout->getInnerText());
        }
