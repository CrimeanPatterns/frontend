...
{
    public $mailFiles = "british/it-859754688.eml, british/it-904186150.eml";
    public $subjects = [
        'Your booking confirmation',
        'Your booking change confirmation',
    ];
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        if ($this->http->XPath->query("//text()[{$this->contains(['Thank you for booking with', 'Thank you for changing your booking with'])}]")->length > 0
    ...
    public function parseFlights(Email $email)
    {
        ...
        foreach ($nodes as $root) {
            ...
            if (preg_match("/Departs:\n(?<depDate>.+\d{4})\n(?<depName>.+)\n(?<depTime>[\d\:]+)\n(?<depCode>[A-Z]{3})[\s\-]*(?:Terminal\b\s*(?<depTerminal>\S.*))?$/", $depInfo, $m)) {
                $s->departure()
                    ->name($m['depName'])
                    ->code($m['depCode'])
            ...
            if (preg_match("/Arrives:\n(?<arrDate>.+\d{4})\n(?<arrName>.+)\n(?<arrTime>[\d\:]+)\n(?<arrCode>[A-Z]{3})[\s\-]*(?:Terminal\b\s*(?<arrTerminal>.*))?(?:\n|$)/", $arrInfo, $m)) {
                $s->arrival()
                    ->name($m['arrName'])
                    ->code($m['arrCode'])
