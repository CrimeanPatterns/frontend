...
class ConfirmationOrder extends \TAccountChecker
{
    ...
    public $subjects = [
        ...
        // is
        'Ferðin þín er staðfest. Pöntunarnúmer',
    ];
    ...
    public static $dictionary = [
        ...
        "hu" => [
            ...
            'Stop(s)'           => ['Közbenső állomás(ok)', 'megállás'],
            ...
            'Seat reservations'    => 'Helyfoglalások',
            'Additions Fee'        => ['Helyfoglalások', 'Járattal kapcsolatos hírek SMS-ben', 'Foglalási azonosító küldése SMS-ben'],
            'Sum'                  => ['Összeg'],
        ...
        "ru" => [
            ...
            'Seat reservations'    => 'Бронирование мест',
            'Additions Fee'        => ['Бронирование мест', 'Обслуживание багажа', 'Пакет перебронирования авиабилетов',
                'Защита от банкротства авиакомпаний', 'Гарантия возможности отмены', ],
            'Sum'                  => ['Сумма'],
        ...
        "pl" => [
            ...
            'Stop(s)'           => ['Międzylądowania', 'międzylądowania'],
        ...
        "fi" => [
            ...
            'Additional Products:' => 'Lisätuotteet:',
        ],
        "no" => [
            // Html
            ...
            // 'Seat reservations'    => 'Paikkavaraus',
            'Additions Fee'        => ['Innsjekket bagasje*', 'Håndbagasje*', 'Fleksibel billett'],
            'Sum'                  => ['Summa'],
            'Total amount'         => 'Totalt',
            'Additional Products:' => 'Tilleggsprodukter:',
        ],
        "ja" => [
            // Html
            'Order Status'      => 'ご予約状況',
        ...
        "th" => [
            ...
            'Seat reservations'    => 'การสำรองที่นั่ง',
            'Additions Fee'        => ['การสำรองที่นั่ง'],
            'Sum'                  => ['รวม'],
            'Total amount'         => 'ยอดรวม',
        ...
        "is" => [
            ...
            'Your Trip Summary' => 'Ferðayfirlit þitt',
            'Stop(s)'           => 'breytingar',
            'First name'        => 'Fornafn',
            'Last name'         => 'Eftirnafn',
            'Total'             => 'Samtals',
            ...
            // Pdf
            'Order number:'        => 'Pöntunarnúmer:',
            'Payment Overview'     => 'Yfirlit yfir greiðslur',
            'Trip'                 => 'Ferð',
            'Description'          => 'Lýsing',
            'Price'                => 'Verð',
            'Taxes & charges'      => 'Skattar & gjöld',
            // 'Seat reservations'    => 'Κρατήσεις θέσης',
            // 'Additions Fee'        => ['Κρατήσεις θέσης', 'Κωδικός κράτησης με γραπτό μήνυμα', 'Χειραποσκευές*',
            //     'Αποσκευές για check-in*', ],
            'Sum'                  => ['Samtala'],
            'Total amount'         => 'Samtals',
            'Additional Products:' => 'Viðbótarvörur:',
        ],
    ];
    ...
    public function ParseFlight(Email $email, string $pdfText = null)
    {
        ...
        if (!empty($pdfText)
            ...
            // delete page number
            $pdfText = preg_replace("/\n {20,}\d{1,2} ?\/ ?\d{1,2}\s*\n(.*\n){0,2} *(?:Gotogate Pty|Flight Network UK) Ltd,.+/", '', $pdfText);
            $pdfText = preg_replace("/\n {20,}\d{1,2} ?\/ ?\d{1,2}\s*\n(.*\n){0,2} *.+ Org no\. .+, VAT no\. .+/", '', $pdfText);
        ...
        if (preg_match("/^\s*(?:\+\d\n\s*)?([[:alpha:]\p{Thai}]+(?: [[:alpha:]\p{Thai}]+)? •) .*(?:\b|\D)\d{4}(?:\b|\D).*\n/u", $flightsText, $m)) {
            $segments = $this->split("/\n\n\n({$m[1]})/u", "\n\n\n\n" . $flightsText);
        } else {
            $segments = $this->split("/\n\n\n((?:.+\n?){1,3}?\s+→\s+(?:.+\n?){1,3}?\n(?:.+\n?){1,3}?\s+[→]\s+[^\n]+)/u",
                "\n\n\n\n" . $flightsText);
        ...
        $re2 = "/^\s*(?:\+\d\n\s*)?[[:alpha:]\p{Thai}]+(?: [[:alpha:]\p{Thai}]+)? •\s*(?<date>.+?)\n\s*(?<depTime>\d+\:\d+(?: ?[AP]M)?)\s+(?<arrTime>\d+\:\d+(?: ?[AP]M)?)(?<overnight>\s+[\-+]\d)?\n\s*"
            . "(?<duration>[\w \.\p{Thai}]+?)( *[,,] *(?:乗り換え\s*)?(?:(?<stops>\d+) {$this->opt($this->t('Stop(s)'))}|{$this->opt($this->t('Stop(s)'))} (?<stops2>\d+)))?\n(?:.*\n)?\s*"
            . "(?<depPart1>.+?) ?\((?<depCode>[A-Z]{3})\)\n\s*(?<arrPart1>.+?) ?\((?<arrCode>[A-Z]{3})\)"
            . "(?<namesPart2>\n.*)/us";
        // $this->logger->debug('$re2 = '.print_r( $re2,true));

        $reCabinFlight = "/^ *(?:(?<cabin>[\w\p{Thai}](?: ?[\w\-\p{Thai}])+?)[ ]{3,})?(?:\S ?)+\s*[•]\s*(?<an>(?:[A-Z][A-Z\d]|[A-Z\d][A-Z]))(?<fn>\d{1,5})\s*$/u";
        ...
        foreach ($segments as $i => $sText) {
            ...
            if (preg_match($re, $sText, $m)
                ...
                $m['stops'] = !empty($m['stops2']) ? $m['stops2'] : $m['stops'] ?? null;
                ...
                if (isset($pdfTrips[$i])
                    && preg_match("/\n *" . str_replace(' ', '\s+', $this->opt($depNameP1)) . " *- *" . str_replace(' ', '\s+', $this->opt($arrNameP1)) . "\s*,\s*(?:[\d\/\s]+|[\d\-\s]+|[\d\.\s]+(?:г\.)?)\s*\n([\s\S]+?)\s*$/u", $pdfTrips[$i], $pm)
                ) {
    ...
    public function cityAlternativeName($name): array
    {
        $result = [];
        $cityNameReplaces = [
            // 'название в сегменте' => 'названия в местах'
            ...
            'Chișinău International Airport' => ['Кишинёв'],
        ];
