    ...
    private function parseEmailHtml(Email $email): void
    {
        ...
        foreach ($segments as $root) {
            ...
            */
            ...
            || preg_match($pattern2 = "/^(?<code>[A-Z]{3})\n(?<time>[\d\:]+\s*A?P?M)\n*(?:[•]|$)/", $departureText, $m)) {
            ...
            if (preg_match($pattern, $arrivalText, $m) || preg_match($pattern2, $arrivalText, $m)) {
            ...
            if ($s->getDepCode() && $s->getArrCode() && array_key_exists($s->getDepCode() . $s->getArrCode(), $seats)) {
                foreach ($seats[$s->getDepCode() . $s->getArrCode()] as $seat) {
                    $pax = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Seat:'))}]/following::text()[normalize-space()][1][{$this->contains($seat)}]/preceding::h5[1][{$this->eq($s->getDepCode() . ' - ' . $s->getArrCode())}]/ancestor::table[2]/descendant::text()[string-length()>3][1][not(contains(normalize-space(), 'TBAADT'))]");
