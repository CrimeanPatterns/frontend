...
{
    public $mailFiles = "fontain/it-718098139.eml, fontain/it-716550565.eml, fontain/it-899882560.eml, fontain/it-903357671.eml";

    public $lang = '';

    public static $dictionary = [
        'en' => [
            'hotelNameStart' => ['Thank you for reserving a stay at', 'THANK YOU FOR RESERVING A STAY AT', 'thank you for reserving a stay at'],
            'confNumber'     => ['Confirmation Number'],
            'statusPhrases'  => ['Your reservation is', 'YOUR RESERVATION IS'],
            'statusVariants' => ['confirmed'],
            'feeNames'       => ['Resort Fees (Total)', 'Total Taxes (Includes Resort Fee Taxes)', 'Total Taxes (includes Resort Fee taxes and Add On taxes)'],
    ...
    private function parseHotel(Email $email): void
    {
        ...
        ?? $this->http->FindSingleNode("//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1][{$this->eq($this->t('Room Type'), "translate(.,':','')")}] ]/*[normalize-space()][2]");
        ...
        if (preg_match("/^(\d{1,3})$/", $numberOfGuests, $m)
            || preg_match("/\b(\d{1,3})\s*{$this->opt($this->t('Adult'))}/i", $numberOfGuests, $m)
        ) {
            $h->booked()->guests($m[1]);
        }
        ...
        $cancellationText = implode("\n", $this->http->FindNodes("descendant::text()[{$this->eq($this->t('cancellationPolicy'), "translate(.,':','')")}][1]/ancestor::tr[1]/descendant::text()[normalize-space()]"));
        $cancellation = preg_match("/^{$this->opt($this->t('cancellationPolicy'))}\n+(.+)$/s", $cancellationText, $m) ? $m[1] : null;
        $h->general()->cancellation($cancellation, false, true);

        if (preg_match("/^Cancell? by (?<hour>{$patterns['time']})(?: local hotel time)? at least (?<prior>\d{1,3}\s*(?:days?|hours?)) prior to arrival to avoid a \w+[-\s]+nights? cancell?ation fee(?:\s*[.;!]|$)/i", $cancellation, $m)
        ) {
        ...
        $hotelName = $this->http->FindSingleNode("//text()[{$this->contains($this->t('hotelNameStart'))}]", null, true, "/{$this->opt($this->t('hotelNameStart'))}\s+(.{2,75}?)[\s.;!]*$/")
        ?? $this->http->FindSingleNode("//img[normalize-space(@alt)]/@alt[{$this->contains($this->t('hotelNameStart'))}]", null, true, "/{$this->opt($this->t('hotelNameStart'))}\s+(.{2,75}?)[\s.;!]*$/");
        $address = null;

        if ($hotelName) {
            $hotelNameVariants = array_unique([$hotelName, strtoupper($hotelName), strtolower($hotelName), ucwords($hotelName)]);
            $address = $this->http->FindSingleNode("//text()[{$this->contains($this->t('hotelNameStart'))}]/following::text()[{$this->starts($hotelNameVariants)}]", null, true, "/{$this->opt($hotelNameVariants)}(?:\s*\|\s*)+(.{3,95}?)[\s.;!]*$/i")
            ?? $this->http->FindSingleNode("//text()[{$this->contains($this->t('hotelNameStart'))}]/following::text()[{$this->eq($hotelNameVariants, "translate(.,'|','')")}]/following::text()[string-length(normalize-space())>2][1]", null, true, "/^[\|\s]*(.{3,95}?)[\s.;!]*$/")
            ?? $this->http->FindSingleNode("//img[normalize-space(@alt)]/@alt[{$this->contains($this->t('hotelNameStart'))}]/following::img[normalize-space(@alt)]/@alt[{$this->starts($hotelNameVariants)}]", null, true, "/{$this->opt($hotelNameVariants)}[:\|\s]+(.{3,95}?)[\s.;!]*$/i");
        }

        $phone = $this->http->FindSingleNode("//text()[{$this->contains($this->t('hotelNameStart'))}]/following::text()[{$this->eq($this->t('Reservations'), "translate(.,':|','')")}]/following::text()[normalize-space()][1]", null, true, "/^{$patterns['phone']}$/")
        ?? $this->http->FindSingleNode("//img[normalize-space(@alt)]/@alt[{$this->contains($this->t('hotelNameStart'))}]/following::img[normalize-space(@alt)]/@alt[{$this->starts($this->t('Reservations'))}]", null, true, "/^{$this->opt($this->t('Reservations'))}[:\|\s]+({$patterns['phone']})$/");
