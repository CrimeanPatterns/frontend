...
//                throw new CheckException($error, ACCOUNT_PROVIDER_ERROR);
    ...
    public function ParseOfUSA()
    {
        ...
        if (
            ...
            // Total Cash Back
            $this->SetProperty("LifetimeCashBack", $response->lifeTimeCashBack);
            // Lifetime Points
            $this->SetProperty("LifetimeMRPoints", !empty($response->TotalCashBack) ? $response->TotalCashBack :
                $this->http->FindSingleNode("//div[contains(@data-testid,'account-left-member-life-cb')]/div[1]", null,
                    false, '/^\s*([\d.,]+)\s*Lifetime Points/'));
            $this->SetProperty("Pending", $this->http->FindSingleNode("//div[contains(@data-testid,'pending-cb-amount')]", null, false, '/^([\d.,]+)/'));
        } else {
            // Cash Paid
            $this->SetProperty("CashPaid", $response->CashPaid ?? null);
            // Total Cash Back
            $this->SetProperty("LifetimeCashBack", !empty($response->TotalCashBack) ? $response->TotalCashBack :
                $this->http->FindSingleNode("//div[contains(@data-testid,'account-left-member-life-cb')]/div[1]", null,
                    false, '/^\s*(\$[\d.,]+)\s*Lifetime Cash Back/'));
            $this->SetProperty("CashbackPending", $this->http->FindSingleNode("//div[contains(@data-testid,'pending-cb-amount')]", null, false, '/(\$[\d.,]+)/'));
        }
