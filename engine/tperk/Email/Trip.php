...
{
    public $mailFiles = "tperk/it-12289931.eml, tperk/it-12418264.eml, tperk/it-156651009.eml, tperk/it-162632935.eml, tperk/it-201694955.eml, tperk/it-284441906.eml, tperk/it-424796840.eml, tperk/it-657330576.eml, tperk/it-911564689.eml, tperk/it-911599029.eml, tperk/it-912029325.eml, tperk/it-912039830.eml, tperk/it-912176570.eml";
    ...
    public static $dictionary = [
        'en' => [
            ...
            'Your trip ID is'   => ['Your trip ID is'],
            'otaConfTitle'      => 'trip ID',
            ...
            // Hotel
            'nights at '       => ['nights at ', 'night at '],
            'Check-in'         => ['Check-in'],
            'Check-out'        => ['Check-out'],
            'until'            => ['until', 'to'],
            "Hotel's phone"    => ["Hotel's phone", "Hotel's Phone"],
            "Hotel's Email"    => ["Hotel's Email", "Hotel's email"],
            'Confirmation #'   => ['Confirmation #', 'Provider references', 'Reservation number', 'Provider reference'],
            // Car
        ...
        'fr' => [
            ...
            'otaConfTitle'                    => 'référence de votre voyage',
            // Flight / Train
            ...
            // Hotel
            'nights at '        => ['nuit au ', 'nuits au '],
            'Check-in'          => ['Arrivée'],
            'Check-out'         => ['Départ'],
            'from'              => ['de'],
            'until'             => ['à'],
            "Hotel's phone"     => "Téléphone de l'hôtel",
            "Hotel's Email"     => ["E-mail de l'hôtel"],
            'Rooms'             => 'Chambres',
            'Guests'            => 'Clients',
            'Cancellation'      => 'Annulation',
            'Confirmation #'    => ['Numéro de confirmation #', 'Référence du prestataire', 'Numéro de réservation'],
            // Car
            // 'Pick-up'  => ['Pick-up', 'Pickup'],
            // 'Drop-off' => ['Drop-off', 'Dropoff'],
            // 'Loyalty:' => ['Loyalty:', 'Loyalty'],
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if ($tripBookingDate) {
            ...
            $date = EmailDateHelper::getEmailDate($this, $parser);
        ...
        if (preg_match("/{$this->opt($this->t('Your trip ID is'))}[:\s]+([-A-Z\d]{5,})(?:\s*[-—]|\.|$)/u", $tripIDText, $m)) {
            $email->ota()->confirmation($m[1], $this->t('otaConfTitle'));
        } else {
        ...
        $xpath = "//{$this->xpath['headRow']}/ancestor-or-self::tr[ preceding-sibling::tr[normalize-space()] or following-sibling::tr[normalize-space()] ][1][ not(preceding-sibling::tr[normalize-space()]) and following-sibling::tr[normalize-space()] ]/..";
        // $this->logger->debug('$xpath = '.print_r( $xpath,true));
        $tripSegments = $this->http->XPath->query($xpath);

        if ($tripSegments->length == 0) {
            $xpath = "//{$this->xpath['headRow']}/ancestor-or-self::tr[ preceding-sibling::tr[normalize-space()] or following::tr[normalize-space()] ][1][ not(preceding-sibling::tr[normalize-space()]) and following::tr[normalize-space()] ]/../ancestor::table[2]";
            // $this->logger->debug('$xpath = '.print_r( $xpath,true));
            $tripSegments = $this->http->XPath->query($xpath);
        }

        if ($tripSegments->length == 0) {
            $segmentsStart = "{$this->eq($this->t('Depart from'))} or {$this->eq($this->t('Check-in'))} or {$this->eq($this->t('Operator'))}";
            $xpath = "//text()[{$segmentsStart}]/preceding::text()[normalize-space()][1]/ancestor-or-self::table[.//text()[{$segmentsStart}]][1]";
            // $this->logger->debug('$xpath = '.print_r( $xpath,true));
            $tripSegments = $this->http->XPath->query($xpath);
        }
        ...
        foreach ($tripSegments as $sRoot) {
            if ($this->http->XPath->query("descendant::*[ preceding-sibling::*[normalize-space()][1][{$this->eq($this->t('Depart from'))}] and following-sibling::*[normalize-space()][1][{$this->eq($this->t('Arrive at'))}] and following-sibling::*[{$this->eq($this->t('Flight duration'))}] ]", $sRoot)->length > 0) {
                ...
                if (empty($pnr)) {
                    if ($this->http->XPath->query("descendant::text()[normalize-space()][1][{$this->contains(' - ')}]", $sRoot)->length === 0
                        && $this->http->XPath->query("ancestor::table[1][count(.//text()[{$this->eq($this->t('Depart from'))}]) = 1]/descendant::text()[normalize-space()][1][{$this->contains(' - ')}]", $sRoot)->length > 0
                    ) {
                        $sRoot = $this->http->XPath->query("ancestor::table[1]", $sRoot)->item(0);
                    }
        ...
        if (!empty($flightPrice) || !empty($rentalPrice) || !empty($hotelPrice) || !empty($trainPrice)) {
            foreach ($email->getItineraries() as $it) {
                ...
                if ($it->getType() === 'train' && !empty($trainPrice)) {
                    if (preg_match('/^(?<currency>[^\-\d)(]+?)[ ]*(?<amount>\d[,.\'\d ]*)$/', $trainPrice, $matches)
                        || preg_match('/^(?<amount>\d[,.\'\d ]*)[ ]*(?<currency>[^\-\d)(]+?)$/', $trainPrice, $matches)) {
                        // €207.47
                        $matches['currency'] = $this->currency($matches['currency']);
                        $currencyCode = preg_match('/^[A-Z]{3}$/', $matches['currency']) ? $matches['currency'] : null;
                        $it->price()
    ...
    private function parseTrain(Email $email, string $pnr, array $segments): void
    {
        ...
        foreach ($segments as $root) {
            ...
            if (empty($train)) {
                $train = $this->http->FindSingleNode("descendant::text()[normalize-space()][2]", $root);
            }

            if (preg_match('/(?:^|\s+-\s+)(?<name>[[:alpha:] ][[:alpha:] ]*?)[ ]*(?<number>\d+)$/', $train, $m)) {
                $s->extra()
                    ->service($m['name'])
                    ->number($m['number']);
            }
            ...
            */
            ...
            $coach = array_unique(array_filter($this->http->FindNodes("descendant::*[preceding-sibling::*[normalize-space()][1][{$this->eq($this->t('Passengers'))}] and not(following-sibling::*[normalize-space()])]/descendant::text()[contains(normalize-space(), 'Coach')]/ancestor::td[1]", $root, "/{$this->opt($this->t('Coach'))}[\:\s]+(\d+)/u")));
    ...
    private function parseHotel(Email $email, \DOMNode $root): void
    {
        ...
        if ($this->http->XPath->query("descendant::text()[normalize-space()][1][{$this->contains($this->t('nights at '))}]", $root)->length === 0
            && $this->http->XPath->query("ancestor::table[1]/descendant::text()[normalize-space()][1][{$this->contains($this->t('nights at '))}]", $root)->length > 0
        ) {
            $root = $this->http->XPath->query("ancestor::table[1]", $root)->item(0);
        }
        ...
        if (preg_match("/{$this->opt($this->t('nights at '))}\s*(.+)/", $hotelName, $m)) {
            $hotelName = $m[1];
        } else {
            $hotelName = null;
        }
        ...
        if (empty($phone)) {
            $phone = $this->http->FindSingleNode("descendant::*[ preceding-sibling::*[normalize-space()][1][{$this->eq($this->t("Hotel's phone"))}] and following-sibling::*[normalize-space()][1][{$this->eq($this->t("Hotel's Email"))}] ]",
                $root, true, "/^{$this->patterns['phone']}$/");
        }
        ...
        foreach ($this->t('Confirmation #') as $confTitle) {
            $confirmationVal = $this->http->FindSingleNode("descendant::tr[not(.//tr) and {$this->starts($confTitle)}][not({$this->eq($confTitle)})]",
                $root);

            if (empty($confirmationVal)) {
                $confirmationVal = implode(", ",
                    $this->http->FindNodes(".//text()[{$this->eq($confTitle)}]/ancestor::*[following-sibling::*[normalize-space()][1]][1]/following-sibling::*[normalize-space()][1]//text()[normalize-space()]",
                        $root));

                if (!empty($confirmationVal)) {
                    $confirmationVal = $this->http->FindSingleNode("(.//text()[{$this->eq($confTitle)}])[1]",
                            $root) . ":" . $confirmationVal;
                }
            }

            if (preg_match("/^({$this->opt($confTitle)})[:\s]*([-A-Z\d, ]{5,})$/", $confirmationVal, $m)) {
        ...
        if (preg_match("/^(.{6,}?)\s+{$this->opt($this->t('from'))}\s+({$this->patterns['time']})(?:\s*{$this->opt($this->t('until'))}\s*[\d\:]+\s*A?P?M?)?$/", $checkIn, $m)) {
        ...
        if (preg_match("/Free (?i)cancell?ation before (?<date>.+? \d{4}) at (?<time>{$this->patterns['time']})\s*(?:[.!(]|$)/", $cancellation, $m)
            // fr
            || preg_match("/Annulation gratuite avant le (?<date>.+? \d{4}) à (?<time>{$this->patterns['time']})\s*(?:[.!(]|$)/u", $cancellation, $m)
        ) {
