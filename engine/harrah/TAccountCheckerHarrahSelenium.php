    ...
    public function Login()
    {
        try {
            $this->waitForElement(WebDriverBy::xpath($xpath = '
                //div[contains(text(),"reward credits")] 
                | //a[contains(., "Sign Out")] 
                | //div[@id = "errorMsg"]/div[contains(@class, "index-module_notification-content")] 
                | //div[@data-testid="toast-message"]//div[@dir and not(@style)]
        ...
        if ($maybeLater = $this->waitForElement(WebDriverBy::xpath('//button[contains(.,"Maybe Later")]'), 0)) {
            $maybeLater->click();
            sleep(2);
            $this->waitForElement(WebDriverBy::xpath($xpath), 25);
            $this->saveResponse();
        }
        ...
        if ($message = $this->http->FindSingleNode('
                ...
                | //div[contains(text(),"Invalid username or password. Please try again.")]
            ')
            ?? $this->http->FindSingleNode('(//div[@data-testid="toast-message"]//span)[1]')
        ) {
