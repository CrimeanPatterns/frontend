...
class ConfirmationEmailPdf extends \TAccountChecker
{
    ...
    public static $dictionary = [
        'it' => [ // before it
            'Travel Itinerary/Invoice'       => ['Itinerario di viaggio/Fattura', 'Itinerario di viaggio/ricevuta'],
            ...
            'Taxes and Fees'                 => ['Taxes and Fees/ Tasse e supplementi', 'Tax, Fees and Charges/ Tasse e'],
            'Taxes and Fees SecondRow'       => 'supplementi',
    ...
    {
        if (array_key_exists('from', $headers) && stripos($headers['from'], 'no-reply@amadeus.com') !== false) {
            return false;
        }
...
//        $this->logger->debug('$parts = ' . print_r($parts, true));
        ...
        if (preg_match_all("/(?:^|\n)[ ]*(\S.*?\S)[ ]*[:]+[ ]*-[ ]*.+\n+[ ]*(?:{$this->opt($this->t('Ticket success!'))}[ ]*)?{$this->opt($this->t('Air e-ticket numbers'))}/", $partTraveller, $m)) {
            $f->general()
                ->travellers($m[1], true);
        } else {
            $f->general()
        ...
        if (preg_match_all("/(.+)\n+.*{$this->opt($this->t('Air e-ticket numbers'))}[: ]+({$patterns['eTicket']})[ ]*(?:[[:alpha:]]|\n)/", $partTraveller, $ticketMatches, PREG_SET_ORDER)) {
            foreach ($ticketMatches as $m) {
                $passengerName = $this->re("/^[ ]*(\S.*?\S)[ ]*[:]+[ ]*-[ ]*.+$/", $m[1]);
                $f->issued()->ticket($m[2], false, $passengerName);
            }
        } else {
            $f->issued()
                ->tickets([], false);
...
//        $this->logger->debug('$segments = ' . print_r($segments, true));
        foreach ($segments as $sText) {
            ...
            if (preg_match("/^\s*(?<aircraft>.+(?:\n.+)?)\n\n[ ]*(?<cabin>.+(?:\n.*)?)\([ ]*(?<code>[A-Z]{1,2})[ ]*\)\n/", $table[3] ?? '', $m)
                || preg_match("/^\s*(?<aircraft>.+)\n[ ]*(?<cabin>\S.+)[ ]*\n?[ ]*\([ ]*(?<code>[A-Z]{1,2})[ ]*\)\n/", $table[3] ?? '', $m)
                || preg_match("/^\s*(?<aircraft>\S.+(?:\n[ ]*.*\d.*)?)\n\n[ ]*\([ ]*(?<code>[A-Z]{1,2})[ ]*\)\n[ ]*(?<cabin>\S.+ Class)[ ]*\n\n/", $table[3] ?? '', $m)
            ) {
                //  Boeing 777-
                //   300/300ER
                //
