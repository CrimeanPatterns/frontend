...
class ChoiceExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface, ParseItinerariesInterface, ParseHistoryInterface
{
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        ...
        try {
            ...
            // TODO refs#25331 - The form does not respond, there is a 403 in the console only for this acc.
            if ($tab->evaluate('//h2[contains(text(),"Sign in to Choice Privileges rewards")]',
                    EvaluateOptions::new()->timeout(0)->allowNull(true))
                && $tab->evaluate('//input[@id="cpSignInUsername"]',
                    EvaluateOptions::new()->timeout(0)->allowNull(true))) {
        ...
        if (strstr($result->getAttribute('class'), 'member-firstname')) {
            ...
            $enterCode = $tab->evaluate('//h3[contains(text(),"Enter your code.")]', EvaluateOptions::new()->timeout(60)->allowNull(true));
            if ($enterCode) {
    ...
    public function parse(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
        ...
        $st->addProperty('Number', $account->guestProfile->loyaltyAccountNumber
            ?? $account->loyaltyAccounts[0]->loyaltyAccountNumber
            ?? null);
        ...
        $st->addProperty('MemberSince', strtotime($loyaltyAccount->memberSince));
