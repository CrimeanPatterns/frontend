...
{
    public $mailFiles = "choice/it-175973027.eml, choice/it-44205016.eml, choice/it-56559538.eml, choice/it-66367003.eml, choice/it-912557731.eml";
    ...
    private $subject = [
        'en' => [
            'Your Stay With Us Is Just Days Away',
            'Your Reservation at the ',
            'Thank you for staying with us at the ',
    ...
    private $reBody2 = [
        'en' => [
            ...
            ', thank you for staying with us at:',
        ],
        'es' => [
    ...
    private function parseHtml(Email $email): void
    {
        ...
        $checkIn = implode(' ', $this->http->FindNodes("//tr[{$this->eq($this->t('Check-in'), "translate(.,'*:','')")}]/following-sibling::tr[normalize-space()][last()]/descendant::text()[normalize-space()]"));

        $checkOut = implode(' ', $this->http->FindNodes("//tr[{$this->eq($this->t('Check-out'), "translate(.,'*:','')")}]/following-sibling::tr[normalize-space()][last()]/descendant::text()[normalize-space()]"));

        if (empty($checkIn) && $this->http->XPath->query("//*[{$this->starts($this->t('Check-in'))}]")->length === 0
            && empty($checkOut) && $this->http->XPath->query("//*[{$this->starts($this->t('Check-out'))}]")->length === 0
            && $this->http->XPath->query("//*[{$xpathTime}]")->length === 0
            && !empty($email->getStatement())
        ) {
            // it-912557731.eml
            $this->logger->debug('Skip hotel reservation, because dates not found! Parsed statement only.');
        ...
        $account = $this->http->FindSingleNode("descendant::text()[{$this->contains($this->t('account'))}][1]/following-sibling::node()[normalize-space()][1]", null, false, '/^[:#\s]*([A-Z\d]+\d[A-Z\d]+)$/');
        ...
        $hotelName = $this->http->FindSingleNode("({$xpathHotel}/preceding-sibling::tr/td)[1]");
    ...
    private function normalizeDate(?string $text): string
    {
        ...
        $in = [
            ...
            // ven., 30 mai 2025 12 h 00
            "/^\s*(?:[[:alpha:]]{2,}[\.]?\s*,)?\s*(\d{1,2})\s+([[:alpha:]]{3,})\s+(\d{4})\s+(\d{1,2}) h (\d{2})\s*$/u",
        ];
        $out = [
            '$2 $1 $3, $4',
            '$1.$2.$3, $4',
