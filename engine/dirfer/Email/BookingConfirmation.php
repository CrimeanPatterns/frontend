...
{
    public $mailFiles = "dirfer/it-149815128.eml, dirfer/it-154199629.eml, dirfer/it-34773831.eml, dirfer/it-35702343.eml, dirfer/it-35708498.eml, dirfer/it-415967808.eml, dirfer/it-899319902.eml";
    ...
    public static $dict = [
        'en' => [
            ...
            'totalPrice'           => ['Total Debited', 'Total paid', 'Total paid:'],
        ],
        'de' => [
    ...
    private function parseEmail(Email $email)
    {
        ...
        if (empty($totalPrice)) {
            $totalPrice = $this->http->FindSingleNode("//text()[{$this->eq($this->t('totalPrice'))}]/following::text()[normalize-space()][1]");

            $anotherFormat = true;
        }

        $nodes = $this->http->FindNodes("//text()[{$this->eq($this->t('Passengers Details'))}]/ancestor::table[1][{$this->starts($this->t('Passengers Details'))}]/descendant::tr[position()>1][count(./td) = 2]");
        ...
        foreach ($nodes as $root) {
            ...
            $meal = $this->http->FindSingleNode("./following-sibling::table[normalize-space()!=''][1]/descendant::text()[{$this->starts($this->t('Meals'))}]/following::text()[normalize-space()!=''][1]", $root);
            ...
            $accommodation = $this->http->FindSingleNode("./following-sibling::table[normalize-space()!=''][1]/descendant::text()[{$this->starts($this->t('Accommodation'))}]/following::text()[normalize-space()!=''][1]", $root);

            if ($accommodation == null){
                $accommodation = $this->http->FindSingleNode("./ancestor::div[1]/following-sibling::div[{$this->contains($this->t('Accommodation'))}]/descendant::text()[{$this->starts($this->t('Accommodation'))}]/following::text()[normalize-space()!=''][1]", $root);
            }
            ...
            $this->logger->debug("./descendant::text()[{$ruleTime}][1]");
            $depDate = $this->http->FindSingleNode("./descendant::text()[{$ruleTime}][1]", $root);

            if ($depDate == null) {
                $depDate = $this->http->FindSingleNode("./descendant::text()[normalize-space()][1]", $root);
                $depTime = $this->http->FindSingleNode("./ancestor::div[1]/following-sibling::div[normalize-space()][1]/descendant::text()[{$ruleTime}][1]", $root);
            ...
            $name = $this->http->FindSingleNode("./descendant::text()[{$ruleTime}][1]/following::text()[normalize-space()!=''][1]", $root);

            if ($name == null) {
                $name = $this->http->FindSingleNode("./ancestor::div[1]/following-sibling::div[normalize-space()][1]/descendant::text()[{$ruleTime}][1]/following::text()[normalize-space()][1]", $root);
            }
            ...
            $arrDate = $this->http->FindSingleNode("./descendant::text()[{$ruleTime}][2]", $root, false, "#(.+?)\s*(?:\(\s*\+\s*\d+\s*{$this->t('day')}|$)#");

            if ($arrDate == null) {
                $arrTime = $this->http->FindSingleNode("./ancestor::div[1]/following-sibling::div[normalize-space()][1]/descendant::text()[{$ruleTime}][2]", $root);

                if (preg_match("#(.+?)\s*\(\s*\+\s*(\d+)\s*days?\)$#", $arrTime, $m)){
                    $arrTime = $m[1];

                    $days = $m[2];
                }

                if ($depDate !== null && $arrTime !== null){
                    if (!empty($days)) {
                        $s->arrival()
                            ->date($this->normalizeDate($depDate . ' ' . $arrTime) + $days * 86400);
                    } else {
                        $s->arrival()
            ...
            if ($name == null) {
                $name = $this->http->FindSingleNode("./ancestor::div[1]/following-sibling::div[normalize-space()][1]/descendant::text()[{$ruleTime}][2]/following::text()[normalize-space()][1]", $root);
            }
