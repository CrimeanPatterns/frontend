...
class FerriesBookingPDF extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            'Lead Pax Name' => ['Lead Pax Name', 'Passenger Name'],
        ],
    ];
    ...
    public function ParseFerryPDF(Email $email, $text)
    {
        ...
        $paxText = $this->re("/\n+(\s*{$this->opt($this->t('Lead Pax Name'))}.+\n+\s*.+)\n+/", $text);
        $paxTable = $this->splitCols($paxText, [0, 25, 45, 60]);
        $f->general()
            ->traveller($this->re("/{$this->opt($this->t('Lead Pax Name'))}\s*(.+)/us", $paxTable[0]));
        ...
        if (preg_match_all("/\n+(\s*From.+\n+\s*(?:.+\n){1,})\s*{$this->opt($this->t('Lead Pax Name'))}/u", $text, $match)) {
