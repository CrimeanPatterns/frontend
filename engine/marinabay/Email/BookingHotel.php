...
class BookingHotel extends \TAccountChecker
{
    ...
    public function detectEmailByHeaders(array $headers)
    {
        if (array_key_exists('subject', $headers) && stripos($headers['subject'], 'your stay + exclusive privileges at Marina Bay Sands begin on') !== false
            || array_key_exists('subject', $headers) && stripos($headers['subject'], 'we look forward to seeing you in') !== false
            || array_key_exists('subject', $headers) && stripos($headers['subject'], ', skip the queue when you arrive!') !== false
            || array_key_exists('subject', $headers) && stripos($headers['subject'], ', your reservation has been modified') !== false) {
            return true;
        }
    ...
    private function hotel(Email $email): void
    {
        ...
        if ($this->http->FindSingleNode("//img[normalize-space(@alt)='MARINA BAY SANDS SINGAPORE']") !== null
            || $this->http->FindSingleNode("//img[@width>500 and contains(@src,'.marinabaysands.com/') and ancestor::a[contains(@href,'.marinabaysands.com/') or contains(@href,'.marinabaysands.com%2F') or contains(@e-editable,'logo_link')]]/@src[not(contains(normalize-space(), 'HOTEL_Awards') or contains(normalize-space(), 'casino/banner') or contains(normalize-space(), 'md_'))]", null, true, "/\/[-_A-z\d]*logo[-_A-z\d]*\.[A-z]/i") !== null
        ) {
