...
class TAccountCheckerStena extends TAccountChecker
{
    ...
    private function ParseItineraryHtml(string $number, ?string $email = null): array
    {
        ...
        for ($i = 0; $i < $nodes->length; $i++) {
            ...
            $this->logger->info("DepDate: $depDate");
            $s->departure()->date(strtotime($depDate));

            // arrDate
            $arrDateInfo = $this->http->FindSingleNode(".//span[normalize-space(text())='Arrives' or normalize-space(text())='Ankunft']/following-sibling::span[1]",
                $node);
