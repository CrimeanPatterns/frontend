...
class BookingDetails extends \TAccountChecker
{
    ...
    private static $dict = [
        ...
        'fr' => [
            ...
            //'Reservation Phone'                               => '',
            'contactFieldNames'                               => ['dorchestercollection.com'],
            'Guest Name:'                                     => 'Nom du client',
            'Thank you very much for making a reservation at' => 'Nous vous remercions d\'avoir choisi',
            // 'guestSummary' => '',
            'Room Details:'                                   => 'Type de chambre',
            // 'Our Check-in time is from' => '',
            // 'Check-out time until' => '',
            // 'Child' => '',
            'Number of Rooms:'                                => 'Nombre de chambres',
            'Arrival time'                                    => 'Arrivée à partir de',
            'Departure time'                                  => 'Départ',
            'Adults'                                          => 'Adultes',
            //'totalPrice'                                      => 'Gesamtpreis Zimmer',
            // 'Number of nights' => '',
            // 'Rate Details:' => '',
    ...
    private function parseHotel(Email $email, ?\DOMNode $root = null, $addXpathCond = ''): void
    {
        ...
        if ($guests === null) {
            $guests = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Adults'))}]/ancestor::tr[1]/descendant::td[normalize-space()][2]/descendant::text()[normalize-space()][1]", $root, false, "/^\d{1,3}/");
        }
        ...
        if ($child === null) {
            $child = $this->http->FindSingleNode("descendant::text()[{$this->eq($this->t('Child'))}]/ancestor::tr[1]/descendant::td[2]/descendant::text()[normalize-space()][last()]", $root, false, "/^\s*(\d{1,3})\s*$/");
        }
        ...
        if (empty($confirmationValue)) {
            $confirmationValue = $this->htmlToText($this->http->FindHTMLByXpath("descendant::text()[{$this->eq($this->t('Confirmation Number:'))}]" . $xpathFragmentNextCell . "/descendant::text()[normalize-space()][contains(normalize-space(), ' - ') or contains(normalize-space(), ', ')]", null, $root));
        }
