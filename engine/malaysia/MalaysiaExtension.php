...
class MalaysiaExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    ...
    {
        $tab->evaluate('//input[@id="signInName"]');
        sleep(2); // bug in safari - Element not found in cache
        $tab->logPageState();
    ...
    public function parse(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
        ...
        $json = $tab->fetch('https://www.malaysiaairlines.com/bin/services/new/getEnrichSummaryLCP')->body;
        ...
        if (isset($response->reachPoints)) {
            $statement->addProperty('ToNextStatus', $response->reachPoints);
        }
