...
//        $this->useCache();
    ...
    public function LoadLoginForm()
    {
        if ($this->attempt == 1) {
            ...
            $this->http->GetURL('https://www.target.com/account?prehydrateClick=true');
        }
        $password = $this->waitForElement(WebDriverBy::xpath('//input[@id="password"]'), 7);
        $username = $this->waitForElement(WebDriverBy::xpath('//input[@id="username"]'), 0);
        $this->saveResponse();

        if (!$password && $username) {
            $this->logger->notice('New form');
            $mover = new MouseMover($this->driver);
            $mover->logger = $this->logger;
            $mover->sendKeys($username, $this->AccountFields['Login'], 5);
//            $button = $this->waitForElement(WebDriverBy::xpath('//button[@id="login"]'), 2);
//            $button->click();
            sleep(3);
            $this->driver->executeScript("try { document.querySelector('button#login').click(); } catch (e) {}");
            sleep(3);
            $this->saveResponse();
            $button = $this->waitForElement(WebDriverBy::xpath('//button[@id="login"]'), 2);
            $button->click();
            ...
            $password = $this->waitForElement(WebDriverBy::xpath('//input[@id="password"]'), 15);
            if (!$password) {
                $username = $this->waitForElement(WebDriverBy::xpath('//input[@id="username"]'), 7);
                if ($username) {
...
//            $button = $this->waitForElement(WebDriverBy::xpath('//button[@id="login"]'), 0);
//            $button->click();
            $this->driver->executeScript("try { document.querySelector('button#login').click(); } catch (e) {}");
            sleep(5);
            $this->saveResponse();
...
//        $password->sendKeys($this->AccountFields['Pass']);
    ...
    public function Login()
    {
        ...
        if (
            stristr($this->http->currentUrl(), 'circle')
