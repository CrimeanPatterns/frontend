...
class TriprewardsExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface, ParseItinerariesInterface, ParseHistoryInterface, ActiveTabInterface
{
    use TextTrait;
    ...
    public function isLoggedIn(Tab $tab): bool
    {
        try {
            ...
            return  json_decode($profile)->success ?? false;
        } catch (\Exception $e) {
            $this->logger->error($e->getMessage());
            return false;
        }

        $loginButton = $tab->evaluate('
        //header[contains(@class,"desktop-nav")]//div[contains(@class,"top-nav-item sign-in")]
        | //header[contains(@class,"mobile-nav")]//div[contains(@class,"top-nav-item account")]
        
        | //div[contains(@class,"homepage-account-info-displays")]//div[@data-binding="AccountInfo.PointBalance"]
        | //div[contains(@class,"dashboard-account-summary ")]//span[@data-binding="Customer.PersonName.GivenName"]',
            EvaluateOptions::new()->nonEmptyString()->visible(true));

        $attr = $loginButton->getAttribute('data-binding');
        return stristr($attr, 'AccountInfo.PointBalance')
            || stristr($attr, 'Customer.PersonName.GivenName');
    }
    ...
    public function logout(Tab $tab): void
    {
        $tab->evaluate("//text()[starts-with(normalize-space(), 'Hi')]/ancestor::div[1]",
            EvaluateOptions::new()->timeout(5))->click();
    ...
    public function login(Tab $tab, Credentials $credentials): LoginResult
    {
        ...
        $tab->evaluate("//div[.//div[contains(text(), 'SIGN IN')] and @data-dropdown='account']",
            EvaluateOptions::new()->timeout(15)->visible(false))->click();

        // IFRAME
        /*$noThanks = $tab->evaluate('//*[@id="closeIconContainer"]',
            EvaluateOptions::new()->timeout(2)->allowNull(true));
        if ($noThanks) {
            $noThanks->click();
        }*/

        $tab->evaluate("//button[contains(@class, 'signin-button')]",
            EvaluateOptions::new()->timeout(2)->visible(false))->click();

        $loginOrVerify = $tab->evaluate("//input[contains(@name, 'username')] | //h1[contains(text(),'Verify Your Account')]",
            EvaluateOptions::new()->timeout(15)->allowNull(true));

        if (!$loginOrVerify) {
            $this->logger->notice('Retry call login form');
            $tab->gotoUrl('https://www.wyndhamhotels.com/wyndham-rewards/login');

            $loginOrVerify = $tab->evaluate("//input[contains(@name, 'username')] | //h1[contains(text(),'Verify Your Account')]");
            $tab->logPageState();
            $this->notificationSender->sendNotification('#24988 Retry call login form // MI');
        }
        ...
        if ($loginOrVerify && strstr($loginOrVerify->getInnerText(), 'Verify Your Account')) {
            ...
            $errorOrTitle = $tab->evaluate('//div[contains(@class,"homepage-account-info-displays")]//div[@data-binding="AccountInfo.PointBalance"]
            | //div[contains(@class,"dashboard-account-summary ")]//span[@data-binding="Customer.PersonName.GivenName"]',
        ...
        | //div[contains(@class,"dashboard-account-summary ")]//span[@data-binding="Customer.PersonName.GivenName"]
        | //header[contains(@class,"desktop-nav")]//div[contains(@class,"top-nav-item sign-in")]
        ', EvaluateOptions::new()->timeout(90)->allowNull(true));

        $tab->logPageState();
        sleep(5);
        $tab->logPageState();

        if (stristr($result->getAttribute('data-binding'), 'AccountInfo.PointBalance')
            || stristr($result->getAttribute('data-binding'), 'Customer.PersonName.GivenName')) {
            return LoginResult::success();
        }
        else if ($result->getAttribute('class') == 'ulp-input-error-message') {
            ...
            /*// TODO: The authorization bug appears after the page is reloaded
            if ($result->getAttribute('class') == 'top-nav-item sign-in') {
                $tab->gotoUrl('https://www.wyndhamhotels.com/wyndham-rewards/my-account');
                $result = $tab->evaluate('
            //button[contains(@value, "pick-authenticator")]
            | //span[@class="ulp-input-error-message" and normalize-space()!="" and not(contains(.,"Solve the challenge question to verify you are not a robot."))]
            ...
            if ($errorOrTitle->getNodeName() === 'DIV') {
                $errorOrTitle->getAttribute('class');
                $tab->gotoUrl("https://www.wyndhamhotels.com/wyndham-rewards/my-account");
                return LoginResult::success();
            }
        }
        if ($tab->evaluate('//img[@alt="captcha"]', EvaluateOptions::new()->timeout(5)->allowNull(true))) {
