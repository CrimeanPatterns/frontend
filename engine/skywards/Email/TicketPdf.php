...
class TicketPdf extends \TAccountChecker
{
    ...
    {
        $itineraries = $pdfTexts = [];

        /*
            Step 1: separate pdf-texts
        */
        ...
        foreach ($pdfs as $pdf) {
            ...
            if (empty($textPdfClean) || !$this->assignLang($textPdfClean)) {
                continue;
            }

            $pages = explode("\f", $textPdf);
            $buffer = [];

            foreach ($pages as $pageText) {
                if (preg_match("/^[ ]{0,20}Passenger name[: ]*(?:[ ]{2}|\n)/im", $pageText)) {
                    if (count($buffer) > 0) {
                        $bufferText = implode("\n", $buffer);
                        $lastIndex = count($pdfTexts) > 0 ? count($pdfTexts) - 1 : null;
        ...
        /*
            Step 2: parse pdf-texts
        */

        foreach ($pdfTexts as $textPdf) {
            $this->parsePdf($itineraries, $textPdf);
        }
    ...
    private function parsePdf(array &$itineraries, string $text): void
    {
        ...
        $fareTableText = $this->re("#Fare information\n(.*?)\n\n#ms", $text);
        //PD20.79-RN PD29.05-VV PD1.25- EUR328.00A
        $fareTableText = preg_replace("/(\d+\-\s+)([A-Z]{3}[\d+\.\,\']+[A-Z]\n)/", "$1     $2", $fareTableText);
        $fareTable = $this->splitCols($fareTableText, $this->colsPos($fareTableText));
        ...
        // Passengers
        $passengerName = $this->normalizeTraveller(preg_match("/Passenger name\s+(.*?)(?:\s{2,}|\n+[ ]*Membership)/s", $passTable[0], $m) ? preg_replace('/\s+/', ' ', $m[1]) : null);
        ...
        if (count($fareTable) > 3 && preg_match('/Total fare.*\n+[ ]*(?<points>\d+\s*POINTS)?(?:\s*[+]\n+[ ]*)?(?<currency>[A-Z]{3})[ ]*(?<amount>\d[,.\'\d ]*)/', $fareTable[3], $m)) {
            // TotalCharge
            // Currency
        ...
        if (count($transfer) > 0) {
            foreach ($transfer as $table) {
                ...
                // Passengers
                $passengerName = $this->normalizeTraveller(preg_match("/Passenger name\s+(.*?)\s{2,}/s", $passTable[0], $m) ? preg_replace('/\s+/', ' ', $m[1]) : null);
                $it['Passengers'] = [$passengerName];
    ...
    private function normalizeTraveller(?string $s): ?string
    {
        ...
        if (preg_match("/^(.{2,}?)\s+(?:{$namePrefixes}[.\s]*)+$/is", $s, $m)
            || preg_match("/^(.+?[[:upper:]]){$namePrefixes}$/su", $s, $m)
        ) {
            $s = $m[1];
        }
