...
class InvoiceFor extends \TAccountChecker
{
    ...
    public function ParseCruisePDF(Email $email, $text)
    {
        ...
        $this->date = strtotime($this->re("/DEPARTURE DATE\s*(\w+\s*\d{1,2}\,\s*\d{4})/", $text)) - 259200;
        $this->logger->debug($this->re("/DEPARTURE DATE\s*(\w+\s*\d{1,2}\,\s*\d{4})/", $text));
        ...
        foreach ($segments as $key => $segment) {
            ...
            if (!isset($segTable[3])) {
                $ashore = $this->normalizeDate(str_replace([' ', "\n"], '', $segTable[0]) . ', ' . $segTable[2]);
                $segTable = $this->splitCols($segments[$key + 1]);
                $this->nextRow = true;
                $aboard = $this->normalizeDate(str_replace([' ', "\n"], '', $segTable[0]) . ', ' . $segTable[2]);
            } else {
                $aboard = $this->normalizeDate(str_replace([' ', "\n"], '', $segTable[0]) . ', ' . $segTable[3]);
                $ashore = $this->normalizeDate(str_replace([' ', "\n"], '', $segTable[0]) . ', ' . $segTable[2]);
            }
    ...
    private function normalizeDate(string $string)
    {
        ...
        if (!empty($this->date) && $this->date > strtotime('01.01.2000') && strpos($string, '%year%') !== false && preg_match('/^\s*(?<date>\d{1,2}\/\d{1,2}) %year%(?:\s*,\s(?<time>\d{1,2}:\d{1,2}))?$/', $string, $m)) {
            $string = EmailDateHelper::parseDateRelative($m['date'], $this->date, true, '%D%/%Y%');
