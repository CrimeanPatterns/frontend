...
class TAccountCheckerWendys extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        $this->driver->executeScript('let remMe = document.querySelector(\'[name="keepSignedIn"]\'); if (remMe) remMe.checked = true;');
        $this->saveResponse();
        ...
        $button = $this->waitForElement(WebDriverBy::xpath("//button[contains(., 'Next') and not(@disabled)]"), 3);

        if (!$loginInput || !$button) {
            $this->DebugInfo = "login field not found";
            $this->saveResponse();
        ...
        if (!$passwordInput) {
            ...
            if ($question = $this->waitForElement(WebDriverBy::xpath('//div[p[contains(text(), "Click the link sent to")]]'), 0)) {
                /*
                $this->AskQuestion($question->getText(), null, "VerificationViaLink");
                */
                $this->AskQuestion('To continue updating your account, please copy and paste the verification link from your email.', null, "VerificationViaLink");

                return false;
            }

            $this->DebugInfo = "password field not found";
...
//        }
        ...
        // Access is allowed
        $this->waitForElement(WebDriverBy::xpath("//span[@class = 'header_name user_firstname']"), 10); // TODO: FAKE
        $this->saveResponse();
