...
class ConfirmationFor extends \TAccountChecker
{
    ...
    public function parseFlight(Flight $f): void
    {
        ...
        $nodes = $this->http->XPath->query("//text()[normalize-space()='Departure' or normalize-space()='Connecting Flight' or normalize-space()='Return']/ancestor::tr[1]");
        ...
        foreach ($nodes as $root) {
            ...
            foreach ($travellers as $traveller) {
                $seat = $this->http->FindSingleNode("//text()[normalize-space()='Detailed Traveler Information']/following::text()[{$this->contains($s->getFlightNumber())}]/following::text()[{$this->eq($traveller)}][1]/following::text()[starts-with(normalize-space(), 'Seat Number:')][1]", null, true, "/{$this->opt($this->t('Seat Number:'))}\s*(\d+\-?[A-Z])\s*$/");
