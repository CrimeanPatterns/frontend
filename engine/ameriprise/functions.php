    ...
    public function Login()
    {
        ...
        while ($time < $sleep) {
            ...
            $sq =
                ...
                ?? $this->waitForElement(WebDriverBy::xpath('//p[contains(text(), "sent a text to")]'), 0)
            ;
    ...
    public function parseQuestion()
    {
        ...
        */
        ...
        $question = $this->waitForElement(WebDriverBy::xpath('
            ...
            | //p[contains(text(), "sent a text to")]
        '), self::WAIT_TIMEOUT);
        ...
        // enter an answer
        if (
            !($input = $this->waitForElement(WebDriverBy::xpath("//input[@id = 'tfa-otp-answer' or @id = 'w-mfa-otp-answer']"), 0))
        ) {
            $this->saveResponse();
        ...
        if (
            !($sbm = $this->waitForElement(WebDriverBy::xpath("//button[(@id = 'w-mfa-submit' or @data-analytics = '2fa-verify-btn') and not(@disabled)]"), 2))
        ) {
            $this->saveResponse();
