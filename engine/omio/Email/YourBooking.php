...
{
    public $mailFiles = "omio/it-112243512.eml, omio/it-118025852.eml, omio/it-140173230-es.eml, omio/it-153600164-pt.eml, omio/it-79168915.eml, omio/it-88830172.eml, omio/it-89765521.eml, omio/it-89780409.eml, omio/it-913973959.eml, omio/it-914602751.eml";
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($trainSegments as $key => $tSegment) {
            $key++;
            $s = $train->addSegment();
            ...
            if (preg_match("/^{$this->opt($this->t('Carriage'))}\s*(?<car>\d+)[\s\-]+{$this->opt($this->t('Seat'))}\s*(?<seats>[A-Z\d\,\s]+)$/", $fields['seats'], $m)
                || preg_match("/{$this->opt($this->t('Segment '))}$key\:\s*{$this->opt($this->t('Carriage'))}\s*(?<car>\d+)[\s\-]+{$this->opt($this->t('Seat'))}\s*(?<seats>[A-Z\d\,\s]+)(?:$|{$this->opt($this->t('Segment '))})/u", $fields['seats'], $m)) {
                $s->extra()
                    ->car($m['car'])
        ...
        $this->logger->debug($xpathSegment . "[ *[2]/descendant::img[contains(@src,'/Ferry') or contains(@src,'/ferry.')] ]");
    ...
    private function getSegmentFields(\DOMNode $root): array
    {
        ...
        $cabin = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Class:'))}]", null, true, "/{$this->opt($this->t('Class:'))}\s*(.+)/");

        if (!empty($cabin)) {
            $result['cabin'] = $cabin;
        }

        $seats = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Seats:'))}]/ancestor::tr[1]", null, true, "/{$this->opt($this->t('Seats:'))}\s*(.+)/");
