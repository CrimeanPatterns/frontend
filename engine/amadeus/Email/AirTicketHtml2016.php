...
class AirTicketHtml2016 extends \TAccountChecker
{
    ...
    public static $dict = [
        ...
        'en' => [
            ...
            'Fare / Cabin:'                 => ['Fare / Cabin:', 'Fare / Cabin', 'Fare type/Cabin:', 'Fare type/Cabin', 'Cabin Class:', 'Cabin Class :', 'Class:'],
    ...
    private function parseEmail(): array
    {
        ...
        if ($total === null) {
            $total = $this->http->FindSingleNode("(//td[({$this->xpathArray($this->t('Total for all travellers'), 'normalize-space(.)', 'starts-with')}) and not(.//td)])[1]/preceding-sibling::td[string-length(normalize-space(.)) > 2][1]");
        }

        if (preg_match("/^(.*(?:mile|point).*) \+ (.*)/i", $total, $m)) {
            $it['SpentAwards'] = $m[1];
            $total = $m[2];
        }
...
//                $date = $m['Day'] . ' ' . \AwardWallet\Engine\MonthTranslate::translate(trim($m['Month']), $this->lang) . ' 20' . $m['Year'];
            ...
            if (empty($seg['BookingClass'])) {
                $class = array_unique(array_filter($this->http->FindNodes("following-sibling::tr[normalize-space()][position()<4]/descendant-or-self::tr/*[{$this->eq($this->t('Class'))}]/following-sibling::*[normalize-space()]", $root, "/^\s*([A-Z]{1,2})\s*$/")
                    ?? $this->http->FindNodes("(descendant::tr/*[{$this->eq($this->t('Class'))}]/following-sibling::*[normalize-space()])[1]", $root, "/^\s*([A-Z]{1,2})\s*$/")
                ));

                if (count($class) === 1) {
                    $seg['BookingClass'] = array_shift($class);
                }
            }
