...
{
    public $mailFiles = "eurostar/it-634395085.eml, eurostar/it-891797859.eml, eurostar/it-899786782.eml";
    ...
    public $detectBody = [
        'en' => [
            ...
            'Your train times and seat numbers are confirmed below.',
        ],
        'fr' => [
    ...
    public static $dictionary = [
        'en' => [
            'Booking Reference:'  => ['Booking Reference:', 'Booking reference:', 'Eurostar booking reference:'],
            ...
            // 'Seat'  => '',
            // 'Total paid'  => '',
            'points reward'  => ['points reward', 'reward points'],
        ],
        'fr' => [
        ...
        foreach (self::$dictionary as $lang => $dict) {
            if (
                (!empty($dict["Booking Reference:"]) && $this->http->XPath->query("//*[{$this->contains($dict['Booking Reference:'])}]")->length > 0
                    || !empty($dict["routeName"]) && $this->http->XPath->query("//*[{$this->contains($dict['routeName'])}]")->length > 0
                )
    ...
    private function parseEmailHtml(Email $email): void
    {
        ...
        if (empty($conf)) {
            $conf = $this->http->FindSingleNode("(//a/@href[contains(., 'eurostar.com')][contains(., 'pnr=')])[1]",
                null, true, "/(?:%3F|\?)pnr=([A-Z\d]{5,})(?:%26|&)/");
        }
        $t->general()
            ->confirmation($conf)
        ...
        $rewards = $this->http->FindSingleNode("//*[{$this->eq($this->t('Total paid'))}]/following::text()[normalize-space()][1]/ancestor::*[self::td or self::div][1]/following::text()[normalize-space()][1]",
            null, true, "/^\s*\+\s*(\d[\d., ]*?)\s*{$this->opt($this->t('points reward'))}/");
        ...
        foreach ($nodes as $root) {
            ...
            if (preg_match("/\n\d+ ?hrs \d+ ?mins\b\s*(\d+) *{$this->opt($this->t('Change'))}/u", $node, $m)) {
                $s->extra()
                    ->stops($m[1]);
            }
