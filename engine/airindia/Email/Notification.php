...
{
    public $mailFiles = "airindia/it-685161957.eml, airindia/it-891430447.eml";
    ...
    private function parseEmailHtml(Email $email)
    {
        ...
        $text = $this->http->FindNodes("//img[contains(@src, 'flight-connection')]/ancestor::*[normalize-space()][1]//text()[normalize-space()]");

        if (count($text) < 3) {
            $text = $this->http->FindNodes("//img[contains(@src, 'flight-connection')]/ancestor::*[normalize-space()][1]/ancestor::*[1]//text()[normalize-space()]");
        }
        $text = implode("\n", $text);
        $this->logger->debug('$text = ' . print_r($text, true));

        if (preg_match("/^\s*(?<dCode>[A-Z]{3})\n\s*(?<aCode>[A-Z]{3})\n\s*(?<dName>.+)\n\s*(?<dTime>\d{1,2}:\d{2})\n\s*(?<aName>.+)\n\s*(?<aTime>\d{1,2}:\d{2})\s*$/", $text, $m)
            || preg_match("/^\s*(?<dCode>[A-Z]{3})\n\s*(?<dName>.+)\n\s*(?<dTime>\d{1,2}:\d{2})\n\s*(?<aCode>[A-Z]{3})\n\s*(?<aTime>\d{1,2}:\d{2})\s*$/", $text, $m)
        ) {
            $s->departure()
                ->code($m['dCode'])
                ->name($m['dName'])
