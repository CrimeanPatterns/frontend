...
class TAccountCheckerLufthansaSelenium extends TAccountChecker
{
    ...
    public function InitBrowser()
    {
        ...
        $this->useCamoufox();
        //$this->seleniumOptions->setResolution($resolution);
...
//        $this->http->GetURL('https://www.lufthansa.com/us/en/account-statement');
        */
        ...
        $btnLogin = $this->waitForElement(WebDriverBy::xpath('//*[contains(@class, "btn-login")]'), 5, false);
        $this->saveResponse();
        ...
        if ($btnLogin) {
            ...
            $this->logger->error("Not found btn-login");
            $this->sendNotification('Not found btn-login // MI');
        }

        $loginInput = $this->waitForElement(WebDriverBy::xpath('//input[@name = "loginStepOne"]'), 5);
        $this->saveResponse();

        // TODO
        if (!$loginInput) {
            $this->logger->notice("click by btn one more time");
            $btnLogin = $this->waitForElement(WebDriverBy::xpath('//*[contains(@class, "btn-login")]'), 5);
            $this->saveResponse();
            ...
            $btnLogin = $this->waitForElement(WebDriverBy::xpath('//*[@class="login-flag-modal-body-button"]'), 3);
            $this->saveResponse();
            ...
            $loginInput = $this->waitForElement(WebDriverBy::xpath('//input[@name = "loginStepOne"]'), 12);
            $this->saveResponse();
        }
        ...
        try {
            ...
            $this->driver->executeScript('document.querySelector(".travelid-login__continueButton").click();');
        ...
        $rememberMe = $this->waitForElement(WebDriverBy::xpath('//label[contains(.,"Stay logged in")]'), 0);
        if ($rememberMe) {
            $this->saveResponse();
            $rememberMe->click();
        }
...
//        $button->click();
    ...
    {
        $this->sendNotification("Parse success -> attempt {$this->attempt} // MI");
        ...
        if ($this->ErrorCode == ACCOUNT_ENGINE_ERROR) {
            $this->SetBalance($profile->profile->loyaltyDetails->mileageBalance ?? null);
        }
