...
{
    public $mailFiles = "lufthansa/it-655675245.eml, lufthansa/it-664942417.eml, lufthansa/it-665100182.eml, lufthansa/statements/it-896724036.eml";
    ...
    public function findRoot(): \DOMNodeList
    {
        ...
        if ($this->http->XPath->query($xpath)->length === 0){
            $xpath = "//text()[{$this->eq($this->t('Mileage balance'))}]{$visible}/ancestor::tr[.//img/@src[{$this->contains('status-images')}]][1][count(*[normalize-space()]) = 1 or count(*[normalize-space()]) = 2]"
                . "[following::text()[normalize-space()]{$visible}[1]]";
        }
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if ($date === null) {
            $date = $this->http->FindSingleNode("descendant::text()[normalize-space()][{$this->starts($this->t('Account balance from:'))}]{$visible}",
                $root, true, "/{$this->opt($this->t('Account balance from:'))}\s*(.*)$/u");
        }
        ...
        if (preg_match("/label=HON Circle Points&percentage=[\d.]+&amount=(?<value>\d[\d\.,]*)&threshold=(?<needValue>\d[\d\.,]*)(&.*)?\s*$/m", $imgs, $m)) {
            // https://www.miles-and-more.com/lh_shared/mmg/circleimage.png?version=3&label=HON Circle Points&percentage=0.23&amount=1,390&threshold=6,000
            $m = $this->amount($m);
            $st
                ->addProperty('HONCirclePoints', $m['value'])
                ->addProperty('HONCirclePointsNeededToTheNextLevel', ($m['needValue'] > $m['value']) ? $m['needValue'] - $m['value'] : 0)
            ;
        }
