...
{
    public $mailFiles = "lufthansa/statements/it-903922878.eml, lufthansa/statements/it-96802251.eml, lufthansa/statements/it-96873289.eml";
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        $flightSegments = $this->http->FindSingleNode("*[normalize-space()][2]", $root, true, "/{$this->opt($this->t('Flight segments'))}\s*(\d[,.\'\d ]*)$/");

        if ($flightSegments !== null) {
            $st->addProperty('FlightSegments', $flightSegments);
        }
    ...
    {
        return $this->http->XPath->query("//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1]/descendant::text()[normalize-space()][1][{$this->eq($this->t('Award miles'))}] and *[normalize-space()][2]/descendant::text()[normalize-space()][1][{$this->eq($this->t('Status miles'))} or {$this->eq($this->t('Flight segments'))}] ]");
    }
    ...
    private function assignLang(): bool
    {
        ...
        foreach (self::$dictionary as $lang => $phrases) {
            if (!is_string($lang) || empty($phrases['Award miles']) || (empty($phrases['Status miles']) && empty($phrases['Flight segments']))) {
                continue;
            }
            ...
            if ($this->http->XPath->query("//*[{$this->contains($phrases['Award miles'])}]")->length > 0
                && ($this->http->XPath->query("//*[{$this->contains($phrases['Status miles'])}]")->length > 0
                    || $this->http->XPath->query("//*[{$this->contains($phrases['Flight segments'])}]")->length > 0)
            ) {
                $this->lang = $lang;
