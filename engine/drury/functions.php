...
class TAccountCheckerDrury extends TAccountChecker
{
    ...
    public function Parse()
    {
        ...
        $this->SetBalance(
            $this->http->FindSingleNode("//section[contains(@class, 'welcome-header')]//h2/span[contains(text(), 'Point')]", null, true, "/have\s*([^<]+)\s+Point/")
            ?? $this->http->FindSingleNode("//span[contains(text(), 'MY POINTS:')]", null, true, "/:\s*([^<]+)/")
        );
        // Member #
        ...
        if (empty($this->Properties["Number"])) {
            $this->SetProperty("Number", $this->http->FindSingleNode("//input[@name = 'AwardClubMemberNumber']/@value"));
        }
