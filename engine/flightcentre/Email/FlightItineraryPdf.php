...
{
    public $mailFiles = "flightcentre/it-12624786.eml, flightcentre/it-753364534.eml";
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            if ($this->assignLang($textPdf)) {
                ...
                // collect ota phone
                $phone = $this->re("/\|[ ]+{$this->opt($this->t('Call'))}[ ]+({$this->patterns['phone']})[ ]*\n/", $textPdf);
                $f->ota()->phone($phone);
    ...
    private function parseFlight(Flight $f, string $text): void
    {
        ...
        foreach ($segments as $sText) {
            ...
            if (!empty($airlineRef = $this->re("/{$this->opt($this->t('Airline Reference'))}[: ]+([A-Z\d]{5,10})\b/", $flightText))) {
                $s->airline()->confirmation($airlineRef);
                $airlineReference[] = $airlineRef;
            }

            // remove garbage
            $flightText = preg_replace("/[ ]+{$this->opt($this->t('garbagePhrases'))}[\s\S]*/", '', $flightText);
            $flightText = $this->re("/^(.+?\b\d{4}[ ]*)\n.+$/s", $flightText);

            $tablePos = $this->columnPositions($flightText);
            $table = $this->createTable($flightText, $tablePos);
            ...
            // collect cabin and operator
            if (preg_match("/^{$this->opt($this->t('Fare Class'))}[\: ]+(?<cabin>.+?)[ ]*\n[ ]*(?<operator>.+?)[ ]*$/m", $table[0], $m)) {
                $s->setCabin($m['cabin'])
                    ->setOperatedBy($m['operator']);
            }
    ...
    private function parsePrice($obj, array $invoiceTexts): void
    {
        ...
        foreach ($invoiceTexts as $text) {
            if (!preg_match("/^[ ]*{$this->opt($this->t('BOOKING NUMBER'))}[\: ]+{$this->opt($this->otaConfNumbers)}\b/m", $text)) {
