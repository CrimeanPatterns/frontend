...
{
    public $mailFiles = "flightcentre/it-12547214.eml, flightcentre/it-12550519.eml, flightcentre/it-740095739.eml, flightcentre/it-740760495.eml, flightcentre/it-740760859.eml, flightcentre/it-744336026.eml, flightcentre/it-905322758.eml, flightcentre/it-923085510.eml";
    ...
    public static $dictionary = [
        'en' => [
            ...
            'tripDetailsEnd'        => ['Notes', 'Payment Details', 'Booking conditions:'],
            ...
            'itHeaders'             => ['Flights', 'Flight to', 'Cruises', 'Stays', 'Check-in to', 'Rail', 'Car Hire', 'Tour', 'Transfer', 'Meet your transfer', 'Experience', 'Total Price'],
            'flightHeader'          => ['Flights', 'Flight to'],
            'Stays'                 => ['Stays', 'Check-in to'],
            'Transfer'              => ['Transfer', 'Meet your transfer'],
        ],
    ];
    ...
    private function parsePdf(Email $email, ?string $text): void
    {
        ...
        $tripSegments = $this->splitText($tripDetailsText, "/^([ ]*{$this->opt($this->t('itHeaders'))}(?: [-\')([:alpha:]]+)?(?:[ ].+|\s*$))/m", true);
        ...
        foreach ($tripSegments as $key => $segText) {
            if (preg_match("/^[ ]*{$this->opt($this->t('flightHeader'))}(?:[ ].+| [-\')([:alpha:]]+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Flight");
                $flights[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Cruises'))}(?:[ ].+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Cruise");
                $cruises[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Stays'))}(?:[ ].+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Hotel");
                $hotels[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Rail'))}(?:[ ]{1}.+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Rail");
                $rails[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Car Hire'))}(?:[ ].+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Car");
                $cars[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Tour'))}(?:[ ].+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Event");
                $events[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Transfer'))}(?:[ ].+)?\n+([\s\S]+)$/", $segText, $m)) {
                $this->logger->debug("Trip segment-{$key}: Transfer");
                $transfers[] = $m[1];
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Experience'))}(?:[ ].+|\n|\s*$)/", $segText)
                && preg_match("/^(?:.+\n+){0,4}[ ]*(?:Captain's Value Package|.*\bCombo Tour Public\b.*)(?:[ ]{2}.+|\n|\s*$)/i", $segText)
            ) {
                $this->logger->debug("Trip segment-{$key}: WTF");

                continue;
            } elseif (preg_match("/^[ ]*{$this->opt($this->t('Total Price'))}(?:[ ].+|\n|\s*$)/", $segText)) {
    ...
    private function parseFlight(Email $email, string $text, array $travellers, string $pnr): void
    {
        ...
        foreach ($segments as $key => $sText) {
            ...
            if (preg_match("/{$this->opt($this->t('Room'))}/", $sText)) {
                continue;
            }
            ...
            $this->logger->error($passengerDetailsText);
            ...
            if ($passengerDetailsText && count($travellers) > 0) {
                ...
                $this->logger->error("/^([ ]*(?:[[:alpha:]]+[. ]+)?{$this->opt($travellers)})$/imu");
                $seats = [];

                foreach ($passengersRows as $pRow) {
                    $this->logger->warning($pRow);
                    $passengerName = $this->normalizeTraveller($this->re("/^[ ]*(.{2,})/", $pRow));
                    $this->logger->debug('0-' . $passengerName);
                    ...
                    if (empty($tkt)) {
                        $tkt = $this->re("/{$passengerName}\s*\n*.+{$this->opt($this->t('Ticket No:'))}\s*(?<ticket>[\d\-]{9,})\n/", $pRow);
                    }
    ...
    private function parseCruise(Email $email, string $text, array $travellers): void
    {
        ...
        if (preg_match("/Cabin Details\n+\s*(?<vNumber>[A-Z]\d{1,4})[\s\•]+Room\s+(?<room>\d+)[\s\•]+\D+\b[\s\•]+Deck\s*(?<deck>\d+)[\s\•]+/", $text, $m)) {
            $it->setVoyageNumber($m['vNumber']);
            $it->setRoom($m['room']);
            $it->setDeck($m['deck']);
        }

        $it->setShip($this->re("/nights[\s\•]+(.+)/", $text));

        $depTime = $this->re("/Depart\s+(?:.{5,}\b)[\s\•]+(\d+\:\d+a?p?m?)\n/", $text);
        $arrTime = $this->re("/Return\s+(?:.{5,}\b)[\s\•]+(\d+\:\d+a?p?m?)\n/", $text);

        $segmentsText = $this->re("/Itinerary Details\n(.+\b)\n+\s+Cancellation Policy/msu", $text);

        if (!empty($depTime) && !empty($arrTime)
            && preg_match("/^\s+(?<depDate>\d+\s*\w+\s*\d{4})\n\s+(?<depPort>\D+)\n.+\n\s+(?<arrDate>\d+\s*\w+\s*\d{4})\n\s+(?<arrPort>\D+)$/msu", $segmentsText, $match)) {
            $s = $it->addSegment();
            $s->setAboard(strtotime($match['depDate'] . ', ' . $depTime));
            $s->setName($match['depPort']);

            $s = $it->addSegment();
            $s->setAshore(strtotime($match['arrDate'] . ', ' . $arrTime));
            $s->setName($match['arrPort']);
        }
    ...
    private function parseHotel(Email $email, string $text, array $travellers): void
    {
        ...
        if ($cancellationPolicy) {
            ...
            if (preg_match("/If cancelled on or after\s*(\d+\s*\w+\s*\d{4})\, a cancellation charge of/", $cancellationPolicy, $m)) {
                $it->setDeadline(strtotime($m[1]));
            }
        }
    ...
    private function parseTransfer(Email $email, string $text, array $travellers): void
    {
        ...
        if (stripos($text, "Ferry:") !== false) {
            return;
        }

        $text = preg_replace("/{$this->opt($this->t('Private Transfer: From'))}/", "", $text);
        ...
        $this->logger->warning($transferName);
