...
class TAccountCheckerAplus extends TAccountChecker
{
    ...
    public function checkErrors()
    {
        ...
        if (
            ...
            || ($this->http->Response['code'] == 500 && $this->http->FindPreg('/"message": "Internal Server Error",\s*"details":\s*"Unable to get a response from backend",\s*"code": "500"\s*\}\s*\}/'))
...
//            $exp = str_replace('T00:00:00','', $loyaltyAccount->cardExpirationDate);
    ...
    public function ParseItineraries()
    {
        ...
        if (
            ...
            || $this->http->FindPreg('/^"Incorrect locale information provided"$/')
        ) {
            return [];
        }
...
//            $this->http->Log("<pre>".var_export($response, true)."</pre>", false);
    ...
    private function parseItinerary($item, $data)
    {
        ...
        $this->logger->info("[{$this->currentItin}] Parse Hotel #{$data->number}", ['Header' => 4]);
