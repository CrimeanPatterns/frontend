...
{
    public $mailFiles = "mozio/it-788383424.eml, mozio/it-788651245.eml, mozio/it-815992571.eml, mozio/it-887966584.eml";
    public $subjects = [
        'Your transfer is confirmed with ',
        'Reminder: your upcoming transfer with ',
    ];

    public $lang = 'en';

    public $fromEmails = ['@mozio.com', '@fora.travel'];
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        foreach (self::$dictionary as $dict) {
            if (!empty($dict['Your reservation details']) && !empty($dict['PICK UP']) && !empty($dict['DROP OFF'])
                ...
                || $this->http->XPath->query("//text()[{$this->eq($dict['Manage reservation'])}]")->length > 0)
    ...
    public function ParseTransfer(Email $email)
    {
        ...
        if (empty($pax)) {
            $pax = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Hi,'))}]", null, true, "/^{$this->opt($this->t('Hi,'))}\s*([[:alpha:]][-.\/\'’[:alpha:] ]*[[:alpha:]])$/");
        }
        ...
        foreach ($nodes as $root) {
            ...
            $this->logger->error(var_export($arrs, true));
