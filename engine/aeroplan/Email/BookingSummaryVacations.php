...
class BookingSummaryVacations extends \TAccountChecker
{
    ...
    private function parseEmailHtml(Email $email)
    {
        ...
        if (count($travellers) === 0) {
            $travellers = $this->http->FindNodes("//text()[starts-with(normalize-space(), 'Email:') or starts-with(normalize-space(), 'Aeroplan Number:')]/preceding::text()[normalize-space()][1]/ancestor::div[1][contains(@style, 'bold')]");
        }
        ...
        $this->logger->debug("//text()[{$this->starts($this->t('Aeroplan Number:'))}]/ancestor-or-self::node()[not({$this->eq($this->t('Aeroplan Number:'))})][1]");
        ...
        foreach ($aNodes as $aRoot) {
            ...
            if (!empty($account) && !in_array($account, array_column($email->getTravelAgency()->getAccountNumbers(), 0))) {
                $paxName = $this->http->FindSingleNode("preceding::text()[{$this->starts($this->t('Date of Birth:'))}][1]/preceding::text()[normalize-space()][1]",
                    $aRoot);

                if (empty($paxName)) {
                    $paxName = $this->http->FindSingleNode("preceding::div[contains(@style, 'bold')][1]", $aRoot);
                }
                $email->ota()
                    ->account($account, false, $this->nicePax($paxName),
        ...
        foreach ($hNodes as $hRoot) {
            ...
            $h->general()
                ->noConfirmation()
                ->travellers($this->nicePax($travellers), true);
        ...
        if ($fNodes->length === 0) {
            $xpath = "//*[@src[(contains(normalize-space(.),'fa-plane'))]]/ancestor::table[1]/descendant::text()[contains(normalize-space(),':')]/ancestor::tr[1]";
            //$this->logger->debug('$xpath = '.print_r( $xpath,true));
            $fNodes = $this->http->XPath->query($xpath);
        }
