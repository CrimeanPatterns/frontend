...
{
    public $mailFiles = "aeroplan/it-619354276.eml, aeroplan/it-622316723.eml, aeroplan/it-636102963.eml, aeroplan/it-774036234.eml, aeroplan/it-901511393.eml, aeroplan/it-901927256.eml, aeroplan/it-902241846.eml, aeroplan/it-902563136.eml";
    public $subjects = [
    ...
    public static $dictionary = [
        "en" => [
            ...
            'Return'                            => 'Return',
            "Departure"                         => "Departure",
            'Adult'                             => ['Adult', 'Child', 'Infant on lap'],
        ],
        "fr" => [
            ...
            "Check-in and boarding gate deadlines" => "Délai d'enregistrement et d'embarquement",
            'Adult'                                => ['Adulte', 'Adult', 'Child', 'Infant on lap'],
            'Base fare - '                         => ['Tarif de base -', 'Base fare - '],
        ],
    ];
    ...
    public function ParseFlight(Email $email)
    {
        ...
        $notDisplayed = "not(ancestor::*/@style[contains(translate(., ' ', ''), 'display:none;') or contains(translate(., ' ', ''), 'font-size:1.0pt;')])";
        $xpath = "//text()[{$this->contains($allTitle)}][{$notDisplayed}]/ancestor::*[count(.//text()[{$this->contains($allTitle)}]) = 1][last()][not(contains(normalize-space(), 'sophospsmartbannerend'))]";
        ...
        foreach ($messages as $mesRoot) {
            ...
            $conf = null;
            $confs = array_unique(array_filter($this->http->FindNodes(".//text()[{$this->eq($this->t('Booking Confirmation'))}]/following::text()[normalize-space()][{$notDisplayed}][1]",
                $mesRoot, "/^([A-Z\d]{6})$/")));

            if (empty($confs)) {
                $confs = array_unique(array_filter($this->http->FindNodes(".//a/@href[{$this->contains('.aircanada.')}][{$this->contains('pnr')}]",
                    $mesRoot, "/&pnr=([A-Z\d]{6})\W/")));
            }
            ...
            $f->general()
                ->confirmation($conf)
                ->travellers(array_filter($this->http->FindNodes(".//text()[{$this->starts($this->t('Ticket #:'))}]/preceding::text()[normalize-space()][{$notDisplayed}][not({$this->eq($this->t('(Infant on lap)'))})][1]", $mesRoot)),
                    true);
            ...
            if (count($tickets) > 0) {
                foreach ($tickets as $ticket) {
                    $pax = $this->http->FindSingleNode(".//text()[{$this->eq($this->t('Passengers'))}]/ancestor::table[1]/descendant::text()[{$this->contains($ticket)}]/preceding::text()[normalize-space()][{$notDisplayed}][not({$this->eq($this->t('Ticket #:'))})][not({$this->eq($this->t('(Infant on lap)'))})][1]", $mesRoot);
            ...
            if (count($accounts) > 0) {
                foreach ($accounts as $account) {
                    $pax = $this->http->FindSingleNode(".//text()[{$this->eq($this->t('Passengers'))}]/ancestor::table[1]/descendant::text()[{$this->contains($account)}]/preceding::text()[normalize-space()][{$notDisplayed}][not({$this->contains($tickets)})][not({$this->eq($this->t('(Infant on lap)'))})][1]", $mesRoot);
            ...
            if (preg_match("/^(?<currency>[A-Z]{3})\s*\D*(?<total>[\d\.\,\s]+)\s*$/", $price, $m)
                ...
                if (count(array_filter($costValues)) === 0) {
                    $costValues = $this->http->FindNodes(".//text()[{$this->starts($this->t('Base fare - '))}]/ancestor::tr[1]/following-sibling::tr[1][{$this->starts($this->t('Adult'))}]/descendant::td[2]", $mesRoot, "/^\D*(\d[\d.,]*?)\D*$/");
                    $costValues2 = $this->http->FindNodes(".//text()[{$this->starts($this->t('Base fare - '))}]/ancestor::tr[1]/following-sibling::tr[2][{$this->starts($this->t('Adult'))}]/descendant::td[2]", $mesRoot, "/^\D*(\d[\d.,]*?)\D*$/");
                    $costValues = array_merge($costValues, $costValues2);
                }
                ...
                $feeNodes = $this->http->XPath->query(".//text()[{$this->eq($this->t('Taxes, Fees and Charges'))}]/ancestor::tr[1]/following-sibling::tr[normalize-space()]"
                    . "[following::text()[{$this->starts($this->t('Grand total'))}]]", $mesRoot);
                ...
                foreach ($feeNodes as $feeRoot) {
                    ...
                    if (!empty($feeName) && !empty($feeSumm)) {
                        if (preg_match("/^\s*{$this->opt($this->t('Adult'))}\s*(\(.*\d.*x\d+\))?$/", $feeName)) {
                            $name2 = $this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][1][td[2][not(normalize-space())]][not(.//text()[{$this->eq($this->t('Taxes, Fees and Charges'))}])]/td[normalize-space()][1]", $feeRoot);

                            if (empty($name2)
                                && !empty($this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][1][count(*[normalize-space()]) = 2]/td[normalize-space()][1]", $feeRoot, true,
                                    "/^\s*{$this->opt($this->t('Adult'))}\s*(\(.*\d.*x\d+\))?$/"))
                            ) {
                                $name2 = $this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][2][td[2][not(normalize-space())]][not(.//text()[{$this->eq($this->t('Taxes, Fees and Charges'))}])]/td[normalize-space()][1]", $feeRoot);
                            }

                            if (empty($name2)
                                && !empty($this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][2][count(*[normalize-space()]) = 2]/td[normalize-space()][1]", $feeRoot, true,
                                    "/^\s*{$this->opt($this->t('Adult'))}\s*(\(.*\d.*x\d+\))?$/"))
                            ) {
                                $name2 = $this->http->FindSingleNode("./preceding-sibling::tr[normalize-space()][3][td[2][not(normalize-space())]][not(.//text()[{$this->eq($this->t('Taxes, Fees and Charges'))}])]/td[normalize-space()][1]", $feeRoot);
                            }

                            if (!empty($name2)) {
                                $feeName = $name2 . ' (' . $feeName . ')';
                            }
                        }
                        $f->price()
                ...
                if (empty($fee)) {
                    $fee = $this->http->FindSingleNode(".//text()[{$this->starts($this->t('Carrier surcharges'))}]/ancestor::tr[1]/following-sibling::tr[1]/descendant::td[2]", $mesRoot, true, "/^\D*(\d[\d.,]*?)\D*$/");
                }
            ...
            foreach ($nodes as $root) {
                $s = $f->addSegment();
                $depPoint = implode(" ", $this->http->FindNodes("./preceding::tr[1]/td[1]//text()[normalize-space()][{$notDisplayed}]", $root));

                if (preg_match("/^(?<depName>.+)\s+(?<depCode>[A-Z]{3})\s*$/", $depPoint, $m)
                    || preg_match("/^\s*(?<depCode>[A-Z]{3})\s*$/", $depPoint, $m)
                ) {
                    $s->departure()
                        ->code($m['depCode']);
                ...
                $arrPoint = implode(" ", $this->http->FindNodes("./preceding::tr[1]/td[normalize-space()][2]//text()[normalize-space()][{$notDisplayed}]", $root));

                if (preg_match("/^(?<arrName>.+)\s+(?<arrCode>[A-Z]{3})\s*$/", $arrPoint, $m)
                    || preg_match("/^\s*(?<arrCode>[A-Z]{3})\s*$/", $arrPoint, $m)
                ) {
                    $s->arrival()
                        ->code($m['arrCode']);
                ...
                $arrTime = $this->http->FindSingleNode("./*[normalize-space()][2]", $root);
                ...
                $airInfo = implode("\n", $this->http->FindNodes("./following::tr[3][{$this->contains($this->t('Cabin'))}]/descendant::text()[normalize-space()][{$notDisplayed}]", $root));
                ...
                if (preg_match("/^(?<aName>(?:[A-Z][A-Z\d]|[A-Z\d][A-Z]))\s*(?<fNumber>\d{1,4})[\s\•]+{$this->opt($this->t('Operated by'))}\s*(?<operator>.+)\n/", $airInfo, $m)) {
                    ...
                    $alConfs = array_unique($this->http->FindNodes("//text()[{$this->starts(preg_replace("/(.+)/", $m['operator'] . " $1", $this->t('booking reference:')))}]",
                        null, "/{$this->opt($this->t('booking reference:'))}\s*([A-Z\d]{5,7})\s*$/"));

                    if (count($alConfs) === 1 && !in_array($alConfs[0], array_column($f->getConfirmationNumbers(), 0))) {
                        $s->airline()
                            ->confirmation($alConfs[0]);
                    }
                }
