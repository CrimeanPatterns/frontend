...
//    public $pdfNamePattern = ".*(?:Booking_Confirmation|Buchungsbest_tigung).*pdf";
    ...
    public static $dict = [
        'en' => [
            ...
            'feesEnd'          => ['Total airfare and taxes after options', 'Total airfare and taxes before options (per passenger)', 'Total before options (per passenger)', 'Subtotal', 'GRAND TOTAL (Canadian dollars)'],
        ...
        'fr' => [
            ...
            'feesEnd'            => ['Total avant les options (par passager)', 'Total partiel', 'Total du tarif aérien et des taxes après la'],
    ...
    private function parseEmail($textPDF, Email $email): bool
    {
        ...
        if (preg_match('/^(?<currency>[^\-\d)(]+?)[ ]*(?<integer>\d[,.‘\'\d]*)[. ]*(?<decimals>\d{2})$/u', $totalPrice, $matches)
            ...
            if ($passengersCount !== null) {
                ...
                $travelOptionsFees = $this->re("/^[ ]*{$this->opt($this->t('Travel Options'))}\n+([\s\S]+?)\n+[ ]*(?:{$this->opt($this->t('feesEnd'))}(?:[ ]{2}|$)|\n\n{$this->opt($this->t('GRAND TOTAL'))} *\()/im", $table[1]);
                preg_match_all("/^(?<name>[A-z].+\n*)[ ]{5,}(?<fee>\d[\d\.\,]*)\n(?<name2> {0,3}(\S+ ?)*)?$/mu", $travelOptionsFees, $match, PREG_SET_ORDER);

                foreach ($match as $feeRow) {
                    if (preg_match('/^(?:' . preg_quote($matches['currency'], '/') . ')?[ ]*(?<amount>\d[,.‘\'\d]*)$/u', $feeRow['fee'], $m)) {
                        $feeAmount = PriceHelper::parse($m['amount'], $currencyCode);
                        $f->price()->fee(str_replace("\n", '', $feeRow['name'] . ' ' . $feeRow['name2']), $feeAmount);
                    }
                }
                    ...
                    if (!empty($flSeats[$v[1] . $v[2]])) {
                        ...
                        foreach ($seats as $seat) {
                            ...
                            foreach ($passengerTextArray as $passengerText) {
                                ...
                                $pax = $this->re("/({$this->opt($travellers)})\n(?:.+\n){0,6}.+\s$v[1]$v[2]\s+$seat/u", $passengerText);
