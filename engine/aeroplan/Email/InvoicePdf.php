...
{
    public $mailFiles = "aeroplan/it-125171164.eml, aeroplan/it-426350541.eml, aeroplan/it-535919079.eml, aeroplan/it-657285829.eml, aeroplan/it-671657929-cruise.eml, aeroplan/it-725468147.eml, aeroplan/it-751885262.eml, aeroplan/it-887660011.eml, aeroplan/it-898947830.eml";
    public $subjects = [
        '/Invoice\s*\d[\d\s]{3}\d/',
    ...
    public static $dictionary = [
        "en" => [
            ...
            'Confirmation Number' => ['Confirmation Number', 'CRUISE CONFIRMATION', 'CRUISELINE CONFIRMATION', 'CONFIRMATION', 'CONFIRMATION CRUISE NUMBER'],
            // 'Cruise Description' => '',
        ],
        "fr" => [
    ...
    public function detectProv($text): bool
    {
        ...
        if (strpos($text, 'Vacances Air Canada') === false && strpos($text, 'Air Canada Vacations') === false && empty($this->code)) {
            return false;
        }
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        foreach ($pdfs as $pdf) {
            ...
            foreach (preg_split('/\n+/', $paxText) as $paxRow) {
                if (preg_match("/^[ ]{0,10}(?<number>\d{1,3})[ ]{2,30}(?:(?:MSTR|MRS|MR|MS|M)[ ]+)?(?<name>.*)$/m", $paxRow, $m)) {
                    $passengerName = preg_replace('/[ ]{5}.*/', '', $m['name']);

                    if ($passengerName) {
                        $this->travellers[$m['number']] = $passengerName;
                    }
                } else {
                    $this->logger->debug('Wrong Passenger Information!');
                    $this->travellers = [];

                    break;
                }
            }
            $this->logger->info('Passengers List:');
            $this->logger->debug(print_r($this->travellers, true));

            if ($this->containsText($text, $this->t('Flight Information')) === true) {
                $this->parseFlightsPdf($email, $text);
            }

            if ($this->containsText($text, $this->t('Hotel Details')) === true) {
                $this->parseHotelsAndTransfersPdf($email, $text);
            }
    ...
    private function parseFlightsPdf(Email $email, $text): void
    {
        ...
        foreach ($segments as $sText) {
            ...
            $passengersVal = $this->re("/.{80} (\d{1,3}(?: ?[-,] ?\d{1,3})*)$/m", $sText);
            $passengers = $this->getPassengersByNumbers($passengersVal);
            ...
            if (preg_match($re2, $sText, $m) || preg_match($re, $sText, $m) || preg_match($re4, $sText, $m) || preg_match($re3, $sText, $m) || preg_match($re5, $sText, $m)) {
                ...
                $this->logger->debug($m['arrDate'] . ', ' . $m['arrTime']);
    ...
    private function parseHotelsAndTransfersPdf(Email $email, $text): void
    {
        $this->logger->debug(__FUNCTION__ . '()');
        ...
        foreach ($segments as $sText) {
            if (preg_match("/^\s*{$this->opt($this->t('Transfers Description'))}/u", $sText)) {
            ...
            } elseif (preg_match("/^\s*{$this->opt($this->t('Hotel Description'))}/u", $sText)) {
                ...
                Superior Room Run of House-All Inclusive*/
                    ...
                    $h->booked()
                        ->checkIn($this->normalizeDate($m['arrDate']))
                        ->checkOut($this->normalizeDate($m['depDate']));
                            ...
                            foreach ($email->getItineraries() as $it) {
                                ...
                                if ($it->getType() == 'hotel' && $h->getId() !== $it->getId()) {
                                    if ($h->getHotelName() === $it->getHotelName()
                                        ...
                                        if (count($passengers) > 0) {
                                            foreach ($passengers as $traveller) {
    ...
    private function parseCruisePdf(Email $email, $text): void
    {
        $this->logger->debug(__FUNCTION__ . '()');
        $this->logger->debug(' > CRUISE');
        ...
        if (empty($dateEmbarkation)) {
            $dateEmbarkation = $this->re("/{$this->opt($this->t('Embarkation'))}\s*\:\s*(\w+\s*\d+\s*\d{4})/", $remarksText);
        }

        if (empty($dateDisembarkation)) {
            $dateDisembarkation = $this->re("/{$this->opt($this->t('Disembarkation'))}\s*\:\s*(\w+\s*\d+\s*\d{4})/", $remarksText);
        }

        $this->logger->error($dateDisembarkation);
        ...
        if (preg_match("/^[ ]*{$this->opt($this->t('Embarkation'))}[ ]*[:]+[ ]*(?:\w+\s*\d+\s*\d{4}\s*)?(?<name>\S.+?\S)[ ]*[- ]+[ ]*(?<time>{$this->patterns['time']})/m", $remarksText, $m)) {
            $s->setName($m['name']);
            $timeEmbarkation = $m['time'];
        } elseif (preg_match("/^[ ]*{$this->opt($this->t('Embarkation'))}[ ]*[:]+[ ]*(?:\w+\s*\d+\s*\d{4}\s*)?(?<name>\S.+?\S)\s*\n[ ]*{$this->opt($this->t('Embarkation'))}2[ ]*[:]+[ ]*(?<time>{$this->patterns['time']})/m", $remarksText, $m)) {
            $s->setName($m['name']);
            $timeEmbarkation = $m['time'];
        }
        ...
        if (preg_match("/^[ ]*{$this->opt($this->t('Disembarkation'))}[ ]*[:]+[ ]*(?:\w+\s*\d+\s*\d{4}\s*)?(?<name>\S.+?\S)[ ]*[- ]+[ ]*(?<time>{$this->patterns['time']})/m", $remarksText, $m)) {
            $s->setName($m['name']);
            $timeDisembarkation = $m['time'];
        } elseif (preg_match("/^[ ]*{$this->opt($this->t('Disembarkation'))}[ ]*[:]+[ ]*(?:\w+\s*\d+\s*\d{4}\s*)?(?<name>\S.+?\S)\s*\n[ ]*{$this->opt($this->t('Disembarkation'))}2[ ]*[:]+[ ]*(?<time>{$this->patterns['time']})/m", $remarksText, $m)) {
            $s->setName($m['name']);
            $timeDisembarkation = $m['time'];
        }
    ...
    private function getPassengersByNumbers(?string $passengersVal): array
    {
        $passengers = [];
        $passengerNumbers = preg_split('/(?:\s*,\s*)+/', $passengersVal ?? '');

        foreach ($passengerNumbers as $pNum) {
            if (preg_match("/^(\d+)(?:\s*-\s*)+(\d+)$/", $pNum, $m)) {
                $passengerKeys = range($m[1], $m[2]);

                foreach ($passengerKeys as $key) {
                    if (array_key_exists($key, $this->travellers)) {
                        $passengers[] = $this->travellers[$key];
                    }
                }
            } elseif (array_key_exists($pNum, $this->travellers)) {
                $passengers[] = $this->travellers[$pNum];
            }
        }
    ...
    private function normalizeDate($str)
    {
        ...
        $in = [
            // 27 août 2023
            "#^(\d{1,2})[\s\-]([[:alpha:]]+)\.?[\s\-](\d{4})\s*$#u",
            // 08 mai 2025, 06:30
            "#^(?:[[:alpha:]]+\.?\s*[,\s]\s*)?(\d{1,2})[\s\-]([[:alpha:]]+)\.?[\s\-](\d{4})\s*\,\s*(\d{1,2}:\d{2}(?:\s*[AP]M)?)\s*$#u",
        ];
        $out = [
            "$1 $2 $3",
            "$1 $2 $3, $4",
