...
class TAccountCheckerAeroplanSelenium extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        $this->waitForElement(WebDriverBy::xpath($xpathForm . '//input[@name = "username"] | //input[contains(@class, "gig-tfa-code-textbox")] | //button[@id = "acUserMenu-aco" or @id = "libraUserMenu-signIn"] | //*[contains(@class, "ac-account-signin-button")]//button'), 20);

        if ($btn = $this->waitForElement(WebDriverBy::xpath('//button[@id = "acUserMenu-aco" or @id = "libraUserMenu-signIn"]| //*[contains(@class, "ac-account-signin-button")]//button'), 0)) {
            $btn->click();
    ...
    public function Login()
    {
        ...
        if ($message = $this->http->FindSingleNode('//form[@id = "gigya-login-form"]//*[self::div or self::span][contains(@class, "gigya-error-msg-active")]')) {
            ...
            if (
                ...
                $this->logger->debug('check for captcha progress');
                $this->saveResponse();
                $this->DebugInfo = "FunCaptcha";
                $this->sendNotification('refs #25452 aeropoplan - need to check funcaptcha // IZ');
                ...
                $iframe = $this->waitForElement(WebDriverBy::xpath('//iframe[@data-e2e="enforcement-frame"]'), 10, false);
                $this->driver->switchTo()->frame($iframe);
                $this->driver->executeScript('document.getElementsByName("fc-token")[0].value = "' . $captcha . '";');
                */
...
We determined that the last time you had account activity with Aeroplan was on " . date("m/d/Y", $lastActivity) . ", so the expiration date was calculated by adding 18 months to this date.";
    ...
    public function ParseItineraries()
    {
        ...
        foreach ($response->data->getAeroplanPNRcognito->bookings ?? [] as $booking) {
            ...
            $this->sendNotification('OverrideServlet success // MI');
            /*
...
//        }
    ...
    private function ParsePageHistory($startIndex, $startDate)
    {
        ...
        /*
         * refs #24874 closed, this notification no more needed
