...
//            $selenium->useChromium();
            ...
            $btn->click();
            $selenium->waitForElement(WebDriverBy::xpath("//font[contains(text(), 'The Email Address or Password is incorrect.')] | //a[@data-open = \"PendingCashBack\"]/following-sibling::h6[1]"), 6);
            $err = $selenium->waitForElement(WebDriverBy::xpath("//font[contains(text(), 'The Email Address or Password is incorrect.')]"), 0);
            $this->savePageToLogs($selenium);
            ...
            */
            ...
            if ($currentUrl == 'https://www.mrrebates.com/errors/malformed_link.asp') {
                $retry = true;
            }

            if ($currentUrl == 'https://www.mrrebates.com/') {
                $selenium->http->GetURL(self::REWARDS_PAGE_URL);
                $selenium->waitForElement(WebDriverBy::xpath('//a[@data-open = "PendingCashBack"]/following-sibling::h6[1]'), 6);
                $this->savePageToLogs($selenium);
            }
