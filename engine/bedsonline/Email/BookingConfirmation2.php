...
{
    public $mailFiles = "bedsonline/it-142189356.eml, bedsonline/it-311176485.eml, bedsonline/it-595311836.eml, bedsonline/it-632812684.eml, bedsonline/it-88132004.eml, bedsonline/it-894454467-fr.eml, bedsonline/it-910604104.eml";
    ...
    public static $dictionary = [
        "es" => [
            ...
            'Lead passenger:'                    => ['Pasajero principal:'],
            'SERVICES:'                          => 'SERVICIOS :',
            'SERVICE'                            => 'SERVICIO',
            'Thanks for booking with Bedsonline' => "Muchas gracias por reservar con bedsonline",

            // HOTEL
            'Accommodation'         => ['Hotel'],
            'Cancellation Charges:' => "Gastos de cancelación :",
            'Dates'                 => 'Fechas',
            'Occupancy:'            => 'Ocupación :',
            // 'Room' => '',
            'Adults'   => 'Adultos',
        ...
        "fr" => [
            ...
            // 'Your reference number:' => '',
            'Lead passenger:' => ['Passager principal:', 'Passager principal :'],
            // 'SERVICES:' => '',
            // 'SERVICE' => '',
            'Thanks for booking with Bedsonline' => "Merci d'avoir réservé avec bedsonline",

            // HOTEL
            'Accommodation'         => ['Hébergement', 'Hôtel'],
    ...
    private $detectSubject = [
        "Confirmation de réservation Bedsonline", // fr
        "Booking confirmation Bedsonline", // en
    ];
...
//            return false;
    ...
    private function parsePlain(Email $email, string $text): void
    {
        ...
        $traveller = $this->re("/{$this->opt($this->t('Lead passenger:'))}\s*([[:alpha:]][-.\'’[:alpha:] ]*[[:alpha:]])\s*{$this->opt($this->tPlusEn('SERVICES:'))}/u", $text);

        $segmentText = $this->re("/{$this->opt($this->tPlusEn('SERVICES:'))}(.+){$this->opt($this->t('Thanks for booking with Bedsonline'))}/s", $text);
        $segments = $this->split("/({$this->opt($this->tPlusEn('SERVICE'))}\s*[#]\d+[\s\-]+.+)/", $segmentText);

        $detectedjunkSegments = 0;

        foreach ($segments as $segment) {
            if (preg_match("/{$this->opt($this->t('Accommodation'))}/", $segment)) {
                $this->logger->debug(' > HOTEL');
                $this->parseHotel($email, $segment);
            }

            if (preg_match("/^.* {$this->opt($this->t('Car Rental'))}\s*\n/", $segment)) {
                $this->logger->debug(' > CAR');
                $this->parseRental($email, $segment);
            }

            if (preg_match("/^.* {$this->opt($this->t('transfer'))}\s*\n/", $segment)) {
        ...
        // it-894454467-fr.eml
        $totalAmount = $this->re("/\sTotalAmount[:\s]+(\d[,.’‘\'\d ]*?)\s+TotalCurrency/", $text);
        $totalCurrency = $this->re("/\sTotalCurrency[:\s]+([^:\s][^\-\d)(]*)\s+ServiceType/", $text);

        if (count($its) === 1) {
            if ($totalAmount !== null) {
                $it->price()->total(PriceHelper::parse($totalAmount, $totalCurrency));
            }

            if ($totalCurrency) {
                $it->price()->currency($totalCurrency);
            }
        }
    ...
    private function parseHotel(Email $email, $text): void
    {
        ...
        if (preg_match("/^(.+)\s+{$this->opt($this->t('Status:'))}\s+(\w+)/s", $cancellation, $m)) {
            $cancellation = str_replace("\n", "", $m[1]);

            if (stripos($m[2], $this->t('CANCELLED')) !== false) {
                $h->general()
                    ->cancelled();
            }
        ...
        $h->hotel()
            ->name($this->re("/{$this->opt($this->t('Accommodation'))}\s*[:]+\s*(.{2,}?)\s+{$this->opt($this->t('Dates'))}\s*:/", $text))
            ->noAddress();

        $datesInfo = $this->re("/{$this->opt($this->t('Dates'))}\s*[:]+\s*(.+)/", $text);
    ...
    private function parseRental(Email $email, $text): void
    {
        ...
        if (preg_match("/^([\d\/\-\:T]+)$/", $datePu, $m) && preg_match("/^([\d\/\-\:T]+)$/", $dateDo, $m)) {
    ...
    private function assignLang(?string $text): bool
    {
        ...
        foreach (self::$dictionary as $lang => $phrases) {
            if (!is_string($lang) || empty($phrases['Lead passenger:'])) {
                continue;
            }

            if ($this->striposAll($text, $phrases['Lead passenger:']) !== false) {
                $this->lang = $lang;
    ...
    private function detectDateFormat(string $text): void
    {
