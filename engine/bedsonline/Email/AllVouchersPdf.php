...
class AllVouchersPdf extends \TAccountChecker
{
    ...
    public static $dictionary = [
        'es' => [
            ...
            'refNumber'                          => ['Número de referencia :', 'Número de referencia:', 'Localizador'],
            'paxName'                            => ['Nombre de los pasajeros :', 'Nombre de los pasajeros:', 'Nombre de los pasajeros', 'Viajero principal:'],
        ...
        'en' => [
            ...
            'paxName'          => ['Passenger name:', 'Passenger name :', 'Pax name:', 'Pax name :', 'Guest name:', 'Guest name :', 'Passenger', 'Lead Traveller:'],
    ...
    private function parseHotelPdf(Email $email, $text): void
    {
        ...
        $infoTable = $this->splitCols($info, [0, strlen($this->re("/\n(.+)[ ]{5}(?:Guest name|Booking date|.*?Passenger name|[ ]*name|{$this->t('Lead Traveller')})(?:\\/[^:\n]*)?[ ]*:/", $info))]);
        // Travel Agency
        ...
        $parts = $this->split("/^[ ]{0,15}(\d{1,3} ?x[ ]+\S.+?)/", $roomsText);
        $guests = null;
        $kids = null;
        $rooms = null;

        foreach ($parts as $part) {
            $infoTable = $this->splitCols($part, $this->rowColsPos($this->inOneRow($part)));

            if (strpos(implode("\n", $infoTable), '  ') === false
                && preg_match("/^[ ]*(?<units>\d{1,3}) ?x[ ]+(?<type>\S.+)/us", $infoTable[0], $m0)
                && preg_match("/^\s*(?<adults>\d+)\s*$/u", $infoTable[1], $m1)
                && preg_match("/^\s*(?<children>-|\d+)(?:\s*\(.+\))?\s*$/u", $infoTable[2], $m2)
            ) {
                $m2['children'] = ($m2['children'] === '-') ? '0' : $m2['children'];

                if (is_numeric($m1['adults']) && is_numeric($m2['children'])) {
                    $rooms += $m0['units'];
                    $guests += $m1['adults'];
                    $kids += $m2['children'];

                    for ($i = 1; $i <= $m0['units']; $i++) {
                        $h->addRoom()->setType(preg_replace('/\s+/', ' ', $m0['type']));
                    }
                }

                if (preg_match("/\n *Room type[ ]+Adults[ ]+Children {2,}(\w ?)+ {2,}Travellers\n/", $text)) {
                    $travellers = array_filter(explode("\n", $infoTable[4]));

                    foreach ($travellers as $tr) {
                        if (!in_array($tr, array_column($h->getTravellers(), 0))) {
                            $h->general()
                                ->traveller($tr, true);
                        }
                    }
                }
            } elseif (preg_match_all("/^[ ]*(?<units>\d{1,3}) ?x[ ]+(?<type>\S.+?)[ ]{2,}(?<adults>.+?)[ ]{2,}(?<children>.+?)(?:[ ]{2,}|$)/im", $roomsText, $matches, PREG_SET_ORDER)) {
                foreach ($matches as $m) {
                    $m['children'] = ($m['children'] === '-') ? '0' : $m['children'];
        ...
        if (!empty($guests) || !empty($kids) || !empty($rooms)) {
            $h->booked()
                ->guests($guests)
    ...
    private function parseTicketPdf(Email $email, $text): void
    {
        ...
        if (stripos($r->getAddress(), $this->t('hotel pickup')) !== false) {
