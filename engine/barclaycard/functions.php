...
use AwardWallet\Engine\barclaycard\TAccountCheckerBarclaycardSelenium;
...
class TAccountCheckerBarclaycard extends TAccountChecker
{
    ...
    {
        if (in_array($accountInfo['Login2'], ['USA'])) {
            require_once __DIR__ . "/TAccountCheckerBarclaycardSelenium.php";

            return new TAccountCheckerBarclaycardSelenium();
        }
        return new static();
    }
...
//                    return false;
    ...
    public function Parse()
    {
        ...
        $this->sendNotification('Parse // MI');
        ...
        $links = [];
        $accountIds = explode(';', $this->http->FindPreg("/'productInfo_accountIDList': '(.+?)',/"));

        foreach ($accountIds as $accountId) {
            $url = "https://www.barclaycardus.com/servicing/SwitchAccount.action?accountId=$accountId&userAction=true";
            $links[] = $url;
        }
        ...
        if ($this->http->FindSingleNode('//input[@value="View your credit score"]')
            && $this->http->FindSingleNode('//h2[contains(text(),"Important information about your FICO")]')) {
            $this->throwProfileUpdateMessageException();
        }
        // FICO® SCORE
        ...
        if ($fcioScore && $fcioUpdatedOn) {
            if (isset($this->Properties['SubAccounts']) ) {
                foreach ($this->Properties['SubAccounts'] as $i => $subAccount) {
                    if (is_numeric($subAccount['Balance'])) {
                        $this->SetBalance($subAccount['Balance']);
                        if (isset($subAccount['ExpirationDate'])) {
                            $this->SetExpirationDate($subAccount['ExpirationDate']);
                        }
                        foreach ($subAccount as $key => $value) {
                            $this->SetProperty($key, $value);
                        }
                        unset($this->Properties['SubAccounts'][$i]);
                    }
                }
            }
        ...
        foreach ($accountIds as $accountId) {
            $url = "https://www.barclaycardus.com/servicing/SwitchAccount.action?accountId=$accountId&rnd=" . date('UB') . "&userAction=true";
...
//        $links[] = "https://www.barclaycardus.com/servicing/legacy";
    ...
    public function parseTab()
    {
        ...
        if (
            ...
            $balance = $this->http->FindSingleNode('//dt[contains(text(),"Current Balance:")]/following-sibling::dd');
            if ($balance)
                $this->sendNotification('Rewards.action balance // MI');
            $this->logger->debug("Balance 1 (from main page) -> " . $balance);
        ...
        elseif (isset($tabName) && (!isset($balance) || $closed)) {
            $this->logger->error("Balance not found");
            // This account is closed
            if ($closed) {
