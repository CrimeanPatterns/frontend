...
{
    public $mailFiles = "aman/it-109594620.eml, aman/it-190210591.eml, aman/it-650610991.eml, aman/it-912999044.eml, aman/it-94704385.eml";
    public $subjects = [
    ...
    public static $dictionary = [
        "en" => [
            ...
            'confDesc'                    => ['Confirmation number:', 'confirmation number'],
    ...
    public function ParseHotel(Email $email): void
    {
        ...
        $confDesc = $this->http->FindSingleNode("//text()[{$this->eq($this->t('confDesc'))}]")
            ?? $this->http->FindSingleNode("//text()[{$this->contains($this->t('confDesc'))}]", null, true, "/{$this->opt($this->t('confDesc'))}/");

        $confNumbers = $this->http->FindSingleNode("//text()[{$this->eq($this->t('confDesc'))}]/following::text()[normalize-space()][1]", null, true, "/^\s*([-A-Z\d]{5,}(?:\s+and\s+[-A-Z\d]{5,})?)\s*$/")
            ?? $this->http->FindSingleNode("//text()[{$this->contains($this->t('confDesc'))}]", null, true, "/{$this->opt($this->t('confDesc'))}\s+([-A-Z\d]{5,})\b/");

        preg_match_all("/\b([-A-Z\d]{5,})\b/", $confNumbers, $confMatches);

        foreach ($confMatches[1] as $number => $confNumber) {
            $h = $email->add()->hotel();

            $pos = $number + 1;

            $h->general()
                ->confirmation($confNumber, trim($confDesc, ':'));

            // collect hotel name, address and phone
            ...
            $dateInfo = $this->http->FindSingleNode("//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1][{$this->starts($this->t('Stay dates'))}] ]/*[normalize-space()][2]/descendant::text()[normalize-space()][{$pos}]");

            if (preg_match("/^(?<date1>.+?)\s+{$this->opt($this->t('to'))}\s+(?<date2>.+?)(?:\s+\-\s+|$)/", $dateInfo, $m)) {
            ...
            if (!empty($roomDescription) && preg_match("/^(\D+)\:\s*(.+)/u", $roomDescription, $m)) {
                ...
                $room->setType($m[1]);
            } elseif (!empty($roomType = $this->http->FindSingleNode("//text()[normalize-space() = 'Accommodation details:']/ancestor::tr[1]/descendant::td[string-length()>5][2]/descendant::text()[normalize-space()][{$pos}]"))) {
                $room = $h->addRoom();

                $roomType = $this->re("/^.+?\s+{$this->opt($this->t('to'))}\s+.+?\s+\-\s+(.+?)\s*$/", $roomType) ?? $roomType;

                if (strlen($roomType) < 250) {
                    $room->setType($roomType);
                }

                $roomRate = $this->http->FindSingleNode("//text()[normalize-space() = 'Room rate:']/ancestor::tr[1]/descendant::td[string-length()>5][2]/descendant::text()[normalize-space()][{$pos}]");

                $roomRate = $this->re("/^\s*{$this->opt($this->t('From'))}\s+\d+\s+[^\d\s]+\s+(.+?)\s*$/", $roomRate) ?? $roomRate;
                ...
                $rateType = $this->http->FindSingleNode("//text()[normalize-space() = 'Rate, Exclusive Offer:']/ancestor::tr[1]/descendant::td[string-length()>5][2]")
                    ?? $this->http->FindSingleNode("//tr[ count(*[normalize-space()])=2 and *[normalize-space()][1][{$this->eq($this->t('Rate type:'))}] ]/*[normalize-space()][2]");
            ...
            $totalText = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Grand total:'))}]/ancestor::tr[1]");

            if (preg_match("/^\s*{$this->opt($this->t('Grand total:'))}\s+(?<currency>[^\d\s]{1,3})\s*(?<total>[\d\.\,\']+)\s*$/", $totalText, $m)) {
                ...
                $cost = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Room rate total:'))}]/ancestor::tr[1]", null, true, "/^\s*{$this->opt($this->t('Room rate total:'))}\s*[^\d\s]{1,3}\s*([\d\.\,\']+)\s*$/");
                ...
                $tax = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Taxes and service charge:'))}]/ancestor::tr[1]", null, true, "/{$this->opt($this->t('Taxes and service charge:'))}\s*[^\d\s]{1,3}\s*([\d\.\,\']+)\s*$/");
        ...
        // collect user email
        $userEmail = $this->http->FindSingleNode("//text()[{$this->eq($this->t('This email was intended for'))}]/following::text()[normalize-space()][1]", null, true, "/^\s*(\S+@\S+?)\s*$/")
            ?? $this->http->FindSingleNode("//text()[{$this->starts($this->t('This email was intended for'))}]", null, true, "/^\s*{$this->opt($this->t('This email was intended for'))}\s*(\S+@\S+?)\.\s+/");
