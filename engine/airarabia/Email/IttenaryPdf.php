...
class IttenaryPdf extends \TAccountChecker
{
    ...
    private function parsePdf(&$itineraries)
    {
        ...
        $pos1 = $this->TableHeadPos($this->inOneRow($n));
        $rows = explode("\n", $n);
        $pos2 = $this->TableHeadPos($rows[0]);
        $pos = count($pos1) > count($pos2) ? $pos1 : $pos2;

        $passtable = $this->splitCols($n, $pos);
        ...
        if (!$otherType) {
            ...
            foreach ($rows as $row) {
                ...
                $it['Passengers'][] = preg_replace("/\s+/", ' ', $cols[0]);

                if (!empty(trim($cols[1])) && preg_match("/^\D+$/", $cols[1])) {
                    // add to infants when wil rewritten to object
                    $it['Passengers'][] = preg_replace("/\s+/", ' ', $cols[1]);
                }
            }
        ...
        $it['Passengers'] = preg_replace("/^\s*(Child\.|Baby\.|Ребенок |Enfant |Bébé )\s*/i", '', $it['Passengers']);
        $it['Passengers'] = preg_replace("/^\s*((MS|MRS|MR|DR|MISS|MSTR) )/i", '', $it['Passengers']);
