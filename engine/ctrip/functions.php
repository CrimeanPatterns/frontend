...
//                        break;
                    ...
                    case 'Transfer':
                        ...
                        $guid = $this->http->getCookieByName('GUID');
                        $data = '{
                          ...
                          "reqhead": {
                            ...
                            "cury": "usd",
                            "did": "",
                            "globalTraceId": "491bdc30-0e96-435f-8276-89926361ca98",
                            "gps": {},
                            "host": "www.trip.com",
                            "locale": "en-xx",
                            "mode": "pc",
                            "os": "ios",
                            "packageVersion": "20250508115125",
                            "protocol": "https:",
                            "ptgroup": "",
                            "pttype": "",
                            "sf": "trip_online",
                        ...
                        $host = parse_url($this->http->currentUrl(), PHP_URL_HOST);
                        $this->http->PostURL("https://{$host}/restapi/soa2/21815/TripQueryOrderDetail.json?&_fxpcqlniredt={$this->http->getCookieByName('GUID')}&x-traceID=09034175318306304769-1747202699441-2407775",
                            $data, $headers);
    ...
    private function fetchJsonBookingTransfer($item, $detail)
    {
        ...
        $this->logger->info(sprintf('Parse Transfer #%s', $detail->state->orderId), ['Header' => 3]);

        $r = $this->itinerariesMaster->add()->transfer();

        $r->ota()->confirmation($detail->state->orderId, "Booking No.");
        foreach ($detail->buttons as $item) {
            if ($item->key == 'callDriver') {
                $r->ota()->phone($item->callNumber, $item->nm);
            }
            if ($item->key == 'contactServiceProvider') {
                $r->program()->phone($item->callNumber, $item->nm);
            }
        }
        $r->general()->noConfirmation();

        $r->price()
            ->total(PriceHelper::cost($detail->amount->totalAmount))
            ->currency($detail->amount->currency);


        $s = $r->addSegment();
        /*foreach ($detail->pickupGuide->contents as $guide) {
            if ($guide->type == 'pickupsite') {
                // "23:05:18, May 3, 2025",
                $year = $this->http->FindPreg('/\w+ \d+, (\d{4})$/', false, $detail->state->lastConfirmTimeByLocale);
                $s->departure()
                    // "title": "13:34, May 4  Driver starts waiting",
                    ->date2($this->http->FindPreg('/^(\d+:\d+, \w+ \d+)/', false, $guide->title) . ', ' . $year)
                    ->name($guide->subtitle);
            }
        }*/

        if (isset($detail->bookingAddress->flightInfo->arriveTimeLocal)) {
            $s->departure()
                ->date2($detail->bookingAddress->flightInfo->arriveTimeLocal)
                ->name($detail->bookingAddress->flightInfo->airportName)
                ->code($detail->bookingAddress->flightInfo->airportCode);
        }

        $s->arrival()
            ->noDate()
            ->name($detail->bookingAddress->poiInfo->poiName)
            ->address($detail->bookingAddress->poiInfo->poiAddress
                . ', ' . $detail->bookingAddress->poiInfo->cityName
                . ', ' . $detail->bookingAddress->poiInfo->countryName);


        $s->extra()
            ->type($detail->driverDispatch->carType)
            ->model($detail->driverDispatch->carSeries ?? null, false, true)
            ->image($detail->driverDispatch->carImgUrl);
