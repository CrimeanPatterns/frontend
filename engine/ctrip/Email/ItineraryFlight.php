...
class ItineraryFlight extends \TAccountChecker
{
    ...
    private static $dictionary = [
        ...
        'en' => [
            ...
            // 'Operated by'                    => '',
            'CancelledText' => ['Flight Booking Canceled', 'Your booking has been canceled'],
    ...
    private function parseHtmlSegment2(Flight $f, $mainRoot, $headText): void
    {
        ...
        foreach ($roots as $root) {
            ...
            $airline = $this->http->FindSingleNode("preceding::tr[normalize-space()][not({$this->contains($this->t('Operated by'))})][1]", $root);

            if (preg_match("/^(?<operator>.{2,}?)\s+[·\S]\s+(?<name>[A-Z][A-Z\d]|[A-Z\d][A-Z])(?<number>\d{1,9})\s*\(\s*(?<cabin>.{2,}?)\s*\)/u", $airline, $m)
                || preg_match("/^(?<operator>.{2,}?)\s+[·\S]\s+(?<name>[A-Z][A-Z\d]|[A-Z\d][A-Z])(?<number>\d{1,4})\s+(?<cabin>[[:alpha:] ]+)/u", $airline, $m)
            ) {
                // ITA Airways · AZ674(Classe econômica)
                // Air China · CA1636  Economy class
                $s->airline()
                    ->name($m['name'])
                    ->operator($m['operator'])
                    ->number($m['number']);
...
//        Chegada: 26 de outubro de 2021, 18:55
    ...
    private function parsePdf(Email $email, $text): void
    {
        ...
        if (preg_match("/{$this->opt($this->t('Name'))}\s*{$this->opt($this->t('Ticket Number'))}\s*((?:\n.*?)+){$this->opt($this->t('Flight Details'))}/m",
            ...
            foreach ($rows as $row) {
                ...
                } elseif (!isset($column[1]) && empty($column2)
                    && preg_match("/^(?<name>\D+) +(?<conf>[A-Z\d]{5,7})\s*$/u", $column[0], $m) && strlen(preg_replace("/\D+/", '', $m['conf'])) > 0
                ) {
                    // CELESTE POTTER ROWAN F2IU8F
                    $column[0] = $m['name'];
                }
    ...
    private function normalizeDate($date)
    {
        ...
        $in = [
            ...
            // 8:05 AM, July 5, 2025
            '/^\s*(\d{1,2}:\d{1,2}(?:\s*[ap]m)?)\s*,\s([A-z]+)\s(\d{1,2}),\s*(\d{4})\s*$/sui',
