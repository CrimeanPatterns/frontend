...
//            $this->saveToLogs($selenium);
        ...
        if (!$passwordInput) {
            ...
            $verification = $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "ve sent a verification code to ")]'),
                0);
            if ($verification) {
                ...
                $this->saveResponse();
                // CAPTCHA 2
                if ($this->waitForElement(WebDriverBy::xpath('//div[contains(@class, "cpt-choose-box")] | //div[contains(@class, "slider")]/div[contains(@class, "container")]'), 0)) {
                    return false;
                }
                $this->logger->error("something went wrong");
                ...
                $errorMessage = $this->waitForElement(WebDriverBy::xpath('//div[contains(@class, "s_error_tips")]'), 0);
...
//            $selenium->waitForElement(WebDriverBy::id('ibu_login_submit'), 0)->click();
    ...
    public function parseQuestion()
    {
        ...
        $question = $this->waitForElement(WebDriverBy::xpath('//div[contains(text(), "ve sent a verification code to")]'), 0);
        $this->saveResponse();
    ...
    public function processSecurityCheckpoint()
    {
        ...
        $securityAnswer = $this->waitForElement(WebDriverBy::xpath('(//input[contains(@class,"inputGroup-module__item_input__") or contains(@class,"index-module__code-verify-input__")])[1]'), 0);
        $this->saveResponse();
