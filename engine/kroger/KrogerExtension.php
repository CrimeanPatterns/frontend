...
class KrogerExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface
{
    ...
    private array $headers = [
        ...
        'device-memory' => '8',
        'user-time-zone' => 'America/Anchorage',
        'x-call-origin' => '{"component":"account dashboard","page":"account dashboard"}',
    ...
    public function parse(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
        ...
        // Balance refs#25015
        $domain = $this->getDomain($accountOptions->login2);
        $summaries = $tab->fetch("https://www.$domain/atlas/v1/points/v1/benefit-summaries?filter.archetypes=FUEL&filter.archetypes=CASH_BACK&filter.archetypes=CUSTOMER_CREDIT&filter.archetypes=YEAR_TO_DAY_SAVINGS",
            $options)->body;
        $summary = json_decode($summaries)->data->pointsBenefitsSummary ?? null;
        $balance = 0;
        $minDate = PHP_INT_MAX;
        foreach ($summary->benefitsSummary as $item) {
            if ($item->program->primaryArchetype == 'FUEL') {
                $balance += $item->program->balances->current;
                $endDate = strtotime($item->program->schedule->redemption->end);
                if ($endDate < $minDate) {
                    $minDate = $endDate;
                    $st->addProperty('ExpiringBalance', $item->program->balances->current);
                    $st->setExpirationDate( $endDate);
                }
            }
            elseif ($item->program->primaryArchetype == 'YEAR_TO_DAY_SAVINGS') {
                $st->addProperty('AnnualSavings', '$' . ($item->program->balances->current/100));
            }
        }
        if ($balance > 0) {
