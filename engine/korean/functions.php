...
//                || $this->http->Response['code'] == 401
        ...
        if ($message =
                $this->http->FindSingleNode('//em[contains(@class, "-negative")]')
                ?? $this->http->FindSingleNode('//p[contains(@id, "error-message-")]')
        ) {
            $this->logger->error("[Error]: {$message}");

            if (
                strstr($message, 'No matching member information.')
                || strstr($message, 'Your password was changed')
            ) {
                throw new CheckException($message, ACCOUNT_INVALID_PASSWORD);
            }
