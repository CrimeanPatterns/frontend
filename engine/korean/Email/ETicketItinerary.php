...
{
    public $mailFiles = "korean/it-13214825.eml, korean/it-16735045.eml, korean/it-16803579.eml, korean/it-16918827.eml, korean/it-35988205.eml, korean/it-633937833.eml, korean/it-639209420.eml, korean/it-814997564.eml, korean/it-911131958-cancelled.eml";
    ...
    public static $dictionary = [
        "en" => [
            ...
            'cancelledHeader'         => ['Flight Cancellation Notice', 'Flight Cancelation Notice'],
        ],
        "ko" => [
    ...
    private $reSubject = [
        'en' => 'e-Ticket Itinerary/Receipt of Koreanair for', 'Flight Cancellation Notice', 'Flight Cancelation Notice',
    ...
    public function detectEmailByHeaders(array $headers)
    {
        ...
        foreach ($this->reSubject as $re) {
            if (is_string($re) && array_key_exists('subject', $headers) && stripos($headers['subject'], $re) !== false) {
                return true;
            }
        }
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        $detectProv = $this->detectEmailFromProvider($parser->getCleanFrom()) === true
            || $this->http->XPath->query("//img[contains(@src,'koreanair.com/')]")->length > 0;

        if ($detectProv && $this->http->XPath->query("//tr[starts-with(normalize-space(),'Itinerary')]")->length > 0
            && (
                $this->http->XPath->query("//text()[{$this->contains($this->t('Reservation Information'))}]")->length > 0
                    && $this->http->XPath->query("//text()[{$this->contains(['My Trip', 'Notice', 'you have completed your ticket purchase'])}]")->length > 0
                || $this->isCancelled()
            )
        ) {
            return true;
    ...
    {
        return $this->http->XPath->query("//h1[{$this->eq($this->t('cancelledHeader'))}]")->length > 0;
    }
    ...
    private function flight2(Email $email, string $pdftext): void
    {
        ...
        if (preg_match("#\n([ ]*(?:\w+ ){0,2}[ ]*Passenger Name[ ]+[^\n]+(\s+[\s\S]+?))\n\s*(?:\w+ ){0,2}[ ]*Itinerary#u", $text, $m)) {
            ...
            if (count($table) > 0 && preg_match("/^\s*([.[:alpha:]][-.'’[:alpha:] \/]*[[:alpha:]])\s*(?:\(?([A-Z\d*]+)\))?.*$/su", $table[0], $mat)) {
            ...
            if (count($table) > 1 && preg_match("/^\s*[-\d ]+\s*$/", $table[1])) {
            ...
            if (count($table) > 2 && preg_match("/([A-Z\d\-]{5,})\s*\(\s*([A-Z\d]{5,8})\s*\)(?:\s|$)/", $table[2], $mat)
        ...
        foreach ($segments as $stext) {
            ...
            if (preg_match("/(?<code>[A-Z]{3})\s+(?<name>[\s\S]+?)\n[ ]*(?<date>\d{1,2}[[:alpha:]]+\d{2}.+)\s+Terminal No\s*:\s*(?<term>.+)/u", $tableRoute[0], $m)) {
                $s->departure()
            ...
            if (preg_match("/(?<code>[A-Z]{3})\s+(?<name>[\s\S]+?)\n[ ]*(?<date>\d{1,2}[[:alpha:]]+\d{2}.+)\s+Terminal No\s*:\s*(?<term>.+)/u", $tableRoute[1], $m)
                || preg_match("/(?<code>[A-Z]{3})\s+(?<name>[\s\S]+?)\n[ ]*(?<date>\d{1,2}[[:alpha:]]+\d{2}.+)$/u", $tableRoute[1], $m)
            ) {
                $s->arrival()
                    ->name(trim(preg_replace("#\s+#", ' ', $m['name'])))
                    ->code($m['code'])
    ...
    private function flightHTML(Email $email): void
    {
        ...
        } elseif (preg_match("/(?<confTitle>Booking Reference)[\s\:]+(?<conf>[A-Z\d]{5,8})$/", $confText, $m)) {
            $f->general()->confirmation($m['conf'], $m['confTitle']);
        }

        $segments = $this->http->XPath->query("//text()[normalize-space()='Itinerary']/following::img[contains(@src,'flight')]/ancestor::tr[1]");

        if ($cancelled && $segments->length === 1) {
            $f->general()->cancelled();
        } elseif ($cancelled && $segments->length !== 1) {
            $this->logger->debug('Please check this cancelled format!');
