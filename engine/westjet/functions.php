...
class TAccountCheckerWestjet extends TAccountChecker
{
    ...
    public function IsLoggedIn()
    {
        ...
        $this->http->GetURL("https://apigw.westjet.com/dps/v1/guests/my/rewards/account", $headers);
        ...
        if (!empty($name) && isset($response->rewardsAccount->wspBalance)) {
...
//        }
        ...
        if ($message) {
            ...
            if (
                strstr($message, 'WestJet Rewards accounts are currently unavailable')
                || strstr($message, 'Your account is unavailable while we launch the all-new WestJet Rewards')
            ) {
                throw new CheckException($message, ACCOUNT_PROVIDER_ERROR);
            }
    ...
    public function Parse()
    {
        ...
        if ($this->http->currentUrl() !== "https://apigw.westjet.com/dps/v1/guests/my/rewards/account") {
            $this->http->GetURL("https://apigw.westjet.com/dps/v1/guests/my/rewards/account", $headers);
        }
        ...
        $this->SetBalance($response->rewardsAccount->wspBalance ?? null);
...
//        $this->SetProperty("AccountNumber", $this->http->JsonLog(null, 0)->westjetId ?? null);
        ...
        $this->http->GetURL("https://apigw.westjet.com/dps/v1/guests/my/rewards/account/program?lang=en", $headers);
        $response = $this->http->JsonLog(null, 3, false, 'currentAnniversaryStartDate');

        /*
        if (isset($response->programDetails->anniversaries->currentAnniversary)) {
            // Earned this qualifying year
            $this->SetProperty("EarnedThisQualifyingYear", $response->programDetails->anniversaries->currentAnniversary->currentQualifyingSpend . " CAD");
            // Current qualifying year
            $this->SetProperty("QualifyingYear", date("M d, Y", strtotime($response->programDetails->anniversaries->currentAnniversary->currentAnniversaryStartDate)) . " - " . date("M d, Y", strtotime($response->programDetails->anniversaries->currentAnniversary->currentAnniversaryEndDate)));
            // Qualifying spend this year
            $this->SetProperty("QualifyingSpend", $response->programDetails->anniversaries->currentAnniversary->currentCreditCardSpend . " CAD");
        }
        */

        if (isset($response->programDetails->tiers->nextTier->requiredForNextTier)) {
            // Spend to maintain tier
            $this->SetProperty("SpendToMaintainTier", '$ ' . $response->programDetails->tiers->nextTier->requiredForNextTier);
        }

        if (isset($response->programDetails->anniversaries->currentAnniversary->currentQualifyingSpend)) {
            // Tier qualifying spend earned
            $this->SetProperty("TierQualifyingSpendEarned", '$ ' . $response->programDetails->anniversaries->currentAnniversary->currentQualifyingSpend);
        }

        if (isset($response->programDetails->milestones->milestoneQualifyingSpend)) {
            // Milestone qualifying spend
            $this->SetProperty("MilestoneQualifyingSpend", '$ ' . $response->programDetails->milestones->milestoneQualifyingSpend);
        }

        $awardList = $response->programDetails->milestones->nextAwardList ?? [];

        if (count($awardList) > 0) {
            uasort($awardList, function ($a, $b) {
                if ($a->requiredForNextAward == $b->requiredForNextAward) {
                    return 0;
                }

                return ($a->requiredForNextAward < $b->requiredForNextAward) ? -1 : 1;
            });

            foreach ($awardList as $award) {
                if ($award->requiredForNextAward > 0) {
                    // Spend to the next milestone award
                    $this->SetProperty("SpendToTheNextMilestoneAward", $award->requiredForNextAward);
        ...
        $this->http->GetURL("https://apigw.westjet.com/dps/v1/guests/my/rewards/account/expiringPoints", $headers);
        $response = $this->http->JsonLog();
        $expNodes = $response->expirations ?? [];
        ...
        */
        ...
        $this->http->GetURL("https://apigw.westjet.com/dps/v1/guests/my/rewards/vouchers?lang=en", $headers);
    ...
    private function uniqueStateKeys()
    {
        ...
        try {
            ...
            */
            ...
            $signIn = $selenium->waitForElement(WebDriverBy::xpath('//button[@id = "sign-in-button" or @name = "sign-in-my-account-cta"] | //div[@id = "profile-apps-sign-in-button"]//button'), 10);
            ...
            */
            ...
            foreach ($requests as $n => $xhr) {
                ...
                if (strstr($xhr->request->getUri(), 'oauth2/userinfo')) {
                    $this->logger->debug("xhr response {$n} body: " . json_encode($xhr->response->getBody()));
                    $responseNameData = json_encode($xhr->response->getBody());
                    $this->logger->notice("[Form $responseData]: " . $responseData);
                    $this->getJsonName($responseNameData);
                }

                if (!empty($response) && !empty($this->lastName)) {
                    break;
                }
            }
    ...
    private function getJsonName($responseNameData = null): ?string
    {
        $this->logger->notice(__METHOD__);
        /*
        $headers = [
        ...
        */
        ...
        if ($responseNameData === null && !empty($this->State['NameData'])) {
            $responseNameData = $this->State['NameData'];
        } elseif (!empty($responseNameData)) {
            $this->State['NameData'] = $responseNameData;
        }

        $data = $this->http->JsonLog($responseNameData, 3, true);
        $firstName = urldecode(ArrayVal($data, 'given_name', ''));
        $lastName = urldecode(ArrayVal($data, 'family_name', ''));
        ...
        $this->SetProperty("Name", beautifulName($name));
        // WestJet Rewards ID
        $this->SetProperty("AccountNumber", ArrayVal($data, 'subname', ''));
