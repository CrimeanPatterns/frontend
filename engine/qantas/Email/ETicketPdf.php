...
class ETicketPdf extends \TAccountChecker
{
    ...
    public function parsePdf(Email $email): void
    {
        ...
        foreach ($segments as $stext) {
            ...
            $s->departure()
                ->name($this->niceDepArrName($depName))
            ...
            if (!empty($stopName)) {
                $s->arrival()
                    ->name($this->niceDepArrName($stopName))
                    ->noCode()
                    ->noDate();
                ...
                $s->arrival()
                    ->name($this->niceDepArrName($arrName))
                    ->date($arrDate)
                    ->noCode();
    ...
    public function niceDepArrName($airport)
    {
        $airport = preg_replace("/Intl$/i", "International Airport", $airport);
        $airport = str_replace(["/", "\n"], " ", $airport);
