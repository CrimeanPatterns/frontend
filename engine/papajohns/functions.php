...
//                if ($this->http->FindPreg("/pageRedirect\|\|\%2f\|/")) {
 ...
 <br><br>We determined that last time you had account activity with Papa John's Pizza on {$lastActivity}, so the expiration date was calculated by adding 6 months to this date.");
            ...
            default:
                // Balance - 0/75 Points
                $this->SetBalance($this->http->FindSingleNode('(//text()[contains(., "points")]/preceding-sibling::strong)[1]', null, false, "/([\d.,]+)\/[\d.,]+/i"));

                if ($myPapaDough = $this->http->FindSingleNode('//span[contains(text(), "Papa Dough®")]/../preceding-sibling::div')) {
                    $this->AddSubAccount([
