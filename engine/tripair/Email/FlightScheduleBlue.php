...
{
    public $mailFiles = "tripair/it-108806044.eml, tripair/it-11996426.eml, tripair/it-40884801.eml, tripair/it-41980269.eml, tripair/it-42228880.eml, tripair/it-42320902.eml, tripair/it-42321708.eml, tripair/it-424144247.eml, tripair/it-4819113.eml, tripair/it-49957461.eml, tripair/it-534746385.eml, tripair/it-60812230.eml, tripair/it-6566906.eml, tripair/it-6657845.eml, tripair/it-73514139.eml, tripair/it-916635217.eml, tripair/it-97720789.eml";
    public $lang;
    public static $dict = [
        'en' => [
    ...
    private function parseEmail(Email $email): void
    {
        ...
        //Cruise
        $xpath = "//text()[{$this->contains($this->t('Ship:'))}]/ancestor::table[{$this->contains($this->t('Stateroom:'))}][1]";
        $cruise = $this->http->XPath->query($xpath);

        if ($cruise->length > 0) {
            $this->logger->debug("[XPATH-car] " . $xpath);
            $this->parseCruise($cruise, $email);
        }
    ...
    private function parseCruise(\DOMNodeList $segments, Email $email): void
    {
        foreach ($segments as $root) {
            $c = $email->add()->cruise();

            if (count($this->pax) > 0) {
                $c->general()->travellers($this->pax, true);
            }

            $company = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Ship:'))}]/ancestor::tr[1]/preceding-sibling::tr[1]/descendant::td[1]");

            if (!empty($company)) {
                $c->setVoyageNumber($company);
            }

            $class = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Category:'))}]/ancestor::td[1]", $root, true, "/{$this->opt($this->t('Category:'))}\s*(.+)$/");
            ...
            if (!empty($this->tripNum)) {
                $c->ota()->confirmation($this->tripNum, rtrim($this->t('Trip ID:'), ': '));
            } elseif (!empty($this->tripLoc)) {
                $c->ota()->confirmation($this->tripLoc, rtrim($this->t('Trip Locator:'), ': '));
            }

            $confirmation = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Confirmation:'))}]/ancestor::td[1]", $root);
            ...
            $c->setShip($this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Ship:'))}]/ancestor::td[1]", $root, true, "/^{$this->opt($this->t('Ship:'))}\s*(.+)/"));

            $room = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Stateroom:'))}]/ancestor::td[1]", $root, true, "/^{$this->opt($this->t('Stateroom:'))}\s*(\d+)$/");
            ...
            $s = $c->addSegment();
            $s->setName($this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Depart:'))}]/ancestor::td[1]", $root, true, "/^{$this->opt($this->t('Depart:'))}\s*(.+)$/"));
            $dateAboard = $this->http->FindSingleNode("./preceding::text()[normalize-space()][2][contains(translate(normalize-space(.),'0123456789','dddddddddd'),'dddd')]", $root);
            $timeAboard = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Depart:'))}]/ancestor::tr[1]/following-sibling::tr[1]/descendant::td[1]", $root, true, "/^(\d+\:\d+\s*A?P?M?)$/iu");

            if (!empty($dateAboard) && !empty($timeAboard)) {
                $s->setAboard(strtotime($dateAboard . ', ' . $timeAboard));
            } else if (!empty($dateAboard)) {
                $s->setAboard(strtotime($dateAboard));
            }
            $s = $c->addSegment();

            $s->setName($this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Arrive:'))}]/ancestor::td[1]", $root, true, "/^{$this->opt($this->t('Arrive:'))}\s*(.+)$/"));
            $dateAshore = $this->http->FindSingleNode("./descendant::text()[starts-with(normalize-space(), 'Arrive:')]/ancestor::tr[1]/following-sibling::tr[1]/descendant::td[1][contains(translate(normalize-space(.),'0123456789','dddddddddd'),'dddd')]");
            $timeAshore = $this->http->FindSingleNode("./descendant::text()[{$this->starts($this->t('Arrive'))}]/ancestor::tr[1]/following-sibling::tr[2]/descendant::td[1]", $root, true, "/^(\d+\:\d+\s*A?P?M?)$/iu");

            if (!empty($dateAshore) && !empty($timeAshore)) {
                $s->setAshore(strtotime($dateAshore . ', ' . $timeAshore));
            } else if (!empty($dateAshore)) {
                $s->setAboard(strtotime($dateAshore));
            }
        }
    }
    ...
    {
        foreach ($segments as $root) {
            if ($this->http->XPath->query("./descendant::text()[{$this->eq($this->t('Check Out:'))}]/following::text()[normalize-space()][1][{$this->eq($this->t('Cost:'))}]", $root)->length > 0) {
                continue;
            }
        ...
        foreach ($airs as $rl => $roots) {
            ...
            foreach ($roots as $root) {
                ...
                if (count($seats) === 0) {
                    ...
                    // SEATS 2E AND 2F
                    $seatsValue = $this->http->FindSingleNode("descendant::tr[{$this->eq($this->t('Remarks:'))}]/following-sibling::tr[{$this->contains($this->t('SEATS'))}]", $root, true, "/{$this->opt($this->t('SEATS'))}\s+(\d+[A-Z\d ]*?[A-Z])(?:\s+RESERVED|\s+CONFIRMED$)/");
