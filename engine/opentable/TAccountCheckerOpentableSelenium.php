...
class TAccountCheckerOpentableSelenium extends TAccountChecker
{
    ...
    public function Parse()
    {
        ...
        $json = $this->http->FindSingleNode('//script[@id="primary-window-vars" and @type="application/json"]', null, false, '/(\{.*\})/');
        $windowVariables = $this->http->JsonLog($json, 3, false);
        $diningDashboard = $windowVariables->windowVariables->__INITIAL_STATE__->diningDashboard ?? null;

        if (!isset($diningDashboard)) {
            return;
        }
        // Balance
        $this->SetBalance($diningDashboard->points);
        // You are only 2,800 points away from a $50 reward!
        $this->SetProperty("NeededNextReward", $diningDashboard->nextReward->requiredPoints ?? null);
        // Expiring balance
        $this->SetProperty("ExpiringBalance", $diningDashboard->userExpiringPoints->expiringPoints ?? null);

        if (
            isset($diningDashboard->userExpiringPoints->expiringPoints)
            && $diningDashboard->userExpiringPoints->expiringPoints > 0
            && $diningDashboard->userExpiringPoints->eligibleForExpiration == true
        ) {
            $this->SetExpirationDate(strtotime($diningDashboard->userExpiringPoints->expirationDate));
        }

        $this->history = $diningDashboard->pastReservations;
        ...
        if (
            !empty($lastActivity)
            && isset($diningDashboard->userExpiringPoints->expiringPoints, $diningDashboard->userExpiringPoints->expirationDate)
            && $diningDashboard->userExpiringPoints->expiringPoints == 0
        ) {
    ...
    public function ParseItineraries()
    {
        ...
        $result = $links = [];
        $json = $this->http->FindSingleNode('//script[@id="primary-window-vars" and @type="application/json"]', null, false, '/(\{.*\})/');
        $windowVariables = $this->http->JsonLog($json, 3, false, 'upcomingReservations');
        $noLinkReservations = $windowVariables->windowVariables->__INITIAL_STATE__->diningDashboard->upcomingReservations;
