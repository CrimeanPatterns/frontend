...
//            return false;
    ...
    public function processSecurityCheckpoint()
    {
        ...
        $q = $this->waitForElement(WebDriverBy::xpath("
            ...
            | //h2[contains(text(), 'Enter the code we')]
        ...
        for ($i = 0; $i < strlen($this->Answers[$question]) && $i < 6; $i++) {
            $securityAnswer = $this->waitForElement(WebDriverBy::xpath("//input[
                ...
                or @id = 'phone-verification-code-form__code-input'
            ]"), 0);
        ...
        if ($btn = $this->waitForElement(WebDriverBy::xpath("//button[contains(text(),'Continue') and not[@disabled]]"), 2)) {
            $this->saveResponse();
            $btn->click();
        }
