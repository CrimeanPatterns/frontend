...
class ReservationConfirmation2 extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            ...
            'Show all reservation details' => ['Show all reservation details', 'Show more details'], // after main info, before price
        ...
        "es" => [ // it-107224110.eml
            ...
            'Show full itinerary'          => ['Ver itinerario completo', 'Ver el itinerario completo', 'Mostrar itinerario completo', 'Mostrar más información'], // after dates
            ...
            'Check-in is any time after'   => ['Después de las', 'Horario de llegada: de', 'La hora de llegada es a partir de las', 'La hora de llegada es', 'La llegada es',
                'El horario de llegada es de las', ],
            'flexible'                   => 'flexible',
            'Check-in'                   => 'Llegada', // Check-in and Checkout in two column
            'Checkout'                   => 'Salida',
            'Checkout by'                => ['S‌a‌l‌i‌d‌a‌ ‌a‌n‌t‌e‌s‌ ‌d‌e‌ ‌l‌a‌s‌', 'Salida a las', 'Salida antes de las', 'Hasta las'],
        ...
        "pt" => [
            ...
            'Show all reservation details' => ['Mostrar todas as informações da reserva', 'Mostrar todos os detalhes da reserva', 'Mostrar mais informações'], // after main info, before price
            ...
            'hosted by'                  => ['hospedado por', 'hospedada por ', 'Anfitriã(o):'],
            'Address'                    => 'Endereço',
            //'badAddress' => [''],
            ...
            'Check-in is any time after' => ['O check-in é a qualquer momento depois das', 'O check-in é entre', 'O horário de check-in é',
                'O check-in é a qualquer hora depois das', 'Após', ],
            'flexible'                   => 'flexível',
            'Check-in'                   => 'Check-in', // Check-in and Checkout in two column
            'Checkout'                   => ['Checkout', 'Check-out'],
            'Checkout by'                => ['Checkout até', 'Check-out até às', 'Até'],
            'Guests'                     => 'Hóspedes',
            'adult'                      => 'adult',
        ...
        "fr" => [
            ...
            'Show all reservation details' => ['Afficher tous les détails de la réservation', 'Afficher plus d\'informations'], // after main info, before price
            // 'Change reservation' => '',
    ...
    public function parseHTML(Email $email): void
    {
        ...
        foreach ($nodes as $root) {
            ...
            if (empty($checkInText) || preg_match("/^{$this->opt($this->t('Check-in is any time after'))}\s*\d+\:\d+\s*$/", $checkInText)) {
                $checkInArray = array_filter($this->http->FindNodes("//text()[{$this->contains($this->t('Check-in is any time after'))}]/ancestor::td[{$this->contains($this->t('Check-in'))}][1]/descendant::text()[normalize-space()][not({$this->contains($this->t('Check-in'))})]"));

                if (count($checkInArray) > 0) {
                    $checkInText = implode(" ", $checkInArray);
                }
            }
            ...
            if (preg_match("/^\s*[[:alpha:]\-]*,\s*(?<date>(?:\d{1,2}|[[:alpha:]]+)\s*[.,]?(?:\bde\b)?\s*([[:alpha:]]+|\d{1,2})\s*[.,]?(?:\bde\b)?\s*\d{4}|\d+년\s*\d+월\s*\d+일)\D+(?<time>{$this->patterns['time']}|{$this->opt($this->t('flexible'))}|flexible)/u",
                ...
                || preg_match("/^\s*{$this->opt($this->t('Check-in is any time after'))}\s*(?<time>{$this->patterns['time']}|{$this->opt($this->t('flexible'))}|flexible).*\s*[-[:alpha:]]+\s*,\s*(?<date>.{6,})/u",
                    $checkInText, $m)
                || preg_match("/^\s*(?<date>[[:alpha:]\-]+\.?\s*[,\s]\s*.{6,}?)\s*{$this->opt($this->t('Check-in is any time after'))}\s*(?<time>{$this->patterns['time']})$/u",
                    $checkInText, $m)
            ...
            if (empty($checkOutText) || preg_match("/^{$this->opt($this->t('Checkout by'))}\s*\d+\:\d+\s*$/", $checkOutText)) {
                $checkOutArray = array_filter($this->http->FindNodes("//text()[{$this->contains($this->t('Checkout by'))}]/ancestor::td[{$this->contains($this->t('Checkout'))}][1]/descendant::text()[normalize-space()][not({$this->contains($this->t('Checkout'))})]"));

                if (count($checkOutArray) > 0) {
                    $checkOutText = implode(" ", $checkOutArray);
                }
            }
            ...
            if (preg_match("/^\s*[[:alpha:]\-]*,\s*(?<date>(?:\d{1,2}|[[:alpha:]]+)\s*[.,]?(?:\bde\b)?\s*([[:alpha:]]+|\d{1,2})\s*[.,]?(?:\bde\b)?\s*\d{4})\D*(?<time>{$this->patterns['time']}|{$this->opt($this->t('flexible'))})/u",
                ...
                || preg_match("/{$this->opt($this->t('Checkout by'))}\s*(?<time>{$this->patterns['time']})\s*[-[:alpha:]]+\,\s(?<date>.+)/u",
                    $checkOutText, $m)
                || preg_match("/^\s*(?<date>[[:alpha:]\-]+\.?\s*[,\s]\s*.{6,}?)\s*{$this->opt($this->t('Checkout by'))}\,?\s*(?<time>{$this->patterns['time']})$/u",
                    $checkOutText, $m)
                || preg_match("/^\s*[[:alpha:]\-]+\.?\s*[,\s]\s*(?<date>.{6,}?)\s*(?<time>{$this->patterns['time']}|{$this->opt($this->t('flexible'))}|flexible)$/u",
                    $checkOutText, $m)
            ) {
    ...
    private function detectDeadLine(\AwardWallet\Schema\Parser\Common\Hotel $h): void
    {
        ...
        if (
            ...
            || preg_match("/Cancelamento gratuito antes das (?<time>{$this->patterns['time']}) do dia (?<date>\d{1,2}\s+[[:alpha:]\s]+)\./u", $cancellationText, $m) // tr
            || preg_match("/Se você cancelar antes do horário de check-in às (?<time>{$this->patterns['time']}) em (?<date>\d{1,2}\s+[[:alpha:]\s]+)\.\,/u", $cancellationText, $m) // tr
            || preg_match("/Se você cancelar antes das (?<time>{$this->patterns['time']}) em (?<date>\d{1,2}\s+[[:alpha:]\s]+)\.\,/u", $cancellationText, $m) // tr
        ) {
        ...
        if (
          ...
          || preg_match("/^This reservation is non-refundable./", $cancellationText) // en
          || preg_match("/^Esta reserva não é reembolsável./", $cancellationText) // en
          || preg_match("/^Esta reserva no es reembolsable./", $cancellationText) // en
        ) {
            $h->booked()->nonRefundable();
        }

        if (preg_match("/Annulation gratuite pendant (?<hours>\d+) heures\./", $cancellationText, $m)
        || preg_match("/Cancelamento gratuito por (?<hours>\d+) horas./", $cancellationText, $m)) {
            $h->booked()
    ...
    private function normalizeDate($str)
    {
        ...
        $in = [
            ...
            // 30. Apr., 16:00 Uhr
            // jeu. 27 avr. 16:00
            "/^\s*(?:[-[:alpha:]]+\.?\s*[,\s]\s*)?(\d{1,2})\.?[ ]*(?:de\s+)?([[:alpha:]]+)\.?[ ]*[,\s][ ]*(?:\w+\s+)?({$this->patterns['time']})\s*$/u",
            ...
            // Tue, Aug 22 4:00 PM
            "/^(\w+)\.?\,\s*(\w+)\.?\s*(\d+)\s*([\d\:]+\s*A?\.?P?\.?M\.?)\s*$/iu",
            // Thu, Apr 17 After 3:00 PM
            "/^(\w+)\,\s+(\w+)\s+(\d+)\s+(?:After|By)\s+([\d\:]+\s*[AP]M)$/",
        ];
        $out = [
            "$1 $2 $year $3",
            "$1 $2 $3, $4",
