...
class TAccountCheckerEgyptair extends TAccountChecker
{
    ...
    protected const REESE84_CACHE_KEY = 'egyptair_reese84';
    ...
    private function getCookieFromCache()
    {
        $reese84 = Cache::getInstance()->get(self::REESE84_CACHE_KEY);

        if (empty($reese84) || $this->attempt > 0) {
            return $this->selenium();
        }

        $this->logger->notice("set reese84 from cache");
        $this->http->setCookie("reese84", $reese84, ".www.egyptairplus.com");
    ...
    private function selenium()
    {
        ...
        try {
            ...
            $selenium->http->start();
            $selenium->Start();
            $selenium->http->GetURL("https://www.egyptairplus.com/StandardWebsite/Login.jsp?activeLanguage=EN");
            $selenium->waitForElement(WebDriverBy::xpath("//input[@id = 'txtUser']"), 10);
            $this->saveLogs($selenium);
            ...
            foreach ($cookies as $cookie) {
                ...
                $this->logger->notice("set new reese84");
                Cache::getInstance()->set(self::REESE84_CACHE_KEY, $reese84, 60 * 60);
