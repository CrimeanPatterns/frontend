...
class TAccountCheckerCtmoney extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        $this->triSession = Cache::getInstance()->get('ctmoney_tri_session');
        $this->logger->debug('ctmoney_tri_session' . print_r($this->triSession, true));

        if (empty($this->triSession)) {
            $this->selenium();
        }
        ...
        */
        ...
        $headers = [
            ...
            "Origin"                    => "https://customerauth.triangle.com",
            "Referer"                   => "https://customerauth.triangle.com/",
            "Content-Type"              => "application/json",
            "service-client"            => "triangle/web",
            "X-TMX-Session-ID"          => $this->triSession,
            "x-web-host"                => "customerauth.triangle.com",
        ];
        $this->http->PostURL("https://apim.triangle.com/v1/authorization/signin/rba-tmx", json_encode($data), $headers);
        $this->http->RetryCount = 2;
    ...
    public function Login()
    {
        ...
        if (
            isset($response->errorDetails)
            && $response->errorDetails == 'Login Failed'
        ) {
            Cache::getInstance()->set('ctmoney_tri_session', null, 60 * 60);

            throw new CheckRetryNeededException(3, 0, self::PROVIDER_ERROR_MSG, ACCOUNT_PROVIDER_ERROR);
        }

        if (isset($this->triSession)) {
            Cache::getInstance()->set('ctmoney_tri_session', $this->triSession, 60 * 60);
        }
    ...
    private function getTRISession()
    {
        return;
    }
    ...
    private function selenium()
    {
        ...
        try {
            ...
            $triSession = $selenium->driver->executeScript("return localStorage.getItem('tmxSessionIdInfo');");
            $this->logger->debug($triSession);

            if (isset($triSession)) {
                $triSessionDecoded = $this->http->JsonLog($triSession);
                $this->triSession = $triSessionDecoded->sessionId ?? null;
            }
        } finally {
