...
class CongratsOn extends \TAccountChecker
{
    ...
    public function ParseEmail(Email $email)
    {
        ...
        if (empty($balance)) {
            $balance = $this->http->FindSingleNode("(//text()[{$this->starts($this->t('CT Money'))}])[1]/ancestor::td[1]", null, true, "/\S[:]\s*\D*(\d[\d\.]*)\D*/");
        }
