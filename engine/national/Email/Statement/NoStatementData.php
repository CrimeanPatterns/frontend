...
class NoStatementData extends \TAccountChecker
{
    ...
    public static $dictionary = [
        "en" => [
            'detectPhrases'  => ['Emerald Club member', 'your Emerald Club profile'],
            "Member Number:" => ['Member Number:', 'Member #:', 'Emerald Club #:'],
            'Tier'           => ['Executive', 'Executive Elite'],
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        if ($this->detectLang($parser->getSubject()) == true) {
            if ($this->http->XPath->query("//text()[contains(normalize-space(), '{$this->provDetect}')]")->length > 0) {
                if (
                    $this->http->XPath->query("//text()[{$this->contains($this->t('detectPhrases'))}]")->length > 0
                    && $this->http->XPath->query("//text()[{$this->contains($this->t('Member Number:'))}]")->length == 0
                    && $this->http->XPath->query("//text()[{$this->contains($this->t('Tier'))}]")->length == 0
