...
class GoldcrownExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface, ParseItinerariesInterface
{
    ...
    public function isLoggedIn(Tab $tab): bool
    {
        ...
        // $loginFieldOrLogOut = $tab->evaluate('//form[contains(@action, "reservation-index.html")] | //p[@id="rewards-card-number"] | //span[@id="nav-logged-in-expander-points-amount"]');
        $loginFieldOrLogOut = $tab->evaluate('//input[@name="identifier"] | //p[@id="rewards-card-number"]');
        if ($loginFieldOrLogOut->getNodeName() == 'INPUT') {
            return false;
        }
    ...
    {
        $tab->evaluate('//input[@name="identifier"]')->setValue($credentials->getLogin());
        $tab->evaluate('//input[@value="NEXT"]')->click();
        $tab->evaluate('//input[@name="credentials.passcode"]')->setValue($credentials->getPassword());
        $tab->evaluate('//input[@value="VERIFY"]')->click();
        $errorOrLogOut = $tab->evaluate($xpath = '
            //div[contains(@class,"errorMessage__")]
            | //div[@id = "credentials-failed-error-msg"]//span[contains(@class, "defaultMessage")]
            | //p[@id="rewards-card-number"] | //iframe[contains(@src,"/captcha/")]
        ');
        ...
        if ($this->findPreg('/^Account /', $errorOrLogOut->getInnerText())) {
            ...
            if (
                strstr($error, "We are unable to process your request. Please try again. If you continue to have difficulties, please contact a")
