...
{
    public $mailFiles = "chinaeastern/it-772971330-zh.eml, chinaeastern/it-773443606-fr.eml, chinaeastern/it-779473882-de.eml, chinaeastern/it-903516174.eml, chinaeastern/it-906387577-kr.eml, chinaeastern/it-906869271-ru.eml, chinaeastern/it-9094222.eml"; // +1 bcdtravel(html)[en]
    ...
    public $reBody = [
        ...
        'Обратитесь в службу поддержки клиентов компании China Eastern Airlines', // ru
    ];
    public $reSubject = [
    ...
    private static $dict = [
        ...
        'kr' => [
            'The order No. is' => '예약 주문 번호:',
            'Flight No.'       => '항공편명',
            'Total Amount'     => '총 금액',
            'Ticket fares'     => '항공권 운임',
            'Tax & Fees'       => '세금 및 수수료',
            'Passenger type'   => '승객 유형',
            'Passenger name'   => '탑승자 성함',
            'ID No.'           => "신분증 번호",
        ],
        'ru' => [
            'The order No. is' => 'Номер заказа:',
            'Flight No.'       => 'Номер рейса',
            'Total Amount'     => 'Общая сумма',
            'Ticket fares'     => 'Тарифы билетов',
            'Tax & Fees'       => 'Налоги и сборы',
            'Passenger type'   => 'Тип пассажира',
            'Passenger name'   => 'Фамилия / Имя пассажира',
            'ID No.'           => "Номер удостоверения личности",
        ],
    ];
    ...
    {
        if ($this->http->XPath->query("//*[{$this->contains($this->reBody)} or {$this->contains(['@ceair.com', 'sg.ceair.com', 'us.ceair.com', 'au.ceair.com', 'ca.ceair.com', 'de.ceair.com', 'fr.ceair.com', 'kr.ceair.com', 'ru.ceair.com'])}]")->length === 0
            && $this->http->XPath->query("//a[{$this->contains(['//sg.ceair.com', '//us.ceair.com', '//au.ceair.com', '//ca.ceair.com', '//de.ceair.com', '//fr.ceair.com', '//kr.ceair.com', '//ru.ceair.com'], '@href')}]")->length === 0
        ) {
            return false;
        }
    ...
    private function parseEmail(Email $email): void
    {
        ...
        $tripNumber = $this->http->FindSingleNode("//text()[{$this->contains($this->t('The order No. is'))}]/following::text()[normalize-space(.)!=''][1]",
            null, true, '/^([A-Z\d]{5,})$/');
        if ($this->http->FindSingleNode("//text()[{$this->contains($this->t('The order No. is'))}]/following::text()[normalize-space(.)!=''][1]") !== null && $tripNumber !== null){
        ...
        if (preg_match('/^(?<amount>\d[,.‘\'\d ]*?)[ ]*(?<currency>[^\-\d)(]+?)$/u', $totalPrice, $matches)) {
            ...
            $baseFares = $this->http->FindNodes("//text()[{$this->eq($this->t('Ticket fares'), "translate(.,':：','')")}]/ancestor::div[1][count(descendant::text()[normalize-space()])>1]", null,  "/^{$this->opt($this->t('Ticket fares'))}[\s:：]+(.*\d.*)$/");
            $faresArray = [];

            foreach ($baseFares as $baseFare){
                if (preg_match('/^(?<amount>\d[,.‘\'\d ]*?)[ ]*(?:' . preg_quote($matches['currency'], '/') . ')?$/u', $baseFare, $m)) {
                    $faresArray[] = PriceHelper::parse($m['amount'], $currencyCode);
                }
            }

            if (!empty($faresArray)){
                $r->price()->cost(array_sum($faresArray));
            }

            $taxes = $this->http->FindNodes("//text()[{$this->eq($this->t('Tax & Fees'), "translate(.,':：','')")}]/ancestor::div[1][count(descendant::text()[normalize-space()])>1]", null, "/^{$this->opt($this->t('Tax & Fees'))}[\s:：]+(.*\d.*)$/");
            $taxesArray = [];

            foreach ($taxes as $tax){
                if (preg_match('/^(?<amount>\d[,.‘\'\d ]*?)[ ]*(?:' . preg_quote($matches['currency'], '/') . ')?$/u', $tax, $m)) {
                    $taxesArray[] = PriceHelper::parse($m['amount'], $currencyCode);
                }
            }

            if (!empty($taxesArray)){
                $r->price()->tax(array_sum($taxesArray));
            } else {
