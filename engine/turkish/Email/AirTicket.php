...
{
    public $mailFiles = "turkish/it-104641218.eml, turkish/it-10472836.eml, turkish/it-104760471.eml, turkish/it-104760588.eml, turkish/it-10695101.eml, turkish/it-10942629.eml, turkish/it-11032252.eml, turkish/it-11178203.eml, turkish/it-11216976.eml, turkish/it-11943638.eml, turkish/it-235008798.eml, turkish/it-5660848.eml, turkish/it-5710839.eml, turkish/it-5913226.eml, turkish/it-6210075.eml, turkish/it-832093767.eml, turkish/it-890436126.eml, turkish/it-9953010.eml";
    ...
    private static $dict = [
        ...
        'de' => [
            ...
            // 'Seat selection'             => [''],
            // '. Flight'                   => '',
            'Main contact for this trip' => 'Hauptkontakt',
        ],
        'fr' => [
            //            'statusText' => '',
            //            'statusVariants' => [''],
        ...
        'ru' => [
            ...
            '. Flight'                   => '. Рейс',
            'Main contact for this trip' => 'Контактное лицо',
        ],
        'tr' => [
            //            'statusText' => '',
            //            'statusVariants' => [''],
    ...
    private function parseEmail(Email $email)
    {
        ...
        $title = ['Mr', 'Ms', 'Mrs', 'Sra', 'Dr', '先生', '女士', 'والسادة', 'Bay', 'Г-жа', 'Г-н', '', '', '', '', ''];

        if (!empty($passengers)) {
            $f->general()
                ->travellers(preg_replace("/^\s*({$this->opt($title)})[.\s]+/ui", '', $passengers));
        } else {
            ...
            if (!empty($passenger)) {
                $f->general()
                    ->traveller(preg_replace("/^\s*({$this->opt($title)})[.\s]+/ui", '', $passenger));
            }
        }
        ...
        $xpath = "//tr[{$this->starts($this->t('FLIGHT'))}][{$this->contains($this->t('DURATION'))}]" . '[not(.//tr)]/following-sibling::tr[count(./td)>=3 and (count(./td[contains(.,":")])>1 or count(./td[contains(.,".")])>1)]';
        ...
        $airportsRoots = $this->http->XPath->query("//tr[count(*) >= 3][*[1][{$timeXpath}]][*[3][{$timeXpath}]]");

        foreach ($airportsRoots as $aRoot) {
            $names = implode("\n", $this->http->FindNodes("*[position() = 1 or position() = 3]/descendant::text()[normalize-space()][2]", $aRoot));

            if (preg_match_all("/^\s*(.+?)\s*\(([A-Z]{3})\)\s*$/mu", $names, $m)) {
                foreach ($m[0] as $i => $v) {
                    $airportsCode[$m[1][$i]] = $m[2][$i];
                }
            }
        }
        $airportsRoots = $this->http->XPath->query("//td[not(.//td)][count(.//text()[{$timeXpath}]) = 2]");

        foreach ($airportsRoots as $aRoot) {
            $names = implode("\n", $this->http->FindNodes(".//text()[contains(., '(') and contains(., ')')]", $aRoot));
        ...
        foreach ($segments as $root) {
            ...
            $name = trim($this->http->FindSingleNode("./descendant::td[string-length(normalize-space())>1][2]/descendant::text()[normalize-space()][last()][not(contains(normalize-space(), 'Direkt'))]/preceding::text()[normalize-space()][1]", $root, true, "/^(\D+)\s*\(/"));

            if (!empty($airportsCode[$name])) {
                $s->departure()
                    ->code($airportsCode[$name]);
            } elseif (preg_match("/^\s*{$this->opt(mb_strstr($name, ',', true))}\s*$/m", implode("\n", array_keys($airportsCode)), $m)) {
                $airportsCode[$name] = $airportsCode[$m[0]];
                $s->departure()
                    ->code($airportsCode[$m[0]]);
            } else {
                $s->departure()->noCode();
            }
            ...
            $name = trim($this->http->FindSingleNode("./descendant::td[string-length(normalize-space())>1][3]/descendant::text()[normalize-space()][last()][not(contains(normalize-space(), 'Direkt'))]/preceding::text()[normalize-space()][1]", $root, true, "/^(\D+)\s*\(/"));
            ...
            if ($this->http->FindSingleNode("preceding-sibling::tr[normalize-space()][1][{$this->starts($this->t('FLIGHT'))}][{$this->contains($this->t('DURATION'))}]", $root)) {
                if (!empty($groupFlights) && (!empty($groupSegment['airportNameFrom']) || !empty($groupSegment['airportNameTo']) || !empty($groupSegment['segmentsId']))) {
                    $groupFlights[] = $groupSegment;
                    $groupSegment = [];
                }

                if (empty($groupSegment['airportNameFrom'])) {
                    $groupSegment['airportNameFrom'] = $s->getDepName();
                    $groupSegment['airportCodeFrom'] = $s->getDepCode();
                } else {
                    $groupFlights[] = $groupSegment;
                    $groupSegment = [];
                }
            }
            $groupSegment['segmentsId'][] = $s->getId();

            if (empty($this->http->FindSingleNode("following-sibling::tr[normalize-space()][1]", $root))) {
                // $i = 1;
                if (empty($groupSegment['airportNameTo'])) {
                    $groupSegment['airportNameTo'] = $s->getArrName();
                    $groupSegment['airportCodeTo'] = $s->getArrCode();
                } else {
                    $groupFlights[] = $groupSegment;
                    $groupSegment = [];
                }
        ...
        if ($this->http->XPath->query("//text()[{$this->eq($this->t('Seat selection'))}]")->length > 0) {
            $segmentsSeats = [];

            foreach ($groupFlights as $gFlight) {
                if (empty($gFlight['airportNameFrom']) || empty($gFlight['airportNameTo']) || empty($gFlight['segmentsId'])) {
                    continue;
                }
                $anFrom = array_filter([$gFlight['airportNameFrom'], preg_replace("/^(.+)-(.+)$/", '$1 ($2)', $gFlight['airportNameFrom']), $gFlight['airportCodeFrom']]);
                $anTo = array_filter([$gFlight['airportNameTo'], preg_replace("/^(.+)-(.+)$/", '$1 ($2)', $gFlight['airportNameTo']), $gFlight['airportCodeTo']]);
                $seatNodes = $this->http->XPath->query("//text()[{$this->eq($this->t('Seat selection'))}]/following::text()[normalize-space()][1]/ancestor::*[not(.//text()[{$this->eq($this->t('Seat selection'))}])][last()]"
                    . "/descendant::tr[{$this->contains($anFrom)}][{$this->contains($anTo)}][last()]");

                foreach ($seatNodes as $snRoot) {
                    if (preg_match("/^\s*{$this->opt($anFrom)}\s*\([A-Z]{3}\)\s*-\s*{$this->opt($anTo)}\s*\([A-Z]{3}\)\s*/u", $snRoot->nodeValue)
                        || preg_match("/^\s*.+?s*\({$this->opt($anFrom)}\\)\s*-\s*.+\s*\({$this->opt($anTo)}\)\s*/u", $snRoot->nodeValue)
                    ) {
                        $seatsText = implode("\n", $this->http->FindNodes(".//text()[{$this->contains($this->t('. Flight'))}]/ancestor::td[1]//text()[normalize-space()][1]", $snRoot));

                        foreach ($gFlight['segmentsId'] as $i => $seg) {
                            $seats = preg_split('/\s*\/\s*/',
                                $this->re("/(?:^|\n)\s*(?:" . ($i + 1) . ")\s*{$this->opt($this->t('. Flight'))}\s*(.+?)(?:\s+\d+\s*{$this->opt($this->t('. Flight'))}|\s*$)/", $seatsText));
                            $segmentsSeats[$seg] = array_filter($seats);
                        }
                    }
                }
            }
            ...
            foreach ($f->getSegments() as $s) {
                if (!empty($segmentsSeats[$s->getId()])) {
                    $s->extra()
                        ->seats($segmentsSeats[$s->getId()]);
                }
            }
        }
    }
