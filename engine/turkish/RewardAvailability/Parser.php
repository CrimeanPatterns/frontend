...
//            }
        ...
        if ($message = $this->http->FindSingleNode('//p[contains(., "We are unable to process your request at this time. Please try again later.")]')) {
...
//            $this->randomClickSearchForm();
    ...
    public function ParseCalendar(array $fields)
    {
        ...
        try {
            ...
            if (!$this->waitForElement(\WebDriverBy::xpath("//label[normalize-space()='From']"), 15)) {
                $this->logger->warning('Page not loaded. Need Retry!');
            ...
            foreach ($requests as $n => $xhr) {
                if (stripos($xhr->request->getUri(), 'information/messages/en/int') !== false) {
                    $this->headers = $xhr->request->getHeaders();

                    break;
                }
            }
        } catch (\UnknownServerException | \WebDriverException | \WebDriverCurlException $e) {
            $this->logger->error($e->getMessage());

            throw new \CheckRetryNeededException(5, 0);
        }

        $response = $this->ParseRewardCalendar($fields);
        $response = $this->http->JsonLog($response, 1, true);

        return ['fares' => $this->parseRewardCalendarJson($response)];
    }
...
//        }
    ...
    private function hasAirport($http2, $airport, $x_bfp, $isCheck = false): bool
    {
        ...
        if (!isset($data) || $data === false || !is_array($data)) {
            ...
            if ($http2->FindSingleNode('//span[contains(., "Take a short break from your passion for travel")]')) {
                $this->logger->error('Not found data airport. Need restart.');
...
//            $this->markProxyAsInvalid();
    ...
    private function ParseRewardCalendar($fields)
    {
        $PageRequestID = $this->generateRequestId();
        ...
        try {
            ...
            xhr.open('POST', 'https://www.turkishairlines.com/api/v1/availability/cheapest-prices', false);
            ...
            xhr.setRequestHeader('X-requestId', '{$this->generateRequestId()}');
            xhr.setRequestHeader('X-conversationId', '{$conversationId}');
            xhr.setRequestHeader('X-clientId', '{$this->headers['X-clientId']}');
            xhr.setRequestHeader('X-bfp', '{$this->headers['X-bfp']}');
    ...
    private function ParseReward($fields, $PageRequestID)
    {
        ...
        if (strpos($this->http->currentUrl(), 'book-award-tickets') == false) {
            $this->http->GetURL("https://www.turkishairlines.com/en-int/miles-and-smiles/book-award-tickets");
...
//        $headers['X-clientId'] = str_replace('"', '', $this->driver->executeScript("return localStorage.getItem('clientId');"));
//        $headers['X-bfp'] = str_replace('"', '', $this->driver->executeScript("return localStorage.getItem('bfp');"));

        $this->logger->info(var_export($this->headers, true), ['pre' => true]);
...
//            xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
        ...
        if (!isset($data['data']['originDestinationInformationList'])) {
            ...
            if ($data['statusDetailList'][0]['code'] === 'Error-BOS-52015') {
                throw new \CheckRetryNeededException(5, 0);
            }
...
//                    $this->sendNotification('hasNoData // ZM');
        ...
        $headers = [
            ...
            "X-clientId"      => $this->headers['X-clientId'],
            "X-bfp"           => $this->headers['X-bfp'],
            "X-country"       => "int",
    ...
    private function parseRewardCalendarJson($data)
    {
        $this->logger->notice(__METHOD__);
        $result = [];

        if (!isset($data)) {
            $this->logger->error('Error get data');

            throw new CheckRetryNeededException(5, 0);
        }

        if (!isset($data['data']['dailyPriceList'])) {
            if ($data['statusDetailList'][0]['code'] === 'Error-BOS-52015') {
                throw new \CheckRetryNeededException(5, 0);
            }
        ...
        foreach ($data['data']['dailyPriceList'] as $item) {
            if (empty($item['price'])) {
                continue;
            }

            $dateTime = \DateTime::createFromFormat("d-m-Y", $item['date']);
            $result[] = [
                'date'        => $dateTime->format("Y-m-d"),
                'redemptions' => ['miles' => $item['price']['amount']],
                'payments'    => [
                    'currency' => 'USD',
                    'taxes'    => null,
                    'fees'     => null,
...
//            $headers['Cache-Control'] = 'no-cache';
            ...
            foreach ($list['fareCategory'] as $fare => $fareDetails) {
                ...
                foreach ($fareDetails['bookingPriceInfoList'] as $bookingPriceInfoList) {
                    if ((time() - $this->requestDateTime) > 280) {
                        $this->SetWarning('Not all flights are collected.');
