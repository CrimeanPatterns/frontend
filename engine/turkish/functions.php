...
class TAccountCheckerTurkish extends TAccountChecker
{
    ...
    public static function GetAccountChecker($accountInfo)
    {
        if (!empty($accountInfo['ConfNo']) && !empty($accountInfo['Code'])) {
            ...
            require_once __DIR__ . "/TAccountCheckerTurkishSelenium.php";

            return new TAccountCheckerTurkishSelenium();
        }
    }
...
//        $this->http->SetInputValue('j_password', $this->AccountFields['Pass']);
    ...
    {
        $this->logger->notice(__METHOD__);
        $key = "6LeSMSIqAAAAAP9RxtQIkvaGmFGYWuR4elfglTqM";
        ...
        $postData = [
            ...
            "websiteURL"  => "https://www.turkishairlines.com/en-us/index.html",
            "websiteKey"  => $key,
            "isInvisible" => true,
        ];
    ...
    public function Login()
    {
        ...
        if (isset($response->data->otpRequired) && $response->data->otpRequired) {
            ...
            $recaptchaInfo = new stdClass();
//            $recaptchaInfo->response = $captcha;

            $data = [
                "ffId"          => $response->data->username,
                "reasonType"    => "MEMBER_LOGIN",
            ...
            $this->http->RetryCount = 0;
            $this->http->PostURL("https://www.turkishairlines.com/api/v1/miles-and-smiles/otp", json_encode($data), $headers);
            $this->http->RetryCount = 2;
            $response = $this->http->JsonLog();

            if (isset($response->data->phoneNumber)) {
                $this->AskQuestion("Please verify your one-time password sent to your {$response->data->phoneNumber} registered phone number to continue.", null, "Question");
                return false;
            }

            if (isset($response->statusDetailList[0]->translatedMessage)) {
                $message = $response->statusDetailList[0]->translatedMessage;
                $this->logger->error("[Error]: {$message}");

                if (strstr($message, 'We are currently unable to process your request.')) {
    ...
    public function ProcessStep($step)
    {
        ...
        $data = [
            ...
            "username"       => $this->AccountFields["Login"],
            "password"       => $this->AccountFields['Pass'],
            "captchaInfo"    => new stdClass(), // "token":{} or "token":{"token":""}
            "loginOperation" => "HOME",
            "otp"            => $answer,
        ];
        $headers = [
        ...
        $this->http->RetryCount = 0;
        $this->http->GetURL("https://www.turkishairlines.com/api/v1/miles-and-smiles/member-credential/login", json_encode($data), $headers);
        $this->http->RetryCount = 2;
        $response = $this->http->JsonLog();
