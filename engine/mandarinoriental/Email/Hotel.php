...
class Hotel extends \TAccountChecker
{
    ...
    {
        $hotelEmails = ['moprg-reservations@mohg.com', 'modoh-reservations@mohg.com', 'mozrh-reservations@mohg.com', 'molon-reservations@mohg.com'];
        ...
        if (!empty($this->http->FindSingleNode("//text()[contains(normalize-space(), ' been cancelled')]"))) {
            $h->general()
                ->cancelled()
                ->status('cancelled')
        ...
        if (empty($hotelInfo)) {
            ...
            if ($this->http->XPath->query("//text()[{$this->contains($hotelName_temp)}]")->length > 0
