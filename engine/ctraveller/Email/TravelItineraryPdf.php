...
{
    public $mailFiles = "ctraveller/it-119057906.eml, ctraveller/it-119735761.eml, ctraveller/it-154537248.eml, ctraveller/it-154673585.eml, ctraveller/it-156533193.eml, ctraveller/it-246701658.eml, ctraveller/it-558631489.eml, ctraveller/it-842910282.eml, ctraveller/it-891748426-fcmtravel.eml, ctraveller/it-888726657.eml";
    ...
    public static $dictionary = [
        'en' => [
            ...
            'hTravellers'           => ['Traveller(s)', 'Traveler(s)'],
            ...
            'rateEnd'          => ['per night', 'average nightly'],
            // CARS
    ...
    private $patterns = [
        ...
        'terminal'      => '[\s\S]*\bTERMINAL\b[\s\S]*|[\s\S]+\sTERM|[\s\S]+\sINTL|[\s\S]*\bINTERNATIONAL\b[\s\S]*|AEROGARE\b[\s\S]*',
    ];
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        $email->setType('TravelItineraryPdf' . $type . ucfirst($this->lang));
    ...
    private function parsePdf(Email $email, $text): void
    {
        ...
        if (preg_match("/\n[ ]{6,}{$this->opt($this->t('Passenger Information'))}.*\n+(?<head>[ ]{0,10}{$this->opt($this->t('hTravellers'))}.*)\n+(?<body>[\S\s]+?)\n+[ ]{6,}{$this->opt($this->t('Travel Summary'))}\n/", $text, $m)) {
            $table = $this->splitCols($m['body'], $this->rowColsPos($m['head']));
            $travellerText = count($table) > 0 ? $table[0] : '';
            ...
            foreach ($travellerRows as $tRow) {
                ...
                if (preg_match("/^[ ]*({$this->patterns['travellerName']})[ ]*$/u", $tRow, $m2)) {
                    $travellers[] = $this->normalizeTraveller($m2[1]);
                } else {
                    $this->logger->debug('Wrong taveller table!');
                    $travellers = [];
        ...
        foreach ($flightMatches[0] as $fText) {
            ...
            if (preg_match("/^\s*(?<code>[A-Z]{3})(?i)(?:[ ]*Arrows)?\n+(?<name>[\s\S]{3,}?)\n+[ ]*{$this->opt($this->t('DEPARTURE'))}\n+[ ]*(?<time>{$this->patterns['time']})\s*(?<terminal>{$this->patterns['terminal']})?\s*$/", $middleTable[0], $m)) {
                $s->departure()
                    ->code($m['code'])
                ...
                if (!empty($m['terminal'])) {
                    $s->departure()->terminal(preg_replace(['/\s+/', "/\s*\b(?:Terminal|AEROGARE)\b\s*/i"], [' ', ''], trim($m['terminal'])));
                }
            }

            if (preg_match("/^\s*(?<code>[A-Z]{3})(?i)\n+(?<name>[\s\S]{3,}?)\n+[ ]*{$this->opt($this->t('ARRIVAL'))}\n+[ ]*(?<time>{$this->patterns['time']})(?:\s*[+]\s*(?<overnight>\d{1,3}))?\s*(?<terminal>{$this->patterns['terminal']})?\s*$/", $middleTable[1], $m)) {
                $s->arrival()
                    ->code($m['code'])
                    //->name(preg_replace('/\s+/', ' ', $m['name']))
                ...
                if (!empty($m['terminal'])) {
                    $s->arrival()->terminal(preg_replace(['/\s+/', "/\s*\b(?:Terminal|AEROGARE)\b\s*/i"], [' ', ''], trim($m['terminal'])));
                }
            }
            ...
            if (preg_match("/(?:^|\n)([ ]*{$this->opt($this->t('Frequent Flyer'))}.{2,}?)(?:\n\n|$)/s", $middleTable[2], $m)) {
                ...
                if (preg_match("/^([-A-Z\d]{5,})$/", $ff, $m2)
                    || !empty($s->getAirlineName()) && preg_match("/^([-A-Z\d]{5,})(?i)\s*{$this->opt($s->getAirlineName())}$/", $ff, $m2)
                ) {
                    // XXXXX2565 WestJet
                    $ffNumbers[] = $m2[1];
                }
            ...
            if (preg_match("/(?:^|\n)[ ]*{$this->opt($this->t('Seat'))}[ ]*:\s*([\s\S]*?)(?:\n+.+:|\s*$)/", $bottomText, $matches)) {
                foreach (preg_split('/(?:[ ]*\n[ ]*)+/', $matches[1]) as $seatRow) {
                    if (preg_match("/^(?<seat>\d+[A-Z])(?:\s*{$this->opt($this->t('Confirmed'))})?(?:\s+-\s+(?<passenger>{$this->patterns['travellerName']}))?$/u", $seatRow, $m)) {
                        $passengerName = empty($m['passenger']) ? null : $this->normalizeTraveller($m['passenger']);
        ...
        foreach ($hotelMatches[0] as $hText) {
            ...
            if (preg_match("/^[ ]*({$this->patterns['date']})\s*({$this->patterns['date']})(?:\b|\D|$)/mu", $dates, $matches)) {
                $checkIn = $matches[1];
                $checkOut = $matches[2];
            } else {
                $tablePos = [0];

                if (preg_match("/ {$this->opt($this->t('CHECK OUT'))}\n+[ ]{0,10}({$this->patterns['date']}\s*)/u", $dates, $matches)
                    || preg_match("/^(.+[ ]{2}){$this->opt($this->t('CHECK OUT'))}/m", $dates, $matches)
                ) {
                    $tablePos[] = mb_strlen($matches[1]);
                }
                ...
                $checkIn = count($table) > 0 && preg_match("/{$this->opt($this->t('CHECK IN'))}[:\s]+([\s\S]{6,}?)\s*$/", $table[0], $m) ? preg_replace('/\s+/', ' ', $m[1]) : null;
                $checkOut = count($table) > 1 && preg_match("/{$this->opt($this->t('CHECK OUT'))}[:\s]+([\s\S]{6,}?)\s*$/", $table[1], $m) ? preg_replace('/\s+/', ' ', $m[1]) : null;
            }

            if ($checkOut && preg_match("/ ID(\n|$)/", $checkOut)) {
            ...
            $roomsCount = $this->re("/^[ ]*{$this->opt($this->t('Number of Rooms'))}[ ]*[:]+[ ]*(\d{1,3})$/m", $bottomText);
            ...
            $roomRate = preg_match("/^[ ]*{$this->opt($this->t('Rate'))}[ ]*:[ ]*(.*?\d.*?(?:\n {6,}.*\d.*?){0,5})\s+{$this->opt($this->t('rateEnd'))}/m", $bottomText, $m) ? $m[1] : null;
            ...
            $h->booked()->rooms($roomsCount, false, true)->guests($guestCount);

            if ($cancellation) {
                $h->general()->cancellation($cancellation);

                if (preg_match("/^\d{1,3}[ ]*[DH] CANCELL? (?<prior>\d{1,3}[ ]*(?:DAYS?|HOURS?)) PRIOR TO ARRI?V?A?L?[-.\s]*$/i", $cancellation, $m)
                    || preg_match("/^CXL (?<prior>\d{1,3}[ ]*(?:DAYS?|HOURS?)) PRIOR TO ARRI?V?A?L?[-.\s]*$/i", $cancellation, $m)
                ) {
                    $h->booked()->deadlineRelative($m['prior'], '00:00');
                } elseif (preg_match("/^\d{1,2}P CANCELL? BY (?<hour>{$this->patterns['time']}) DAY OF ARRI?V?A?L?[-.\s]*$/i", $cancellation, $m)
                ) {
                    $h->booked()->deadlineRelative('0 days', $m['hour']);
                }
            }

            $total = $this->re("/^[ ]*(?:{$this->opt($this->t('Est Hotel Total'))}|{$this->opt($this->t('Est. Total'))})[ ]*[:]+[ ]*(.+?)(?:[ ]+plus .+)?$/im", $bottomText);

            if (preg_match("/^\s*(?<currency>[^\d\s]{1,5})\s*(?<amount>\d[\d., ]*)\s*$/", $total, $m)
                || preg_match("/^\s*(?<amount>\d[\d., ]*)\s*(?<currency>[^\d\s]{1,5})\s*$/", $total, $m)
            ) {
                $h->price()->total(PriceHelper::parse($m['amount'], $m['currency']))->currency($m['currency']);
            }
        }

        // CARS
        //$this->logger->debug($text);
        $cRegexp = "/^[ ]{6,40}\S.+\n+.+[ ]{2}{$this->opt($this->t('Status'))}[ ]*:.*(?:\n.*){1,12}\n[ ]*{$this->opt($this->t('PICK UP'))}[ ]{2,}{$this->opt($this->t('DROP OFF'))}(?:[ ]{2}.+)?(?:\n.*){1,10}\n[ ]*(?:{$this->opt($this->t('Rate'))}|{$this->opt($this->t('Est Car Total'))}|{$this->opt($this->t('Est. Total'))})[ ]*:.*(?:\n*{$this->opt($this->t('Note:'))}.*)?/m";
        $cRegexp2 = "/^[ ]{6,40}\S.+(?:\n.+\b20\d{2}\b.*)?\n+[ ]*{$this->opt($this->t('PICK UP'))}[ ]{2,}{$this->opt($this->t('DROP OFF'))}(?:[ ]{2}.+)?(?:\n.*){1,10}\n[ ]*(?:{$this->opt($this->t('Rate'))}|{$this->opt($this->t('Est Car Total'))}|{$this->opt($this->t('Est. Total'))})[ ]*:.*(?:\n*{$this->opt($this->t('Note:'))}.*)?/m";
        ...
        foreach ($carSegments as $carText) {
            ...
            if (preg_match("/^(?:[ ]*[A-Z]{3}\n+)?([\S\s]+?)\n+([ ]*{$this->opt($this->t('PICK UP'))}[ ]{2,}{$this->opt($this->t('DROP OFF'))}[\S\s]+)$/", $middleTable[0], $m)) {
                ...
                $locationPickup = $locationDropoff = $phonePickup = $phoneDropoff = $faxPickup = $faxDropoff = null;

                $tablePos = [0];

                if (preg_match("/^(.{15,}[ ]{2}){$this->patterns['phone']}$/m", $addressPhones, $matches)) {
                ...
                if (count($table) === 2) {
                    $pickupText = trim($table[0]);
                    $dropoffText = trim($table[1]);
                } else {
                    $pickupText = trim($addressPhones);
                    $dropoffText = '';
                }

                // pickup

                if (preg_match("/^(?<location>[\s\S]+?)(?:\s*;\s*)+(?:Tel|Fax)\s*:/i", $pickupText, $matches)
                    || preg_match("/^(?:(?<location>[\s\S]*?)\n+)?[ ]*(?<phone>{$this->patterns['phone']})(?:\n+[ ]*{$this->patterns['phone']})*$/i", $pickupText, $matches)
                ) {
                    $locationPickup = empty($matches['location']) ? '' : preg_replace('/\s+/', ' ', $matches['location']);
                    $pickupText = preg_replace('/(\d)-\n+[ ]*(\d)/', '$1-$2', $pickupText);
                    $phonePickup = empty($matches['phone']) ? null : $matches['phone'];
                }

                if (preg_match("/[;\s]Tel\s*[:]+\s*({$this->patterns['phone']})(?:\s*;|[;\s]*Fax|\s*$)/i", $pickupText, $matches)) {
                    $phonePickup = $matches[1];
                }

                if (preg_match("/[;\s]Fax\s*[:]+\s*({$this->patterns['phone']})(?:\s*;|[;\s]*Tel|\s*$)/i", $pickupText, $matches)) {
                    $faxPickup = $matches[1];
                }

                if (empty($locationPickup)) {
                    $locationPickup = preg_replace('/\s+/', ' ', $pickupText);
                }

                // dropoff

                if (preg_match("/^(?<location>[\s\S]+?)(?:\s*;\s*)+(?:Tel|Fax)\s*:/i", $dropoffText, $matches)
                    || preg_match("/^(?:(?<location>[\s\S]*?)\n+)?[ ]*(?<phone>{$this->patterns['phone']})(?:\n+[ ]*{$this->patterns['phone']})*$/i", $dropoffText, $matches)
                ) {
                    $locationDropoff = empty($matches['location']) ? '' : preg_replace('/\s+/', ' ', $matches['location']);
                    $dropoffText = preg_replace('/(\d)-\n+[ ]*(\d)/', '$1-$2', $dropoffText);
                    $phoneDropoff = empty($matches['phone']) ? null : $matches['phone'];
                }

                if (preg_match("/[;\s]Tel\s*[:]+\s*({$this->patterns['phone']})(?:\s*;|[;\s]*Fax|\s*$)/i", $dropoffText, $matches)) {
                    $phoneDropoff = $matches[1];
                }

                if (empty($phoneDropoff) && $dropoffText === '') {
                    $phoneDropoff = $phonePickup;
                }

                if (preg_match("/[;\s]Fax\s*[:]+\s*({$this->patterns['phone']})(?:\s*;|[;\s]*Tel|\s*$)/i", $dropoffText, $matches)) {
                    $faxDropoff = $matches[1];
                }

                if (empty($faxDropoff) && $dropoffText === '') {
                    $faxDropoff = $faxPickup;
                }

                if (!$phonePickup && $locationDropoff === '' && $phoneDropoff) {
                    $phonePickup = $phoneDropoff;
                }

                if (empty($locationDropoff)) {
                    $locationDropoff = $dropoffText === '' || $locationDropoff === '' ? $locationPickup : preg_replace('/\s+/', ' ', $dropoffText);
                }

                $locationPickup = preg_replace("/\s[A-Z\d]{2}\s+\d{2,4}$/", "", $locationPickup);
                $locationDropoff = preg_replace("/\s[A-Z\d]{2}\s+\d{2,4}$/", "", $locationDropoff);

                $car->pickup()->location($locationPickup)->phone($phonePickup, false, true)->fax($faxPickup, false, true);
                $car->dropoff()->location($locationDropoff)->phone($phoneDropoff, false, true)->fax($faxDropoff, false, true);
            ...
            } elseif (preg_match("/{$this->opt($this->t('DROP OFF'))}\s+([\s\S]{6,})$/", $table[1], $matches)
                && preg_match("/^{$this->patterns['date']}$/u", trim($matches[1]), $m)
            ) {
                // it-888726657.eml
                $car->dropoff()->noDate();
            }
            ...
            if (preg_match("/(?:^|\n)([ ]*{$this->opt($this->t('Frequent Renter ID'))}.{2,}?)(?:\n\n|$)/s", $middleTable[1], $m)) {
                ...
                if (preg_match("/^[-A-Z\d]{5,}$/", $frID)) {
                    $car->program()->account($frID, preg_match("/XXX/i", $frID) || preg_match("/^XX/i", $frID));
                }
            }

            $car->car()->type($this->re("/^[ ]*{$this->opt($this->t('Type'))}[ ]*:[ ]*(.+?)\s*(?:\s+{$this->opt($this->t('Corp. Discount:'))}|$)/m", $bottomText), false, true);

            $total = $this->re("/^[ ]*(?:{$this->opt($this->t('Est Car Total'))}|{$this->opt($this->t('Est. Total'))})[ ]*[:]+[ ]*(.+?)(?:[ ]+plus .+)?$/im", $bottomText);

            if (preg_match("/^\s*(?<currency>[^\d\s]{1,5})\s*(?<amount>\d[\d., ]*)\s*$/", $total, $m)
                || preg_match("/^\s*(?<amount>\d[\d., ]*)\s*(?<currency>[^\d\s]{1,5})\s*$/", $total, $m)
            ) {
                $car->price()->total(PriceHelper::parse($m['amount'], $m['currency']))->currency($m['currency']);
            }

            if (preg_match("/VEHICLE\-(.+)/", $carText, $m)) {
                $car->setCarType($m[1]);
            }
        }

        $this->logger->debug('Parsed: FLIGHTS - ' . count($parsed['flights']) . ', TRAINS - ' . count($parsed['trains']) . ', HOTELS - ' . count($parsed['hotels']) . ', CARS - ' . count($parsed['cars']));

        $tripPriceText = $this->re("/\n[ ]*{$this->opt($this->t('Estimated Trip Total'))}(?:[ ]{2}.+)?\n+([\s\S]+?)(?:\n{4}|$)/", $text);

        if (preg_match_all("/^[ ]*{$this->opt($this->t('Air'))}[ ]{2,}(\S.*)$/im", $tripPriceText, $matches)
            && count($parsed['flights']) === 1
        ) {
            /** @var \AwardWallet\Schema\Parser\Common\Flight $f */
            $f = $parsed['flights'][0];

            if ($f->getPrice() && $f->getPrice()->getTotal() !== null) {
                $matches[1] = [];
            }

            $amounts = $currencies = [];

            foreach ($matches[1] as $priceVal) {
                if (preg_match('/^(?<amount>\d[,.’‘\'\d ]*?)[ ]*(?<currency>[^\-\d)(]+?)$/u', $priceVal, $m)) {
                    $amounts[] = PriceHelper::parse($m['amount'], $m['currency']);
                    $currencies[] = $m['currency'];
                }
            }

            if (count(array_unique($currencies)) === 1 && !in_array(null, $amounts, true)) {
                $f->price()->total(array_sum($amounts))->currency($currencies[0]);
            }
        }

        if (preg_match_all("/^[ ]*{$this->opt($this->t('Rail'))}[ ]{2,}(\S.*)$/im", $tripPriceText, $matches)
            && count($parsed['trains']) === count($matches[1])
        ) {
            foreach ($parsed['trains'] as $key => $train) {
                /** @var \AwardWallet\Schema\Parser\Common\Train $train */
                if ($train->getPrice() && $train->getPrice()->getTotal() !== null) {
                    continue;
                }

                if (preg_match('/^(?<amount>\d[,.’‘\'\d ]*?)[ ]*(?<currency>[^\-\d)(]+?)$/u', $matches[1][$key], $m)) {
                    $train->price()->total(PriceHelper::parse($m['amount'], $m['currency']))->currency($m['currency']);
                }
            }
        }

        if (preg_match_all("/^[ ]*{$this->opt($this->t('Hotel'))}[ ]{2,}(\S.*)$/im", $tripPriceText, $matches)
            && count($parsed['hotels']) === count($matches[1])
        ) {
            foreach ($parsed['hotels'] as $key => $h) {
                /** @var \AwardWallet\Schema\Parser\Common\Hotel $h */
                if (preg_match('/^(?<amount>\d[,.’‘\'\d ]*?)[ ]*(?<currency>[^\-\d)(]+?)$/u', $matches[1][$key], $m)) {
                    $h->price()->total(PriceHelper::parse($m['amount'], $m['currency']))->currency($m['currency']);
                }
            }
        ...
        if (preg_match_all("/^[ ]*{$this->opt($this->t('Car'))}[ ]{2,}(\S.*)$/im", $tripPriceText, $matches)
            && count($parsed['cars']) === count($matches[1])
        ) {
            foreach ($parsed['cars'] as $key => $car) {
                /** @var \AwardWallet\Schema\Parser\Common\Rental $car */
                if (preg_match('/^(?<amount>\d[,.’‘\'\d ]*?)[ ]*(?<currency>[^\-\d)(]+?)$/u', $matches[1][$key], $m)) {
                    $car->price()->total(PriceHelper::parse($m['amount'], $m['currency']))->currency($m['currency']);
                }
            }
        ...
        $totalPrice = $this->re("/(?:^|\n)[ ]*(?:{$this->opt($this->t('Air'))}|{$this->opt($this->t('Rail'))}|{$this->opt($this->t('Hotel'))}|{$this->opt($this->t('Car'))}).*\n+[ ]*{$this->opt($this->t('Estimated Total Cost'))}[ ]+(.*\d.*?)\s*$/i", $tripPriceText);
    ...
    private function parsePdfInvoice(Email $email, $text): void
    {
        $this->logger->debug(__FUNCTION__ . '()');
...
//                $this->logger->debug('Rental Segment :' . "\n" .  print_r($stext, true));
    ...
    private function getField(string $name, string $source): array
    {
        ...
        return count($table) === 2 ? [
            ...
            'value' => str_replace('*', '', preg_replace('/\s+/', ' ', trim($table[1]))),
        ] : [
            'name'  => null,
            'value' => null,
        ];
