...
//        foreach($this->reBody2 as $lang=>$re){
    ...
    private function parseHtml(Email $email, PlancakeEmailParser $parser)
    {
        ...
        if (count($travellers) == 0) {
            $travellers = array_filter(str_replace(['Mr. ', 'Mrs. ', 'Ms. ', 'Master ', 'Miss'], '', array_unique($this->http->FindNodes("//text()[normalize-space()='Full Name']/ancestor::td[1]", null, "/{$this->opt($this->t('Full Name'))}\s*(.+)/"))));
        }
    ...
    private function parseFlights(Email $email): void
    {
        ...
        foreach ($nodes as $root) {
            ...
            */
            $regexp = "/^\s*(?<dName>.+)\s*\(\s*(?<dCode>[A-Z]{3})\s*\)\s*to\s*(?<aName>.+)\s*\(\s*(?<aCode>[A-Z]{3})\s*\)(?:\s*Next day\s+)?\s*(?<dTime>\d{1,2}:\d{2}(?:\s*[aApP\.]+[mM\.]+)?)\s*-\s*(?<aTime>\d{1,2}:\d{2}(?:\s*[aApP\.]+[mM\.]+)?)(?<nextDay>\s*Next day)?\n(?<cabin>\D+)?\s*\((?<BookingCode>([A-Z]))\)?/";
