...
class IsraelExtension extends AbstractParser implements LoginWithIdInterface, ParseInterface, ParseItinerariesInterface, LoginWithConfNoInterface, RetrieveByConfNoInterface
{
    use TextTrait;
    ...
    public function parse(Tab $tab, Master $master, AccountOptions $accountOptions): void
    {
        ...
        // Status
        $st->addProperty('CurrentClubStatus', $tab->findText("//em[contains(@class,'status')]"));
        // Status valid until
        ...
        if (isset($statusExpiration))
            $st->addProperty('StatusExpiration', strtotime($statusExpiration));
        // Name
        $st->addProperty('Name',
    ...
    {
        return 'https://www.elal.com/CheckIn/Home/New_Identification/c?language=eng';
    }
    ...
    public function loginWithConfNo(Tab $tab, array $confNoFields, ConfNoOptions $options): LoginWithConfNoResult
    {
        $tab->saveScreenshot();
        $tab->saveHtml();
        $tab->evaluate('//input[@name="EticketRequest.EticketNumber"]')->setValue($confNoFields['ConfNo']);
        $tab->evaluate('//input[@name="EticketRequest.LastName"]')->setValue($confNoFields['LastName']);
        $tab->evaluate('//button[@id="eticketBtn"]')->click();
        $tab->saveScreenshot();
        $tab->saveHtml();
        ...
        if ($loginResult) {
            ...
            if (
                stristr($error, "One of inserted parameters is incorrect. Resubmit your request or try again with using E-ticket number")
            ) {
