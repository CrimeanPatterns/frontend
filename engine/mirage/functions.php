...
//        $this->http->PostURL('https://www.mgmresorts.com/mgm-web/authentication/en/v1/login', $data, $headers);
    ...
    public function selenium()
    {
        ...
        try {
            ...
            $welcomeText = $selenium->waitForElement(WebDriverBy::xpath("//div[@data-testid = 'account number']/div[1] | //*[self::h6 or self::p][contains(text(), 'Welcome, ')] | //h3[contains(text(),'My Profile')]"), 15);
            $closeBtn = $selenium->waitForElement(WebDriverBy::xpath('//button[@data-testid="onboarding-modal-icon" and @aria-label="Close"]'), 0);
            // save page to logs
            $this->savePageToLogs($selenium);
            ...
            if ($selenium->waitForElement(WebDriverBy::xpath("
                ...
                $welcomeText = $selenium->waitForElement(WebDriverBy::xpath("//div[@data-testid = 'account number']/div[1] | //*[self::h6 or self::p][contains(text(), 'Welcome, ')] | //h3[contains(text(),'My Profile')]"), 25);
                // save page to logs
                $this->savePageToLogs($selenium);
                ...
                if ($selenium->waitForElement(WebDriverBy::xpath("
                    ...
                    | //*[contains(text() , 'loading...')]
                "), 0)) {
                    $welcomeText = $selenium->waitForElement(WebDriverBy::xpath("//div[@data-testid = 'account number']/div[1] | //*[self::h6 or self::p][contains(text(), 'Welcome, ')] | //h3[contains(text(),'My Profile')]"), 45);
                    // save page to logs
                    $this->savePageToLogs($selenium);
                }
            }

            if (!$welcomeText && !$this->http->FindSingleNode("//div[@data-testid = 'account number']/div[1] | //*[self::h6 or self::p][contains(text(), 'Welcome, ')]")) {
...
//                ])
        ...
        // refs #22673
        if (is_numeric($statusPoints = str_replace(',', '', $this->http->FindSingleNode('//p[contains(text(),"Tier Credits")]/preceding-sibling::p') ?? $this->http->FindSingleNode('//span[contains(text(),"Tier Credits")]', null, true, "/(.+) Tier/")))) {
        ...
        $this->SetProperty("Account", $this->http->FindSingleNode('//button[@data-testid="card-container"]/following-sibling::p[contains(text(),"#")]',
            null, false, '/#\s*(.+)/'));
        // Name
        $this->SetProperty("Name", beautifulName($this->http->FindSingleNode("//*[self::h6 or self::p][contains(text(), 'Welcome, ')]", null, true, "/,\s*([^!]+)/")));
        // Holiday Gift Points
