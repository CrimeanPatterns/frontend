...
//            throw new \CheckException("Unsupported captcha proxy, only BrightData is supported");
    ...
    protected function setProxyNetNut($newSession = null, $country = "us", $siteURL = null, $headers = []): ?array
    {
        ...
        */
        $ipInfoURL = "https://awardwallet-public.s3.amazonaws.com/healthcheck.html";
//        $ipInfoURL = "https://api.netnut.io/myIP.aspx";
...
//        $domain = 'gw-g.ntnt.io'; // TODO: get back gw.ntnt.io, now netnut has multiple unresponsive proxies behind this dns name, May 2022
        ...
        if ($this->isRewardAvailability && $this->AccountFields["Partner"] === "juicymiles") {
...
//        $ipInfoURL = "https://ipinfo.io/ip";
    ...
    protected function setProxyGoProxies($newSession = null, $country = 'us', $city = null, $state = null, $siteURL = null, $headers = [])
    {
        ...
        if ($this->isRewardAvailability && $this->AccountFields['Partner'] === 'awardwallet' && isset($this->AccountFields['ProviderCode'])) {
            [$proxyUserName, $proxyPwd] = $this->createGoCredentials($this->AccountFields["Method"] === "reward-availability-hotel" ? "hs_" : "fs_", $this->AccountFields['ProviderCode']);
        }
        elseif ($this->isRewardAvailability && isset($this->AccountFields['ProviderCode'])) {
            [$proxyUserName, $proxyPwd] = $this->getGoCredentials($this->AccountFields['ProviderCode']);
        }
        elseif (!$this->isRewardAvailability && isset($this->AccountFields['ProviderCode'])) {
            [$proxyUserName, $proxyPwd] = $this->createGoCredentials("p_", $this->AccountFields['ProviderCode']);
        }
...
//        [$proxyUserName, $proxyPwd] = [GOPROXIES_USERNAME, GOPROXIES_PASSWORD];
        ...
        */
        ...
        if ($proxyURL === null) {
            ...
            if ($this->isRewardAvailability && $this->AccountFields["Partner"] !== "awardwallet") {
                $this->setProxyNetNut();
            } else {
                $this->setProxyBrightData(null, 'static', $country);
            }
    ...
    protected function setLpmProxy(Port $port, string $siteURL = null) : void
    {
        ...
        $lpmInternalIp = $lpm->getInternalIp();

        if (strpos($this->http->GetProxy(), $lpmInternalIp) !== false) {
            $this->logger->info("already using lpm proxy");
        ...
        try {
            ...
            $proxyURL = $lpmInternalIp . ':' . $createPortResponse->getPortNumber();
        }
        ...
        $this->onCheckFinished[] = function() use ($lpm, $createPortResponse, $port) {
            $stats = $lpm->getRecentStats();
            $logPortStats = function(?array $portStats, string $portName) use ($port) {
                if ($portStats !== null) {
                    $portStats['total_bw'] = $portStats['in_bw'] + $portStats['out_bw'];
                    $this->globalLogger->info("lpm $portName port stats: " . json_encode($portStats), ["Total" => $portStats['total_bw'], "Host" => $port->getExternalProxyName(), "Provider" => $this->AccountFields["ProviderCode"]]);
                }
            };
    ...
    protected function getProxyHost(string $proxyType = 'dop', string $country = 'us', string $zone = 'static')
    {
        switch ($proxyType) {
            ...
            case 'netnut':
                $params = $this->getLiveProxyNetNut(null, $country, "https://awardwallet-public.s3.amazonaws.com/healthcheck.html");
    ...
    private function createGoCredentials(string $prefix, string $providerCode) : array
    {
        $userName = $prefix . $providerCode;
...
//            ['api_key' => MOUNT_APIKEY], 20);
    ...
    protected function requestProxyManager(\AwardWallet\Common\Parsing\Web\Proxy\ProxyRequestInterface $request)
    {
        if (!isset($this->State['ProxyManagerState'])) {
            $this->State['ProxyManagerState'] = [];
        }

        $proxyManager = new \AwardWallet\Common\Parsing\Web\Proxy\ProxyManager(
            $this->logger,
            $this->State['ProxyManagerState'],
            $this->services->get(\AwardWallet\Common\Parsing\Web\Proxy\ProxyProviders::class)->getProxyProviders(),
            $this->services->get(\HttpDriverInterface::class),
            new \AwardWallet\Common\Parsing\Web\Proxy\ProxyContext($this->AccountFields['ProviderCode'])
        );

        $proxy = $proxyManager->get($request);
        $this->http->setProxyParams(["proxyAddress" => $proxy->host, "proxyHost" => $proxy->host, "proxyPort" => $proxy->port, "proxyLogin" => $proxy->username, "proxyPassword" => $proxy->password]);

        if ($request instanceof \AwardWallet\Common\Parsing\Web\Proxy\Provider\MountRotatingRequest) {
            $this->logger->info("tracking mount traffic through lpm");
