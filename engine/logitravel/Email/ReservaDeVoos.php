...
class ReservaDeVoos extends \TAccountChecker
{
    ...
    public $reBody = [
        ...
        'pt6'  => ['DETALHES DO SEU ORÇAMENTO', 'Voos'],
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if (count($this->pax) === 0) {
            $this->pax = $this->http->FindNodes("//text()[normalize-space()][{$this->eq($this->t('Titular:'))}]/following::text()[normalize-space()][1]", null, "/^{$this->patterns['travellerName']}$/u");
        }
    ...
    private function parseFlights(Email $email): void
    {
        ...
        foreach ($airs as $rl => $roots) {
            ...
            } else if ($this->http->XPath->query("//text()[{$this->contains($this->t('Localizador companhia aérea'))}]")->length === 0){
                $f->general()->noConfirmation();
            }
