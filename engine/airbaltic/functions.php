...
class TAccountCheckerAirbaltic extends TAccountChecker
{
    ...
    private const REWARDS_PAGE_URL = "https://www.airbalticclub.com/int-en/my-account/card";
    /**
...
//        $this->http->removeCookies();
    ...
    public function Parse()
    {
        ...
        // Exp date
        $this->http->GetURL("https://www.airbalticclub.com/int-en/my-account");
        $points = $this->http->FindSingleNode("//p[contains(text(), 'will expire on')]", null, true, "/p ([\d\.\, ]+) will/");
        $month = $this->http->FindSingleNode("//p[contains(text(), 'will expire on')]", null, true, "/expire on\s*(.+)/");
        $date = str_replace(' / ', '/1/', $month);
        $this->logger->debug($date . " / " . $points . " points");

        if ($points > 0 && strtotime($date)) {
            // PINS to Expire
            $this->SetProperty("PointsToExpire", $points);
            $this->SetExpirationDate(strtotime("last day of ".$date));
        }// if ($points > 0 && strtotime($date))

//        $this->http->GetURL("https://www.airbaltic.com/en/airbaltic-club-new");
...
//            $this->SetBalance($user->points ?? null);
        ...
        */
        ...
        $this->http->GetURL("https://www.airbalticclub.com/int-en/my-account/profile");
        // Name
