    ...
    private function selenium()
    {
        ...
        try {
            ...
            foreach ($requests as $xhr) {
                ...
                if (strpos($xhr->request->getUri(), 'hotels/v3/profiles/') !== false) {
                    $resp = $xhr->response->getBody();

                    if (isset($resp['hotelContent'])) {
                        $hotelsProfile[] = $resp['hotelContent'][0];
                    }
                }
            }
...
//        $this->http->JsonLog(json_encode($hotelsProfile), 1);
        ...
        foreach ($hotelsProfile as $profile) {
            if (!isset($hotelsWithPoints[$profile['hotelCode']])) {
                continue;
            }

            $hotelsWithPoints[$profile['hotelCode']] += [
                'brandName' => $profile['brandInfo']['brandName'],
                'profile'   => [
                    'name'          => $profile['profile']['name'][0]['value'],
                    'averageReview' => $profile['profile']['averageReview'] ?? null,
                    'totalReviews'  => $profile['profile']['totalReviews'] ?? null,
                    'latitude'      => $profile['profile']['latLong']['latitude'],
                    'longitude'     => $profile['profile']['latLong']['longitude'],
    ...
    private function parseData($response)
    {
        ...
        foreach ($response as $hotel) {
            ...
            $address = $this->makeAddress($hotel['address']['translatedMainAddress']);
    ...
    {
        $address = (isset($data['line1'][0]['value'])) ? $data['line1'][0]['value'] . ', ' : '';
        $address .= (isset($data['city'][0]['value'])) ? $data['city'][0]['value'] . ', ' : '';
        $address .= (isset($data['state'][0]['value']) && isset($data['state'][0]['value'])) ? $data['state'][0]['value'] . ', ' : '';
        $address .= (isset($data['zipcode'][0]['value'])) ? $data['zipcode'][0]['value'] . ', ' : '';
        $address .= (isset($data['country'][0]['value']) && isset($data['country'][0]['value'])) ? $data['country'][0]['value'] . ', ' : '';
