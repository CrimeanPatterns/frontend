...
{
    public $mailFiles = "ichotelsgroup/statements/it-61223250.eml, ichotelsgroup/statements/it-61327511.eml, ichotelsgroup/statements/it-61394646.eml, ichotelsgroup/statements/it-61418594.eml, ichotelsgroup/statements/it-62058901.eml, ichotelsgroup/statements/it-62439081.eml, ichotelsgroup/statements/it-62545737.eml, ichotelsgroup/statements/it-62659521.eml, ichotelsgroup/statements/it-72865057.eml, ichotelsgroup/statements/it-73005072.eml, ichotelsgroup/statements/it-896335068.eml, ichotelsgroup/statements/it-896336023.eml, ichotelsgroup/statements/it-896336984.eml, ichotelsgroup/statements/it-896337112.eml, ichotelsgroup/statements/it-896924480.eml, ichotelsgroup/statements/it-896932138.eml, ichotelsgroup/statements/it-896933596.eml, ichotelsgroup/statements/it-896965767.eml, ichotelsgroup/statements/it-896972700.eml, ichotelsgroup/statements/it-897066075.eml";
    private $lang = '';
    ...
    private $reBody = [
        'en' => [
            'Log in to your account profile with your IHG',
            'Log in to your account profile with your IHG One Rewards number and password.',
        ...
        'pt' => [
            ...
            'Inicie sessão no perfil da sua conta com o seu IHG One Rewards número e palavra-passe. ',
        ],
        'ko' => [
        ...
        'ja' => [
            ...
            'IHG One Rewards会員番号とパスワードを使ってアカウントにログインしてください。',
            'にアクセスしてください。 IHG One Rewards会員番号とパスワードを使ってアカウントにログインしてください',
        ],
        'it' => [
    ...
    private static $dictionary = [
        ...
        'en' => [
            ...
            'Hi'                      => ['Hi', 'HI'],
            'status through'          => ['status through', 'status until'],
            'Elite Qualifying Points' => ['Elite Qualifying Points', 'Elite Qualifying points'],
            'Elite Qualifying Nights' => ['Elite Qualifying Nights', 'Elite Qualifying nights'],
            'Points earned'           => ['Points earned', 'points earned'],
            'Summary'                 => ['Summary', 'Check out your stats.'],
            'You have'                => ['You have', 'With'],
        ],
        'es' => [
    ...
    public function ParsePlanEmailExternal(PlancakeEmailParser $parser, Email $email)
    {
        ...
        // dictionary for all langs (finding of IHG logo)
        $logoDict = [
        ...
        // provider logo from which find account information
        $xpathMainLogo = "(({$this->contains($this->t('IHG'), '@alt')} and ({$this->contains($logoDict['Rewards'], '@alt')} or {$this->contains($logoDict['Logo'], '@alt')})) or {$this->contains($logoDict['/IHG'], '@src')})";

        // account info after provider logo
        // it-61223250.eml, it-62659521.eml, it-896336023.eml
        $rowsNodes = $this->http->XPath->query("(//img[{$xpathMainLogo}])[1]/following::table[contains(translate(normalize-space(.),'0123456789','dddddddddd--'),'ddddddddd')][1]/descendant::text()[normalize-space()]");

        // account info before provider logo
        // it-896335068.eml
        if ($rowsNodes->length === 0) {
            $rowsNodes = $this->http->XPath->query("(//img[{$xpathMainLogo}])[1]/preceding::table[contains(translate(normalize-space(.),'0123456789','dddddddddd--'),'ddddddddd')][1]/descendant::text()[normalize-space()]");
        }
        ...
        */
        $accountPatterns['v1'] = "/^"
            ...
            . "[ ]*(?<level>[\w ]+)[ ]*\n"
            . "[ ]*(?:{$this->opt($this->t('Member #:'))}\s+)?(?<number>\d{5,})[ ]*(?:\n|$)"
            . "/u";
        ...
        */
        $accountPatterns['v4'] = "/^"
            . "\s*(?<name>{$patterns['travellerName']})[ ]*\n"
            . "[ ]*{$this->opt($this->t('Member #:'))}[ ]+(?<number>\d{5,})[ ]*\n"
            . "[ ]*(?:{$this->opt($this->t('Membership Level:'))}[ ]+)?(?<level>[\w ]+)[ ]*(?:\n|$)"
            . "/u";
        ...
        */
        ...
        foreach ($accountPatterns as $pattern) {
            if (preg_match($pattern, $text, $m)) {
                ...
                if (!empty($m['level'])) {
                    ...
                    $st->addProperty('Level', preg_replace('/^[^(]*\(\s*([^)(]{4,}?)\s*\)[^)]*$/', '$1', $m['level']));
                }
        ...
        if (preg_match("/^{$this->opt($this->t('Points Balance'))}[: ]+(\d[,.\'\d ]*)[ ]+{$this->opt($this->t('as of'))}[ ]+(.{6,})$/", $pointsBalanceCell, $m)) {
            ...
            if (preg_match_all('/\b(\d+)\s*\/\s*\d+\s*\/\s*\d{4}\b/', $m[2], $dateMatches)) {
        ...
        if (preg_match_all("/asofdate=(\d\S*?)(?:&|\s|$)/i", $urlData, $m)
            ...
            if (preg_match_all('/\b(\d+)\s*\/\s*\d+\s*\/\s*\d{4}\b/', $m[1][0], $dateMatches)) {
                foreach ($dateMatches[1] as $simpleDate) {
                    if ($simpleDate > 12) {
                        $this->enDatesInverted = true;
        ...
        // it-896336023.eml, it-896965767.eml
        $balanceTextNodes = $this->http->XPath->query("(//text()[{$this->eq($this->t('YOUR POINTS BALANCE IS'))}])/ancestor::tr[1]");

        if ($balanceTextNodes->length > 0) {
            $name = $this->re("/^\s*{$this->opt($this->t('Hi'))}\s+({$patterns['travellerName']})\,\s+/u", $balanceTextNodes[0]->nodeValue);

            if (!empty($name)) {
                $st->addProperty('Name', $name);
            }

            $balance = $this->http->FindSingleNode("./following::text()[normalize-space()][1]", $balanceTextNodes[0], true, "/^\s*([\d\.\,\']+)\s*$/");
            $st->setBalance($this->normalizeAmount($balance));

            $levelExpiryDate = $this->http->FindSingleNode("//text()[{$this->contains($this->t('status through'))}]", null, true, "/^.+?(\b\d+\s*\/\s*\d+\s*\/\s*\d{4})\s*$/u");
            $balanceExpireDate = $this->http->FindSingleNode("//text()[{$this->starts('your points WILL expire on')}]", null, true, "/^\s*{$this->opt($this->t('your points WILL expire on'))}\s*(.+\d{4})\s*$/");
            $curStatusDate = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Account status valid as of'))}]", null, true, "/^.+?(\b\d+\s*\/\s*\d+\s*\/\s*\d{4})\s*$/u");

            $dates = array_filter([$levelExpiryDate, $curStatusDate, $balanceExpireDate]);

            foreach ($dates as $date) {
                if ($this->re("/^(\d+)\//", $date) > 12) {
                    $this->enDatesInverted = true;
            ...
            if (!empty($levelExpiryDate)) {
                $st->addProperty('LevelExpiry', strtotime($this->normalizeDate($levelExpiryDate)));
            }

            if (!empty($balanceExpireDate)) {
                $st->parseExpirationDate($this->normalizeDate($balanceExpireDate));
            }

            if (!empty($curStatusDate)) {
                $st->parseBalanceDate($this->normalizeDate($curStatusDate));
            }

            $pointsEarned = $this->http->FindSingleNode($xpath = "//text()[{$this->eq($this->t('Elite Qualifying Points'))}]/ancestor::tr[1]/following-sibling::tr[normalize-space()][1]/descendant::td[not(.//td)][normalize-space()][1]", null, true, "/^\s*([\d\.\,\']+)\s+{$this->opt($this->t('point(s)'))}\s*$/u");

            if ($pointsEarned !== null) {
                $st->addProperty('PointsEarned', $this->normalizeAmount($pointsEarned));
            }

            $qualifyingNights = $this->http->FindSingleNode("//text()[{$this->eq($this->t('Elite Qualifying Nights'))}]/ancestor::tr[1]/following-sibling::tr[normalize-space()][1]/descendant::td[not(.//td)][normalize-space()][1]", null, true, "/^\s*(\d+)\s+{$this->opt($this->t('night(s)'))}\s*$/u");

            if ($qualifyingNights !== null) {
                $st->addProperty('QualifyingNights', $qualifyingNights);
            }
        }

        // it-896336984.eml, it-896924480.eml
        $totalPointsEarned = $this->http->FindSingleNode("//text()[{$this->starts($this->t('Points earned'))}]/preceding::text()[normalize-space()][1]", null, true, "/^\s*([\d\.\,\']+)\s*$/u");

        if ($totalPointsEarned !== null) {
            $st->addProperty('TotalPointsEarned', $this->normalizeAmount($totalPointsEarned));
        }

        // it-897066075.eml, it-896972700.eml
        $balanceText = $this->http->FindSingleNode("//text()[{$this->contains($this->t('points are set to expire on'))}]");

        if (preg_match("/\s*(?<balance>[\d\.\,\']+)\s*{$this->opt($this->t('points are set to expire on'))}\s+(?<expireDate>.+\d{4})/", $balanceText, $m)) {
            $st->setBalance($this->normalizeAmount($m['balance']));
            $st->parseExpirationDate($this->normalizeDate($m['expireDate']));
        }

        // it-896933596.eml, it-896932138.eml
        $balance = $this->http->FindSingleNode("//text()[{$this->starts($this->t('You have'))}]", null, true, "/^\s*{$this->opt($this->t('You have'))}\s*([\d\.\,\']+)\s*{$this->opt($this->t('points'))}/")
            ?? $this->http->FindSingleNode("//text()[{$this->contains($this->t('your current points balance of'))}]", null, true, "/{$this->opt($this->t('your current points balance of'))}\s*([\d\.\,\']+)\s*{$this->opt($this->t('to'))}/");
        ...
        $userEmail = $this->http->FindSingleNode("(//text()[{$this->starts($this->t('You are subscribed as:'))}])[1]", null, true, "/^\s*{$this->opt($this->t('You are subscribed as:'))}\s+([\w\.]+\@[\w\.]+\b)/u")
            ?? $this->http->FindSingleNode("(//text()[{$this->starts($this->t('You are subscribed as:'))}])[1]/following::text()[normalize-space()][1]", null, true, "/^\s*([\w\.]+\@[\w\.]+\b)/u");
    ...
    private function normalizeDate(?string $date): ?string
    {
        ...
        // 3/9/2025 => 03/09/2025
        $date = preg_replace("/\b(\d)\//", "0$1/", $date);
        ...
        $out = [
            $this->enDatesInverted ? '$1.$2.$3' : '$2.$1.$3',
            '$1 $2 $3',
            '$2 $1 $3',
        ];
