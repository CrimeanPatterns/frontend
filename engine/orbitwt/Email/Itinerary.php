...
class Itinerary extends \TAccountChecker
{
    ...
    private static $bodies = [
        'orbitwt' => [
            ...
            '@travelcall.com.au', //while not this provider in our list
        ...
        foreach ($airs as $airline => $sTexts) {
            ...
            if (!empty($accounts)) {
                foreach ($accounts as $account) {
                    ...
                    if (!empty($pax)) {
                        if (preg_match("/(?<desc>.+)[ ]+(?<number>\d+)/", $account, $m)) {
                        ...
                        if (preg_match("/(?<desc>.+)[ ]+(?<number>\d+)/", $account, $m)) {
                            $r->addAccountNumber($m['number'], false, null, $m['desc']);
                        } else {
