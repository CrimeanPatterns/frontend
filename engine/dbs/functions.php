...
//        $this->http->SetInputValue('txtCreditCardNumber', $this->AccountFields['Pass']);
    ...
    public function Login()
    {
        ...
        */
        ...
        try {
            $this->waitForElement(WebDriverBy::xpath('//div[@id = "errormsg" and normalize-space(.) != ""] | //h1[strong[contains(text(), "Two Step Verification")]]'), 10);
            $this->saveResponse();
...
//        $logout->click();
    ...
    private function twoStepVerification($generateNewOTP = false)
    {
        ...
        if (!$authenticate || !$elements) {
            return false;
        }

        if ($generateNewOTP && ($generateOTP = $this->waitForElement(WebDriverBy::xpath('//p/button[@id = "resenddiv" and contains(text(), "GENERATE")]'), 0, false))) {
            $generateOTP->click();
            $this->saveResponse();
        }
        ...
        if (strlen($this->Answers[self::OTP]) < 6) {
            $generateOTP = $this->waitForElement(WebDriverBy::xpath('//p/button[@id = "resenddiv" and contains(text(), "GENERATE")]'), 0, false);
            if (!$generateNewOTP && $generateOTP) {
                $generateOTP->click();
                $this->saveResponse();
            }
