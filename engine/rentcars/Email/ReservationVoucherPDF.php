...
{
    public $mailFiles = "rentcars/it-117227630-pt.eml, rentcars/it-118044981.eml, rentcars/it-901399863-pt.eml";
    private $subjects = [
        'Confirmação de Reserva -',
        'Atualizações em sua reserva -',
    ...
    private $detectLang = [
        'pt' => ['RETIRADA', 'Voucher da Reserva'],
        'en' => ['PICKUP'],
    ];

    private $pdfPattern = '.+\.pdf';
    private $detectBody = [
        'pt' => ['Instruções de retirada', 'Código da Solicitação Rentcars'],
    ...
    private static $dictionary = [
        "pt" => [
            ...
            'mainTableEnd'        => ['ATENÇÃO', 'Instruções de retirada', 'INSTRUÇÕES DE RETIRADA'],
            'RENTER'              => ['LOCATÁRIO', 'CONDUTOR'],
            'PICKUP'              => 'RETIRADA',
            'RETURN'              => 'DEVOLUÇÃO',
            'ATTENTION:'          => 'ATENÇÃO:',
            'RENTAL COMPANY:'     => 'LOCADORA:',
            'Your reservation:'   => ['Sua reserva:', 'SUA RESERVA'],
            'Vehicle'             => ['Veículo e Categoria', 'Veículo'],
        ...
        "en" => [
            ...
            'mainTableEnd'   => ['ATTENTION', 'Pickup instructions', 'PICKUP INSTRUCTIONS'],
    ...
    {
        return preg_match('/[@.]rentcars\.com$/i', $from) > 0;
    }
    ...
    public function ParseRental(Email $email, $text): void
    {
        ...
        $textHeader = $this->re("/^([\s\S]+?)\n+.+ {$this->opt($this->t('mainTableStart'))}[: ]*\n/", $text);

        $r->general()
            ->confirmation($this->re("/^.{60,}?[ ]{3}([A-Z\d]{7})$/m", $textHeader))
            ->traveller($this->re("/{$this->opt($this->t('RENTER'))}[ ]*[:]+[ ]*({$patterns['travellerName']})(?:[ ]{2}|\n)/u", $text) ?? $this->re("/{$this->opt($this->t('RENTER'))}.*\n[ ]*({$patterns['travellerName']})\n/u", $text), true);

        $mainTableText = $this->re("/(.+ {$this->opt($this->t('mainTableStart'))}[: ]*\n+[\s\S]+?)\n+[ ]*{$this->opt($this->t('mainTableEnd'))}[:\s]/m", $text);
        $tablePos = [0];
