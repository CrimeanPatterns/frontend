...
class TAccountCheckerCinemark extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        $login = $this->waitForElement(WebDriverBy::xpath("//input[@id = 'EmailAddress' or @id = 'Email' or @id = 'username']"), 20);
        $this->saveResponse();
        ...
        try {
            if (!$login && $this->waitForElement(WebDriverBy::xpath("//span[contains(text(), 'Checking your browser before accessing')] | //h2[contains(text(), 'Checking if the site connection is secure')]"), 0)) {
                ...
                $this->saveResponse();
                $this->http->GetURL('https://www.cinemark.com/Membership/SignIn');
                $login = $this->waitForElement(WebDriverBy::xpath("//input[@id = 'EmailAddress' or @id = 'Email' or @id = 'username']"), 15);
            }
        ...
        $this->saveResponse();
        $pass = $this->waitForElement(WebDriverBy::xpath("//input[@id = 'Password' or @id = 'password']"), 5);
        $signInButton = $this->waitForElement(WebDriverBy::xpath("//input[@id = 'signInButton'] | //button[@id = 'kc-login']"), 0);
        ...
        $this->driver->executeScript('let rememberMe = document.querySelector(\'#RememberMe, #rememberMe\'); if (rememberMe) rememberMe.checked = true;');
        $this->saveResponse();
    ...
    public function Login()
    {
        ...
        $res = $this->waitForElement(WebDriverBy::xpath('
            ...
            | //div[contains(@id, "input-error-")]/span
            | //p[contains(text(), "Our Privacy Policy has been updated. Please review and confirm that you agree.")]
            | //h1[contains(text(), "Update Account Information")]
            | //span[contains(text(), "You need to change your password to activate your account.")]
        '), 25);
        $this->saveResponse();
        ...
        if ($this->http->FindSingleNode('//h1[contains(text(), "Update Account Information")]')) {
            $this->throwProfileUpdateMessageException();
        }

        // Errors
        $message = $this->http->FindNodes('//form[@id="signInForm"]/div[@class = "validation-summary-errors"]/ul/li/text() | //div[contains(@id, "input-error-")]/span | //span[contains(text(), "You need to change your password to activate your account.")]');
