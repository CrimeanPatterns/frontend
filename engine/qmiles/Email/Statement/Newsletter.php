...
class Newsletter extends \TAccountChecker
{
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        $membershipNo = $this->http->FindSingleNode("preceding::tr[normalize-space()='Membership No.']/preceding-sibling::tr[normalize-space()][1]", $root, true, '/^[-A-Z\d]{5,}$/')
            ...
            ?? $this->http->FindSingleNode("preceding::td[{$this->eq(['Membership No.', 'Membership Number', 'Membership number', 'N. socio', '会员号', 'Mitgliedschafts-Nr.', 'N. Socio', 'Αριθμός μέλους', 'Nº de membro', '회원번호',
                '会員番号 :', '会員番号：', 'Numero socio', 'Numéro de membre', 'Numero di iscrizione', 'Member no', 'رقم العضوية', 'No. de membro', 'No. Socio', ])}]/following-sibling::td[normalize-space()][1]", $root, true, '/^\d{5,}$/')
            ?? $this->http->FindSingleNode("preceding::td[{$this->eq(['رقم العضوية'])}]/preceding-sibling::td[normalize-space()][1]", $root, true, '/^\d{5,}$/')
        ;
        ...
        $name = $this->http->FindSingleNode("preceding::text()[contains(normalize-space(),'we remain committed to')]", $root, true, "/^({$patterns['travellerName']})\s*,\s*we remain committed to/iu")
            ...
            ?? $this->http->FindSingleNode("preceding::text()[starts-with(normalize-space(),'Congratulations')]", $root, true, "/^Congratulations\s+({$patterns['travellerName']}), you have arrived at your destination/u")
            ?? $this->http->FindSingleNode("preceding::text()[contains(normalize-space(),', need more Avios?')]", $root, true, "/^\s*({$patterns['travellerName']}), need more Avios\? You can simply buy them/u")
            ?? $this->http->FindSingleNode("preceding::text()[contains(normalize-space(),', we hope you are enjoying ')]", $root, true, "/^\s*({$patterns['travellerName']}), we hope you are enjoying /u")
            ?? $this->http->FindSingleNode("preceding::text()[contains(normalize-space(),', we hope you have enjoyed ')]", $root, true, "/^\s*({$patterns['travellerName']}), we hope you have enjoyed /u")
        ;
    ...
    private function findRoot(): \DOMNodeList
    {
        ...
        if ($roots->length !== 1) {
            $roots = $this->http->XPath->query("//text()[contains(normalize-space(), '©') and contains(normalize-space(), 'الحقوق محفوظة للخطوط الجوية القطرية')]");
            $this->logger->debug('debug' . $roots->length);
        }
