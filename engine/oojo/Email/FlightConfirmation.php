...
{
    public $mailFiles = "oojo/it-807146626.eml, oojo/it-821812115.eml, oojo/it-824507481.eml, oojo/it-907748938.eml";
    public $subjects = [
    ...
    public static $dictionary = [
        'en' => [
            ...
            'Depart'                        => 'Depart',
            'Oojo International B.V.'       => ['Oojo International B.V.', 'TravelTech corp.'],
        ],
    ];
    ...
    public function detectEmailByBody(PlancakeEmailParser $parser)
    {
        ...
        foreach (self::$dictionary as $dict) {
            if (!empty($dict['detectPhrase']) && $this->http->XPath->query("//*[{$this->contains($dict['detectPhrase'])}]")->length > 0
                ...
                && !empty($dict['Depart']) && $this->http->XPath->query("//*[{$this->eq($dict['Depart'])}]")->length > 0
            ) {
                return true;
            }
        }
    ...
    public function Flight(Email $email)
    {
        ...
        foreach ($segmentNodes as $root) {
            ...
            $year = $this->http->FindSingleNode("//text()[{$this->contains($this->t('Oojo International B.V.'))}]", $root, false, "/^\©\s*(\d{4})\s*{$this->opt($this->t('Oojo International B.V.'))}\,/");
            ...
            if (preg_match("/^(?<code>[A-Z]{3})\s*\,\s*\D+$/", $departureCode, $m)) {
                $s->departure()
                    ->code($m['code']);
            } elseif (preg_match("/^(?<name>.+)\s*\b(?<code>[A-Z]{3})\s*$/", $departureCode, $m)) {
                $s->departure()
                    ->name($m['name'])
                    ->code($m['code']);
            }
            ...
            if (preg_match("/^(?<code>[A-Z]{3})\s*\,\s*\D{2}$/", $arrivalCode, $m)) {
                $s->arrival()
                    ->code($m['code']);
            } elseif (preg_match("/^(?<name>.+)\s*\b(?<code>[A-Z]{3})\s*$/", $arrivalCode, $m)) {
                $s->arrival()
                    ->name($m['name'])
                    ->code($m['code']);
            }
