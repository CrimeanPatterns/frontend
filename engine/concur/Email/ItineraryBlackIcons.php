                ...
                if (!empty($dateFormat) && !empty($h->getCheckInDate())) {
                    ...
                    foreach ($ratesStr as $rateStr) {
                        if (preg_match("/^\s*(?<from>{$dateFormat})\s*-\s*(?<till>{$dateFormat})\s+(?<rate>.+)/u", $rateStr, $m)) {
                            $dateFrom = $this->dateStringToEnglish($m['from']);
                            $dateTill = $this->dateStringToEnglish($m['till']);
                            $day = date_diff(
                                date_create('@' . EmailDateHelper::parseDateRelative($dateFrom, strtotime("-1 day", $h->getCheckInDate()))),
                                date_create('@' . EmailDateHelper::parseDateRelative($dateTill, strtotime("-1 day", $h->getCheckInDate())))
                            )->format('%a');
