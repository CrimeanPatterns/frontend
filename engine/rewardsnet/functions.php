...
//            $selenium->useGoogleChrome(\SeleniumFinderRequest::CHROME_94);
            ...
            $loginInput = $selenium->waitForElement(WebDriverBy::xpath('//*[@id = "username"]'), 10);
            $passwordInput = $selenium->waitForElement(WebDriverBy::xpath('//*[@id = "password"]'), 0);
            // save page to logs
            $this->savePageToLogs($selenium);
...
//            $loginInput->sendKeys($this->AccountFields['Login']);
            ...
            */
            ...
            $selenium->driver->executeScript("
                document.querySelector('#username').shadowRoot.querySelector('input[name=\"username\"]').value = \"{$this->AccountFields['Login']}\";
            ");
            $this->logger->debug("enter password");
            $selenium->driver->executeScript("
                document.querySelector('#password').shadowRoot.querySelector('input[name=\"password\"]').value = \"" . str_replace(["\\", '"'], ["\\\\", '\"'], $this->AccountFields['Pass']) . "\";
            ");
            sleep(2);
            $this->savePageToLogs($selenium);
            $this->logger->debug("click by btn");
            $selenium->driver->executeScript("document.querySelector('.adc-button').shadowRoot.querySelector('#button').click();");

            /*
            $button = $selenium->waitForElement(WebDriverBy::xpath('//button[@id = "button_login" and not(@disabled)] | //*[@class="adc-button" and not(@disabled)]'), 3);
            $this->savePageToLogs($selenium);
