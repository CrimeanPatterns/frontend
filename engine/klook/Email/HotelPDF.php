...
class HotelPDF extends \TAccountChecker
{
    ...
    public function ParseHotelPDF(Email $email, $text): void
    {
        ...
        */
        ...
        $confText = preg_replace("/\n *This booking was made through .+\s*$/", '', $confText);
        ...
        } elseif (preg_match("/^\s*({$this->opt($this->t('Booking reference ID'))})[\s:]+([-A-Z\d\s]{4,40}?)\s*$/", $confTextTransform, $m)) {
            $otaConfirmation = preg_replace('/\s+/', '', $m[2]);
        ...
        if (preg_match("/(?:^\s*|\n[ ]*)({$this->opt($this->t('Supplier reference no.'))})[\s:]+([-A-Z\d][-A-Z\d\s]{4,40}?)\n+[ ]*{$this->opt($this->t('Hotel confirmation no.'))}/", $confTextTransform, $m)) {
            $otaConfirmation = preg_replace('/\s+/', '', $m[2]);
        ...
        if (preg_match("/(?:^\s*|\n[ ]*)({$this->opt($this->t('Hotel confirmation no.'))})[\s:]+([-A-Z\d\s]{4,40}?)\s*$/", $confTextTransform, $m)) {
            ...
            $h->general()->noConfirmation();
                ...
                || preg_match("/^\s*({$this->opt($this->t('Booking reference ID'))})[\s:]+[-A-Z\d\s]{0,40}\s*\n\s*{$this->opt($this->t('Supplier reference no.'))}[\s:]+[-A-Z\d\s]{0,40}\s*$/", $confTextTransform, $m)
            ) && count($this->otaConfNumbers) > 0) {
            $h->general()->noConfirmation();
        }
