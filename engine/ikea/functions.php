...
class TAccountCheckerIkea extends TAccountCheckerExtended
{
    ...
    private function processQuestion()
    {
        ...
        $this->http->PostURL('https://de.accounts.ikea.com/cim/de/en/v2/passwordless/start', json_encode($data), $headers);
        ...
        if (!isset($response->verification_ticket)) {
            return $this->checkErrors();
        }

        $this->State['verification_ticket'] = $response->verification_ticket;
        $this->AskQuestion('We just sent a one-time login code to ' . $this->AccountFields['Login'] . ', please check your inbox.', null, 'Question');
        return true;
    }
    ...
    public function ProcessStep($step)
    {
        ...
        $data = [
            ...
            'client_id' => 'po7A70WYX6DeSRiKRKBJocsCxZHLyUUq',
            'consumer' => 'OWF',
            'verification_ticket' => $this->State['verification_ticket'],
            'flow' => 'PWDLESS_LOGIN_EMAIL',
        ];
        ...
        $this->http->PostURL('https://de.accounts.ikea.com/cim/de/en/v2/passwordless/verify', json_encode($data), $headers);
        ...
        foreach($response->violations ?? [] as $violation) {
            if (strstr($violation->message, "Wrong username or verification code")) {
                $this->AskQuestion($this->Question, $violation->message, 'Question');

                return false;
            }

            if (strstr($violation->message, "Too many attempts")) {
                throw new CheckException($violation->message, ACCOUNT_PREVENT_LOCKOUT);
            }

            $this->logger->error("[Error]: {$violation->message}");
            $this->DebugInfo = $violation->message;
            return false;
        }

        $_csrf = $this->http->getCookieByName('_csrf', null, '/usernamepassword/login');
        $this->http->GetURL("https://de.accounts.ikea.com/passwordless/verify_redirect?client_id=po7A70WYX6DeSRiKRKBJocsCxZHLyUUq&response_type=code&redirect_uri=https://www.ikea.com/de/en/profile/login/&audience=https://retail.api.ikea.com&_csrf={$_csrf}&state=hKFo2SBRZmYxa0k4RUlTTXRfd2lpMTRUOXpGNWxVMGRYcmJvVaFupWxvZ2luo3RpZNkgZDYyT2ZlQzBBTDNxbXVqYkVpTFJvMHQ3a1Z0aU1mZzWjY2lk2SBwbzdBNzBXWVg2RGVTUmlLUktCSm9jc0N4WkhMeVVVcQ&protocol=oauth2&connection=email&verification_code=629447&email=veresch80@yahoo.com&remember=false&nonce=&scope=openid profile email offline_access");
    ...
    public function LoadLoginForm()
    {
        ...
        switch ($this->AccountFields['Login2']) {
            ...
            case 'Germany':
                $this->http->GetURL('https://de.accounts.ikea.com/login?state=hKFo2SBRZmYxa0k4RUlTTXRfd2lpMTRUOXpGNWxVMGRYcmJvVaFupWxvZ2luo3RpZNkgZDYyT2ZlQzBBTDNxbXVqYkVpTFJvMHQ3a1Z0aU1mZzWjY2lk2SBwbzdBNzBXWVg2RGVTUmlLUktCSm9jc0N4WkhMeVVVcQ&client=po7A70WYX6DeSRiKRKBJocsCxZHLyUUq&protocol=oauth2&redirect_uri=https://www.ikea.com/de/en/profile/login/&response_type=code&ui_locales=en-DE&code_challenge=i06LU0eDsp7ERmPpjVkkgoz2t_-HJO0Jm7VLLl8rlTM&code_challenge_method=S256&scope=openid profile email offline_access&audience=https://retail.api.ikea.com&registration={}&consumer=OWF&auth0Client=eyJuYW1lIjoiYXV0aDAuanMiLCJ2ZXJzaW9uIjoiOS4yMS4wIn0=');
                if (!$this->http->FindSingleNode('//title[contains(text(), "IKEA Deutschland")]')) {
                    return $this->checkErrors();
                }
