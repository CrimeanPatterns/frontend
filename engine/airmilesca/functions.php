...
class TAccountCheckerAirmilesca extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        $login = $this->waitForElement(WebDriverBy::xpath('//input[@id="login-page-user-id-field" or @id = "email"]'), self::WAIT_TIMEOUT);
        $this->saveResponse();
        ...
        if ($cookieButton = $this->waitForElement(WebDriverBy::xpath('//button[@id="onetrust-accept-btn-handler"]'), self::WAIT_TIMEOUT)) {
            $cookieButton->click();
        }

        $continue = $this->waitForElement(WebDriverBy::xpath('//button[@id="login-submit-btn"] | //a[@id = "login-continue"]'), 4);
        ...
        $continue->click();
        */
        $this->driver->executeScript("document.querySelector('#login-submit-btn, #login-continue').click();");
        $password = $this->waitForElement(WebDriverBy::xpath('//input[@id="login-page-password-field" or @id = "password"]'), self::WAIT_TIMEOUT);
        ...
        $continue = $this->waitForElement(WebDriverBy::xpath('//button[@id = "login-submit-btn" or @id = "login-form-submit"]'), 0);
        $this->saveResponse();
        ...
        $continue->click();
        */
        $this->driver->executeScript("document.querySelector('#login-submit-btn, #login-form-submit').click();");
