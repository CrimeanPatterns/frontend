...
{
    public $mailFiles = "thetrainline/it-105369465.eml, thetrainline/it-111794651.eml, thetrainline/it-11188052.eml, thetrainline/it-144513061.eml, thetrainline/it-166374823.eml, thetrainline/it-45833713.eml, thetrainline/it-51917284.eml, thetrainline/it-52799251.eml, thetrainline/it-53460958.eml, thetrainline/it-53745158.eml, thetrainline/it-652963572.eml, thetrainline/it-73776615.eml, thetrainline/it-79005389.eml, thetrainline/it-82325985.eml, thetrainline/it-88963823.eml, thetrainline/it-919403730-nsinter.eml"; // +2 bcdtravel(html)[en]
    public $lang = '';
    public static $dictionary = [
        'en' => [ // it-73776615.eml, it-52799251.eml, it-11188052.eml
            'reSubject'                        => ['Your Booking Confirmation', 'Ticket collection reference'],
            'route'                            => ['Outward journey', 'Return journey', 'Outbound', 'Return', 'OUT'],
            ...
            // 'Transaction ID' => '',
            'Ticket PNR reference'             => ['Ticket PNR reference', 'Ticket collection reference', 'Booking reference', 'Order ID:', 'Booking no:', 'Booking code', 'Bookingcode'],
            // 'cabin' => '',
        ],
        'fr' => [ // it-79005389.eml
    ...
    private $subjects = [
        ...
        'en' => [
            'Confirmation, name:', 'Your booking confirmation for',
    ...
    public function ParsePlanEmailExternal(\PlancakeEmailParser $parser, Email $email)
    {
        ...
        if (empty($paxText)) {
            $paxText = $this->http->FindNodes("//text()[{$this->eq(['en' => 'Passengers and seats', 'nl' => 'Passagiers en zitplaatsen'])}]/ancestor::tr[1]/following-sibling::tr/descendant::text()[normalize-space()][1]");
        }
    ...
    private function parseSegments(Train $t, Bus $b)
    {
        ...
        if ($xpath->length === 0) {
            $xpath = $this->http->XPath->query("(//span[{$this->starts(['en' => 'Track', 'nl' => 'Spoor'])}]/preceding::table[1])[position() mod 2 = 1 ]/ancestor::tr[2]");
        }
        ...
        foreach ($xpath as $root) {
            ...
            Venezia Mestre
            */
            $text = preg_replace("/({$this->opt(['en' => 'Track', 'nl' => 'Spoor'])}\s*(?:[A-z\d]+|(?i)unknown(?-i))\s*{$this->opt(['en' => 'Transfer', 'nl' => 'Overstap'])}.*)/", "\n$1", $text);

            if (preg_match("/(?<timeDep>{$this->patterns['time']}|—)\s+(?<airportDep>.+?)\n+{$this->opt(['en' => 'Track', 'nl' => 'Spoor'])}.+\n+(?<number>[\s\S]*\n+)?(?<timeArr>{$this->patterns['time']}|—)\s*(?<airportArr>.*\S)/", $text, $matches)
                ...
                $seatText = $this->http->FindSingleNode("./following::text()[{$this->eq($this->t('Passengers and seats'))}][1]/following::text()[{$this->starts($depNameStrong)}][1]/ancestor::tr[1]/following::tr[normalize-space()][1]", $root);

                if (empty($seatText)) {
                    $seatText = $this->http->FindSingleNode("./following::text()[{$this->eq($this->t('Passengers and seats'))}][1]/following::text()[{$this->starts($depNameOrinigal)}][1]/ancestor::tr[1]/following::tr[normalize-space()][1]", $root);
                }
                $seats = [];
                ...
                $matches['airportArr'] = preg_replace("/\s*{$this->opt(['en' => 'Track', 'nl' => 'Spoor'])}.+$/i", '', $matches['airportArr']);
