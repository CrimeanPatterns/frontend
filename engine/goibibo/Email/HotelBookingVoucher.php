...
class HotelBookingVoucher extends \TAccountChecker
{
    ...
    private function parseEmail(Email $email): void
    {
        ...
        $pnr = $this->http->FindSingleNode("//text()[{$this->eq($this->t('PNR :'))}]/following::text()[normalize-space(.)!=''][1]",
            null, false, "#^\s*([A-Z\d]+)\s*$#");
        ...
        if (empty($rooms)) {
            $rooms = $this->http->FindSingleNode("descendant::tr[ (count(*[normalize-space()])=2 or count(*[normalize-space()])=3) and *[normalize-space()][last()]/descendant::text()[normalize-space()][1]]/descendant::text()[starts-with(translate(normalize-space(), '0123456789', 'dddddddddd'), 'd') and contains(normalize-space(), 'Room')]", null, true, "/^\s*(\d{1,3})\s*{$this->opt($this->t('Room'))}/i");
        }
        ...
        $this->logger->debug($xpathRoom . "/*[normalize-space()][last()-1]/ancestor-or-self::td[ preceding-sibling::td[normalize-space()] ][1]/preceding-sibling::td[normalize-space()][1]");

        // Rooms
        $roomType = $this->http->FindNodes($xpathRoom . "/*[normalize-space()][last()-1]/descendant::text()[normalize-space()][1]");

        if (empty($roomType)) {
            $roomType = $this->http->FindNodes("//text()[starts-with(translate(normalize-space(), '0123456789', 'dddddddddd'), 'd') and contains(normalize-space(), 'Room')]/ancestor::td[1]/following-sibling::td[1]/descendant::text()[contains(normalize-space(), 'Adults')]/preceding::text()[normalize-space()][2]");
        }

        $roomDesc = $this->http->FindNodes($xpathRoom . "/*[normalize-space()][last()-1]/descendant::text()[normalize-space()][2]", null, "/.+?\s*\+\s*\d+\s*$/");

        if (empty($roomDesc)) {
            $roomDesc = $this->http->FindNodes("//text()[starts-with(translate(normalize-space(), '0123456789', 'dddddddddd'), 'd') and contains(normalize-space(), 'Room')]/ancestor::td[1]/following-sibling::td[1]/descendant::text()[contains(normalize-space(), 'Adults')]/preceding::text()[normalize-space()][1]", null, "/.+?\s*\+\s*\d+\s*$/");
        }
        ...
        $pax = $this->http->FindSingleNode("//a[{$this->contains($this->t('Edit Guest Name'))}]/ancestor::td[ preceding-sibling::td[normalize-space()] ][1]/preceding-sibling::td[normalize-space()][1]/descendant::text()[normalize-space()][not(contains(normalize-space(), 'Room'))][1]");
        $pax = preg_replace("/(\s[+]\d+.*)/", "", $pax);

        if (!empty($pax) && in_array($pax, $h->getTravellers()[0])) {
            $h->program()
