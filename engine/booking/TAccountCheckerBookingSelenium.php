...
//            $this->http->GetURL('https://secure.booking.com/myreservations.en-us.html');
    ...
    public function processSecurityCheckpoint()
    {
        ...
        $q = $this->waitForElement(WebDriverBy::xpath("
            ...
            | //*[self::p or self::span][contains(text(), 'We sent a verification code to ')]
...
//        $this->logger->debug(var_export($walletData, true), ['pre' => true]);
    ...
    public function parseItineraryFlight($item, $headers)
    {
        ...
        $f->price()->currency($data->totalPrice->total->currencyCode);
        if ($data->totalPrice->baseFare->units > 0) {
...
//        }
    ...
    private function parseItineraryHotel($data)
    {
        ...
        if (mb_strlen($data->propertyData->name) > 250) {
            $name = preg_split('/\s+(?:-|by|are all)\s+/', $data->propertyData->name)[0];
        } else {
...
//            ->confirmation($data->id);
        ...
        try {
            ...
            $this->browser->PostURL("https://taxis.booking.com/bookings/{$data->bookingRef}", json_encode(['emailAddress' => $data->bookerEmail]), [
                'Accept' => '*/*',
            ...
            $this->browser->GetURL("https://taxis.booking.com/bookings/{$data->bookingRef}?lang=en-gb");
        ...
        // Car Info
        $s->extra()->type($this->browser->FindSingleNode('//*[@data-test="your-vehicle__vehicleType"]'));
        $duration = $this->browser->FindSingleNode('//*[@data-test="pick-up-location__journey-time"]', null, false, '/About (.+)/');
        $s->extra()->duration($duration);
        // Departure
        $s->departure()->address($this->browser->FindSingleNode('//*[@data-test="pick-up-location__title"]'));
        $s->departure()->date2($this->browser->FindSingleNode('//*[@data-test="pick-up-location__date"]'));

        // Arrival
        $s->arrival()->address($this->browser->FindSingleNode('//*[@data-test="drop-off-location__title"]'));
        $s->arrival()->date(strtotime($duration, strtotime($this->browser->FindSingleNode('//*[@data-test="drop-off-location__date"]'))));
