...
//        $this->logger->debug('$text = '.print_r( $text,true));
        ...
        if (preg_match('/^(?:(?:.*\n+){0,3}.*\.[bB]ooking\.com.*\n+|)(?:\s*MTA\s*-\s*Mobile[ ]+Travel[ ]+Agents\s*\n)?(?:\d\/\d\n+)?((?:.+\n+){1,4})\s+' . $this->opt($this->t('Address:')) . '\s*([\s\S]{1,500}?)(?:\n\s*' . $this->t("Star rating:") . '[\s\S]*)?' . $this->t('Phone:') . '\s*(.+)/', $text, $m)) {
            ...
            if (preg_match("/^\s*[\(\)\+]*\d[\d\(\)\-\+ \.]{5,}(?:$|\n)/", $m[3])) {
                $h->hotel()->phone(trim($m[3]));
            } elseif (preg_match("/{$this->preg_implode($this->t('Phone:'))}\s*\n+\:?\n*\s*([\(\)\+]*\d[\d\(\)\-\+ \.]{5,})(?:$|\n)/", $text, $match)) {
                $h->hotel()->phone($match[1]);
            }
        }

        $this->logger->error('/' . $this->preg_implode($this->t('CHECK-IN')) . $patterns['date'][$this->lang] . '/uis');
...
//            || preg_match("#^até\s(?<date>\d{1,2}\sde\s[A-z]+\sde\s\d{4}\s\d{1,2}:\d{1,2})#ui", $cancellationText, $m)
            ...
            || preg_match("#^jusqu'au\s+(?<date>\d{1,2}\s[[:alpha:]]+\s\d{4}\s\d{1,2}\s*:\s*\d{1,2}) \[[A-Z]{3,4}\]\s*:\s*(?:[^\d\s]{1,5}[ ]?0|(?:\D )?0[ ]?[^\d\s]{1,5}|\D{1,3}\s*\D\s*0)[ ]+#ui", $cancellationText, $m)
        ) {
            $h->booked()
                ->deadline($this->normalizeDate(str_replace(' : ', ':', $m['date'])));
