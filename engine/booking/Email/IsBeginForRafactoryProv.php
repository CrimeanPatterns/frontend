...
class IsBeginForRafactoryProv extends \TAccountChecker
{
    ...
    public static $dictionary = [
        ...
        "es" => [
            ...
            "Check-in"         => ["Entrada", "Llegada", 'Check-in'],
...
//                null, false, "#^(.+\[.+)$#"))
    ...
    private function normalizeDate($strDate, $lastTime = false)
    {
        ...
        $in = [
            ...
            28 Mart 2025 Cuma 15:00
            */
            "#^\s*(\d+)[.]?\s+([^\d\s]+)\s+(\d{4})[,\s]\s*[^\d\s]*(?:\s+|\s*\(\D*)(\d{1,2}:\d{2}(?:\s*[AP][M])?)\D*$#ui",
            ...
            "#^\s*(?:\D+\s+|)(\d{1,2})[.]?\s+(?:de\s+|d\')?([^\d\s\.\,،]+)[.،]?\s+(?:de\s+|ค\.ศ\.\s+)?(\d{4})[.,]?(?:\s*[гр]\.)?\s*(?:\s+|\s*\(\D*)(\d{1,2})[.:](\d{2}(?:\s*[AP][M])?)\D*$#ui",
