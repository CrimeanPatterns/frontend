...
class TAccountCheckerPestana extends TAccountChecker
{
    ...
    private const REWARDS_PAGE_URL = 'https://secure.pestana.com/en/myaccount/points';
    ...
    public function LoadLoginForm()
    {
        ...
        $this->http->GetURL('https://www.pestana.com/en/my-account');
        $login = $this->waitForElement(WebDriverBy::xpath('//input[@name = "loginfmt"]'), 5);
        $nextBtn = $this->waitForElement(WebDriverBy::xpath('//input[@value = "Next"]'), 0);
        $this->saveResponse();
        ...
        $this->processQuestuion();

        return false;
    }

    private function processQuestuion()
    {
        $this->logger->notice(__METHOD__);

        $question = $this->waitForElement(WebDriverBy::xpath('//span[@id = "otcDesc"]'), 5);
        $otc = $this->waitForElement(WebDriverBy::xpath('//input[@name = "npotc"]'), 0);
        $signInBtn = $this->waitForElement(WebDriverBy::xpath('//input[@value = "Sign in"]'), 0);
        $this->saveResponse();

        if (!$question || !$otc || !$signInBtn) {
            return false;
        }
    ...
    private function loginSuccessful()
    {
        ...
        if (
            $this->http->FindSingleNode("//a[contains(@href, 'logoffsso')]/@href")
            ?? $this->http->FindSingleNode("//div[contains(text(), 'Current balance')]")
        ) {
            return true;
        }
