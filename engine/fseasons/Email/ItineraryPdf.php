...
{
    public $mailFiles = "fseasons/it-155008104.eml, fseasons/it-398080953.eml, fseasons/it-41974722.eml, fseasons/it-42118999.eml, fseasons/it-631494812.eml, fseasons/it-652068587-dorchester.eml, fseasons/it-65389712.eml, fseasons/it-79528547.eml, fseasons/it-885698621.eml, fseasons/it-892934491.eml, fseasons/it-904837336.eml, fseasons/it-922634334.eml";
    ...
    private static $detectProviders = [
        'fseasons'   => ['www.fourseasons.com', 'Four Seasons Hotel', '@fourseasons.com'],
    ...
    private function parseEmailPdf($textPDF, Email $email): void
    {
        ...
        $segmentsStarts = [
            "[ ]*{$this->t('Offering:')}(?:.*\n){2}",
            "[ ]*{$this->t('Date:')}[^\n]+\n[ ]*(?:{$this->t('Time:')}|{$this->t('Request Type:')})(?:.*\n){2}",
            "[ ]*{$this->opt($this->t('Date and Time'))}\s+(?:.*\n){2}",
        ];
        $segments = $this->splitter("#\n(" . implode("|", $segmentsStarts) . ")#", $textPDF);
        ...
        foreach ($segments as $key => $segment) {
            ...
            switch (true) {
                case $type === 'As Directed Transportation':
                        break;
                ...
                case stripos($type, 'Porter Service') !== false:
                case $type === 'Tennis':
                case $type === 'Activity':
    ...
    private function parseTransfer($textPDF, Email $email): bool
    {
        ...
        $addressDep = $this->normalizeAddr($depAddr, $arrAddr);
        $addressArr = $this->normalizeAddr($arrAddr, $depAddr);

        if (preg_match('/^\s*[A-Z]{3}\s*$/', $addressDep)) {
            $s->departure()->code($addressDep);
        } else {
        ...
        if (preg_match('/^\s*[A-Z]{3}\s*$/', $addressArr)) {
            $s->arrival()->code($addressArr);
        } else {
    ...
    private function normalizeAddr(?string $target, ?string $source): ?string
    {
        ...
        if (preg_match("/^Four Seasons(?: Hotel)?$/i", $target) > 0
            ...
            && mb_strlen($m[1]) > mb_strlen($target)
        ) {
            $target = $m[1];
        }

        $placesByCountries = [
            'Italy' => ['Four Seasons Hotel Milano'],
            'United States' => [
                'Four Seasons Hotel Denver', 'Four Seasons Resort Hualalai', "Four Seasons Resort Lana'i",
                'Four Seasons Resort Orlando',
            ],
            'Switzerland' => ['Four Seasons Hotel des Bergues'],
            'Russian Federation' => ['Four Seasons Hotel Moscow'],
        ];

        foreach ($placesByCountries as $country => $places) {
            foreach ((array) $places as $placeName) {
                if (!preg_match("/\b{$this->opt($country)}\b/i", $target)
                    && preg_match("/^{$this->opt($placeName)}$/i", $target)
                    && preg_match("/\b{$this->opt($country)}\b/i", $this->pdfFull)
                ) {
                    $target .= ', ' . $country;
    ...
    private function parseEvent($textPDF, Email $email): bool
    {
        ...
        $address = $this->re("/{$this->opt($this->t('Vendor Address:'))}[ ]+(.{3,})/", $textPDF)
            ...
            ?? (preg_match("/Porter Service.*\s+[-]+\s+(?-i)(?<al>[A-Z][A-Z\d]|[A-Z\d][A-Z])\s*(?<fn>\d+)\s*(?:[,\n]|$)/i", $eventName, $m)
                && preg_match("/\n[ ]*{$this->opt($this->t('Pick-up Location:'))}[: ]*([^:\s].+)\n+(?:.+\n+){1,3}?.+[: ]{$m['al']} ?{$m['fn']}\n/", $this->pdfFull, $m2)
                ? $m2[1] : null // it-922634334.eml
            )
            ?? $eventName
        ;
    ...
    private function parseNotes(string $text): ?string
    {
        $notesText = $this->re("/^([ ]*{$this->opt($this->t('Guest Notes:'))}[: ]*[^:\s].*)/ms", $text) ?? '';
        $leftPadding = preg_match("/^([ ]*{$this->opt($this->t('Guest Notes:'))}[: ]*)[^:\s]/", $notesText, $m) ? (mb_strlen($m[1]) - 1) : 9999;

        if (preg_match("/^[ ]*{$this->opt($this->t('Guest Notes:'))}[: ]*([^:\s].*(?:\n+[ ]{" . $leftPadding . "}[ ]*\S.+)*)/", $notesText, $m)) {
