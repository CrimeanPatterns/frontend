...
class FolioPdf extends \TAccountChecker
{
    ...
    public static $dictionary = [
        ...
        'en' => [ // always last!
            ...
            'confNumber'  => ['Conf. No.', 'Conf No', 'Confirm No.', 'Confirmation No.', 'Conf. Number', 'Confirmation #'],
    ...
    private function parsePdf(Email $email, string $text, string $subject, ?string $from): void
    {
        if (stripos($text, 'Rodrigo da Fonseca') !== false) {
            ...
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Resort O'ahu at Ko Olina/u", $subject, $m)
            || stripos($text, '92-1001 Olani St, Kapolei, HI 96707') !== false
        ) {
            $this->hotelNameID = 'Resort O\'ahu at Ko Olina';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Safari Lodge, Serengeti/u", $subject, $m)
            || stripos($text, 'P.O Box 14321, Arusha, Tanzania') !== false
        ) {
            $this->hotelNameID = 'Safari Lodge, Serengeti';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Hotel Hong Kong -/u", $subject, $m)
            || stripos($text, '8 Finance Street, Central, Hong Kong') !== false
        ) {
            $this->hotelNameID = 'Hotel Hong Kong';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Hotel Mumbai - /u", $subject, $m)
            || stripos($text, 'PLOT NO-1/136, DR E MOSES ROAD') !== false
        ) {
            $this->hotelNameID = 'Hotel Mumbai';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Hotel Tunis Guest Folio/u", $subject, $m)
            || stripos($text, 'Zone Touristique Cap Gammarth') !== false
        ) {
            $this->hotelNameID = 'Hotel Tunis';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Hotel Riyadh - Guest Folio/u", $subject, $m)
            || stripos($text, 'P.O BOX 231000, RIYADH ') !== false
        ) {
            $this->hotelNameID = 'Hotel Riyadh';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Hotel Hampshire - Guest Folio/u", $subject, $m)
            || stripos($text, 'HAMPSHIRE RG27 8TD') !== false
        ) {
            $this->hotelNameID = 'Hotel Hampshire';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Hotel Istanbul at Sultanahmet - /u", $subject, $m)
        ) {
            $this->hotelNameID = 'Hotel Istanbul at Sultanahmet';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Resort The Nam Hai -/u", $subject, $m)
            || stripos($text, 'Block Ha My Dong B, Dien Duong Ward') !== false
        ) {
            $this->hotelNameID = 'Resort The Nam Hai';
            $this->hotelMiami($email, $text);
        } elseif (preg_match("/Four Seasons Resort Sharm El Sheikh -/u", $subject, $m)
        ) {
            $this->hotelNameID = 'Resort Sharm El Sheikh';
            $this->hotelMiami($email, $text);
        }
    ...
    private function hotelMiami(Email $email, string $text): void
    {
        ...
        $checkOutVal = $this->re("/^[ ]*{$this->opt($this->t('checkOut'))}[:\s]+(\d{1,2}\/\d{1,2}\/\d{2,4})[ ]+\d.+\d[ ]*{$this->opt('‫ﺗﺎﺭﻳﺦ ﺍﻟﻤﻎ‬')}$/mu", $table[1])
            ...
            ?? $this->re("/(?:^[ ]*| {3,})(?:[- .[:alpha:]]{1,15} ?\/ ?)?{$this->opt($this->t('checkOut'))} *: *(.+)/mu", $table[1])
        ;
        $checkOut = strtotime($this->normalizeDate($checkOutVal));
        $h->booked()->checkIn($checkIn)->checkOut($checkOut);
        ...
        if (preg_match("/^\s*(?<amount>\d[,.‘\'\d ]*?)\s*$/u", $totalPrice, $matches)) {
            ...
            if (empty($currencyCode)) {
                $currencyTemp = $this->re("/^[ ]*{$this->opt($this->t('Balance'))} +(\D{1,5}) *\d[,.‘\'\d ]*?\s*$/mu", $paymentText)
                    ?? $this->re("/^[ ]*{$this->opt($this->t('Balance'))} +.+[ ]{2,}\d[,.‘\'\d]*[ ]*([A-Z]{3})$/mu", $paymentText)
                ;
                $currencyTemp = preg_replace("/^\s*£\s*$/", 'GBP', $currencyTemp);

                if (preg_match('/^[A-Z]{3}$/', $currencyTemp)) {
                    $currencyCode = $currencyTemp;
                }
            }
            $h->price()->currency($currencyCode, false, true)->total(PriceHelper::parse($matches['amount'], $currencyCode));
        ...
        if (($this->hotelNameID === 'Resort Whistler' || $this->hotelNameID === 'Hotel Cairo at Nile Plaza'
            ...
            || preg_match("/\n *(?<address>\d+.+\bU\.?S\.?A\.?)\s+(?<phones>Tel\: *.+\s+Fax\: *.+\d)/", $text, $matches)
            || preg_match("/\n *(?<address>P\.O Box .+)\s+(?<phones>Tel\. *.+[\s*\|]+Fax\. *.+\d)/", $text, $matches)
            || preg_match("/\n\n\n *(?<address>.+)\s+(?<phones>Tel: *.+\s+Fax: *.+\d)/", $text, $matches)
            || preg_match("/\n\n *(?<address>.+)\s+(?<phones>Tel: *.+?) +(?:www\.)?fourseasons\.com(?:\/\S+)?(?:\n|$)/i", $text, $matches)
        ) {
            if (empty($h->getHotelName()) && $hotelName) {
                ...
                $h->hotel()->chain('Four Seasons')->name($this->hotelNameID);
            } elseif (!empty($this->hotelNameID)) {
                $h->hotel()->chain('Four Seasons')->name($this->hotelNameID);
            }

            $matches['address'] = preg_replace("/^\s*Four Seasons .+? at (?<address>[^:\n]{3,})$/iu", '$1', $matches['address']);
            $h->hotel()->address(preg_replace('/\s+/', ' ', $matches['address']));
