...
class TAccountCheckerNorwegian extends TAccountChecker
{
    ...
    public function LoadLoginForm()
    {
        ...
        if ($this->selenium) {
            ...
            $loginFormBtnXpath = '//button[contains(@id, "nas-button-" ) and contains(., "Sign In")]';
            $loginFormBtn = $this->waitForElement(WebDriverBy::xpath($loginFormBtnXpath), 7);
            ...
            if (!$loginFormBtn) {
                ...
                $loginFormBtn = $this->waitForElement(WebDriverBy::xpath($loginFormBtnXpath), 0);
            ...
            if (!$login && ($loginFormBtn = $this->waitForElement(WebDriverBy::xpath($loginFormBtnXpath), 0))) {
                $loginFormBtn->click();
                $login = $this->waitForElement(WebDriverBy::xpath('//input[@id = "nas-element-login-box-0-username"] | //form[@id = "gigya-login-form"]//input[@name = "username"]'), 7);
                $this->savePageToLogs($this);
...
//                    $this->logger->info(htmlspecialchars(var_export($this->history, true)), ['pre' => true]);
    ...
    private function getExpDateFromSecondSite()
    {
        ...
        | //button[@onclick = "$(\'#acceptAllForm\').submit()"]'), 10);

        $acceptAllForm = $this->waitForElement(WebDriverBy::xpath('//button[@onclick = "$(\'#acceptAllForm\').submit()"]'),
            0);
        ...
        $this->waitForElement(WebDriverBy::xpath('//input[@id = "Username"] | //span[@id="my-account-link"]'), 7);

        $result = $this->waitForElement(WebDriverBy::xpath('//span[@id="my-account-link"]'), 0);
        if (!$result) {
        ...
        $result = $this->waitForElement(WebDriverBy::xpath('//a[@id = "log-out-user"] | //span[@id="my-account-link"]'), 7);
