...
*/
        ...
        if (empty($address)) {
            if (!empty($r->getCheckInDate()) && !empty($r->getCheckOutDate())) {
                $hotel = $this->getHotelAddress($r->getHotelName());
    ...
    private function getHotelAddress($propertyId = null): array
    {
        ...
        if (!empty($propertyUrl)) {
            ...
            if (stripos($propertyUrl, 'vrbo.com') !== false || stripos($propertyUrl, 'homeaway.com') !== false
                ...
                if (empty($http1->Response['body']) && stripos($propertyUrl, 'vrbo.com') !== false && !empty($propertyId)) {
