...
class TravelReservationPdf extends \TAccountChecker
{
    ...
    public static $dict = [
        ...
        'en' => [
            ...
            // Hotel
            'hotel after table' => ['TAX AND\/OR SURCHARGE', 'RATES AND EFFECTIVE DATES', 'Additional Details:', '\n\n\n', '$'],
            //            'Confirmation:' => '',
...
//            $this->logger->debug('Air Taxi Segment: ' . "\n" . print_r( $segments, true));
        ...
        foreach ($cars as $ctext) {
            ...
            if (strpos($ctext, $this->t('DROP OFF')) === false) {
                ...
                /*DROP OFF: THURSDAY 17
                PICK UP: SATURDAY 12 APR                    6 DAY(S)
                                                            APR*/
                if (preg_match("/^\s*{$this->t('DROP OFF')}\:?\s*\w+\s+\d+$/u", $dropOffRow)) {
                    $month = $this->re("/^.*{$this->t('DROP OFF')}.*\n{$this->opt($pickUpRow)}\n\s+([A-Z]{3})\s*\n/mu", $text);

                    if (!empty($month)) {
                        $dropOffRow = str_replace("\n", "", $dropOffRow . ' ' . $month) . "\n";
                        $ctext = preg_replace("/^\s+{$month}\s*\n/mu", "", $ctext);
                    }
                }
            ...
            if (empty($date)) {
                $date = $this->normalizeDate($this->re("/{$this->opt($this->t("DROP OFF"))}[ ]*:\s+(.+)/", $ctext));
            }
