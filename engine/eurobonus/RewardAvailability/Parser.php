...
//        }
    ...
    public function getRewardAvailabilitySettings()
    {
        ...
        return [
            ...
            'defaultCurrency'          => 'USD',
            'priceCalendarCabins'      => ["premiumEconomy", "business", "economy", "unknown"],
        ];
    }
    ...
    public function ParseCalendar(array $fields)
    {
        ...
        $calendar = $this->getCalendarData($fields);

        if (empty($calendar['outbound'])) {
            $this->SetWarning('There are no flights for the this month');

            return ['fares' => []];
        }

        return ['fares' => $this->parseCalendarJson($calendar, $fields)];
    }
...
//        }
    ...
    protected function parseCaptcha($selenium)
    {
        $this->logger->notice(__METHOD__);

        if (!$sitekey = $selenium->waitForElement(\WebDriverBy::xpath("//div[@class='h-captcha']"), 5)) {
            $this->logger->debug('captcha not load');
        ...
        $parameters = [
            "method"  => "hcaptcha",
            "pageurl" => $selenium->http->currentUrl(),
    ...
    private function getCabinFields(): array
    {
        $cabins = [
            ...
            'premiumEconomy' => ['awardType' => ['PLUS', 'PREMIUM']],
            'firstClass'     => [],
    ...
    private function getProductName($cabinID): ?string
    {
        $cabin = null;
    ...
    private function selenium($fields)
    {
        ...
        $cookiesForCache = \Cache::getInstance()->get('ra_eurobonus_coockies');

        if (!$cookiesForCache || !is_array($cookiesForCache)) {
            $cookiesForCache = [];
            ...
            try {
                ...
                try {
                    ...
                    $selenium->http->GetURL("https://www.flysas.com/us-en/book/flights/?search={$search}&view=upsell&bookingFlow=points&sortBy=stop");

                    if ($selenium->http->FindSingleNode("//h1[contains(.,'403 - Forbidden')]
                    | //h2[contains(text(),'Forbidden')]")) {
                        $this->logger->error('403 - Forbidden');
                ...
                $selenium->waitForElement(\WebDriverBy::xpath("//div[@class='h-captcha']"), 10);
                $selenium->saveResponse();
                $this->setCaptcha($selenium);
                ...
                foreach ($cookies as $cookie) {
                    ...
                    if (in_array($cookie['name'], ['ASLBSACORS', 'reese84', 'ASLBSA'])) {
                        $cookiesForCache[] = $cookie;
                    }
                }
            } catch (\ScriptTimeoutException | \TimeOutException $e) {
            ...
            if (!empty($cookiesForCache)) {
                \Cache::getInstance()->set('ra_eurobonus_coockies', $cookiesForCache, 5 * 60);
            }
        }

        foreach ($cookiesForCache as $cookie) {
    ...
    {
        $cookie = $selenium->waitForElement(\WebDriverBy::xpath('//button[@id="deny-consent-button"]'), 5);
    ...
    private function getCalendarData($fields)
    {
        $dateStr = date("Ym", $fields['DepDate']);
        $headers = [
            'Accept'        => 'application/json, text/plain, */*',
            'Authorization' => $this->access_token,
            'Content-Type'  => 'application/json',
        ...
        $query = http_build_query([
            ...
            'market'  => 'us-en',
            'month'   => $dateStr,
            'product' => $this->getProductName($fields['Cabin']),
            'to'      => $fields['ArrCode'],
        ...
        $this->http->RetryCount = 0;
        $this->http->GetURL(
            "https://www.sas.se/v2/cms-www-api/flights/calendar/prices/?{$query}",
            $headers
        );
        $this->http->RetryCount = 2;
    ...
    private function parseCalendarJson($calendar, $fields)
    {
        ...
        foreach ($calendar['outbound'] as $date => $value) {
            $dateTime = \DateTime::createFromFormat('Ymd', $date);

            $result[] = [
                'date'        => $dateTime->format('Y-m-d'),
                'redemptions' => ['miles' => $value['totalPrice']],
                'payments'    => [
                    'currency' => 'USD',
                    'taxes'    => null,
                    'fees'     => null,
                ],
                'cabin'        => $fields['Cabin'] == 'firstClass' ? 'business' : $fields['Cabin'],
                'brandedCabin' => $this->getProductName($fields['Cabin']),
            ];
        }
    ...
    private function setCaptcha($selenium)
    {
        $this->logger->notice(__METHOD__);
        ...
        //['request']
        $selenium->waitForElement(\WebDriverBy::xpath('//button[contains(@class, "cash-money-banner")]'), 10);
