...
//            return null;
//        }
        $this->http->GetURL("https://digital.etihad.com/selfservice/en/ssg-landing-page?pnr={$arFields['ConfNo']}&lastName={$arFields['LastName']}");
        $paramKey = $this->http->FindPreg('/#encryptedParameters=(.+)/', false, $this->http->currentUrl());
        ...
        $data = [
            'encryptedParameters' => $paramKey,
            'language' => 'en',
        ];
        $this->http->RetryCount = 0;
        $this->http->PostURL("https://digital.etihad.com/ada-services/bff-pnr-retrieval/service/gateway-orchestrator/v2/order",
            json_encode($data), $headers, 20);
        $this->http->RetryCount = 2;
    ...
    public function parseItineraryFlight($item)
    {
        ...
        $f->general()->confirmation($item->recordLocator);
        $travellers = [];
        foreach ($item->segments as $segment) {
            foreach ($segment->legs as $leg) {
                $s = $f->addSegment();
                $s->airline()->name($leg->operatingAirlineCode);
                $s->airline()->number($leg->flightNumber);

                $s->departure()->name($leg->originAirportName);
                $s->departure()->code($leg->originAirportCode);
                $leg->departureTime = $this->http->FindPreg('/^(\d+:\d+)/', false, $leg->departureTime);
                $s->departure()->date2("$leg->departureDate $leg->departureTime");
                $s->departure()->terminal($leg->departureTerminal, false, true);
                $s->arrival()->name($leg->destinationAirportName);
                $s->arrival()->code($leg->destinationAirportCode);
                $leg->arrivalTime = $this->http->FindPreg('/^(\d+:\d+)/', false, $leg->arrivalTime);
                $s->arrival()->date2("$leg->arrivalDate $leg->arrivalTime");
                $s->arrival()->terminal($leg->arrivalTerminal, false, true);
                $s->extra()->aircraft($leg->airCraftDetails->model);
                $s->extra()->cabin($leg->cabin);
                $s->extra()->bookingCode($leg->bookingClass);

                // 06:05
                $duration = explode(':', $leg->flightDuration);
                if (count($duration) == 2) {
                    $hours = $duration[0];
                    $minutes = $duration[1];
                    // 06 hrs, 05 mins
                    $s->extra()->duration(sprintf('%01d hrs %02d mins', $hours, $minutes));
                }
                foreach ($leg->travelers as $traveler) {
                    $travellers[] = beautifulName("$traveler->firstName $traveler->lastName");
                }
            }
        }
