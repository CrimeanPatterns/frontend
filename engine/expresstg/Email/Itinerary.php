...
class Itinerary extends \TAccountChecker
{
    ...
    private function parseEmailPdf(Email $email, ?string $textPdf = null)
    {
        ...
        foreach ($its as $i => $part) {
            ...
            $f->general()
                ->traveller($pax = preg_replace(['/\s+(mr|ms|mrs|mstr|dr|miss)\s*$/i', "/(.+?) *\/ *(.+)/"], ['', "$2 $1"],
                    trim($this->re("/^\s*(.+)\n/", $part))));

            $f->issued()
                ->ticket($this->re("/^\s*TICKET NO +([\d\-]{10,})\s*$/m", $mailInfo), false, $pax);

            $account = $this->re("/FREQUENT FLYER NO\s*([A-Z]{2}\*\d{8,})$/", $mailInfo);
...
//                $this->logger->debug('$segment = '.print_r( $segment,true));
                ...
                $re = "/^\s*(?<name>.*?)\s*\((?<code>[A-Z]{3})\)\s+(?<date>[^\n]+)\n(?<name2>.+?(?:Airport|Schiphol))\s*(?<terminal>\n(?i).*\bterminal\b.*)?\s*$/s";

                // Departure
                if (preg_match($re, $table[0], $m)) {
                    $m['name'] = preg_replace("/(Operated by.+\,(?:\s*inc\.)?)/ui", "", $m['name']);
                    $s->departure()
