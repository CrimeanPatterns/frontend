...
class YourItinerary extends \TAccountCheckerExtended
{
    ...
    private function parseFlight(Email $email)
    {
        ...
        $confInfo = $this->http->FindSingleNode("(//text()[{$this->eq($this->t('RecordLocator'))}])[1]/ancestor::tr[1][normalize-space()]");

        if (preg_match("/^\s*(?<desc>{$this->opt($this->t('RecordLocator'))})\s*(?<number>[\w-]+)\s*$/", $confInfo, $m)) {
            $f->general()
        ...
        foreach ($nodes as $root) {
            ...
            if (strlen($route) == 7) {
                $passengerNodes = $this->http->XPath->query("//text()[{$this->eq($route)}]/ancestor::tr[1]");

                foreach ($passengerNodes as $passengerNode) {
                    $flightCodes = $this->http->FindNodes("./td[{$this->contains($route)}]/*[normalize-space()]", $passengerNode);

                    // detect flight code position (top or bottom)
                    $routePos = array_search($route, $flightCodes) + 1;

                    $traveller = $this->http->FindSingleNode("./td[{$this->contains($route)}]/preceding-sibling::td[normalize-space()][2]", $passengerNode)
                        . ' '
                        . $this->http->FindSingleNode("./td[{$this->contains($route)}]/preceding-sibling::td[normalize-space()][1]", $passengerNode);

                    $traveller = str_replace('>', '', $traveller);

                    $seat = $this->http->FindSingleNode("(./td[normalize-space()][last()]/descendant::text()[normalize-space()])[{$routePos}]", $passengerNode);
            ...
            foreach ($pNodes as $pRoot) {
                ...
                if (preg_match("/^\s*" . preg_quote($currency) . "\s*(?<discount>-\s*)?(?<amount>\d[\d\., ]*)\s*$/u",
                    ...
                    if (preg_match("/^\s*{$this->opt($this->t("TotalCharge"))}/u", $name)) {
                        ...
                        if (isset($fees[$name])) {
                            $fees[$name] += PriceHelper::parse($m['amount'], $currency);
                        } else {
                            $fees[$name] = PriceHelper::parse($m['amount'], $currency);
                        }
                    }
