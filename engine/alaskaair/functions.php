...
class TAccountCheckerAlaskaair extends TAccountChecker
{
    ...
    public static function GetAccountChecker($accountInfo)
    {
        ...
        require_once __DIR__ . "/TAccountCheckerAlaskaairSelenium.php";

        return new TAccountCheckerAlaskaairSelenium();
    }
...
//        $this->http->SetProxy($this->proxyWhite(), false);
    ...
    public function Login()
    {
        ...
        if (
            ...
            && $this->http->ParseForm(null, "//form[@data-form-primary=\"true\"]")
        ...
        if ($this->http->currentUrl() == 'https://www.alaskaair.com/account/token') {
            $this->http->GetURL("https://www.alaskaair.com/account/overview");
        }

        if ($this->http->currentUrl() == 'https://www.alaskaair.com/content/page-not-found') {
    ...
    public function Parse()
    {
        ...
        $tokenInfo = $this->http->JsonLog($this->http->FindSingleNode("//pre[not(@id)]"), 0);
        ...
        $this->headers = [
            ...
            "Ocp-Apim-Subscription-Key" => "2484ecb576844a5b991ca8ece8e7917b", // https://apis.alaskaair.com/1/guestServices/admin/headerFooter/footer.js
        ];
    ...
    public function ParseItineraries()
    {
        ...
        $res = $this->http->JsonLog($this->http->FindSingleNode("//pre[not(@id)]"));
        ...
        if (isset($res->upcoming) && is_array($res->upcoming)) {
            foreach ($res->upcoming as $upcoming) {
                ...
                if ($this->http->FindSingleNode("//h1[contains(text(), 'Confirm Your Schedule Change')]")
                    ...
                    $this->http->setOriginHeader = true;
                    $this->http->PostForm(['Origin' => 'https://www.alaskaair.com']);
                    ...
                    $this->parseItineraryHtmlV2($upcoming);
                } else {
...
//            $this->parseItineraryHtml();
    ...
    private function guestUpgrades()
    {
        ...
        $this->http->RetryCount = 2;
        $response = $this->http->JsonLog($this->http->FindSingleNode("//pre[not(@id)]"), 3, false, 'validVouchers');
    ...
    private function myWallet($tokenInfo)
    {
        ...
        if (!empty($myWalletBalance) && $this->http->Response['code'] == 200) {
            ...
            $certificates = $this->http->JsonLog($this->http->FindSingleNode("//pre[not(@id)]"));
...
//                $this->logger->debug(var_export($expirationList, true), ['pre' => true]);
    ...
    private function getHistory()
    {
        ...
        $this->http->GetURL("https://apis.alaskaair.com/1/Retain/Mileage/Plan/mpactivitybff/member/activities?Range=24&Offset=0", $headers);
        $response = $this->http->JsonLog($this->http->FindSingleNode("//pre[not(@id)]"), 1);
    ...
    function parseItineraryHtmlV2($item = null)
    {
        $this->logger->notice(__METHOD__);
        ...
        foreach ($segments as $segmentNode) {
            ...
            foreach ($details as $detail) {
                ...
                $arrDateStr = $this->http->FindSingleNode($rootSegmentXpath . '//div[contains(text(),"Arrives")]/following-sibling::div[@class="itinerary-details-time-display"]', $segmentNode);
                $this->logger->debug("Arrives: $date");
                // Thu, Nov 14 | 01:09 PM 12:50 PM
                $arrDateStr = $this->http->FindPreg('/^(\w{3},.+?\d+:\d+ [PA]M)/', false, $arrDateStr);
                $arrDateStr = preg_replace(['/\|/', '/\+\d+ days*/'], '', $arrDateStr);
                $this->logger->debug("date str: $arrDateStr");
                $arrDate = strtotime($arrDateStr, $s->getDepDate());
                $diff = abs($arrDate - $depDate);
                if ($depDate > $arrDate && $diff > DateTimeUtils::SECONDS_PER_DAY * 360 && $diff < DateTimeUtils::SECONDS_PER_DAY * 365
                    && date('m', $depDate) === '12' && date('m', $arrDate) === '01') {
                    if (empty($item->endDate)) {
                        $s->arrival()->date(strtotime('+1 year', $arrDate));
                    } else {
                        $this->logger->debug("item end date: $item->endDate");
                        $s->arrival()->date(strtotime($arrDateStr, strtotime($item->endDate)));
                    }
                } else {
                ...
                foreach ($seats as $seatNode) {
                    $s->extra()->cabin($this->http->FindSingleNode('.//div[@class="seating-cabin"]', $seatNode, false, '/(\w+) \([A-Z]+\)/'), false, true);
                    $s->extra()->bookingCode($this->http->FindSingleNode('.//div[@class="seating-cabin"]', $seatNode, false, '/\w+ \(([A-Z]+)\)/'), false, true);
    ...
    function parseItineraryHtml()
    {
        ...
        if ($error = $this->http->FindSingleNode("//table[@id='flightInformationTable']//div[contains(.,'There are no flights currently attached to this reservation')] | //div[@id = 'contact-info-error' and not(@style = 'display: none;') and contains(., 't display your reservation right now, but we')]")) {
            $this->logger->error($error);
    ...
    function parseChangedItineraryV2($date = null, $pax = [])
    {
        $this->logger->notice(__METHOD__);
    ...
    private function parseItineraryV2($date = null)
    {
        ...
        for ($i = 0; $i < $nodes->length; $i++) {
            ...
            $seg->setCabin($this->http->FindPreg("/\(([A-Z]+)\)/ims", false, $cabinInfo), false, true);
            // BookingClass
