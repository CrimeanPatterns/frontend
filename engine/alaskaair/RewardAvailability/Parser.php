...
//        $this->seleniumRequest->setHotSessionPool(self::class, $this->AccountFields['ProviderCode']);
    ...
    public function ParseCalendar(array $fields): array
    {
        ...
        $url = "https://www.alaskaair.com/planbook/shoppingstart?A={$fields['Adults']}&O={$fields['DepCode']}&D={$fields['ArrCode']}&OD={$dateFormat}&OT=Anytime&RT=false&ShoppingMethod=onlineaward&UPG=none&FRM=avail&RequestType=Calendar&SITE_PREF=full&int=flightresultsmicrosite%3Aviewby-calendar";
        ...
        if ($this->http->FindSingleNode("//span[contains(., 'This site can')]")) {
            try {
                $this->http->GetURL($url);
            } catch (\Facebook\WebDriver\Exception\UnknownErrorException $e) {
                $this->logger->error("UnknownErrorException: " . $e->getMessage(), ['HtmlEncode' => true]);
        ...
        if (!$this->waitForElement(\WebDriverBy::xpath('//div[@id="CalendarResultColumn"]'), 30)) {
            $error = $this->driver->executeScript('return window.validationSummary');
            $error = $this->http->JsonLog(json_encode($error), 1, true);
            ...
            $this->SetWarning($error['Errors'][0]['ErrorText']);

        }


        $calendar = $this->driver->executeScript('return window.calendars');
        $this->http->JsonLog(json_encode($calendar), 1, true);

        $result = $this->parseCalendarFlightsJson($calendar);
    ...
    private function decodeCabin($cabin)
    {
        switch ($cabin) {
            ...
            case 'Business':
            case 'ContainsBusinessCabin':
            case 'Partner Business':
                return 'business';

            case 'First':
            case 'ContainsFirstCabin':
            case 'First Class':
                return 'firstClass';
        }
...
//            $this->runScriptFetch();
            */
            ...
            if ($msg = $this->http->FindSingleNode('//auro-alert[contains(.,"No results available")]')) {
...
//            $this->response = $this->driver->executeScript('return localStorage.getItem("searchbff");');
            ...
            $this->getInfo('/data:({departureStation:?.*]})/');
...
//                $stops = $seg['num_stops'] + 1;
    ...
    private function parseCalendarFlightsJson($data)
    {
        $this->logger->notice(__METHOD__);
        $result = [];

        foreach ($data as $item) {
            foreach ($item['CalendarDays'] as $calendarDay) {
                if (($calendarDay['Text'] == 'Not available')
                    || ($calendarDay['CssClass'] == 'calpastday')) {
                    continue;
                }

                preg_match('/\$(\d+)/', $calendarDay['GrandTotal'], $matches);
                $taxes = (int) $matches[1];

                preg_match('/(\d{1,3}(?:,\d{3})*)/', $calendarDay['ToolTip'], $matches);
                $miles = (int) str_replace(',', '', $matches[0]);

                $dateTime = new \DateTime($calendarDay['Date']);
                $date = $dateTime->format("Y-m-d");

                $result[] = [
                    'date'        => $date,
                    'redemptions' => ['miles' => $miles],
                    'payments'    => [
                        'currency' => 'USD',
                        'taxes'    => $taxes,
                        'fees'     => null,
                    ],
                    'cabin'            => $this->decodeCabin($item['Cabin']),
                    'brandedCabin'     => $this->decodeBrandedCabinType($item['Cabin']),
                ];
            }
        }
    ...
    private function decodeBrandedCabinType($cabin)
    {
        switch ($cabin) {
            case 'Saver':
                return 'Saver';
...
//                     "adrum": "isAjax:true",
    ...
    private function getInfo(string $regexp): void
    {
        $response = $this->http->FindPreg($regexp);
