...
class TAccountCheckerHertzSelenium extends TAccountChecker
{
    ...
    public function processQuestion()
    {
        ...
        */
        ...
        $this->saveResponse();
        $button = $this->waitForElement(WebDriverBy::xpath('//div[@id="val-mem-button"]/a'), 0);
        $this->saveResponse();
        ...
        $button->click();
        //$this->driver->executeScript("document.querySelector('#val-mem-button a').click()");

        sleep(7);
        $this->saveResponse();

        if (
            $message = $this->waitForElement(WebDriverBy::xpath('//div[@class="val-mem-error"]/p'), 10)
    ...
    {
        if ($this->alwaysSelenium) {
//
//            $this->curlDrive->GetURL("https://www.hertz.com");
//
//            $result = $this->curlDrive->FindSingleNode('//div[@class = "cardUserDetails"]/text()[1]') !== null
//            || $this->curlDrive->FindSingleNode("//span[contains(@class, 'memberNumber')]") !== null;
//
//            if ($result) {
//                return true;
//            }

            // curl failed, try selenium
            try {
                $this->http->GetURL("https://www.hertz.com");
            } catch (UnexpectedJavascriptException | UnknownServerException $e) {
                $this->logger->error("Exception: " . $e->getMessage(), ['HtmlEncode' => true]);
            }

            $result = $this->http->FindSingleNode('//div[@class = "cardUserDetails"]/text()[1]') !== null
                || $this->http->FindSingleNode("//span[contains(@class, 'memberNumber')]") !== null;
    ...
    public function LoadLoginForm()
    {
        ...
        if (!$this->alwaysSelenium && !$this->curlDrive->ParseForm("submitLogin") && !$this->curlDrive->FindPreg('/_Incapsula_Resource/')) {
        ...
        try {
            ...
            if ($loginForm = $this->waitForElement(WebDriverBy::xpath('//button[@id = "loginLink"] | //li[@id = "loginLinkClub"]/a'), 0)) {
                ...
                try {
                    ...
                    if (strstr($e->getMessage(), 'Extension manifest must request permission to access the respective host')) {
                        throw new CheckRetryNeededException(3, 0);
                    }
                }
            }
