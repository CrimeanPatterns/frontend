...
class TAccountCheckerSingaporeair extends TAccountChecker
{
    ...
    private function akamaiWorkaround($form, $formURL, $headers)
    {
        $this->logger->notice(__METHOD__);
        $xpathRecaptchaFrame = '//iframe[contains(@provider, "recaptcha")]/@data-key';

        // distill workaround
        if ($key = $this->http->FindSingleNode($xpathRecaptchaFrame)) {
        ...
        if ($key = $this->http->FindSingleNode($xpathRecaptchaFrame)) {
            ...
            if ($this->http->FindSingleNode($xpathRecaptchaFrame)) {
                throw new CheckRetryNeededException(3, 7);
            }
        }
    }
    ...
    public function Login()
    {
        ...
        $this->akamaiWorkaround($form, $formURL, $headers);
        ...
        $this->logger->notice("[Current URL]: {$currentUrl}");

        if ($currentUrl == 'https://www.singaporeair.com/kfLogin.form?errorKey=kfaccount.login.errorcodes.N2129&errorCategory=login') {
            throw new CheckException(self::PROVIDER_ERROR_MSG, ACCOUNT_PROVIDER_ERROR);
        }
        ...
        if (
            ...
            if (
                $currentUrl == 'https://www.singaporeair.com/kfLogin.form?filterFlowExecutionURL=kfDashBoardPPS.form'
                && in_array($this->http->Response['code'], [200])
                && $this->http->ParseForm('kfLoginForm')
            ) {
        ...
        if ($this->http->FindPreg("#akamai-custom-pages/singaporeair-captcha.html#ims")) {
            $this->markProxyAsInvalid();
            throw new CheckRetryNeededException(3);
        }
