...
export async function createCreditCardAccount(account: CreditCardAccount): Promise<HTMLAnchorElement> {
    const logoPath = (await import(`@Bem/images/favicons/${account.logo.split('/')[1]}`)).default;

    const creditCardAccountElement = (
        <a href={account.link} className={bem('credit-card-account')}>
            <img src={logoPath} className={bem('credit-card-account', 'logo')} alt="provider logo" />
            <span className={bem('credit-card-account', 'name')}>{account.displayName}</span>
            <span className={bem('credit-card-account', 'owner')}>{account.owner}</span>
            <span className={bem('credit-card-account', 'balance')}>{account.balance}</span>
            <div className={bem('credit-card-account', 'status-block', [`${account.expirationState}`])}>
                <svg
                    className={bem('credit-card-account', 'status-icon')}
                    viewBox="0 0 18 18"
                    fill="none"
                ...
                <span className={bem('credit-card-account', 'expiration-date', ['desktop'])}>
                    {account.expirationDate}
                </span>
                <span className={bem('credit-card-account', 'expiration-date', ['mobile'])}>
            ...
            <svg
                ...
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className={bem('credit-card-account', 'arrow-icon')}
            >
                <path
    ...
    return creditCardAccountElement as unknown as HTMLAnchorElement;
}
