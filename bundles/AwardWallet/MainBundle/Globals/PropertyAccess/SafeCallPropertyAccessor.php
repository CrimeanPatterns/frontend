...
class SafeCallPropertyAccessor implements PropertyAccessorInterface
{
    /**
     * @internal
     */
     ...
     */
    public function __construct($magicCall = false, ?CacheItemPoolInterface $cacheItemPool = null)
    {
    ...
    public function setValue(&$objectOrArray, $propertyPath, $value)
    {
        ...
        $overwrite = true;
        $isSafeCallSupported = $propertyPath instanceof SafeCallPropertyPathInterface;
        ...
        try {
            ...
            for ($i = \count($propertyValues) - 1; 0 <= $i; --$i) {
                $isSafeCall = $isSafeCallSupported && $propertyPath->isSafeCall($i);
        ...
        for ($i = 0; $i < $lastIndex; ++$i) {
            ...
            if ($isIndex) {
                ...
                try {
                    ...
                    if ($isSafeCall) {
                        $propertyValues[] = self::$resultProto;
    ...
    private function getReadAccessInfo($class, $property)
    {
        ...
        if ($reflClass->hasMethod($getter) && $reflClass->getMethod($getter)->isPublic()) {
            ...
            $access[self::ACCESS_NAME] = $getter;
        } elseif ($reflClass->hasMethod($property)) {
            $access[self::ACCESS_TYPE] = self::ACCESS_TYPE_METHOD;
            $access[self::ACCESS_NAME] = $property;
        } else {
    ...
    private function getPropertyPath($propertyPath)
    {
        ...
        $propertyPathInstance = new SafeCallPropertyPath($propertyPath);
