...
class TaxiConverter extends AbstractConverter implements ItineraryConverterInterface
{
    /**
     * @param Rental|Itinerary $itinerary
     * @param TaxiModel|AbstractModel $model
     */
    public function convert(Itinerary $itinerary, AbstractModel $model)
    {
        $this->baseConverter->convert($itinerary, $model);
        $model->setTaxiCompany($itinerary->getRentalCompanyName());
    ...
    public function reverseConvert(AbstractModel $model, Itinerary $itinerary)
    {
        if (empty($model->getPickUpAddress()) || empty($model->getDropOffAddress())) {
            throw new \LogicException('Trying to save taxi ride without pick-up or drop-off location');
        }

        if (is_null($model->getPickUpDate()) || is_null($model->getDropOffDate())) {
            throw new \LogicException('Trying to save taxi ride without pick-up or drop-off dates');
        }

        $this->baseConverter->reverseConvert($model, $itinerary);
        $itinerary->setRentalCompanyName($model->getTaxiCompany());
        ...
        $itinerary->setType(Rental::TYPE_TAXI);
    }
}
