...
class TrainConverter extends AbstractSegmentConverter implements ItinerarySchema2EntityConverterInterface
{
    public function __construct(
        LoggerFactory $loggerFactory,
        BaseConverter $baseConverter,
        Helper $helper,
        TrainRideSegmentMatcher $segmentMatcher,
    ...
    public function convert(
        ...
        $this->helper->validateObject($schemaItinerary, SchemaTrain::class);
        $update = !is_null($entityItinerary);

        if (!$update) {
            $entityItinerary = new EntityTrip();
            $entityItinerary->setCategory(EntityTrip::CATEGORY_TRAIN);
    ...
    /**
     * @param SchemaTrainSegment $schemaSegment
     */
    public function convertSegment(
        ...
        $this->helper->validateObject($schemaSegment, SchemaTrainSegment::class);
        ...
        // scheduleNumber
        if (!is_null($scheduleNumber = $schemaSegment->scheduleNumber)) {
            $entitySegment->setFlightNumber($scheduleNumber);
        }
        ...
        // serviceName
        if (!is_null($serviceName = $schemaSegment->serviceName)) {
            $entitySegment->setServiceName($serviceName);
        }

        // trainInfo
        if (!is_null($model = $schemaSegment->trainInfo->model ?? null)) {
            $entitySegment->setAircraftName($model);
        }

        // car
        if (!is_null($car = $schemaSegment->car)) {
            $entitySegment->setCarNumber($car);
        }
