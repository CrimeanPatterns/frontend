...
class BusConverter extends AbstractSegmentConverter implements ItinerarySchema2EntityConverterInterface
{
    public function __construct(
        LoggerFactory $loggerFactory,
        BaseConverter $baseConverter,
        Helper $helper,
        BusRideSegmentMatcher $segmentMatcher,
    ...
    public function convert(
        ...
        $this->helper->validateObject($schemaItinerary, SchemaBus::class);
        $update = !is_null($entityItinerary);

        if (!$update) {
            $entityItinerary = new EntityTrip();
            $entityItinerary->setCategory(EntityTrip::CATEGORY_BUS);
    ...
    public function convertSegment(
        ...
        $this->helper->validateObject($schemaSegment, SchemaBusSegment::class);
        ...
        // busInfo
        if (!is_null($model = $schemaSegment->busInfo->model ?? null)) {
